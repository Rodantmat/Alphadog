...
// Real, explicit day-completeness gate: identical fix pattern applied across metrics/classification/
// baseline - a candidate date is only eligible once every real game on it is genuinely
// final/available_for_stats (or a legitimate exception). Replaces the naive MAX(game_date) check.
...
async function runDeltaRecalculateAffectedPlayers(sql, input) {
  ...
  if (dayInfo.no_data_yet) return { ok: true, data_ok: true, mode: "delta_recalculate_affected_players", status: "DELTA_CLASSIFICATION_NOOP_NO_NEW_DAY_AVAILABLE", watermark: dayInfo.watermark, next_date: dayInfo.next_date, latest_available: dayInfo.latest_available, blocked_reason: dayInfo.blocked_reason, metrics_gate: dayInfo.metrics_gate, continuation_required: false };
