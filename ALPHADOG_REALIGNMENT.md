...
**8. Player-level trailing hit rate must be checked against post-hoc selection bias before counting as real signal.** A raw "≥X% trailing over ≥Y observations" filter is one specific choice out of many possible thresholds, and testing only that one pair is a post-hoc hyperparameter search, not a validated finding. This system has now seen the failure mode directly: Sleeper singles/less showed an apparent +5.44pp player-level lift that was, on proper testing, indistinguishable from "hit 2 of last 3" binomial noise once corrected for the multiple-comparisons problem inherent in picking a threshold after seeing data. Before trusting any player-level filter: (a) compare "no prior history" legs against "has history" legs directly - if they perform the same, the pool is uniformly strong and a player-level filter is unnecessary (not risky, just pointless); (b) if a real split exists, check whether the weaker sub-bucket still clears breakeven on its own - if it does, filtering it out is the mirror-image mistake (the "inverse Sleeper trap": discarding real, still-positive volume for a marginal, statistically insignificant gain); (c) require a corrected significance threshold (not raw p<0.05) given the threshold itself was chosen after seeing the data.

**9. Full granular prop × line × side × variant × tier grid, sample size stated per cell, before declaring any single cell dead or alive.** Never evaluate a prop/line/side combination pooled across variants or averaged across tiers - build the full breakdown and read each cell on its own terms. Two hard rules that have already caused real errors when ignored:
   - **Goblin and Demon move in OPPOSITE directions as tier distance grows** (Goblin: farther from anchor = easier = lower real multiplier; Demon: farther = harder = higher real multiplier). Averaging across variants, or assuming one variant's tier pattern applies to the other, produces meaningless numbers. Always split by variant first, tier second.
   - **`goblin_demon_tier` must be reconstructed via the anchor-join method, never trusted as a raw column.** Confirmed directly: the column does not exist at all on `score.prop_outcome_history` (only on `score.final_board_current`/`final_board_history`), and even there it is documented empty for most of the real historical window. Reconstruct via `ROUND(ABS(line_value - anchor_line))` where `anchor_line` comes from that player+game's real standard-lane (`is_goblin=0 AND is_demon=0`) row - and when a genuine anchor doesn't exist for a given day (a real market-structure fact on days a prop had no standard line offered at all, not a data bug), report that population separately rather than dropping it silently, since it may still carry a valid, real outcome rate worth including.
...
| — | Day-level significance test | **clustered t = 1.31, daily mean +8.21%, 95% CI [−5.1%, +21.5%] — INCONCLUSIVE, not disproven** |

**⚠️ CORRECTION TO THIS ENTRY, and it is important enough to sit inside the case study rather than beside it.** The final line above originally read "clustered t = 1.31 vs required 2.89 — FAILS," and that framing was wrong in two ways:
1. **The 2.89 bar was the wrong standard.** It derives from Bonferroni α=0.05/13, built for the Item 5 *scan across thirteen factors*. This was a **single, pre-registered, confirmatory test on one pre-chosen line** — no multiplicity to correct for. The correct bar is uncorrected α=0.05, i.e. **t ≈ 2.13** (15 df). At t=1.31 the result still does not clear it, but by a far smaller margin than "1.31 versus 2.89" implied. **Carrying a scan-corrected threshold into a confirmatory test is itself a benchmark error — the seventh on this thread, and one that pushed in the conservative direction rather than the optimistic one.**
2. **"Fails" overstated what 16 noisy days can establish.** Daily mean +8.21% with daily SD 25.04 gives a **95% CI of [−5.1%, +21.5%]** — an interval containing both zero and a substantial exploitable edge. Detecting an effect of that size at 80% power would require roughly **57 days; there are 16.** The correct verdict is **INCONCLUSIVE — insufficient evidence, not evidence of absence.** Sample size, not the absence of an effect, is the most likely reason this looks shut.

**REVISED H2 STATUS: inconclusive at n=16 days, worth re-checking once more days accumulate.** The retraction of the +32.68%/+34.12% figure stands — that number was genuinely invalid. The retraction of the "mechanistically coherent" prop-ordering claim stands — that reasoning was genuinely wrong. **But the corrected +8.21% effect is not disproven, and this thread should be re-run rather than treated as closed.**

**The generalisable lesson, which survives the correction:** every benchmark tightening shrank the effect, and that monotone relationship remains the diagnostic worth carrying forward. **But note the symmetry the original entry missed — benchmark errors run in both directions.** Six of the seven inflated the effect; the seventh (an over-strict significance bar) deflated it. A process that only hunts for optimistic errors will systematically under-call real effects, which is exactly the one-directional bias the audit in item 14 was written to prevent. **Check the benchmark in both directions, every time.**
...
| **PrizePicks own line movement** (`archive.board_leg_history`) | not yet validly measured | — | **Blocked on a data defect, not a negative result.** Variant lives only in a stringified JSON blob; regex tagging returned 223,326 `demon` vs 27,025 `standard` (inverted), so grouping measured the simultaneous ladder rather than movement — 99.66% of legs appeared to "move" by 2.27 units. **Unblock: join on `prepared_row_id` (populated on 100% of 307,351 rows; 79,976 keys join) to inherit correct `is_goblin`/`is_demon` from `final_board_history`.** Then verify row count unchanged, verify standard outnumbers demon, verify one line per rung per capture — before measuring anything. Post-join coverage ~26%, so quantify power before concluding. |
