...
async function runUmpireContext(pg, input) {
  ...
  const realBoardDateRows = await pg.unsafe(`SELECT DISTINCT official_date::text AS official_date FROM score.board_prepared_current WHERE pickable_safe = 1 AND official_game_time_utc IS NOT NULL`);
  ...
  try {
    ...
    const recentlyProcessed = await pg`SELECT game_pk FROM daily.umpire_context_current WHERE official_date IN ${pg(retention.dates)} AND COALESCE(is_temporary_derived,0) = 0`;
