...
<div class="wrap">
  <header class="hero">
    <div class="brand"><img class="logo" src="/main_alphadog_logo.png" alt="AlphaDog"><div><h1>AlphaDog</h1><div class="sub">v0.2.19 - Slip Builder + Player Profile</div></div></div>
  ...
  <section id="slipsScreen" class="hidden"><div class="panel"><button id="slipsBack" class="btn">← Main Board</button><h2>Saved Slips</h2><button id="autoCreateSlipsBtn" class="btn" style="margin-bottom:10px">⚡ Create Slips</button><div id="autoCreateResults"></div><div id="slipsBody" class="dossierBody"><div class="empty">Loading slips...</div></div></div></section>
...
function renderGenerated(){const el=$('generatedSlips');if(!lastGeneratedSlips.length){el.innerHTML='<div class="empty">No valid slips generated. Check same-team/source constraints.</div>';$('saveSlips').classList.add('hidden');return}el.innerHTML='<h3>Custom Slips</h3>'+lastGeneratedSlips.map((s,i)=>'<div class="slipCard"><div class="slipHead"><label><input type="checkbox" class="genSlipPick" data-i="'+i+'" checked> '+esc(String(s.source_key||'').toUpperCase())+' '+esc(s.structure_label||s.slip_type)+'</label><span class="'+evClass(s.estimated_ev_per_unit_stake)+'" style="font-weight:950">'+evLabel(s.estimated_ev_per_unit_stake)+'</span></div><div class="small">'+notesLines(s.strategy_notes)+'</div><div class="slipLegs">'+(s.legs||[]).map(leg=>'<div class="legMini"><span>'+legLine(leg)+'</span><b>'+pct(leg.hit_probability_0_100)+'</b></div>').join('')+'</div></div>').join('');$('saveSlips').classList.remove('hidden')}
...
function propFamilyKeys(isP){return isP?['pitcher_strikeouts','pitcher_outs','earned_runs','hits_allowed','walks_allowed','runs_allowed']:['hits','total_bases','home_runs','runs','rbis','walks','singles','doubles','stolen_bases','hitter_strikeouts','hits_runs_rbis']}
