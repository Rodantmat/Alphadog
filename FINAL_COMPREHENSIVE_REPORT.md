...
- **The enrichment-layer question is now completely, rigorously resolved, including a full close-out of every data-coverage question**: The original premise driving the whole enrichment investigation — "the current enrichment layer destroys baseline's real discrimination" — was itself found to rest on a leakage-corrupted baseline reference. Once corrected, baseline and the full current enrichment layer perform almost identically. A rigorous test of the best possible case for enrichment (the three factors with the strongest individual validation in the entire ~19-factor system) found that even this best case does not improve baseline's real, day-level correlation with outcomes — with one genuine, positive exception found afterward through further work: `opposing_pitcher_quality`, when tested with a validated, full-coverage proxy (see Section 5.6), shows a real, statistically significant improvement specifically for `hitter_strikeouts` (t=5.913 across 133 real days) — a finding the original, data-limited test was too thin to detect. One real, confirmed exception in the other direction also exists: `rbis` (and `hits_runs_rbis`, which shares the mechanism) has a specific, actively harmful enrichment-layer bug, independently confirmed twice.
...
### 5.6 Closing the data-coverage question completely, and a genuine positive finding discovered along the way

Section 5.2's conclusion was reached using each of the three real factors' existing, limited real-data coverage (roughly 21-30 days). Given the standing requirement that any coverage claim be genuinely closed rather than assumed sufficient, all three factors were traced to their real raw data sources and tested for whether full, gap-free, through-today coverage was achievable.

**`opposing_pitcher_quality` and `catcher_framing` have a genuine, hard data ceiling.** Both depend on external quality-metric snapshots that exist, system-wide, for exactly three dates: August 20, 23, and 30 — not a rolling window, and `catcher_framing` has a second, compounding gap in its own per-game catcher-assignment data. Closing this would require standing up new, ongoing data collection and waiting for real history to accumulate — genuine infrastructure work, not further analysis. This is accepted as a real limitation, not something further backtesting can resolve.

**Rather than stop there, a validated substitute was built and tested for `opposing_pitcher_quality`.** Fielding Independent Pitching (FIP, the standard sabermetric formula `(13×HR + 3×BB − 2×K)/IP`) was computed as a genuine, point-in-time season-to-date value from data with full coverage since March — and validated directly against the real snapshot data before being trusted (r=-0.515, correct sign, R²=0.27 after calibration, n=858 real pitchers). This gave real, gap-free coverage across 137 days and 26,560 legs, versus the original 12 days and 3,124 legs.

**The result reversed the earlier, thin-sample finding for one specific prop, with real statistical strength.** For `hitter_strikeouts`, the properly-extended test showed the combined version beating baseline's day-level correlation on 93 of 133 real days, t=5.913 — strongly significant, in sharp contrast to the original 21-day test's negative trend (t=-1.968). Checked directly for the tier-cancellation pattern central to this whole investigation and confirmed clean (all four skill tiers move by under 0.3 percentage points, no masked swing). The most likely explanation: the original test's data was too sparse to detect a real but modest effect, and could plausibly show a wrong-direction trend from pure noise.

**Extended to `opposing_pitcher_quality`'s other four wired props, and the result is genuinely mixed, not a uniform confirmation**: `hits` showed a borderline positive trend (t=2.090), `total_bases` similarly borderline (t=1.928, just under conventional significance), `walks` not significant (t=1.639), and `home_runs` showed essentially zero effect (t=0.014). **The honest, defensible conclusion: `opposing_pitcher_quality` is worth including for `hitter_strikeouts` specifically, on the strength of its result — the case for its other four wired props remains weak-to-absent.**

**`schedule_travel_fatigue` was also fully closed.** A real, verified 30-team venue-timezone reference and per-game travel-direction computation were built (spot-checked for correctness against real, geographically sensible transitions) and tested with full 138-day coverage against the three props with the largest originally-documented effect sizes. None showed a significant effect (`total_bases` t=-0.585, `runs` t=-0.204, `home_runs` t=-0.446) — consistent with, not contradicting, the earlier finding. The most likely explanation, consistent with a pattern seen elsewhere in this investigation: the original validation's documented effect held within its own narrower window but did not replicate at full scale.

**This closes every open data-coverage question in the enrichment investigation.** Nothing in this section remains blocked by data availability — the one meaningful update to Section 5.2's conclusion is the genuine, positive, narrowly-scoped finding for `opposing_pitcher_quality` on `hitter_strikeouts` specifically.
