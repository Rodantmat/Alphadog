...
- Phase 1 (recognition): substantial progress, not yet complete.
  ...
  - Certifier's real current logic (verified by reading the deployed file): reads `SCORE_DB.score_board_prepared_current` filtered by `pickable_safe=1, matchup_status='calendar_matched', player_match_status='matched'` (source-agnostic — good sign for Underdog compatibility), cross-references all 7 `daily_*_current` tables, computes a per-row status (READY_FULL_CONTEXT / READY_WITH_WARNINGS / READY_PARTIAL_ENRICHMENT / BLOCKED_* / NOT_APPLICABLE_STARTED_OR_EXPIRED), writes `daily_context_readiness_current`/`_issues` rebuilt fresh each run for today/tomorrow only. Real gaps against the new spec: only binary available/missing per layer (no real/derived/partial/complete/temporary tally), no fallback-triggering logic at all, only runs last (no first-pass mode), window logic checks today+tomorrow but doesn't classify/report which of the 3 slate shapes applies.
  - Spot-checked `daily-weather.js`: also reads `score_board_prepared_current` with the identical filter (not any raw per-source board table) — meaning it's likely already Underdog-safe by construction, and already has its own retention/purge logic (`retention_date_start/end`, deletes stale current/snapshot/issue rows). **Not yet verified across the other 5 real workers** — do not assume they all follow this same safe pattern until each is individually checked.
  - Remaining for Phase 1: verify `daily-probable-pitchers`, `daily-lineups`, `daily-player-availability`, `daily-bullpen-availability`, `daily-schedule`, and `daily-usage-pulse` (umpire) each individually the same way — board-read source, retention/purge behavior, and overall structure — before Phase 1 is actually complete.
