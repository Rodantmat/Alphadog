...
- Phase 2 (research + lock additional daily-context data candidates): IN PROGRESS.
  ...
  - **Additional daily-context data candidates — researched and locked**:
    - **NEW LAYER TO BUILD: Catcher Context** (who's catching today, regular/personal catcher vs. backup, pitch-framing quality tier). Real, established prop-modeling systems (THE BAT X per ESPN's MLB betting coverage, SportsGrid, Dimers) explicitly list catcher pitch-framing as a real factor alongside ballparks/bullpens/weather/umpires — the same category of factor as what we already mine. Genuinely new: no current layer tracks catcher assignment or framing. Minable the same way Umpire Context already works (MLB StatsAPI boxscore/live feed for assignment; framing-quality tier can start as a simple derived proxy from the catcher's own defensive stats already available in STATS_HITTER_DB, refined later if a better public framing metric source is found). Locked for build in Phase 5.
...
- Phase 3 (certifier redesign — run first, own the tally/calendar per layer and data-level): starting next.
