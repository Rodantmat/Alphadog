...
async function promoteCertifiedStage(sql, batchId, requestId) {
  ...
  await sql`INSERT INTO ref.rosters
    ...
    SELECT roster_key, slate_date, roster_date, snapshot_type, team_id, mlb_team_id, player_id, player_name, roster_status, role, position_abbreviation, source_key, 1, raw_json, now(), ${requestId}, now()
  ...
  try {
    ...
    for (const snapshot of rosterSnapshots) {
      ...
      await sql`INSERT INTO ref.rosters_stage
        (batch_id, roster_key, roster_date, snapshot_type, team_id, mlb_team_id, player_id, player_name, roster_status, role, position_abbreviation, source_key, active, raw_json, updated_at, last_seen_request_id, last_seen_at)
        VALUES (${batchId}, ${rosterKey}, current_date::text, 'STATIC_40MAN_SNAPSHOT', ${snapshot.team.team_id}, ${numOrNull(snapshot.team.mlb_team_id)}, ${snapshot.player.player_id}, ${snapshot.player.full_name}, ${snapshot.player.roster_status}, ${snapshot.player.primary_position}, ${snapshot.player.position_abbreviation}, ${SOURCE_KEY}, 1, ${snapshot.player.raw_json}, now(), ${requestId}, now())
