const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.13-base-pitcher-game-logs-source-probe-route";
  ...
  if (job === "orchestrator_enqueue_base_pitcher_game_logs") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        note: "Existing Base Pitcher Game Logs source-probe queue row found. Use ORCHESTRATOR > Wake. Browser does not run the job."
      });
    }

    const requestId = "base_pitcher_logs_probe_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_base_pitcher_logs_probe_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Pitcher Game Logs",
      mode: "source_probe",
      backend_scheduled_continuation: false,
      source_probe_only: true,
      base_backfill_cutoff_date: "2026-05-18",
      delta_reserved_start_date: "2026-05-19",
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-pitcher-game-logs', 'alphadog-v2-base-pitcher-game-logs', 'Base', 'base', 'Base Pitcher Game Logs Source Probe', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_pitcher_game_logs', 'INFO', 'queued_base_pitcher_game_logs_source_probe', 'Queued exact Base Pitcher Game Logs v0.1.0 source-probe job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"BASE > Pitcher Game Logs", source_probe_only:true, no_live_promotion:true})).run();
    ...
    return jsonResponse({
      ...
      note: "Queued source-probe only. Use ORCHESTRATOR > Wake once, then ORCHESTRATOR > Logs and MANUAL SQL > Run. No live promotion, no full base, no delta."
    });
  }
