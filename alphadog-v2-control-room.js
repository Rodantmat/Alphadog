const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.45-base-starter-history-promotion";
  ...
  if (job === "orchestrator_enqueue_base_starter_history") {
    ...
    if (existing) {
      ...
      return jsonResponse({
        ...
        base_promotion_stage_clean: true,
    ...
    const requestId = "starter_history_base_promote_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_starter_history_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Starter History",
      mode: "base_promotion_stage_clean",
      certified_stage_batch_id: "starter_base_stage_batch_mphbfevk_49z9fq",
      base_backfill_cutoff_date: "2026-05-18",
      delta_reserved_start_date: "2026-05-19",
      probe_only: false,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-starter-history', 'alphadog-v2-base-starter-history', 'Base', 'base', 'Base Starter History Promotion v0.3.0', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_starter_history', 'INFO', 'queued_base_starter_history_promotion', 'Queued Base Starter History v0.3.0 certified stage promotion and cleanup', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "BASE > Starter History", mode: "base_promotion_stage_clean", certified_stage_batch_id: "starter_base_stage_batch_mphbfevk_49z9fq", base_backfill_cutoff_date: "2026-05-18", no_live_promotion: false, backend_hot_continuation: true })).run();

    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(callOrchestrator(env, "/pump", {
        source: "control_room_base_starter_history_promotion_auto_start",
        max_cycles: 18,
        max_jobs_per_cycle: 1,
        max_ms: 70000,
        pump_depth: 0,
    ...
    return jsonResponse({
      ...
      note: "Queued Base Starter History v0.3.0 certified stage promotion and cleanup. Control Room launches one backend orchestrator pump via ctx.waitUntil. Browser may close now. ORCHESTRATOR > Wake is rescue only. No delta execution, no scoring, no board mutation."
    });
  }
