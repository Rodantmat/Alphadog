const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.57-bullpen-promote-clean";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_base_bullpen_history") {
    ...
    if (existing) {
      if (ctx && typeof ctx.waitUntil === "function") {
        ctx.waitUntil(callOrchestrator(env, "/pump", {
          source: "control_room_base_bullpen_history_existing_promote_hot_continue",
          max_cycles: 8,
      ...
      return jsonResponse({
        ...
        target_batch_id: "bullpen_base_stage_batch_mphjoj0p_rwldp7",
        backend_scheduled_continuation: true,
        auto_pump_triggered: true,
        browser_auto_pump: false,
        note: "Existing Base Bullpen History queue row found. Control Room launched one backend orchestrator pump as a hot-continuation nudge. Browser does not run the job. ORCHESTRATOR > Wake is rescue/acceleration only."
      });
    }

    const requestId = "bullpen_history_base_promote_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_bullpen_history_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Bullpen History",
      mode: "base_promote_clean",
      batch_id: "bullpen_base_stage_batch_mphjoj0p_rwldp7",
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-bullpen-history', 'alphadog-v2-base-bullpen-history', 'Base', 'base', 'Base Bullpen History Promote Clean v0.3.0', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_bullpen_history', 'INFO', 'queued_base_bullpen_history_base_promote_clean', 'Queued Base Bullpen History v0.3.0 base promote-clean for certified stage batch only', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "BASE > Bullpen History", mode: "base_promote_clean", batch_id: "bullpen_base_stage_batch_mphjoj0p_rwldp7", max_rows_per_tick: 500, no_mining: true, no_new_batch: true, no_source_calls: true, no_delta_update_execution: true, no_daily_bullpen_availability: true, backend_hot_continuation: true })).run();

    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(callOrchestrator(env, "/pump", {
        source: "control_room_base_bullpen_history_base_promote_auto_start",
        max_cycles: 10,
        max_jobs_per_cycle: 1,
        max_ms: 60000,
        pump_depth: 0,
    ...
    return jsonResponse({
      ...
      note: "Queued Base Bullpen History v0.3.0 promote-clean for the certified stage batch only. Browser may close now. ORCHESTRATOR > Wake is rescue/acceleration only. No mining, no new batch, no source calls, no delta, no Daily Bullpen Availability, no scoring, no board mutation."
    });
  }
