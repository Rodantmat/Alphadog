const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.141-market-context-allowlist-fix";
  ...
  if (job === "orchestrator_enqueue_market_context_source_probe") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "MARKET > Context Probe",
        note: "Existing Market Context Source Probe queue row found. Use ORCHESTRATOR > Wake and ORCHESTRATOR > Logs."
      });
    }

    const requestId = "market_context_probe_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_market_context_probe_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "MARKET > Context Probe",
      ...
      probe_only_real_worker_shape: true,
      ...
      parlay_player_prop_coverage_test_only: true,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'market-normalizer', 'alphadog-v2-market-normalizer', '05 Market', 'market_context', 'Market Context Source Probe', 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_market_context_source_probe', 'INFO', 'queued_market_context_source_probe', 'Queued exact Market Context Source Probe worker job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"MARKET > Context Probe", queued_job_key:"market-normalizer", queued_worker_name:"alphadog-v2-market-normalizer", no_market_current_lines_writes:true, no_score_db_mutation:true})).run();
    ...
    return jsonResponse({
      ...
      probe_only_real_worker_shape: true,
      today_tomorrow_retention_only: true,
      note: "Queued Market Context Source Probe only. This may create/write MARKET_DB probe evidence tables for today/tomorrow only. It does not write market_current_lines, mutate prepared board, score, rank, build matrix packets, or write final board. Tap ORCHESTRATOR > Wake."
    });
  }
