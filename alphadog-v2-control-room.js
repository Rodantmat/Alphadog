const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.195-v2-shadow-no-like-worker-guard";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  const shadowV2Specs = {
    orchestrator_enqueue_score_enrichment_v2_shadow: {
      ...
      display_name: "Expansion V2 Shadow - Score Enrichment V2",
      visible_button: "EXPANSION / V2 BASELINE > Enrich V2 Shadow",
      request_prefix: "score_enrichment_v2_shadow",
      chain_prefix: "chain_score_enrichment_v2_shadow",
    ...
    orchestrator_enqueue_hit_probability_v3_shadow: {
      ...
      display_name: "Expansion V2 Shadow - Hit Probability V3",
      visible_button: "EXPANSION / V2 BASELINE > HP V3 Shadow",
      request_prefix: "hit_probability_v3_shadow",
      chain_prefix: "chain_hit_probability_v3_shadow",
    ...
    orchestrator_enqueue_final_score_v2_shadow: {
      ...
      display_name: "Expansion V2 Shadow - Final Score V2",
      visible_button: "EXPANSION / V2 BASELINE > Score V2 Shadow",
      request_prefix: "final_score_v2_shadow",
      chain_prefix: "chain_final_score_v2_shadow",
    ...
    orchestrator_enqueue_final_board_v3_shadow: {
      ...
      display_name: "Expansion V2 Shadow - Final Board V3 Review",
      visible_button: "EXPANSION / V2 BASELINE > Board V3 Shadow",
      request_prefix: "final_board_v3_shadow",
      chain_prefix: "chain_final_board_v3_shadow",
      logical_worker_name: "alphadog-v2-final-board-v3-shadow",
      priority: 9
    }
  };

  if (shadowV2Specs[job]) {
    const spec = shadowV2Specs[job];
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = ? AND status IN ('pending','running','partial_continue') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).bind(spec.job_key).first();
    if (existing) {
      return jsonResponse({
        ok: true,
        data_ok: true,
        ...
        request_id: existing.request_id,
        existing,
        visible_button: spec.visible_button,
        expansion_v2_shadow: true,
        review_testing_only: true,
        not_production_final_board: true,
        not_live_plays: true,
        note: "Existing shadow queue row found. Use ORCHESTRATOR > Wake / Logs to continue or inspect."
      });
    }
    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-score-audit', ?, 'Expansion / V2 Baseline', 'expansion_v2_shadow', ?, 1, 'https://alphadog-v2-score-audit.rodolfoaamattos.workers.dev', 'SCORE_AUDIT_WORKER', 1, ?, CURRENT_TIMESTAMP)"
    ).bind(spec.job_key, spec.display_name, "Shadow V2/V3 scoring lane. Orchestrator dispatches only; alphadog-v2-score-audit owns heavy calculation and writes only v2_* shadow tables. Review/testing only, not production final board, not live plays.").run();
    const requestId = spec.request_prefix + "_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = spec.chain_prefix + "_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: spec.visible_button,
      mode: spec.mode,
      created_at: now,
      logical_worker_name: spec.logical_worker_name,
      deployed_worker_slot: "alphadog-v2-score-audit",
      exact_worker_only: true,
      orchestrator_dispatch_only: true,
      worker_owned_shadow_logic: true,
      worker_owned_schema_creation: true,
      expansion_v2_baseline_section: true,
      shadow_review_testing_only: true,
      not_production_final_board: true,
      not_live_plays: true,
      writes_v2_shadow_tables_only: true,
      production_tables_read_only: true,
      no_current_scoring_mutation: true,
      no_score_enrichment_current_mutation: true,
      no_hit_probability_v2_mutation: true,
      no_final_score_v1_mutation: true,
      no_score_final_board_v2_mutation: true,
      no_old_production_touch: true,
      live_playable_forced_zero: true
    };
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, ?, 'alphadog-v2-score-audit', 'Expansion / V2 Baseline', 'expansion_v2_shadow', ?, 'pending', ?, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, spec.job_key, spec.display_name, spec.priority, JSON.stringify(input)).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_expansion_v2_shadow_stage', 'Queued Expansion / V2 Baseline shadow scoring stage for score-audit worker-owned execution', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, job, JSON.stringify({ request_id:requestId, chain_id:chainId, queued_job_key:spec.job_key, queued_worker_name:"alphadog-v2-score-audit", visible_button:spec.visible_button, orchestrator_dispatch_only:true, worker_owned_shadow_logic:true })).run();
    return jsonResponse({
      ok: true,
      data_ok: true,
      version,
      ...
      writes_v2_shadow_tables_only: true,
      no_current_scoring_mutation: true,
      note: "Queued under EXPANSION / V2 BASELINE. Tap ORCHESTRATOR > Wake. Stage is shadow/review only and writes only v2_* tables."
    });
  }
