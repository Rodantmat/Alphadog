const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.105-incremental-morning-full-run-button";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_incremental_morning_full_run") {
    ...
    const input = {
      ...
      created_at: now,
      approved_chain_order: ["base-hitter-game-logs", "base-pitcher-game-logs", "base-team-game-logs", "base-starter-history", "base-bullpen-history", "base-hitter-splits", "base-pitcher-splits", "base-hitter-metrics", "base-pitcher-metrics"],
      child_modes: {
    ...
    return jsonResponse({ ok: true, data_ok: true, version, job, status: "queued", request_id: requestId, chain_id: chainId, visible_button: "DELTA > Full Run", queued_job_key: "incremental-morning-full-run", queued_worker_name: "alphadog-v2-orchestrator", approved_chain_order: input.approved_chain_order, backend_chain_only: true, auto_pump_triggered: true, browser_auto_pump: false, note: "Queued one parent Incremental Morning Full Run request. Backend service-binding auto-pump was launched; browser may close now. ORCHESTRATOR > Wake is manual testing/rescue only." });
  }
