const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.157-scoring-v0.2.3-db-config";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_scoring_simulation") {
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'scoring-engine-simulation', 'alphadog-v2-score-audit', '11 Scoring', 'scoring', 'Scoring Engine Simulation Shadow v0.2.3 DB Config', 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_scoring_engine_simulation', 'Queued exact Scoring Engine Simulation Shadow v0.2.3 DB Config job', ?, CURRENT_TIMESTAMP)"
