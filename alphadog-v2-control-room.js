const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.168-hard-cleanup-market-full-autopump";
...
async function runJob(request, env, ctx) {
  ...
  try {
    ...
    if ([
      ...
      "orchestrator_enqueue_scoring_simulation",
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_market_scoring_full_run") {
    ...
    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-orchestrator','market-scoring-full-run','12 Full Runs','market_scoring','Market + Factors + Matrix + Scoring Full Run',1,'https://alphadog-v2-orchestrator.rodolfoaamattos.workers.dev','ORCHESTRATOR_WORKER',1,'Backend parent chain: Market Teams, Market Hitters, Market Pitchers, Factor Hitters, Factor Pitchers, Matrix Build, Scoring Simulation, Final Board. No source board mutation and no manual SQL schema requirement.',CURRENT_TIMESTAMP)"
    ).run();

    await env.CONFIG_DB.prepare(
      "INSERT OR REPLACE INTO config_worker_definitions (worker_name, job_key, worker_group, phase_key, display_name, enabled, schedule_profile_key, notes) VALUES ('alphadog-v2-orchestrator','market-scoring-full-run','12 Full Runs','market_scoring','Market + Factors + Matrix + Scoring Full Run',1,'manual_or_orchestrated','Parent backend chain for market context, prop factors, matrix, scoring simulation, and final board. Created by Control Room; no schema SQL required.')"
    ).run();
  ...
  if (job === "orchestrator_enqueue_scoring_engine") {
    ...
    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-score-audit','scoring-engine','11 Scoring','scoring','Scoring Engine Framework/Profile Gate',1,'https://alphadog-v2-score-audit.rodolfoaamattos.workers.dev','SCORE_AUDIT_WORKER',1,'Existing score-audit slot repurposed as Scoring Engine framework/profile gate. Worker-owned idempotent schema creation; one row per matrix variation; side-aware/source-line-type-aware/variation-aware; no final threshold lock, no ranking, no final board.',CURRENT_TIMESTAMP)"
    ).run();
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'scoring-engine', 'alphadog-v2-score-audit', '11 Scoring', 'scoring', 'Scoring Engine Framework/Profile Gate', 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_scoring_engine_framework', 'Queued exact Scoring Engine Framework/Profile Gate job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, job, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"SCORING > Engine", queued_job_key:"scoring-engine", queued_worker_name:"alphadog-v2-score-audit", logical_worker_name:"alphadog-v2-scoring-engine", no_final_thresholds_locked:true, no_ranking:true, no_final_board:true})).run();
    ...
    return jsonResponse({
      ...
      final_qualification_threshold_locked: false,
      one_score_row_per_matrix_row: true,
      note: "Queued Scoring Engine framework/profile gate. Worker creates idempotent SCORE_DB/ARCHIVE_DB schema on first run, preserves one row per matrix variation, keeps Goblin/Demon More-only, does not calculate final scores, rank, or write final board. Tap ORCHESTRATOR > Wake."
    });
  }
  ...
  if (job === "orchestrator_enqueue_scoring_simulation") {
    ...
    const requestId = "scoring_simulation_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_scoring_simulation_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "SCORING > Simulation",
