const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.188-expansion-baseline-stale-requeue";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job.startsWith("orchestrator_enqueue_expansion_baseline_")) {
    const modeMap = {
      orchestrator_enqueue_expansion_baseline_mining: ["expansion-baseline-mining", "expansion_baseline_mining", "Expansion Baseline Mining", "EXPANSION > Mining"],
      orchestrator_enqueue_expansion_baseline_sanity: ["expansion-baseline-sanity", "expansion_baseline_sanity", "Expansion Baseline Sanity", "EXPANSION > Sanity"],
      orchestrator_enqueue_expansion_baseline_hp: ["expansion-baseline-hp", "expansion_baseline_hp", "Expansion Baseline HP", "EXPANSION > Baseline HP"],
      orchestrator_enqueue_expansion_baseline_certifier: ["expansion-baseline-certifier", "expansion_baseline_certifier", "Expansion Baseline Certifier", "EXPANSION > Certifier"],
      orchestrator_enqueue_expansion_baseline_full_run: ["expansion-baseline-full-run", "expansion_baseline_full_run", "Expansion Baseline Full Run", "EXPANSION > Full Baseline"]
    };
    const cfg = modeMap[job];
    if (!cfg) return jsonResponse({ok:false, data_ok:false, version, job, error:"unknown_expansion_baseline_enqueue_job"}, 400);
    const [jobKey, mode, displayName, visibleButton] = cfg;
    const existing = await env.CONTROL_DB.prepare(
    ...
    if (existing) {
      const stale = await env.CONTROL_DB.prepare(
        "SELECT request_id FROM control_job_queue WHERE request_id=? AND status IN ('running','partial_continue') AND datetime(updated_at) <= datetime('now','-8 minutes') LIMIT 1"
      ).bind(existing.request_id).first();
      if (!stale) return jsonResponse({ok:true, data_ok:true, version, job, status:"already_queued", request_id:existing.request_id, existing, visible_button:visibleButton, expansion_only:true, baseline_only:true});
      await env.CONTROL_DB.prepare(
        "UPDATE control_job_queue SET status='cancelled', finished_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP, error_code='expansion_baseline_stale_running_requeued', error_message='Cancelled stale expansion baseline row before requeue; source-only patch keeps production tables untouched.' WHERE request_id=?"
      ).bind(existing.request_id).run();
      await env.CONTROL_DB.prepare(
        "UPDATE control_job_runs SET status='failed', data_ok=0, certification_status='EXPANSION_BASELINE_STALE_RUNNING_REQUEUED', finished_at=CURRENT_TIMESTAMP, error_code='expansion_baseline_stale_running_requeued', error_message='Retired stale expansion baseline run before requeue.' WHERE request_id=? AND finished_at IS NULL"
      ).bind(existing.request_id).run();
      await env.CONTROL_DB.prepare(
        "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'WARN', 'expansion_baseline_stale_running_requeued', 'Retired stale Expansion Baseline running row before requeue', ?, CURRENT_TIMESTAMP)"
      ).bind(existing.request_id, job, JSON.stringify({stale_request_id:existing.request_id, stale_status:existing.status, stale_updated_at:existing.updated_at, requeue_requested:true, production_tables_untouched:true})).run();
    }
    const requestId = jobKey.replace(/-/g, "_") + "_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_expansion_baseline_" + Date.now().toString(36);
    const input = {source:"control_room",visible_button:visibleButton,mode,expansion_mode:mode,created_at:now,expansion_only:true,baseline_only:true,writes_expansion_tables_only:true,write_table_whitelist_regex:"^expansion_",no_current_baseline_mutation:true,no_factor_mutation:true,no_matrix_mutation:true,no_scoring_mutation:true,no_final_board_mutation:true,no_scheduler_mutation:true,no_full_run_integration:true,no_market_context:true,no_daily_context:true,no_scoring_context:true,full_depth_base:true,mining_game_chunk_size:60,target_established_sample:25};
    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-phase3a-first-inning-pitcher-context', ?, 'Expansion', 'expansion_baseline', ?, 1, 'https://alphadog-v2-phase3a-first-inning-pitcher-context.rodolfoaamattos.workers.dev', 'PHASE3A_FIRST_INNING_PITCHER_CONTEXT_WORKER', 1, 'Baseline-only expansion shadow routed through existing first-inning pitcher context slot. Writes expansion_ tables only; no factor, matrix, scoring, final board, scheduler, or production baseline mutation.', CURRENT_TIMESTAMP)"
    ).bind(jobKey, displayName).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, ?, 'alphadog-v2-phase3a-first-inning-pitcher-context', 'Expansion', 'expansion_baseline', ?, 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, jobKey, displayName, JSON.stringify(input)).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_expansion_baseline_job', 'Queued isolated Expansion Baseline job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, job, JSON.stringify({request_id:requestId, chain_id:chainId, queued_job_key:jobKey, queued_worker_name:"alphadog-v2-phase3a-first-inning-pitcher-context", visible_button:visibleButton, expansion_only:true, baseline_only:true, no_production_mutation:true})).run();
    if (ctx && ctx.waitUntil) ctx.waitUntil(callOrchestrator(env, "/pump", {source:"control_room_expansion_baseline_enqueue", max_cycles:5, max_jobs_per_cycle:1, max_ms:45000, no_browser_loop:true, cron_rescue_only:true}));
    return jsonResponse({ok:true, data_ok:true, version, job, status:"queued", request_id:requestId, chain_id:chainId, visible_button:visibleButton, queued_job_key:jobKey, queued_worker_name:"alphadog-v2-phase3a-first-inning-pitcher-context", expansion_only:true, baseline_only:true, writes_expansion_tables_only:true, no_current_baseline_mutation:true, no_scoring_mutation:true, no_final_board_mutation:true});
  }
