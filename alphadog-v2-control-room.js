const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.76-clear-base-delta-labels";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_base_hitter_metrics") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "BASE > Hitter Metrics Snapshot Prep",
        backend_scheduled_continuation: true,
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing BASE > Hitter Metrics Snapshot Prep queue row found. Tap ORCHESTRATOR > Wake once only if it has not processed. Browser does not run the job."
      });
    }
    ...
    const input = {
      ...
      created_at: now,
      worker_scope: "v0_3_3_snapshot_prep_stage_only",
      ...
      v0_3_3_snapshot_prep_stage_only: true,
    ...
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"BASE > Hitter Metrics Snapshot Prep", no_promotion:true, no_external_mlb_calls:true, no_scoring:true})).run();
  ...
  if (job === "orchestrator_enqueue_delta_hitter_metrics") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "DELTA > Hitter Metrics Live Repair",
        backend_scheduled_continuation: true,
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing DELTA > Hitter Metrics Live Repair queue row found. Tap ORCHESTRATOR > Wake once only if it has not processed. Browser does not run the job."
      });
    }
    ...
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"DELTA > Hitter Metrics Live Repair", no_external_mlb_calls:true, no_source_mutation:true, no_scoring:true})).run();
  ...
  if (job === "orchestrator_enqueue_base_starter_history") {
    ...
    const requestId = "starter_history_delta_noop_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-starter-history', 'alphadog-v2-base-starter-history', 'Base', 'base', 'Base Starter History Scoped Repair v0.4.3', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_starter_history', 'INFO', 'queued_base_starter_history_scoped_source_repair', 'Queued Base Starter History v0.4.4 scoped repair order fix check', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "BASE > Starter History", mode: "delta_scoped_source_repair", delta_start_date: "2026-05-19", delta_end_date: new Date().toISOString().slice(0,10), base_backfill_cutoff_date: "2026-05-18", no_live_promotion: false, backend_hot_continuation: true, stage_retained: true })).run();

    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(callOrchestrator(env, "/pump", {
        source: "control_room_base_starter_history_scoped_source_repair_auto_start",
        max_cycles: 18,
        max_jobs_per_cycle: 1,
        max_ms: 70000,
        pump_depth: 0,
    ...
    return jsonResponse({
      ...
      note: "Queued Base Starter History v0.4.4 scoped repair order fix check. Control Room launches one backend orchestrator pump via ctx.waitUntil. Browser may close now. ORCHESTRATOR > Wake is rescue only. Refetches only live + retained-stage missing delta keys from official boxscore; no full sweep, no new batch, no scoring, no board mutation."
    });
  }
  ...
  if (job === "orchestrator_enqueue_delta_team_game_logs") {
    ...
    const input = {
      source: "control_room",
      visible_button: "DELTA > Team Game Logs",
      mode: "delta_retained_stage_restore_before_queue",
    ...
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "DELTA > Team Game Logs", mode: "delta_retained_stage_restore_before_queue", delta_start_date: "2026-05-19", delta_end_date_policy: "auto_latest_completed_final_mlb_game_date", retained_stage_required: true })).run();
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued Delta Team Game Logs v0.3.1. Tap ORCHESTRATOR > Wake once to dispatch. Worker auto-discovers latest completed MLB final date, appends only missing final dates after retained stage, and retains stage for no-op/restore testing."
    });
  }
  ...
  if (job === "orchestrator_enqueue_delta_hitter_splits") {
    ...
    const input = {
      ...
      mode: "delta_retained_stage_restore_before_queue",
      created_at: now,
      worker_scope: "delta_noop_restore_scoped_repair_daily_affected_refresh_gate_v0_4_2",
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-hitter-splits', 'alphadog-v2-base-hitter-splits', 'Delta', 'delta', 'Delta Hitter Splits Scoped Repair + Daily Affected Refresh v0.4.2', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_delta_hitter_splits', 'INFO', 'queued_delta_hitter_splits_scoped_repair_gate', 'Queued exact Delta Hitter Splits v0.4.2 scoped repair + daily affected-player refresh gate job', ?, CURRENT_TIMESTAMP)"
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued v0.4.2 delta no-op / retained-registry restore / scoped-repair / daily affected-player refresh gate in CONTROL_DB. Tap ORCHESTRATOR > Wake once to dispatch. It must not run a full hitter universe refresh."
    });
  }
  ...
  if (job === "orchestrator_enqueue_delta_pitcher_splits") {
    ...
    const input = {
      ...
      created_at: now,
      worker_scope: "delta_noop_restore_scoped_repair_daily_affected_refresh_gate_v0_5_3",
    ...
    return jsonResponse({ ok: true, data_ok: true, version, job, status: "queued", request_id: requestId, chain_id: chainId, visible_button: "DELTA > Pitcher Splits", queued_job_key: "base-pitcher-splits", queued_worker_name: "alphadog-v2-base-pitcher-splits", delta_noop_restore_scoped_repair_gate: true, retained_stage_restore_before_queue: true, daily_affected_pitcher_refresh_when_new_final_date: true, no_balanced_split_assumption: true, outcome_rows_staged_non_authoritative: true, no_full_pitcher_universe_refresh: true, backend_scheduled_continuation: true, auto_pump_triggered: false, browser_auto_pump: false, note: "Queued v0.5.3 delta no-op / retained-restore / scoped-repair / daily affected-pitcher refresh gate. It must not run a full pitcher-universe refresh. Tap ORCHESTRATOR > Wake once to dispatch." });
  }
