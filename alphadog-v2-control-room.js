const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.33-hitter-splits-daily-affected-refresh";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_delta_pitcher_splits") {
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-pitcher-splits', 'alphadog-v2-base-pitcher-splits', 'Delta', 'delta', 'Delta Pitcher Splits No-op / Restore / Scoped Repair v0.5.1', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_delta_pitcher_splits', 'INFO', 'queued_delta_pitcher_splits_scoped_repair_gate', 'Queued Delta Pitcher Splits v0.5.1 no-op / retained-restore / scoped-repair gate', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "DELTA > Pitcher Splits", delta_noop_restore_scoped_repair_gate: true, retained_stage_restore_before_queue: true, no_balanced_split_assumption: true })).run();
    return jsonResponse({ ok: true, data_ok: true, version, job, status: "queued", request_id: requestId, chain_id: chainId, visible_button: "DELTA > Pitcher Splits", queued_job_key: "base-pitcher-splits", queued_worker_name: "alphadog-v2-base-pitcher-splits", delta_noop_restore_scoped_repair_gate: true, retained_stage_restore_before_queue: true, no_balanced_split_assumption: true, outcome_rows_staged_non_authoritative: true, backend_scheduled_continuation: true, auto_pump_triggered: false, browser_auto_pump: false, note: "Queued v0.5.1 delta no-op / retained-restore / scoped-repair gate. It must not run a full 740-player refresh. Tap ORCHESTRATOR > Wake once to dispatch." });
  }
