const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.58-bullpen-delta-ready";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_base_bullpen_history") {
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-bullpen-history', 'alphadog-v2-base-bullpen-history', 'Base', 'base', 'Base Bullpen History Delta Update v0.4.0', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_bullpen_history', 'INFO', 'queued_base_bullpen_history_delta_update', 'Queued Base Bullpen History v0.4.0 delta_update / no-op / retained-stage repair gate', ?, CURRENT_TIMESTAMP)"
