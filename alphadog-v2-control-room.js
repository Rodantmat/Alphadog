const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.60-wake-direct-proxy-restore";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_base_bullpen_history") {
    ...
    if (existing) {
      ...
      return jsonResponse({
        ...
        mode: "delta_update",
        backend_scheduled_continuation: true,
        backend_direct_proxy_hot_start: true,
  ...
  if (job === "orchestrator_tick") {
    ...
    const payload = {
      source: "control_room_manual_wake_direct_proxy",
      max_jobs: 5,
      wake_only: true,
      auto_pump: true,
      pump: true,
    ...
    return jsonResponse({
      ...
      version,
      job,
      orchestrator_mode: "control_room_direct_backend_wake_proxy",
      status: "backend_pump_completed_or_started",
      backend_pump_launched: true,
      backend_direct_proxy_awaited: true,
      ...
      no_browser_loop: true,
      cron_rescue_only: true,
      note: "Wake now directly proxies one bounded backend orchestrator pump through Control Room and waits for the pump response. The browser still does not fetch the orchestrator directly and does not run a loop; cron is fallback rescue only.",
