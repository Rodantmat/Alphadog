const SYSTEM_VERSION = "alphadog-v2-control-room-v1.5.24-base-hitter-enqueue-route-fix";
  ...
  if (job === "orchestrator_enqueue_base_hitter_game_logs") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Base Hitter Game Logs queue row found. Control Room queues only. Orchestrator scheduled backend/cron owns continuation; ORCHESTRATOR > Wake is manual backend testing only."
      });
    }
    ...
    const input = {
      ...
      mode: "base_backfill",
      created_at: now,
      worker_scope: "schema_source_lock_probe_only_v0_1_0",
      ...
      no_old_production_touch: true,
      backend_scheduled_continuation: true,
      continuation_strategy: "orchestrator_owned_bounded_chunks_with_persisted_cursor",
      one_active_run_only: true,
      no_browser_auto_pump: true,
      no_control_room_to_orchestrator_fetch: true,
      no_manual_repeated_wake_required: true,
      probe_player_limit: 3,
      max_requests_per_tick: 3
    };

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-hitter-game-logs', 'alphadog-v2-base-hitter-game-logs', 'Base', 'base', 'Base Hitter Game Logs Source-Lock Probe', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_hitter_game_logs', 'INFO', 'queued_base_hitter_game_logs_probe', 'Queued exact Base Hitter Game Logs schema/source-lock probe job', ?, CURRENT_TIMESTAMP)"
    ...
    return jsonResponse({
      ...
      base_backfill_cutoff_date: "2026-05-18",
      delta_reserved_start_date: "2026-05-19",
      schema_source_lock_probe_only_v0_1_0: true,
      backend_scheduled_continuation: true,
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued in CONTROL_DB only. Browser may close now. Orchestrator scheduled backend/cron owns continuation; ORCHESTRATOR > Wake is manual backend testing only. v0.1.0 stages source-shape probe rows only and performs no live promotion."
    });
  }
