const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.184-final-score-v1-active-route-fix";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_final_score_v1") {
    ...
    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-score-audit','final-score-v1','11 Scoring','scoring','Final Score V1 Trust Readiness',1,'https://alphadog-v2-score-audit.rodolfoaamattos.workers.dev','SCORE_AUDIT_WORKER',1,'Same queued score-audit slot is intercepted by orchestrator direct Final Score V1. Reads SCORE_DB.hit_probability_v2_current and score_enrichment_current and writes only final_score_v1_* tables. Does not mutate HP V2, scoring_engine_current, score_final_board_current, prepared board, source boards, ranks, or live/play gates.',CURRENT_TIMESTAMP)"
    ).run();
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'final-score-v1', 'alphadog-v2-score-audit', '11 Scoring', 'scoring', 'Final Score V1 Trust Readiness', 'pending', 8, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_final_score_v1', 'Queued exact Final Score V1 trust/readiness job through score-audit slot', ?, CURRENT_TIMESTAMP)"
