const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.164-past-date-gap-contract-guard";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_scoring_final_board") {
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = 'scoring-final-board' AND status IN ('pending','running') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();

    if (existing) {
      return jsonResponse({ ok: true, data_ok: true, version, job, status: "already_queued", request_id: existing.request_id, existing, visible_button: "SCORING > Final Board", note: "Existing Scoring Final Board queue row found. Use ORCHESTRATOR > Wake / Logs / Queue to continue or inspect." });
    }

    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-score-audit','scoring-final-board','11 Scoring','scoring','Scoring Final Board Generator',1,'https://alphadog-v2-score-audit.rodolfoaamattos.workers.dev','SCORE_AUDIT_WORKER',1,'Generates SCORE_DB.scoring_final_board_current from latest certified STRICT_B simulation live_playable rows only. Replaces current table, archives previous current, no candidate board, no old prop scores.',CURRENT_TIMESTAMP)"
    ).run();

    const requestId = "scoring_final_board_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_scoring_final_board_" + Date.now().toString(36);
    const input = { source: "control_room", visible_button: "SCORING > Final Board", mode: "scoring_final_board_generate", created_at: now, logical_worker_name: "alphadog-v2-scoring-final-board", deployed_worker_slot: "alphadog-v2-score-audit", exact_worker_only: true, final_board_generation: true, source_profile: "STRICT_B", reads_latest_certified_simulation_shadow: true, writes_score_db_final_board_current: true, replaces_final_board_current: true, archives_previous_final_board_current: true, no_score_recalculation: true, no_true_hit_probability_claims: true, no_candidate_board_write: true, no_old_prop_scores_write: true, no_old_production_touch: true };

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'scoring-final-board', 'alphadog-v2-score-audit', '11 Scoring', 'scoring', 'Scoring Final Board Generator', 'pending', 7, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_scoring_final_board', 'Queued Scoring Final Board generation job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, job, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "SCORING > Final Board", queued_job_key: "scoring-final-board", queued_worker_name: "alphadog-v2-score-audit", logical_worker_name: "alphadog-v2-scoring-final-board", final_board_current_table: "SCORE_DB.scoring_final_board_current" })).run();

    return jsonResponse({ ok: true, data_ok: true, version, job, status: "queued", request_id: requestId, chain_id: chainId, visible_button: "SCORING > Final Board", queued_job_key: "scoring-final-board", queued_worker_name: "alphadog-v2-score-audit", logical_worker_name: "alphadog-v2-scoring-final-board", mode: "scoring_final_board_generate", final_board_generation: true, final_board_current_table: "SCORE_DB.scoring_final_board_current", note: "Queued Final Board generation. Tap ORCHESTRATOR > Wake." });
  }
