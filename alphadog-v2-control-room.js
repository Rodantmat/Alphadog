const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.125-daily-player-availability-button";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_daily_lineups") {
    ...
    const input = {
      ...
      writes_daily_lineups_context: false,
      no_calendar_rebuild: true,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'daily-lineups', 'alphadog-v2-daily-lineups', 'Daily', 'daily', 'Daily Lineups Source Probe', 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_daily_lineups', 'INFO', 'queued_daily_lineups_source_probe', 'Queued Daily Lineups source-probe worker', ?, CURRENT_TIMESTAMP)"
    ...
    return jsonResponse({
      ...
      note: "Queued Daily Lineups SOURCE PROBE only. This reads TEAM_DB calendar + SCORE_DB prepared safe rows and probes official MLB /boxscore structure. It does not write lineup tables, mutate boards, score, rank, or write final candidates. Tap ORCHESTRATOR > Wake."
    });
  }
