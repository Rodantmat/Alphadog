const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.167-market-scoring-full-run";
  ...
  if (job === "orchestrator_enqueue_market_scoring_full_run") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        note: "Existing Market/Scoring Full Run parent row found. Use ORCHESTRATOR > Wake / Logs / Queue to continue or inspect."
      });
    }
    ...
    const input = {
      ...
      note: "Market/Factors/Matrix/Scoring/Final Board backend parent chain. Board refresh is intentionally not included."
    };
    ...
    if (ctx && ctx.waitUntil) {
      ctx.waitUntil(callOrchestrator(env, "/pump", { source:"control_room_market_scoring_full_run_enqueue", max_cycles:20, max_jobs_per_cycle:1, max_ms:55000, max_pump_chains:30, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true }));
    }
    ...
    return jsonResponse({
      ...
      no_source_board_mutation: true,
      no_prepared_board_mutation: true,
      note: "Queued Market/Factors/Matrix/Scoring/Final Board full run. Tap ORCHESTRATOR > Wake if the backend pump does not finish it automatically."
    });
  }
