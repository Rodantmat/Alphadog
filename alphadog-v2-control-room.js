const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.65-hitter-metrics-visible-button-fix";
...
<!-- alphadog-v2-control-room-v1.6.65-hitter-metrics-visible-button-fix -->
<style>
...
<div id=\"versionTag\">alphadog-v2-control-room-v1.6.65-hitter-metrics-visible-button-fix</div>
...
</div><div class=\"small\">Dedicated visible route sentinel for Base Hitter Metrics v0.1.0 audit only. No promotion, no MLB calls, no scoring, no board mutation.</div></div>
...
function debugConfig(){
  setOutput(\"DEBUG > Config\",{base:BASE,auto_slate:autoSlateContext(),access_mode:\"single-user-admin-token-disabled\", orchestrator_mode:\"backend-scheduled-continuation\",wake_binding_required:\"ORCHESTRATOR_WORKER -> alphadog-v2-orchestrator\",public_url_fallback_disabled:true,version:\"alphadog-v2-control-room-v1.6.65-hitter-metrics-visible-button-fix\", visible_hitter_metrics_button_required:true, hitter_metrics_route:\"orchestrator_enqueue_base_hitter_metrics\", ui_sentinel:\"BASE_HITTER_METRICS_VISIBLE_BUTTON_FIX_V1_6_65\"});
}
  ...
  if (job === "orchestrator_enqueue_base_hitter_metrics") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Base Hitter Metrics audit queue row found. Tap ORCHESTRATOR > Wake once only if it has not processed. Browser does not run the job."
      });
    }

    const requestId = "base_hitter_metrics_audit_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_base_hitter_metrics_audit_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Hitter Metrics",
      mode: "schema_formula_input_audit",
      created_at: now,
      worker_scope: "v0_1_0_schema_formula_input_config_audit_only",
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-hitter-metrics', 'alphadog-v2-base-hitter-metrics', 'Base', 'base', 'Base Hitter Metrics v0.1.0 Schema/Formula/Input Audit', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_hitter_metrics', 'INFO', 'queued_base_hitter_metrics_schema_formula_input_audit', 'Queued exact Base Hitter Metrics v0.1.0 schema/formula/input/config audit only job', ?, CURRENT_TIMESTAMP)"
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued v0.1.0 audit only. Tap ORCHESTRATOR > Wake once to dispatch. No live metric promotion, no source table mutation, no MLB calls, no scoring."
    });
  }
