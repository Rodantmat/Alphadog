const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.19-base-hitter-splits-source-probe";
  ...
  if (job === "orchestrator_enqueue_base_hitter_splits") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Base Hitter Splits source-probe queue row found. Use ORCHESTRATOR > Wake once as a test accelerator; browser does not run the job."
      });
    }

    const requestId = "base_hitter_splits_probe_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_base_hitter_splits_probe_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Hitter Splits",
      mode: "source_shape_probe",
      created_at: now,
      worker_scope: "schema_source_lock_probe_v0_1_0",
      source_name: "MLB StatsAPI people statSplits hitting endpoint with sitCodes=vl,vr",
      source_key: "mlb_statsapi_people_statSplits_hitting_sitCodes_vl_vr_v0_1_0",
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-hitter-splits', 'alphadog-v2-base-hitter-splits', 'Base', 'base', 'Base Hitter Splits Source-Lock Probe v0.1.0', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_hitter_splits', 'INFO', 'queued_base_hitter_splits_source_probe', 'Queued exact Base Hitter Splits v0.1.0 schema/source-lock probe job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"BASE > Hitter Splits", source_probe_only:true, no_live_promotion:true, sample_size:3})).run();
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued v0.1.0 source-lock probe in CONTROL_DB. Tap ORCHESTRATOR > Wake once to dispatch. No live promotion, no full mining, no delta."
    });
  }
