const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.51-base-bullpen-history-source-probe";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_base_bullpen_history") {
    ...
    if (existing) {
      if (ctx && typeof ctx.waitUntil === "function") {
        ctx.waitUntil(callOrchestrator(env, "/pump", {
          source: "control_room_base_bullpen_history_existing_hot_continue",
          max_cycles: 8,
      ...
      return jsonResponse({
        ...
        mode: "source_lock_probe",
        no_live_promotion: true,
        backend_scheduled_continuation: true,
        auto_pump_triggered: true,
        browser_auto_pump: false,
        note: "Existing Base Bullpen History probe queue row found. Control Room launched one backend orchestrator pump as a hot-continuation nudge. Browser does not run the job. ORCHESTRATOR > Wake is rescue only."
      });
    }

    const requestId = "bullpen_history_source_probe_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_bullpen_history_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Bullpen History",
      mode: "source_lock_probe",
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-bullpen-history', 'alphadog-v2-base-bullpen-history', 'Base', 'base', 'Base Bullpen History Source Probe v0.1.0', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_bullpen_history', 'INFO', 'queued_base_bullpen_history_source_probe', 'Queued Base Bullpen History v0.1.0 source-lock probe only', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "BASE > Bullpen History", mode: "source_lock_probe", sample_date: "2026-05-18", sample_limit: 3, no_live_promotion: true, no_full_base_backfill: true, no_delta_update_execution: true, no_daily_bullpen_availability: true, backend_hot_continuation: true })).run();

    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(callOrchestrator(env, "/pump", {
        source: "control_room_base_bullpen_history_source_probe_auto_start",
        max_cycles: 8,
        max_jobs_per_cycle: 1,
        max_ms: 45000,
        pump_depth: 0,
    ...
    return jsonResponse({
      ...
      note: "Queued Base Bullpen History v0.1.0 source-lock probe only. Control Room launches one backend orchestrator pump via ctx.waitUntil. Browser may close now. ORCHESTRATOR > Wake is rescue only. No live promotion, no full base backfill, no delta, no Daily Bullpen Availability, no scoring, no board mutation."
    });
  }
