const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.46-base-starter-history-delta-retained-stage";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_base_starter_history") {
    ...
    if (existing) {
      ...
      return jsonResponse({
        ...
        delta_update_retained_stage: true,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-starter-history', 'alphadog-v2-base-starter-history', 'Base', 'base', 'Base Starter History Delta v0.4.0', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_starter_history', 'INFO', 'queued_base_starter_history_delta_update', 'Queued Base Starter History v0.4.0 delta update with retained stage', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "BASE > Starter History", mode: "delta_update", delta_start_date: "2026-05-19", delta_end_date: new Date().toISOString().slice(0,10), base_backfill_cutoff_date: "2026-05-18", no_live_promotion: false, backend_hot_continuation: true, stage_retained: true })).run();

    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(callOrchestrator(env, "/pump", {
        source: "control_room_base_starter_history_delta_auto_start",
        max_cycles: 18,
        max_jobs_per_cycle: 1,
        max_ms: 70000,
        pump_depth: 0,
    ...
    return jsonResponse({
      ...
      note: "Queued Base Starter History v0.4.0 delta update with retained stage. Control Room launches one backend orchestrator pump via ctx.waitUntil. Browser may close now. ORCHESTRATOR > Wake is rescue only. No scoring, no board mutation."
    });
  }
  ...
  if (job === "orchestrator_enqueue_delta_team_game_logs") {
    ...
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "DELTA > Team Game Logs", mode: "delta_update", delta_start_date: "2026-05-19", delta_end_date_policy: "auto_latest_completed_final_mlb_game_date", retained_stage_required: true })).run();
