const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.68-base-hitter-metrics-base-stage-route";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_base_hitter_metrics") {
    ...
    const input = {
      ...
      created_at: now,
      worker_scope: "v0_3_0_base_rebuild_stage_only",
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-hitter-metrics', 'alphadog-v2-base-hitter-metrics', 'Base', 'base', 'Base Hitter Metrics v0.3.0 Base-Rebuild Stage-Only', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_hitter_metrics', 'INFO', 'queued_base_hitter_metrics_base_rebuild_stage_only', 'Queued exact Base Hitter Metrics v0.3.0 base-rebuild stage-only job', ?, CURRENT_TIMESTAMP)"
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued v0.3.0 base-rebuild stage-only. Tap ORCHESTRATOR > Wake to dispatch/continue. Stage rows only; no live metric promotion, no source table mutation, no MLB calls, no scoring."
    });
  }
