const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.43-base-starter-history-stage-only";
  ...
  if (job === "orchestrator_enqueue_base_starter_history") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Base Starter History stage-only queue row found. Tap ORCHESTRATOR > Wake once only if needed; backend continuation owns dispatch."
      });
    }
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_starter_history', 'INFO', 'queued_base_starter_history_stage_only_base', 'Queued Base Starter History v0.2.0 stage-only base backfill', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "BASE > Starter History", mode: "base_backfill_stage_only", base_start_date: "2026-03-01", base_backfill_cutoff_date: "2026-05-18", max_games_per_tick: 15, no_live_promotion: true })).run();
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued Base Starter History v0.2.0 stage-only base backfill. Tap ORCHESTRATOR > Wake once to dispatch; backend continuation/cron owns remaining chunks. No live starter_history promotion, no delta execution."
    });
  }
