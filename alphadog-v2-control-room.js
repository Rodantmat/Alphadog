...
async function v12OrchestratorLocalBridge(job, env) {
  ...
  if (job === "orchestrator_enqueue_prizepicks_github_board") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "ORCHESTRATOR > PP Board"
      });
    }
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'prizepicks-github-board', 'alphadog-v2-prizepicks-github-board', 'Market', 'market', 'PrizePicks GitHub Board Source Shape Staging', 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ...
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"ORCHESTRATOR > PP Board"})).run();
  ...
  if (job === "orchestrator_enqueue_static_teams") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "STATIC > Static Teams"
      });
    }
    ...
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"STATIC > Static Teams"})).run();
  ...
  if (job === "orchestrator_tick") {
    ...
    return jsonResponse({
      ...
      version,
      job,
      orchestrator_mode: "wake_now_plus_cron_fallback",
      status: "use_orchestrator_wake_button",
      note: "The visible button is now ORCHESTRATOR > Wake. It calls the real alphadog-v2-orchestrator /tick endpoint once to skip waiting for cron. It does not start a browser loop.",
