const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.170-market-full-engine-order-and-rescue";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_scoring_engine") {
    ...
    const input = {
      source: "control_room",
      visible_button: "SCORING > Simulation",
      mode: "scoring_engine_simulation_shadow_strict_c_realistic_v3_2",
      created_at: now,
      logical_worker_name: "alphadog-v2-scoring-engine-simulation",
      deployed_worker_slot: "alphadog-v2-score-audit",
      exact_worker_only: true,
      simulation_only: true,
      primary_profile: "STRICT_C_REALISTIC_V3_2",
      ...
      no_true_hit_probability_claims: true,
      strict_c_realistic_v3_2_76_plus_floor: 50,
      strict_c_realistic_v3_2_76_plus_ceiling: 300,
      strict_c_realistic_v3_2_82_plus_ceiling: 75,
      strict_c_realistic_v3_2_88_plus_exact: 0,
      regular_lines_two_sided: true,
      goblin_demon_more_only: true,
      goblin_demon_under_blocker: "GOBLIN_DEMON_UNDER_NOT_SELECTABLE",
      no_candidate_board_write: true,
    ...
    ).bind(requestId, job, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"SCORING > Simulation", queued_job_key:"scoring-engine-simulation", queued_worker_name:"alphadog-v2-score-audit", logical_worker_name:"alphadog-v2-scoring-engine-simulation", primary_profile:"STRICT_C_REALISTIC_V3_2", comparison_profile:"HYBRID_CONTROL", shadow_only:true, no_production_mutation:true, no_ranking:true, no_final_board:true})).run();
  ...
  if (job === "orchestrator_enqueue_score_final_board") {
    ...
    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-score-final-board','score-final-board','11 Scoring','scoring','Score Final Board Generator',1,'https://alphadog-v2-score-final-board.rodolfoaamattos.workers.dev','SCORE_FINAL_BOARD_WORKER',1,'Generates SCORE_DB.score_final_board_current from latest completed STRICT_C_REALISTIC_V3_2 scoring simulation live rows. No external calls, no raw board mutation, no simulation mutation.',CURRENT_TIMESTAMP)"
    ).run();
    ...
    return jsonResponse({
      ...
      note: "Queued Score Final Board generator. It reads latest completed STRICT_C_REALISTIC_V3_2 simulation live rows and replaces SCORE_DB.score_final_board_current for the final UI. Tap ORCHESTRATOR > Wake."
    });
  }
