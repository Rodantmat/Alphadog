const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.38-team-game-logs-delta-update";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_delta_team_game_logs") {
    ...
    const input = {
      ...
      visible_button: "DELTA > Team Game Logs",
      mode: "delta_update",
      worker_scope: "v0.3.0_delta_update_retained_stage_2026_05_19",
      created_at: now,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-team-game-logs', 'alphadog-v2-base-team-game-logs', 'Base', 'delta', 'Delta Team Game Logs v0.3.0', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_delta_team_game_logs', 'INFO', 'queued_delta_team_game_logs_update', 'Queued exact Delta Team Game Logs v0.3.0 delta_update for 2026-05-19', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "DELTA > Team Game Logs", mode: "delta_update", delta_start_date: "2026-05-19", delta_end_date: "2026-05-19", retained_stage_required: true })).run();
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued Delta Team Game Logs v0.3.0. Tap ORCHESTRATOR > Wake once to dispatch. Stage should be retained after promotion for no-op/restore testing."
    });
  }
