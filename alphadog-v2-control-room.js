const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.74-hitter-metrics-promote-delta-route";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_base_hitter_metrics") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Base Hitter Metrics queue row found. Tap ORCHESTRATOR > Wake once only if it has not processed. Browser does not run the job."
      });
    }

    const requestId = "base_hitter_metrics_promote_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_base_hitter_metrics_promote_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Hitter Metrics",
      mode: "snapshot_promote_retain",
      created_at: now,
      worker_scope: "v0_4_0_snapshot_promote_retain",
      source_name: "D1 live promotion from certified hitter_metric_snapshot_stage batch",
      source_key: "d1_hitter_metric_snapshot_stage_promote_retain",
      source_season: 2026,
      ...
      no_old_production_touch: true,
      retain_snapshot_stage: true,
      backend_scheduled_continuation: true,
      no_browser_auto_pump: true,
      wake_once_for_test_only: true,
      snapshot_promote_retain: true,
      source_snapshot_prep_required: true
    };

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-hitter-metrics', 'alphadog-v2-base-hitter-metrics', 'Base', 'base', 'Base Hitter Metrics v0.4.0 Snapshot Promote Retain', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_hitter_metrics', 'INFO', 'queued_base_hitter_metrics_snapshot_promote_retain', 'Queued exact Base Hitter Metrics v0.4.0 snapshot promote-retain job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"BASE > Hitter Metrics", retain_stage:true, no_external_mlb_calls:true, no_scoring:true})).run();
    ...
    return jsonResponse({
      ...
      v0_4_0_snapshot_promote_retain: true,
      backend_scheduled_continuation: true,
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued v0.4.0 snapshot promote-retain. Tap ORCHESTRATOR > Wake to dispatch. Promotes certified snapshot stage to live hitter_metric_snapshots, retains stage, no source mutation, no MLB calls, no scoring."
    });
  }
  ...
  if (job === "orchestrator_enqueue_delta_hitter_metrics") {
    ...
    if (existing) {
      return jsonResponse({ ok: true, data_ok: true, version, job, status: "already_queued", request_id: existing.request_id, existing, visible_button: "DELTA > Hitter Metrics", backend_scheduled_continuation: true, note: "Existing Hitter Metrics queue row found. Do not queue a concurrent metrics job." });
    }
    ...
    const input = {
      ...
      created_at: now,
      worker_scope: "v0_4_0_snapshot_delta_gate",
      source_name: "No-op/promote gate for latest certified hitter metric snapshot batch",
      config_profile_id: "hitter_metrics_neutral_v0_3_0_stage_only",
      formula_version: "hitter_metrics_formula_v0_3_0_stage_only",
      no_hitter_game_log_mutation: true,
      no_hitter_split_mutation: true,
      ...
      backend_scheduled_continuation: true,
      no_browser_auto_pump: true,
      snapshot_delta_gate: true
    };

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-hitter-metrics', 'alphadog-v2-base-hitter-metrics', 'Delta', 'delta', 'Delta Hitter Metrics v0.4.0 Snapshot Gate', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_delta_hitter_metrics', 'INFO', 'queued_delta_hitter_metrics_snapshot_gate', 'Queued exact Delta Hitter Metrics v0.4.0 snapshot no-op/promote gate job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"DELTA > Hitter Metrics", no_external_mlb_calls:true, no_scoring:true})).run();

    return jsonResponse({ ok: true, data_ok: true, version, job, status: "queued", request_id: requestId, chain_id: chainId, visible_button: "DELTA > Hitter Metrics", queued_job_key: "base-hitter-metrics", queued_worker_name: "alphadog-v2-base-hitter-metrics", v0_4_0_snapshot_delta_gate: true, backend_scheduled_continuation: true, auto_pump_triggered: false, browser_auto_pump: false, note: "Queued v0.4.0 snapshot delta gate. It no-ops when live snapshot is current, or promotes the latest certified snapshot stage batch if needed. No MLB calls, no source mutation, no scoring." });
  }
