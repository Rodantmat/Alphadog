const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.25-base-pitcher-splits-stage-only";
  ...
  if (job === "orchestrator_enqueue_base_pitcher_splits") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Base Pitcher Splits stage-only queue row found. Do not repeatedly Wake; backend continuation owns any continuation."
      });
    }

    const requestId = "base_pitcher_splits_stage_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_base_pitcher_splits_stage_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Pitcher Splits",
      mode: "base_backfill_stage_only_full_universe",
      created_at: now,
      worker_scope: "base_stage_only_full_universe_v0_2_0",
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_pitcher_splits', 'INFO', 'queued_base_pitcher_splits_stage_only_full_universe', 'Queued exact Base Pitcher Splits v0.2.0 full-universe stage-only job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"BASE > Pitcher Splits", stage_only_full_universe:true, rows_promoted_expected:0, chunk_size:20})).run();
