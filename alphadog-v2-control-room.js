const SYSTEM_VERSION = "alphadog-v2-control-room-v0.7";
const CONTROL_ROOM_HTML = "<!DOCTYPE html>\n<html>\n<head>\n<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">\n<title>AlphaDog V2 Control Room</title>\n<!-- alphadog-v2-control-room-v0.7 - compact phone UI + orchestrator shell buttons -->\n<style>\n:root{--bg:#0b0f14;--line:#30363d;--green:#00ff88;--white:#fff;--muted:#aaa;--debug:#8957e5;--check:#238636;--audit:#0f766e;--sql:#d29922;--clean:#da3633;--orch:#0969da}\n*{box-sizing:border-box}\nbody{background:var(--bg);color:var(--green);font-family:monospace;padding:8px;margin:0;max-width:100vw;overflow-x:hidden}\nh2{font-size:18px;color:var(--green);letter-spacing:.045em;margin:10px 0 8px 0}\nh3{color:var(--white);font-size:12px;letter-spacing:.045em;margin:7px 0}\n.section{border-top:1px solid var(--line);padding-top:7px;margin-top:9px}\n.grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:3px;width:100%}\nbutton{min-width:0;width:100%;padding:4px 1px;font-size:9.5px;border:0;border-radius:5px;background:#1f6feb;color:var(--white);min-height:31px;white-space:normal;overflow:hidden;text-overflow:clip;line-height:1.0;display:flex;align-items:center;justify-content:center;text-align:center;word-break:normal}\n.clean{background:var(--clean)}.check{background:var(--check)}.sql{background:var(--sql)}.debug{background:var(--debug)}.audit{background:var(--audit)}.orch{background:var(--orch)}\n.copy{background:var(--debug);width:100%;margin-top:6px;min-height:34px;font-size:10.5px}\ninput,textarea{width:100%;box-sizing:border-box;background:#111;color:var(--green);border:1px solid var(--line);border-radius:6px;margin-top:6px;padding:7px;font-size:12px}\ntextarea{height:82px}\npre{background:#000;color:var(--green);padding:8px;margin-top:8px;overflow:auto;max-height:420px;min-height:150px;border:1px solid var(--line);border-radius:6px;white-space:pre-wrap;word-break:break-word;font-size:11px}\n.status{background:#111;color:var(--white);padding:7px;border-radius:6px;border:1px solid var(--line);margin:7px 0;white-space:pre-wrap;font-size:11px}\n.small,.muted{font-size:10px;color:var(--muted)}\n#versionTag{color:#9ae6b4;font-family:inherit;font-size:12px;font-weight:800;letter-spacing:.035em;margin-top:5px;margin-bottom:8px}\ntextarea#sqlInput, textarea{user-select:text;-webkit-user-select:text;pointer-events:auto}\n@media (max-width:430px){body{padding:8px}.grid{grid-template-columns:repeat(5,minmax(0,1fr));gap:3px}button{font-size:9px;min-height:30px;padding:3px 1px;border-radius:5px}h2{font-size:17px}h3{font-size:11px}.small,.muted{font-size:9.5px}}\n@media (max-width:370px){button{font-size:8.5px;min-height:29px}.grid{gap:2px}}\n</style>\n</head>\n<body>\n<h2>ALPHADOG CONTROL ROOM</h2>\n<div id=\"versionTag\">alphadog-v2-control-room-v0.7</div>\n<div class=\"small\">PT Now: <span id=\"ptNowLabel\"></span></div>\n<div class=\"small\">Slate: AUTO by game date/time. No manual slate override.</div>\n<div class=\"status\" id=\"status\">READY</div>\n<div class=\"section\"><h3>DEBUG</h3><div class=\"grid\">\n<button class=\"debug\" type=\"button\" onclick=\"debugConfig()\">Config</button><button class=\"debug\" type=\"button\" onclick=\"health()\">Health</button><button class=\"debug\" type=\"button\" onclick=\"diagnostic()\">Diag</button><button class=\"debug\" type=\"button\" onclick=\"testSQL()\">SQL</button><button class=\"debug\" type=\"button\" onclick=\"loadConfig()\">Reload</button>\n</div></div>\n<div class=\"section\"><h3>V2 BOOTSTRAP / SCHEMA</h3><div class=\"grid\">\n<button class=\"check\" type=\"button\" onclick=\"runJobButton('V2 BOOTSTRAP / SCHEMA > Schema','v2_schema_status')\">Schema</button><button class=\"check\" type=\"button\" onclick=\"runJobButton('V2 BOOTSTRAP / SCHEMA > Workers','v2_worker_registry')\">Workers</button><button class=\"check\" type=\"button\" onclick=\"runJobButton('V2 BOOTSTRAP / SCHEMA > Config','v2_config_summary')\">Config</button><button class=\"audit\" type=\"button\" onclick=\"runJobButton('V2 BOOTSTRAP / SCHEMA > Phases','v2_phase_state')\">Phases</button><button class=\"audit\" type=\"button\" onclick=\"runJobButton('V2 BOOTSTRAP / SCHEMA > Markets','v2_market_sources')\">Markets</button><button class=\"audit\" type=\"button\" onclick=\"runJobButton('V2 BOOTSTRAP / SCHEMA > Props','v2_prop_taxonomy')\">Props</button><button class=\"audit\" type=\"button\" onclick=\"runJobButton('V2 BOOTSTRAP / SCHEMA > Certs','v2_certification_rules')\">Certs</button><button class=\"debug\" type=\"button\" onclick=\"runJobButton('V2 BOOTSTRAP / SCHEMA > Bindings','v2_bindings_check')\">Bindings</button>\n</div><div class=\"small\">V2 only. No mining, no scoring, no old production writes.</div></div>\n<div class=\"section\"><h3>ORCHESTRATOR</h3><div class=\"grid\">\n<button class=\"orch\" type=\"button\" onclick=\"runJobButton('ORCHESTRATOR > Status','orchestrator_status')\">Status</button><button class=\"orch\" type=\"button\" onclick=\"runJobButton('ORCHESTRATOR > Enqueue','orchestrator_enqueue_test')\">Enqueue</button><button class=\"orch\" type=\"button\" onclick=\"runJobButton('ORCHESTRATOR > Tick','orchestrator_tick')\">Tick</button><button class=\"orch\" type=\"button\" onclick=\"runJobButton('ORCHESTRATOR > Logs','orchestrator_logs')\">Logs</button><button class=\"orch\" type=\"button\" onclick=\"runJobButton('ORCHESTRATOR > Health','orchestrator_health')\">OHealth</button>\n</div><div class=\"small\">Buttons wake/schedule backend work only. No browser long loops.</div></div>\n<div class=\"section\"><h3>V2 SAFE ACTIONS</h3><div class=\"grid\">\n<button class=\"check\" type=\"button\" onclick=\"runJobButton('V2 SAFE ACTIONS > Queue','v2_queue_status')\">Queue</button><button class=\"check\" type=\"button\" onclick=\"runJobButton('V2 SAFE ACTIONS > Locks','v2_lock_status')\">Locks</button><button class=\"audit\" type=\"button\" onclick=\"runJobButton('V2 SAFE ACTIONS > Snap','v2_health_snapshot')\">Snap</button><button class=\"clean\" type=\"button\" onclick=\"runJobButton('V2 SAFE ACTIONS > Clear Q','v2_clear_open_queue')\">Clear Q</button>\n</div><div class=\"small\">Clear Q only changes v2 CONTROL_DB queue/lock state.</div></div>\n<div class=\"section\"><h3>MANUAL SQL</h3><div class=\"muted\">Output guard active: max 50 rows. Optional first line: -- db: CONFIG_DB</div><textarea id=\"sqlInput\" spellcheck=\"false\" autocomplete=\"off\" autocorrect=\"off\" autocapitalize=\"none\" inputmode=\"text\"></textarea><div class=\"grid\"><button class=\"sql\" type=\"button\" onclick=\"runManualSQL()\">Run</button><button class=\"debug\" type=\"button\" onclick=\"clearSqlInput()\">Clear</button><button class=\"debug\" type=\"button\" onclick=\"selectSqlInput()\">Select</button><button class=\"debug\" type=\"button\" onclick=\"loadExampleSQL()\">Example</button></div></div>\n<pre id=\"output\">Output will appear here.</pre><button class=\"copy\" type=\"button\" onclick=\"copyOutput()\">COPY OUTPUT</button>\n<script>\nconst BASE=\"https://alphadog-v2-control-room.rodolfoaamattos.workers.dev\";const JOB_URL=BASE+\"/tasks/run\";const SQL_URL=BASE+\"/debug/sql\";const HEALTH_URL=BASE+\"/health\";const DIAGNOSTIC_URL=BASE+\"/diagnostic\";const CONFIG_URL=\"https://raw.githubusercontent.com/Rodantmat/Alphadog/main/config.txt\";let TOKEN=\"\";let CONFIG_LOADED=false;\nfunction ptParts(){const parts=new Intl.DateTimeFormat(\"en-CA\",{timeZone:\"America/Los_Angeles\",year:\"numeric\",month:\"2-digit\",day:\"2-digit\",hour:\"2-digit\",minute:\"2-digit\",second:\"2-digit\",hour12:false}).formatToParts(new Date()),m={};parts.forEach(p=>m[p.type]=p.value);return {date:m.year+\"-\"+m.month+\"-\"+m.day,hour:Number(m.hour),time:m.hour+\":\"+m.minute+\":\"+m.second}}\nfunction updateClock(){const p=ptParts();document.getElementById(\"ptNowLabel\").textContent=p.date+\" \"+p.time}function autoSlateContext(){const p=ptParts();let band=\"same-day dominant\";if(p.hour>=12&&p.hour<20) band=\"split slate likely; workers resolve by game date/time\";if(p.hour>=20||p.hour<4) band=\"next-day dominant likely; workers resolve by game date/time\";return {mode:\"AUTO_BY_GAME_DATE_TIME\",pt_now:p,slate_band_hint:band,note:\"Control Room no longer manually overrides slate. Data workers must resolve pickability by actual game date/time and board availability.\"}}\nfunction setStatus(m){document.getElementById(\"status\").textContent=\"[\"+new Date().toLocaleTimeString()+\"] \"+m}function setOutput(l,o){document.getElementById(\"output\").textContent=\"ACTION: \"+l+\"\nTIME: \"+new Date().toISOString()+\"\n\n\"+(typeof o===\"string\"?o:JSON.stringify(o,null,2));window.scrollTo(0,document.body.scrollHeight)}function loading(l,e){setStatus(\"RUNNING: \"+l);setOutput(l,\"Loading...\"+(e?\"\n\"+e:\"\"))}function tokenFingerprint(){return TOKEN?{loaded:true,length:TOKEN.length,starts_with:TOKEN.slice(0,2),ends_with:TOKEN.slice(-2)}:{loaded:false,length:0}}\nasync function loadConfig(){try{loading(\"DEBUG > Reload\");const r=await fetch(CONFIG_URL+\"?t=\"+Date.now(),{cache:\"no-store\"});const t=await r.text();const m=t.match(/^\\s*TOKEN\\s*=\\s*(.+?)\\s*$/m);if(m&&m[1].trim()){TOKEN=m[1].trim().replace(/^[\"']|[\"']$/g,\"\");CONFIG_LOADED=true;setStatus(\"CONFIG LOADED\");setOutput(\"DEBUG > Reload\",{config_loaded:true,token_fingerprint:tokenFingerprint()})}else{TOKEN=\"\";CONFIG_LOADED=false;setStatus(\"CONFIG NOT LOADED\");setOutput(\"DEBUG > Reload\",{config_loaded:false,error:\"TOKEN line not found. Protected buttons need config.txt with TOKEN=your_admin_token.\",config_url:CONFIG_URL})}}catch(e){TOKEN=\"\";CONFIG_LOADED=false;setStatus(\"CONFIG LOAD FAILED\");setOutput(\"DEBUG > Reload\",{config_loaded:false,error:String(e),config_url:CONFIG_URL})}}\nfunction debugConfig(){setOutput(\"DEBUG > Config\",{base:BASE,config_url:CONFIG_URL,auto_slate:autoSlateContext(),token_set:Boolean(TOKEN),config_loaded:CONFIG_LOADED,token_fingerprint:tokenFingerprint(),version:\"alphadog-v2-control-room-v0.7\"})}\nasync function rawRequest(l,u,p){if(p!==null&&(!CONFIG_LOADED||!TOKEN)) await loadConfig();const h={\"Content-Type\":\"application/json\"};if(TOKEN){h[\"x-ingest-token\"]=TOKEN;h[\"x-admin-token\"]=TOKEN;}const o=p===null?{method:\"GET\",headers:h}:{method:\"POST\",headers:h,body:JSON.stringify(p)};try{const r=await fetch(u,o),txt=await r.text();let b;try{b=JSON.parse(txt)}catch(e){b=txt}return {http_status:r.status,body:b}}catch(e){return {ok:false,error:String(e),action:l,url:u}}}\nasync function requestJSON(l,u,p){loading(l);const r=await rawRequest(l,u,p);setStatus(\"DONE: \"+l+\" / HTTP \"+(r.http_status||\"ERR\"));setOutput(l,r);return r}function health(){requestJSON(\"DEBUG > Health\",HEALTH_URL,null)}function diagnostic(){requestJSON(\"DEBUG > Diag\",DIAGNOSTIC_URL,null)}function testSQL(){requestJSON(\"DEBUG > SQL\",SQL_URL,{sql:\"SELECT COUNT(*) AS control_worker_registry_rows FROM control_worker_registry;\",max_rows:50,max_chars:900})}function runJobButton(l,j){requestJSON(l,JOB_URL,{job:j,slate_mode:\"AUTO_BY_GAME_DATE_TIME\",auto_slate_context:autoSlateContext(),backend_only:true})}function runManualSQL(){requestJSON(\"MANUAL SQL > Run\",SQL_URL,{sql:document.getElementById(\"sqlInput\").value,max_rows:50,max_chars:900})}function copyOutput(){navigator.clipboard.writeText(document.getElementById(\"output\").textContent);setStatus(\"COPIED OUTPUT\")}function clearSqlInput(){const el=document.getElementById(\"sqlInput\");if(!el)return;el.removeAttribute(\"readonly\");el.removeAttribute(\"disabled\");el.value=\"\";el.focus()}function selectSqlInput(){const el=document.getElementById(\"sqlInput\");if(!el)return;el.removeAttribute(\"readonly\");el.removeAttribute(\"disabled\");el.focus();el.select()}function loadExampleSQL(){const el=document.getElementById(\"sqlInput\");el.value=\"SELECT COUNT(*) AS control_worker_registry_rows FROM control_worker_registry;\";el.focus();setStatus(\"EXAMPLE SQL LOADED\")}document.addEventListener(\"DOMContentLoaded\",()=>{const el=document.getElementById(\"sqlInput\");if(el){el.removeAttribute(\"readonly\");el.removeAttribute(\"disabled\");el.style.userSelect=\"text\";el.style.webkitUserSelect=\"text\";el.style.pointerEvents=\"auto\"}updateClock();setInterval(updateClock,1000);});\n</script>\n</body>\n</html>";
...
async function runJob(request, env) {
  ...
  try {
    ...
    if (job === "orchestrator_health") return jsonResponse({ok:true, data_ok:true, version:SYSTEM_VERSION, job, orchestrator: await callOrchestrator(env, "/health", null)});
    if (job === "orchestrator_status") return jsonResponse({ok:true, data_ok:true, version:SYSTEM_VERSION, job, orchestrator: await callOrchestrator(env, "/status", null)});
    if (job === "orchestrator_enqueue_test") return jsonResponse({ok:true, data_ok:true, version:SYSTEM_VERSION, job, orchestrator: await callOrchestrator(env, "/enqueue-test", {source:"control_room", backend_only:true})});
    if (job === "orchestrator_tick") return jsonResponse({ok:true, data_ok:true, version:SYSTEM_VERSION, job, orchestrator: await callOrchestrator(env, "/tick", {source:"control_room", backend_only:true, max_jobs:1})});
    if (job === "orchestrator_logs") return jsonResponse({ok:true, data_ok:true, version:SYSTEM_VERSION, job, orchestrator: await callOrchestrator(env, "/logs", null)});
