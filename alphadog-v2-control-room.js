const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.69-base-hitter-metrics-performance-route";
  ...
  if (job === "orchestrator_enqueue_base_hitter_metrics") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Base Hitter Metrics base-stage queue row found. Tap ORCHESTRATOR > Wake once only if it has not processed. Browser does not run the job."
      });
    }

    const requestId = "base_hitter_metrics_base_stage_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_base_hitter_metrics_base_stage_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Hitter Metrics",
      mode: "base_rebuild_stage_only",
      created_at: now,
      worker_scope: "v0_3_1_base_stage_performance_tune",
      source_name: "D1 derived from STATS_HITTER_DB.hitter_game_logs and STATS_HITTER_DB.hitter_splits",
      source_key: "d1_hitter_game_logs_hitter_splits",
      source_season: 2026,
      ...
      v0_3_1_base_stage_performance_tune: true,
      all_eligible_hitters: true
    };

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-hitter-metrics', 'alphadog-v2-base-hitter-metrics', 'Base', 'base', 'Base Hitter Metrics v0.3.1 Performance-Tuned Base-Stage', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_hitter_metrics', 'INFO', 'queued_base_hitter_metrics_base_stage_performance_tune', 'Queued exact Base Hitter Metrics v0.3.1 performance-tuned base-rebuild stage-only job', ?, CURRENT_TIMESTAMP)"
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued v0.3.1 performance-tuned base-rebuild stage-only. Tap ORCHESTRATOR > Wake to dispatch/continue. Stage rows only; no live metric promotion, no source table mutation, no MLB calls, no scoring."
    });
  }
