const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.81-base-pitcher-metrics-full-stage";
  ...
  if (job === "orchestrator_enqueue_base_pitcher_metrics") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Base Pitcher Metrics full-stage queue row found. Tap ORCHESTRATOR > Wake until the queue completes. Browser does not run the job."
      });
    }

    const requestId = "base_pitcher_metrics_base_stage_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_base_pitcher_metrics_base_stage_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Pitcher Metrics",
      mode: "base_rebuild_stage_only",
      created_at: now,
      worker_scope: "v0_3_0_full_granular_stage_only",
      source_name: "D1 audit from certified STATS_PITCHER_DB.pitcher_game_logs and STATS_PITCHER_DB.pitcher_splits",
      source_key: "d1_pitcher_game_logs_and_splits_base_stage",
      source_season: 2026,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-pitcher-metrics', 'alphadog-v2-base-pitcher-metrics', 'Base', 'base', 'Base Pitcher Metrics v0.3.0 Full Granular Stage Only', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_pitcher_metrics', 'INFO', 'queued_base_pitcher_metrics_v0_3_0_full_stage_only', 'Queued exact Base Pitcher Metrics v0.3.0 full granular stage-only job', ?, CURRENT_TIMESTAMP)"
    ...
    return jsonResponse({
      ...
      v0_3_0_base_rebuild_stage_only: true,
      backend_scheduled_continuation: true,
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued v0.3.0 full granular stage-only. Tap ORCHESTRATOR > Wake until pending/running is empty. Stage rows only; no live metric promotion, no source table mutation, no MLB calls, no scoring."
    });
  }
