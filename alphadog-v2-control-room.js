const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.202-final-board-v3-source-forward";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_expansion_baseline_v2_heb") {
    ...
    const input = {
      source:"control_room", visible_button:"EXPANSION > V2 HEB Baseline", mode:"baseline_v2_heb", expansion_mode:"baseline_v2_heb", v2_chunk_size:8, created_at:now,
    ...
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"EXPANSION > V2 HEB Baseline", queued_job_key:"expansion-baseline-v2", queued_worker_name:"alphadog-v2-phase3a-first-inning-pitcher-context", mode:"baseline_v2_heb", v2_chunk_size:8, parallel_v2_only:true, no_current_baseline_mutation:true, backend_hot_continue_triggered:true})).run();
    if (ctx && typeof ctx.waitUntil === "function") {
  ...
  if (job === "orchestrator_enqueue_daily_full_run") {
    ...
    if (existing) {
      return jsonResponse({ ok: true, data_ok: true, version, job, status: "already_queued", request_id: existing.request_id, existing, visible_button: "SCORING > Daily Full", backend_chain_only: true, auto_pump_triggered: false, browser_auto_pump: false, note: "Existing Daily Full Run parent queue row found. It owns Daily Context Full Run, Board Full Run, then Market/Scoring Full Run. Backend auto-pump/watchdog/cron owns continuation; ORCHESTRATOR > Wake is manual testing/rescue only." });
    }
    ...
    if (activeComponent) {
      return jsonResponse({ ok: false, data_ok: false, version, job, status: "component_full_run_active", visible_button: "SCORING > Daily Full", active_component: activeComponent, note: "A component full-run is already active. Do not create a Daily Full parent on top of an independent active full-run. Let the active run finish or use ORCHESTRATOR > Logs/Queue first." }, 409);
    }

    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-orchestrator','daily-full-run','12 Full Runs','daily_master','Daily Full Run: Daily Context + Board + Market/Scoring',1,'https://alphadog-v2-orchestrator.rodolfoaamattos.workers.dev','ORCHESTRATOR_WORKER',1,'Top-level backend parent chain: Daily Context Full Run, Board Full Run including PrizePicks/Sleeper/Score Prep, then Market/Scoring Full Run including hit probability. Prepared for split slate, half slate, and single-game slate. No static/base/delta jobs.',CURRENT_TIMESTAMP)"
    ).run();

    await env.CONFIG_DB.prepare(
      "INSERT OR REPLACE INTO config_worker_definitions (worker_name, job_key, worker_group, phase_key, display_name, enabled, schedule_profile_key, notes) VALUES ('alphadog-v2-orchestrator','daily-full-run','12 Full Runs','daily_master','Daily Full Run: Daily Context + Board + Market/Scoring',1,'manual_or_orchestrated','Top-level twice-daily parent chain for Daily Context Full Run, Board Full Run, and Market/Scoring Full Run. Uses existing component full runs and backend hot continuation; no manual SQL schema requirement.')"
    ).run();
    ...
    const input = {
      source: "control_room",
      visible_button: "SCORING > Daily Full",
      ...
      approved_chain_order: ["daily-context-full-run", "board-full-run", "market-scoring-full-run"],
      approved_stage_order: ["daily_context_full_run", "board_full_run", "market_scoring_full_run"],
      ...
      no_old_production_touch: true,
      note: "Daily Full Run parent chain: Daily Context Full Run -> Board Full Run -> Market/Scoring Full Run. Existing component full runs own their internal stages. Backend auto-pump/watchdog owns continuation; manual Wake is testing/rescue only."
    };

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'daily-full-run', 'alphadog-v2-orchestrator', '12 Full Runs', 'daily_master', 'Daily Full Run: Daily Context + Board + Market/Scoring', 'pending', 3, 1, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();
    ...
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"SCORING > Daily Full", approved_chain_order:input.approved_chain_order, backend_chain_only:true, auto_pump_requested:true, daily_full_run_watchdog_requested:true})).run();
    ...
    return jsonResponse({ ok: true, data_ok: true, version, job, status: "queued", request_id: requestId, chain_id: chainId, visible_button: "SCORING > Daily Full", queued_job_key: "daily-full-run", queued_worker_name: "alphadog-v2-orchestrator", approved_chain_order: input.approved_chain_order, backend_chain_only: true, auto_pump_triggered: true, backend_watchdog_triggered: true, prepared_for_split_slate: true, prepared_for_half_slate: true, prepared_for_single_game_slate: true, includes_daily_context_full_run: true, includes_board_full_run: true, includes_market_scoring_full_run: true, includes_hit_probability: true, note: "Queued Daily Full Run: Daily Context Full Run, Board Full Run, then Market/Scoring Full Run. Backend auto-pump/watchdog was launched; browser may close now. ORCHESTRATOR > Wake is manual testing/rescue only." });
  }
  ...
  if (job === "orchestrator_enqueue_market_scoring_full_run") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "SCORING > Market Full",
    ...
    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-orchestrator','market-scoring-full-run','12 Full Runs','market_scoring','Market + Factors + Matrix + Scoring Full Run',1,'https://alphadog-v2-orchestrator.rodolfoaamattos.workers.dev','ORCHESTRATOR_WORKER',1,'Backend parent chain: Market Teams, Market Hitters, Market Pitchers, Factor Hitters, Factor Pitchers, Matrix Build, Scoring Engine, Final Board, Hit Probability + HP Board. No source board mutation and no manual SQL schema requirement.',CURRENT_TIMESTAMP)"
    ).run();

    await env.CONFIG_DB.prepare(
      "INSERT OR REPLACE INTO config_worker_definitions (worker_name, job_key, worker_group, phase_key, display_name, enabled, schedule_profile_key, notes) VALUES ('alphadog-v2-orchestrator','market-scoring-full-run','12 Full Runs','market_scoring','Market + Factors + Matrix + Scoring Full Run',1,'manual_or_orchestrated','Parent backend chain for market context, prop factors, matrix, real scoring engine current, final board, and same score-audit hit-probability/HP-board display calibration. Created by Control Room; no schema SQL required.')"
    ).run();
    ...
    const input = {
      ...
      note: "Market/Factors/Matrix/Scoring/Final Board/Hit Probability backend parent chain. Board refresh is intentionally not included. Backend auto-pump/watchdog owns continuation; manual Wake is test/rescue only."
    };

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'market-scoring-full-run', 'alphadog-v2-orchestrator', '12 Full Runs', 'market_scoring', 'Market + Factors + Matrix + Scoring Full Run', 'pending', 4, 1, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_market_scoring_full_run', 'INFO', 'queued_market_scoring_full_run', 'Queued Market + Factors + Matrix + Scoring Full Run parent backend chain job', ?, CURRENT_TIMESTAMP)"
    ...
    return jsonResponse({
      ...
      hp_board_display_calibration_required: true,
      no_true_hit_probability_claims: true,
      note: "Queued Market/Factors/Matrix/Scoring/Final Board/Hit Probability full run. Backend auto-pump/watchdog was launched; ORCHESTRATOR > Wake is manual testing/rescue only."
    });
  }
