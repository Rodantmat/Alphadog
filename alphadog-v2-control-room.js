const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.61-wake-service-binding-restore";
...
async function callOrchestrator(env, path, payload = null) {
  ...
  if (env.ORCHESTRATOR_WORKER && typeof env.ORCHESTRATOR_WORKER.fetch === "function") {
    try {
      ...
      return {http_status:null, body:{ok:false, error:String(e && e.message ? e.message : e), route:"service_binding", service_url:serviceUrl}, route:"service_binding", ok:false};
    }
  }

  const publicUrl = "https://alphadog-v2-orchestrator.rodolfoaamattos.workers.dev" + path;
  try {
    const r = await fetch(publicUrl, init);
    const txt = await r.text();
    let body;
    try { body = JSON.parse(txt); } catch (_) { body = txt; }
    return {http_status:r.status, body, route:"public_url_fallback", ok:r.status >= 200 && r.status < 300};
  } catch (e) {
    return {http_status:null, body:{ok:false, error:String(e && e.message ? e.message : e), route:"public_url_fallback", public_url:publicUrl}, route:"public_url_fallback", ok:false};
  }
}
  ...
  if (job === "orchestrator_tick") {
    ...
    return jsonResponse({
      ...
      note: pumpOk
        ...
        : "Wake proxy failed before the backend pump accepted the request. This is not reported as launched; check ORCHESTRATOR_WORKER service binding and orchestrator /pump route.",
