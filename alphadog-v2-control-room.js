const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.137-board-full-run-prep-stale-note";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_board_full_run") {
    ...
    return jsonResponse({ ok: true, data_ok: true, version, job, status: "queued", request_id: requestId, chain_id: chainId, visible_button: "BOARD > Full Run", queued_job_key: "board-full-run", queued_worker_name: "alphadog-v2-orchestrator", approved_chain_order: input.approved_chain_order, backend_chain_only: true, auto_pump_triggered: true, browser_auto_pump: false, note: "Queued Board Full Run only: PrizePicks first, then Sleeper, then required Score Prep. PrizePicks source-stale refresh can retry and then continue nonfatally to Sleeper/Prep; this does not run Incremental Morning Full Run, static, base/delta, scoring, ranking, or final board." });
  }
