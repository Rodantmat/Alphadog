const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.24-base-pitcher-splits-probe";
  ...
  if (job === "orchestrator_enqueue_base_pitcher_splits") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Base Pitcher Splits probe queue row found. Do not repeatedly Wake; backend continuation owns any continuation."
      });
    }

    const requestId = "base_pitcher_splits_probe_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_base_pitcher_splits_probe_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Pitcher Splits",
      mode: "base_backfill_source_shape_probe",
      created_at: now,
      ...
      allowed_writes: ["STATS_PITCHER_DB.pitcher_split_stage", "STATS_PITCHER_DB.pitcher_split_batches", "STATS_PITCHER_DB.pitcher_split_outcomes", "STATS_PITCHER_DB.pitcher_split_cursor", "STATS_PITCHER_DB.pitcher_split_certifications", "STATS_PITCHER_DB.pitcher_schema_migrations"],
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-pitcher-splits', 'alphadog-v2-base-pitcher-splits', 'Base', 'base', 'Base Pitcher Splits Source-Shape Probe v0.1.0', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_pitcher_splits', 'INFO', 'queued_base_pitcher_splits_source_shape_probe', 'Queued exact Base Pitcher Splits v0.1.0 source-shape probe job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"BASE > Pitcher Splits", source_shape_probe_only:true, rows_promoted_expected:0, sample_size:3})).run();
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued v0.1.0 source-shape probe in CONTROL_DB. Tap ORCHESTRATOR > Wake once to dispatch. No live promotion, no full mining, no delta."
    });
  }
