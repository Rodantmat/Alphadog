const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.172-effective-warning-severity-calibration";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_market_scoring_full_run") {
    ...
    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-orchestrator','market-scoring-full-run','12 Full Runs','market_scoring','Market + Factors + Matrix + Scoring Full Run',1,'https://alphadog-v2-orchestrator.rodolfoaamattos.workers.dev','ORCHESTRATOR_WORKER',1,'Backend parent chain: Market Teams, Market Hitters, Market Pitchers, Factor Hitters, Factor Pitchers, Matrix Build, Scoring Engine, Final Board. No source board mutation and no manual SQL schema requirement.',CURRENT_TIMESTAMP)"
    ).run();

    await env.CONFIG_DB.prepare(
      "INSERT OR REPLACE INTO config_worker_definitions (worker_name, job_key, worker_group, phase_key, display_name, enabled, schedule_profile_key, notes) VALUES ('alphadog-v2-orchestrator','market-scoring-full-run','12 Full Runs','market_scoring','Market + Factors + Matrix + Scoring Full Run',1,'manual_or_orchestrated','Parent backend chain for market context, prop factors, matrix, real scoring engine current, and final board. Created by Control Room; no schema SQL required.')"
    ).run();
    ...
    const input = {
      ...
      note: "Market/Factors/Matrix/Scoring/Final Board backend parent chain. Board refresh is intentionally not included. Backend auto-pump/watchdog owns continuation; manual Wake is test/rescue only."
    };
    ...
    return jsonResponse({
      ...
      no_source_board_mutation: true,
      no_prepared_board_mutation: true,
      note: "Queued Market/Factors/Matrix/Scoring/Final Board full run. Backend auto-pump/watchdog was launched; ORCHESTRATOR > Wake is manual testing/rescue only."
    });
  }
