const SYSTEM_VERSION = "alphadog-v2-control-room-v1.5.14-static-players-direct-pump-confirm";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_prizepicks_github_board") {
    ...
    return jsonResponse({
      ...
      note: "Queued in CONTROL_DB. Use ORCHESTRATOR > Wake to process now, then ORCHESTRATOR > Logs and V2 SAFE ACTIONS > Queue."
    });
  }
  ...
  if (job === "orchestrator_enqueue_static_teams") {
    ...
    const input = {
      ...
      no_ranking: true,
      no_final_board: true,
      auto_pump_after_enqueue: true,
      no_manual_repeated_wake: true
    };
  ...
  if (job === "orchestrator_enqueue_static_players") {
    ...
    if (existing) {
      const pumpPayload = {source:"control_room_static_players_existing_auto_pump", request_id:existing.request_id, max_cycles:20, max_jobs_per_cycle:3, max_ms:27000, max_pump_chains:20};
      if (ctx && typeof ctx.waitUntil === "function") {
        ctx.waitUntil(callOrchestrator(env, "/pump", pumpPayload));
      } else {
        await callOrchestrator(env, "/pump", pumpPayload);
      }
      return jsonResponse({
        ok: true,
        data_ok: true,
        version,
        job,
        status: "already_queued_auto_pump_triggered",
        request_id: existing.request_id,
        existing,
        visible_button: "STATIC > Players",
        auto_pump_triggered: true,
        note: "Existing Static Players queue row found. Control Room triggered orchestrator /pump in the background. Use ORCHESTRATOR > Logs and V2 SAFE ACTIONS > Queue; do not repeat manual Wake."
      });
    }
    ...
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"STATIC > Players", auto_pump_after_enqueue:true})).run();

    const pumpPayload = {source:"control_room_static_players_enqueue_auto_pump", request_id:requestId, max_cycles:20, max_jobs_per_cycle:3, max_ms:27000, max_pump_chains:20};
    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(callOrchestrator(env, "/pump", pumpPayload));
    } else {
      await callOrchestrator(env, "/pump", pumpPayload);
    ...
    return jsonResponse({
      ...
      version,
      job,
      status: "queued_auto_pump_triggered",
      request_id: requestId,
      chain_id: chainId,
      visible_button: "STATIC > Players",
      ...
      auto_pump_triggered: true,
      note: "Queued in CONTROL_DB and triggered orchestrator /pump in the background. Use ORCHESTRATOR > Logs and V2 SAFE ACTIONS > Queue; do not repeat manual Wake. Cron remains the fallback/weekly scheduler."
    });
  }
  ...
  if (job === "orchestrator_tick") {
    ...
    return jsonResponse({
      ...
      version,
      job,
      orchestrator_mode: "auto_pump_plus_cron_fallback",
      status: "use_orchestrator_wake_button",
      note: "Static Players uses background orchestrator /pump after enqueue, plus cron fallback. ORCHESTRATOR > Wake now calls bounded /pump instead of single /tick, for diagnostics only.",
