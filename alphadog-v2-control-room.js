const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.67-base-hitter-metrics-sample-stage-route";
  ...
  if (job === "orchestrator_enqueue_base_hitter_metrics") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Base Hitter Metrics sample-stage queue row found. Tap ORCHESTRATOR > Wake once only if it has not processed. Browser does not run the job."
      });
    }

    const requestId = "base_hitter_metrics_sample_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_base_hitter_metrics_sample_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Hitter Metrics",
      ...
      source_season: 2026,
      config_profile_id: "hitter_metrics_neutral_v0_1_0",
      formula_version: "hitter_metrics_formula_v0_1_0_readiness",
      allowed_writes: [
      ...
      v0_2_0_sample_stage_only: true,
      sample_player_cap: 30
    };

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-hitter-metrics', 'alphadog-v2-base-hitter-metrics', 'Base', 'base', 'Base Hitter Metrics v0.2.0 Sample-Stage Calibration', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_hitter_metrics', 'INFO', 'queued_base_hitter_metrics_sample_stage_calibration', 'Queued exact Base Hitter Metrics v0.2.0 sample-stage calibration only job', ?, CURRENT_TIMESTAMP)"
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued v0.2.0 sample-stage calibration only. Tap ORCHESTRATOR > Wake once to dispatch. Stage rows only; no live metric promotion, no source table mutation, no MLB calls, no scoring."
    });
  }
