const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.36-base-team-game-logs-allowlist-fix";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_base_team_game_logs") {
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = 'base-team-game-logs' AND status IN ('pending','running') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Base Team Game Logs probe queue row found. Tap ORCHESTRATOR > Wake once or let backend continuation/cron dispatch it. Browser does not run the job."
      });
    }

    const requestId = "base_team_logs_probe_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_base_team_logs_probe_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Team Game Logs",
      mode: "source_shape_probe",
      worker_scope: "v0.1.0_schema_source_lock_probe_only",
      created_at: now,
      source_name: "MLB StatsAPI schedule plus game boxscore team totals probe",
      ...
      rows_promoted_expected: 0,
      allowed_writes: ["TEAM_DB.team_schema_migrations", "TEAM_DB.team_game_log_batches", "TEAM_DB.team_game_log_stage", "TEAM_DB.team_game_log_outcomes", "TEAM_DB.team_game_log_cursor", "TEAM_DB.team_game_log_certifications"],
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-team-game-logs', 'alphadog-v2-base-team-game-logs', 'Base', 'base', 'Base Team Game Logs Source Lock Probe v0.1.0', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_team_game_logs', 'INFO', 'queued_base_team_game_logs_source_lock_probe', 'Queued exact Base Team Game Logs v0.1.0 source/schema probe-only job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "BASE > Team Game Logs", probe_only: true, sample_date: "2026-05-18", rows_promoted_expected: 0, no_live_promotion: true })).run();
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued Base Team Game Logs v0.1.0 probe-only job. Tap ORCHESTRATOR > Wake once to dispatch. It writes TEAM_DB lifecycle/probe rows only and must not promote live team_game_logs rows."
    });
  }
