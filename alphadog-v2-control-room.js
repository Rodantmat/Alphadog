const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.82-base-pitcher-metrics-snapshot-prep";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_base_hitter_metrics") {
    ...
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"BASE > Hitter Metrics", snapshot_prep_stage_only:true, no_promotion:true, no_external_mlb_calls:true, no_scoring:true})).run();
  ...
  if (job === "orchestrator_enqueue_base_pitcher_metrics") {
    ...
    const requestId = "base_pitcher_metrics_snapshot_prep_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_base_pitcher_metrics_snapshot_prep_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Pitcher Metrics",
      mode: "snapshot_prep_stage_only",
      ...
      source_key: "d1_pitcher_metric_stage_snapshot_prep",
      source_season: 2026,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-pitcher-metrics', 'alphadog-v2-base-pitcher-metrics', 'Base', 'base', 'Base Pitcher Metrics v0.3.3 Snapshot Prep Stage Only', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_pitcher_metrics', 'INFO', 'queued_base_pitcher_metrics_v0_3_3_snapshot_prep_stage_only', 'Queued exact Base Pitcher Metrics v0.3.3 snapshot prep stage-only job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"BASE > Pitcher Metrics", snapshot_prep_stage_only:true, no_promotion:true, no_external_mlb_calls:true, no_scoring:true})).run();
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued v0.3.3 snapshot prep stage-only. Tap ORCHESTRATOR > Wake once to dispatch backend prep. Retained snapshot stage only; no live metric promotion, no source table mutation, no MLB calls, no scoring."
    });
  }
