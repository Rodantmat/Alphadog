const SYSTEM_VERSION = "alphadog-v2-control-room-v1.5.12-static-players-auto-pump-ctx-fix";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_static_players") {
    ...
    if (existing) {
      const pumpPayload = {source:"control_room_static_players_existing_auto_pump", request_id:existing.request_id, max_cycles:12, max_jobs_per_cycle:2, max_ms:25000};
    ...
    const pumpPayload = {source:"control_room_static_players_enqueue_auto_pump", request_id:requestId, max_cycles:12, max_jobs_per_cycle:2, max_ms:25000};
    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(callOrchestrator(env, "/pump", pumpPayload));
    } else {
      await callOrchestrator(env, "/pump", pumpPayload);
  ...
  if (job === "orchestrator_tick") {
    ...
    return jsonResponse({
      ...
      note: "Static Players uses background orchestrator /pump after enqueue, plus cron fallback. ORCHESTRATOR > Wake remains optional for diagnostics only; do not repeat manual Wake.",
