const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.205-final-scoring-route-html-orchestrator-parity";
...
<!-- alphadog-v2-control-room-v1.6.205-final-scoring-route-html-orchestrator-parity -->
<style>
...
<div id="versionTag">alphadog-v2-control-room-v1.6.205-final-scoring-route-html-orchestrator-parity</div>
...
<button class="dailyfull" type="button" onclick="runJobButton('FINAL SCORING SYSTEM > Baseline Base','orchestrator_enqueue_final_scoring_baseline_base')">Baseline Base</button>
<button class="audit" type="button" onclick="runJobButton('FINAL SCORING SYSTEM > Baseline Delta','orchestrator_enqueue_final_scoring_baseline_delta')">Baseline Delta</button>
</div></div>
...
async function runJob(request, env, ctx) {
  ...
  try {
    ...
    if ([
      ...
      "orchestrator_enqueue_final_scoring_baseline_base",
      "orchestrator_enqueue_final_scoring_baseline_delta",
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (shadowScoringJobs[job]) {
    ...
    if (existing) {
      return jsonResponse({ ok:true, data_ok:true, version:SYSTEM_VERSION, job, status:"already_queued", request_id:existing.request_id, existing, visible_button:meta.visible_button, queued_job_key:meta.job_key });
    }
    await env.CONTROL_DB.prepare(
  ...
  if (job === "orchestrator_enqueue_final_scoring_baseline_base" || job === "orchestrator_enqueue_final_scoring_baseline_delta") {
    const isBase = job === "orchestrator_enqueue_final_scoring_baseline_base";
    const visibleButton = isBase ? "FINAL SCORING SYSTEM > Baseline Base" : "FINAL SCORING SYSTEM > Baseline Delta";
    const runType = isBase ? "BASELINE_BASE_ALL_CURRENT" : "BASELINE_DELTA_LATEST_HP_V2_BATCH";
    const existing = await env.CONTROL_DB.prepare(
    ...
    if (existing) {
      if (ctx && typeof ctx.waitUntil === "function") {
        ctx.waitUntil(callOrchestrator(env, "/pump", { source:isBase?"final_scoring_baseline_base_existing":"final_scoring_baseline_delta_existing", max_cycles:18, max_jobs_per_cycle:1, max_ms:65000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, expansion_baseline_v2_watchdog:true }));
      }
      return jsonResponse({ ok:true, data_ok:true, version:SYSTEM_VERSION, job, status:"already_queued", request_id:existing.request_id, existing, visible_button:visibleButton, queued_job_key:"expansion-baseline-v2", backend_hot_continue_triggered:true, note:"Existing baseline-v2 job found. Backend continuation was launched; do not enqueue a duplicate." });
    }
    let sourceHpV2BatchId = "__ALL_HIT_PROBABILITY_V2_CURRENT__";
    let readAllHpV2Current = true;
    let sourceMode = "all_hit_probability_v2_current_full_base";
    let latestBatchRow = null;
    if (!isBase) {
      latestBatchRow = await env.SCORE_DB.prepare("SELECT batch_id, status, certification, updated_at FROM hit_probability_v2_batches ORDER BY datetime(updated_at) DESC LIMIT 1").first();
      if (!latestBatchRow || !latestBatchRow.batch_id) {
        return jsonResponse({ ok:false, data_ok:false, version:SYSTEM_VERSION, job, status:"blocked_no_hp_v2_batch", visible_button:visibleButton, note:"Baseline Delta needs a latest hit_probability_v2_batches batch_id. Run the upstream HP V2/V3 source chain first, or use Baseline Base." }, 200);
      }
      sourceHpV2BatchId = String(latestBatchRow.batch_id);
      readAllHpV2Current = false;
      sourceMode = "latest_hit_probability_v2_batch_delta_only";
    }
    const requestId = (isBase ? "final_scoring_baseline_base_" : "final_scoring_baseline_delta_") + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_" + (isBase ? "final_scoring_baseline_base_" : "final_scoring_baseline_delta_") + Date.now().toString(36);
    const input = {
      source:"control_room", visible_button:visibleButton, mode:"baseline_v2_heb", expansion_mode:"baseline_v2_heb", created_at:now,
      final_scoring_system:true, baseline_run_type:runType, control_room_route_parity_v1_6_205:true, embedded_html_parity:true, standalone_html_parity:true, allowlist_parity:true, 
      ...
      source_hp_v2_batch_id:sourceHpV2BatchId, hp_v2_batch_id:sourceHpV2BatchId, read_all_hp_v2_current:readAllHpV2Current,
      v2_source_mode:sourceMode,
      v2_chunk_size:isBase?96:32, v2_all_current_chunk_size:96, v2_soft_yield_ms:isBase?42000:24000,
      baseline_source_policy:"source_agnostic_unless_formula_profile_is_source_specific",
      fast_safe_chunk_policy:isBase?"full_base_all_current_96_soft_yield_42s":"delta_latest_hp_batch_32_soft_yield_24s",
    ...
    if (latestBatchRow) input.latest_hp_v2_batch = latestBatchRow;
    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-phase3a-first-inning-pitcher-context','expansion-baseline-v2','Final Scoring System','final_scoring_baseline','Final Scoring Baseline V2',1,'https://alphadog-v2-phase3a-first-inning-pitcher-context.rodolfoaamattos.workers.dev','PHASE3A_FIRST_INNING_PITCHER_CONTEXT_WORKER',1,'Final Scoring System baseline buttons with backend allowlist, embedded HTML, standalone HTML, and orchestrator dispatch parity. Base reads all hit_probability_v2_current; Delta reads only latest HP V2 batch. Writes only player_baseline_*_v2 tables.',CURRENT_TIMESTAMP)"
    ).run();
    await env.CONFIG_DB.prepare(
      "INSERT OR REPLACE INTO config_worker_definitions (worker_name, job_key, worker_group, phase_key, display_name, enabled, schedule_profile_key, notes) VALUES ('alphadog-v2-phase3a-first-inning-pitcher-context','expansion-baseline-v2','Final Scoring System','final_scoring_baseline','Final Scoring Baseline V2',1,'manual_or_orchestrated','Base = all-current full baseline. Delta = latest HP V2 batch only. Route/HTML/orchestrator parity locked. Writes player_baseline_*_v2 only.')"
    ).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'expansion-baseline-v2', 'alphadog-v2-phase3a-first-inning-pitcher-context', 'Final Scoring System', 'final_scoring_baseline', ?, 'pending', 7, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, isBase?"Final Scoring Baseline Base":"Final Scoring Baseline Delta", JSON.stringify(input)).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_final_scoring_baseline', 'Queued Final Scoring System baseline job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, job, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:visibleButton, queued_job_key:"expansion-baseline-v2", queued_worker_name:"alphadog-v2-phase3a-first-inning-pitcher-context", baseline_run_type:runType, source_hp_v2_batch_id:sourceHpV2BatchId, read_all_hp_v2_current:readAllHpV2Current})).run();
    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(callOrchestrator(env, "/pump", { source:isBase?"final_scoring_baseline_base_enqueue":"final_scoring_baseline_delta_enqueue", max_cycles:18, max_jobs_per_cycle:1, max_ms:65000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, expansion_baseline_v2_watchdog:true }));
      ctx.waitUntil((async()=>{ await sleepMs(25000); await callOrchestrator(env, "/pump", { source:isBase?"final_scoring_baseline_base_watchdog_25s":"final_scoring_baseline_delta_watchdog_25s", max_cycles:18, max_jobs_per_cycle:1, max_ms:65000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, expansion_baseline_v2_watchdog:true }); })());
    }
    return jsonResponse({ ok:true, data_ok:true, version:SYSTEM_VERSION, job, status:"queued", request_id:requestId, chain_id:chainId, visible_button:visibleButton, queued_job_key:"expansion-baseline-v2", queued_worker_name:"alphadog-v2-phase3a-first-inning-pitcher-context", mode:"baseline_v2_heb", baseline_run_type:runType, source_hp_v2_batch_id:sourceHpV2BatchId, read_all_hp_v2_current:readAllHpV2Current, v2_source_mode:sourceMode, auto_pump_triggered:true, browser_auto_pump:false, note:isBase?"Queued Baseline Base: all-current full V2 baseline rebuild over hit_probability_v2_current." : "Queued Baseline Delta: latest HP V2 batch only; use after upstream delta HP batch exists." });
  }
  ...
  if (job === "orchestrator_enqueue_daily_context_certifier") {
    ...
    if (existing) return jsonResponse({ ok:true, data_ok:true, version:SYSTEM_VERSION, job, status:"already_queued", request_id: existing.request_id, existing, visible_button:"DAILY JOBS > Context Cert", note:"Existing Daily Context Certifier queue row found. Use ORCHESTRATOR > Wake / Logs / Queue to continue or inspect." });
  ...
  if (job === "orchestrator_enqueue_delta_pitcher_game_logs") {
    ...
    if (existing) {
      ...
      return jsonResponse({ ok:true, data_ok:true, version:SYSTEM_VERSION, job, status:"already_queued", request_id:existing.request_id, existing, visible_button:"DELTA > Pitcher Game Logs", backend_scheduled_continuation:true, auto_pump_triggered:true, browser_auto_pump:false, note:"Existing Pitcher Game Logs queue row found. Control Room launched backend pump as a safe continuation nudge; browser does not run the job." });
    }
