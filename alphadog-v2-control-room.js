const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.79-base-pitcher-metrics-audit-route";
  ...
  if (job === "orchestrator_enqueue_base_pitcher_metrics") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Base Pitcher Metrics audit queue row found. Tap ORCHESTRATOR > Wake once only if it has not processed. Browser does not run the job."
      });
    }

    const requestId = "base_pitcher_metrics_audit_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_base_pitcher_metrics_audit_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Pitcher Metrics",
      mode: "schema_formula_input_audit",
      created_at: now,
      worker_scope: "v0_1_0_schema_formula_input_audit_only",
      source_name: "D1 audit from certified STATS_PITCHER_DB.pitcher_game_logs and STATS_PITCHER_DB.pitcher_splits",
      source_key: "d1_pitcher_game_logs_and_splits_audit",
      source_season: 2026,
      config_profile_id: "pitcher_metrics_neutral_v0_1_0_audit",
      formula_version: "pitcher_metrics_formula_v0_1_0_readiness",
      allowed_writes: [
      ...
      no_browser_auto_pump: true,
      wake_once_for_test_only: true,
      schema_formula_input_audit_only: true,
      promotion_locked: true
    };

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-pitcher-metrics', 'alphadog-v2-base-pitcher-metrics', 'Base', 'base', 'Base Pitcher Metrics v0.1.0 Schema/Formula/Input Audit', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_pitcher_metrics', 'INFO', 'queued_base_pitcher_metrics_v0_1_0_audit_only', 'Queued exact Base Pitcher Metrics v0.1.0 schema/formula/input audit-only job', ?, CURRENT_TIMESTAMP)"
    ...
    return jsonResponse({
      ...
      v0_1_0_schema_formula_input_audit_only: true,
      backend_scheduled_continuation: true,
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued v0.1.0 audit-only. Tap ORCHESTRATOR > Wake to dispatch. No live metric promotion, no source table mutation, no MLB calls, no scoring."
    });
  }
