const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.187-expansion-baseline-existing-slot";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job.startsWith("orchestrator_enqueue_expansion_baseline_")) {
    ...
    if (existing) return jsonResponse({ok:true, data_ok:true, version, job, status:"already_queued", request_id:existing.request_id, existing, visible_button:visibleButton, expansion_only:true, baseline_only:true});
