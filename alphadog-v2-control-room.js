const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.59-bullpen-delta-cursor-fix";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_base_bullpen_history") {
    ...
    if (existing) {
      if (ctx && typeof ctx.waitUntil === "function") {
        ctx.waitUntil(callOrchestrator(env, "/pump", {
          source: "control_room_base_bullpen_history_existing_delta_hot_continue",
          max_cycles: 8,
    ...
    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(callOrchestrator(env, "/pump", {
        source: "control_room_base_bullpen_history_delta_update_auto_start",
        max_cycles: 10,
        max_jobs_per_cycle: 1,
        max_ms: 60000,
        pump_depth: 0,
    ...
    return jsonResponse({
      ...
      note: "Queued Base Bullpen History v0.4.0 delta_update. It will no-op if current, restore from retained delta stage before mining if needed, or scoped-repair exact missing delta keys. No Daily Bullpen Availability, no scoring, no board mutation."
    });
  }
