const SYSTEM_VERSION = "alphadog-v2-control-room-v1.5.16-browser-direct-pump-no-worker-fetch";
  ...
  if (job === "orchestrator_enqueue_static_players") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        version,
        job,
        status: "already_queued_browser_direct_pump_required",
        request_id: existing.request_id,
        existing,
        visible_button: "STATIC > Players",
        auto_pump_triggered: true,
        auto_pump_mode: "browser_direct_from_control_room_page",
        auto_pump_client_direct_required: true,
        note: "Existing Static Players queue row found. Browser page will call orchestrator /pump directly. Worker-to-worker fetch is intentionally not used because it returned Cloudflare 1042."
      });
    }
    ...
    return jsonResponse({
      ...
      version,
      job,
      status: "queued_browser_direct_pump_required",
      request_id: requestId,
      chain_id: chainId,
      visible_button: "STATIC > Players",
      ...
      auto_pump_mode: "browser_direct_from_control_room_page",
      auto_pump_client_direct_required: true,
      note: "Queued in CONTROL_DB. Browser page will call orchestrator /pump directly. Cron remains fallback. Worker-to-worker fetch is intentionally not used because it returned Cloudflare 1042."
    });
  }
