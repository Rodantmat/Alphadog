const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.102-pitcher-metrics-delta-route-hardening";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_delta_pitcher_metrics") {
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued v0.5.0 true affected-player recalculation delta. Tap ORCHESTRATOR > Wake. It recalculates only pitchers touched by fresh game logs/splits, then scoped-upserts live snapshots. No source mutation, no MLB calls, no scoring."
    });
  }
