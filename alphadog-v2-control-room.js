const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.42-base-starter-history-probe";
  ...
  if (job === "orchestrator_enqueue_base_starter_history") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Base Starter History probe queue row found. Tap ORCHESTRATOR > Wake once only if needed; backend continuation owns dispatch."
      });
    }

    const requestId = "starter_history_probe_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-starter-history', 'alphadog-v2-base-starter-history', 'Base', 'base', 'Base Starter History Source-Lock Probe v0.1.0', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_starter_history', 'INFO', 'queued_base_starter_history_source_lock_probe', 'Queued Base Starter History v0.1.0 source-lock probe only', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "BASE > Starter History", mode: "source_lock_probe", sample_date: "2026-05-18", sample_limit: 3, no_live_promotion: true })).run();
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued Base Starter History v0.1.0 source-lock probe only. Tap ORCHESTRATOR > Wake once to dispatch. No live starter_history promotion, no full base backfill, no delta execution."
    });
  }
