const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.17-base-pitcher-delta-button";
  ...
  if (job === "orchestrator_enqueue_delta_pitcher_game_logs") {
    ...
    if (existing) {
      ...
      return jsonResponse({
        ...
        auto_pump_triggered: true,
        browser_auto_pump: false,
        note: "Existing Pitcher Game Logs queue row found. Control Room launched backend pump as a safe continuation nudge; browser does not run the job."
      });
    }
    ...
    const input = {
      ...
      mode: "delta_update",
      created_at: now,
      worker_scope: "delta_pitcher_game_logs_retained_stage_engine_v0_4_0",
      source_name: "MLB StatsAPI people gameLog pitching endpoint",
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-pitcher-game-logs', 'alphadog-v2-base-pitcher-game-logs', 'Delta', 'delta', 'Delta Pitcher Game Logs Retained Stage Update', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_delta_pitcher_game_logs', 'INFO', 'queued_delta_pitcher_game_logs_update', 'Queued exact Delta Pitcher Game Logs retained-stage update job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"DELTA > Pitcher Game Logs", backend_scheduled_continuation:true, delta_start_date:"2026-05-19"})).run();

    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(callOrchestrator(env, "/pump", {
        source: "control_room_delta_pitcher_game_logs_auto_start",
        max_cycles: 12,
        max_jobs_per_cycle: 3,
        max_ms: 25000,
        pump_depth: 0,
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: true,
      browser_auto_pump: false,
      note: "Queued in CONTROL_DB. Control Room launches backend orchestrator pump via ctx.waitUntil; orchestrator self-continues bounded chunks. Browser may close now. ORCHESTRATOR > Wake is optional test/rescue only."
    });
  }
