const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.100-hitter-splits-stale-lock-progress-rescue";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_delta_hitter_metrics") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Hitter Metrics queue row found. Tap ORCHESTRATOR > Wake until the queue completes. Browser does not run the job."
      });
    }

    const requestId = "delta_hitter_metrics_snapshot_gate_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_delta_hitter_metrics_snapshot_gate_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "DELTA > Hitter Metrics",
      mode: "snapshot_delta_gate",
      created_at: now,
      worker_scope: "v0_4_1_snapshot_retained_stage_parity_repair",
      source_name: "D1 live hitter_metric_snapshots repaired from retained hitter_metric_snapshot_stage",
      source_key: "d1_hitter_metric_snapshot_stage_retained_repair",
      source_season: 2026,
      config_profile_id: "hitter_metrics_neutral_v0_3_0_stage_only",
      formula_version: "hitter_metrics_formula_v0_3_0_stage_only",
      allowed_writes: [
        "STATS_HITTER_DB.hitter_metric_snapshots",
        "STATS_HITTER_DB.hitter_metric_snapshot_stage.promoted_at"
      ],
      blocked_writes: [
      ...
      no_ranking: true,
      no_final_board: true,
      v0_4_1_snapshot_retained_stage_parity_repair: true
    };

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-hitter-metrics', 'alphadog-v2-base-hitter-metrics', 'Delta', 'delta', 'Delta Hitter Metrics Snapshot Repair Gate', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_delta_hitter_metrics', 'INFO', 'queued_delta_hitter_metrics_snapshot_repair_gate', 'Queued exact Delta Hitter Metrics v0.4.1 snapshot retained-stage parity repair gate', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"DELTA > Hitter Metrics", no_external_mlb_calls:true, no_source_mutation:true, no_scoring:true})).run();
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued v0.4.1 snapshot retained-stage parity repair. Tap ORCHESTRATOR > Wake to dispatch. It restores missing live snapshot rows from retained stage or no-ops when live/stage parity is current. No MLB calls, no source mutation, no scoring."
    });
  }
  ...
  if (job === "orchestrator_enqueue_delta_pitcher_metrics") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Pitcher Metrics queue row found. Backend continuation owns remaining work; browser does not run the job."
      });
    }

    const requestId = "delta_pitcher_metrics_snapshot_gate_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_delta_pitcher_metrics_snapshot_gate_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "DELTA > Pitcher Metrics",
      mode: "snapshot_delta_gate",
      created_at: now,
      worker_scope: "v0_4_1_snapshot_retained_stage_parity_repair",
      source_name: "D1 live pitcher_metric_snapshots repaired from retained pitcher_metric_snapshot_stage",
      source_key: "d1_pitcher_metric_snapshot_stage_retained_repair",
      source_season: 2026,
      ...
      allowed_writes: [
        ...
        "STATS_PITCHER_DB.pitcher_metric_snapshot_stage.promoted_at",
        "STATS_PITCHER_DB.pitcher_metric_snapshot_batches repair metadata only"
      ],
      blocked_writes: [
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-pitcher-metrics', 'alphadog-v2-base-pitcher-metrics', 'Delta', 'delta', 'Delta Pitcher Metrics Snapshot Repair Gate', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_delta_pitcher_metrics', 'INFO', 'queued_delta_pitcher_metrics_snapshot_repair_gate', 'Queued exact Delta Pitcher Metrics v0.4.1 snapshot retained-stage parity repair gate', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"DELTA > Pitcher Metrics", no_external_mlb_calls:true, no_source_mutation:true, no_scoring:true})).run();
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued v0.4.1 pitcher metrics snapshot retained-stage parity repair. Tap ORCHESTRATOR > Wake once. It restores missing live snapshot rows from retained stage or no-ops when live/stage parity is current. No MLB calls, no source mutation, no scoring."
    });
  }
  ...
  if (job === "orchestrator_enqueue_base_team_game_logs") {
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = 'base-team-game-logs' AND status IN ('pending','running','partial_continue') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "BASE > Team Game Logs",
        backend_scheduled_continuation: true,
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Base Team Game Logs base_backfill queue row found. Do not repeatedly Wake; backend continuation/cron owns the remaining chunks."
      });
    }

    const requestId = "base_team_logs_base_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_base_team_logs_base_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Team Game Logs",
      mode: "base_backfill",
      worker_scope: "v0.2.0_base_backfill_through_2026_05_18",
      created_at: now,
      source_name: "MLB StatsAPI schedule plus game boxscore team totals base_backfill",
      source_key: "mlb_statsapi_schedule_boxscore_team_totals_probe_v0_1_0",
      schedule_endpoint_pattern: "/api/v1/schedule?sportId=1&gameTypes=R&startDate={date}&endDate={date}",
      boxscore_endpoint_pattern: "/api/v1/game/{gamePk}/boxscore",
      base_backfill_start_date: "2026-03-01",
      base_backfill_cutoff_date: "2026-05-18",
      delta_reserved_start_date: "2026-05-19",
      probe_only: false,
      base_stage_only: false,
      rows_promoted_expected: "computed_from_final_games_x2",
      allowed_writes: ["TEAM_DB.team_schema_migrations", "TEAM_DB.team_game_log_batches", "TEAM_DB.team_game_log_stage", "TEAM_DB.team_game_log_outcomes", "TEAM_DB.team_game_log_cursor", "TEAM_DB.team_game_log_certifications", "TEAM_DB.team_game_logs"],
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-team-game-logs', 'alphadog-v2-base-team-game-logs', 'Base', 'base', 'Base Team Game Logs Base Backfill v0.2.0', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_team_game_logs', 'INFO', 'queued_base_team_game_logs_base_backfill', 'Queued exact Base Team Game Logs v0.2.0 base_backfill through 2026-05-18', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "BASE > Team Game Logs", mode: "base_backfill", cutoff_date: "2026-05-18", delta_reserved_start_date: "2026-05-19" })).run();
    ...
    return jsonResponse({
      ...
      delta_reserved_start_date: "2026-05-19",
      base_stage_only: false,
      no_delta_update_execution: false,
      backend_scheduled_continuation: true,
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued Base Team Game Logs v0.2.0 base_backfill. Tap ORCHESTRATOR > Wake once to dispatch; backend continuation/cron owns remaining chunks."
    });
  }
  ...
  if (job === "orchestrator_enqueue_base_bullpen_history") {
    ...
    if (!existing) {
      const stageParityRepair = await restoreMissingBullpenDeltaStageFromLive(env, 200);
      if (stageParityRepair.restored && stageParityRepair.restored_rows > 0) {
        await env.CONTROL_DB.prepare(
          "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (NULL, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_bullpen_history', 'INFO', 'repaired_bullpen_delta_stage_from_live_before_queue', 'Restored missing retained bullpen delta stage rows from certified live rows before queue/no-op', ?, CURRENT_TIMESTAMP)"
        ).bind(JSON.stringify(stageParityRepair)).run();
        return jsonResponse({
          ok: true,
          data_ok: true,
          version,
          job,
          status: "REPAIRED_RETAINED_STAGE_FROM_LIVE_BEFORE_QUEUE",
          ...
          no_cleanup: true,
          no_full_sweep: true,
          restored_stage_rows: stageParityRepair.restored_rows,
          missing_before: stageParityRepair.missing_before,
          missing_after: stageParityRepair.missing_after,
          duplicate_stage_keys: stageParityRepair.duplicate_stage_keys,
          first_missing_stage_game_date: stageParityRepair.first_missing_stage_game_date,
          latest_missing_stage_game_date: stageParityRepair.latest_missing_stage_game_date,
          live_delta_rows: stageParityRepair.live_delta_rows,
          sample_missing_before: stageParityRepair.sample_missing_before,
          note: "Retained delta stage parity was repaired from already-certified live bullpen delta rows before enqueue. No backend job, no MLB call, no live mutation, no scoring, no board mutation. Run BASE > Bullpen History again for the normal no-op/current check."
        });
      }
    }

    if (existing) {
      const hotStart = await callOrchestrator(env, "/pump", {
        source: "control_room_base_bullpen_history_existing_delta_direct_proxy_hot_continue",
        max_cycles: 4,
      ...
      return jsonResponse({
        ...
        mode: "delta_update",
        backend_scheduled_continuation: true,
        backend_direct_proxy_hot_start: orchestratorCallOk(hotStart),
        hot_start_route: hotStart.route || null,
        hot_start_http_status: hotStart.http_status,
        hot_start_body: hotStart.body,
        auto_pump_triggered: orchestratorCallOk(hotStart),
        browser_auto_pump: false,
        note: "Existing Base Bullpen History queue row found. Control Room directly proxied one bounded backend orchestrator pump so Wake/queue starts immediately; browser still does not call the orchestrator directly or run a loop. Cron remains fallback rescue only."
      });
    }

    const requestId = "bullpen_history_delta_update_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_bullpen_history_delta_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Bullpen History",
      mode: "delta_update",
      max_games_per_tick: 12,
      repair_limit: 3,
      restore_limit: 200,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-bullpen-history', 'alphadog-v2-base-bullpen-history', 'Base', 'base', 'Base Bullpen History Delta Update v0.4.4', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_bullpen_history', 'INFO', 'queued_base_bullpen_history_delta_update', 'Queued Base Bullpen History v0.4.4 delta_update / no-op / retained-stage repair gate', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "BASE > Bullpen History", mode: "delta_update", max_games_per_tick: 12, repair_limit: 3, restore_limit: 200, no_daily_bullpen_availability: true, backend_hot_continuation: true })).run();

    const hotStart = await callOrchestrator(env, "/pump", {
      source: "control_room_base_bullpen_history_delta_update_direct_proxy_auto_start",
      max_cycles: 4,
      max_jobs_per_cycle: 1,
      max_ms: 30000,
      pump_depth: 0,
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: orchestratorCallOk(hotStart),
      browser_auto_pump: false,
      backend_hot_continuation_launched: orchestratorCallOk(hotStart),
      note: "Queued Base Bullpen History v0.4.4 delta_update and directly proxied one bounded backend pump to start it immediately. It will no-op if current, restore from retained delta stage before mining if needed, or scoped-repair exact missing delta keys. No Daily Bullpen Availability, no scoring, no board mutation."
    });
  }
  ...
  if (job === "orchestrator_enqueue_base_starter_history") {
    ...
    if (existing) {
      if (ctx && typeof ctx.waitUntil === "function") {
        ctx.waitUntil(callOrchestrator(env, "/pump", {
          source: "control_room_base_starter_history_existing_hot_continue",
          max_cycles: 18,
          max_jobs_per_cycle: 1,
          max_ms: 70000,
          pump_depth: 0,
      ...
      return jsonResponse({
        ...
        requested_preflight_behavior: "delta_scoped_source_repair",
        retained_stage_restore_before_queue: true,
        scoped_source_repair: true,
        no_live_promotion: false,
        backend_scheduled_continuation: true,
        auto_pump_triggered: true,
        browser_auto_pump: false,
        note: "Existing Base Starter History queue row found. Control Room launched the backend orchestrator pump as a hot-continuation nudge. Browser does not run the job; no repeated manual Wake required."
      });
    }

    const requestId = "starter_history_delta_update_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_starter_history_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Starter History",
      mode: "delta_update",
      ...
      no_full_sweep: true,
      delta_update_allowed: true,
      daily_incremental_launch: true,
      delta_start_date: "2026-05-19",
      delta_end_date: new Date().toISOString().slice(0,10),
      base_backfill_cutoff_date: "2026-05-18",
      delta_reserved_start_date: "2026-05-19",
      probe_only: false,
      classify_source_shape: false,
      no_live_promotion: false,
      no_full_base_backfill: false,
      no_delta_update_execution: false,
      no_scoring: true,
      no_ranking: true,
      no_final_board: true,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-starter-history', 'alphadog-v2-base-starter-history', 'Base', 'base', 'Base Starter History Delta Update v0.4.0+', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_starter_history', 'INFO', 'queued_base_starter_history_delta_update', 'Queued Base Starter History delta_update daily incremental launch', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "BASE > Starter History", mode: "delta_update", requested_preflight_behavior: "delta_scoped_source_repair", retained_stage_restore_before_queue: true, scoped_source_repair: true, schedule_noop_before_queue: true, no_full_sweep: true, daily_incremental_launch: true, delta_start_date: "2026-05-19", delta_end_date: new Date().toISOString().slice(0,10), base_backfill_cutoff_date: "2026-05-18", no_live_promotion: false, no_delta_update_execution: false, backend_hot_continuation: true, stage_retained: true })).run();

    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(callOrchestrator(env, "/pump", {
        source: "control_room_base_starter_history_delta_update_auto_start",
        max_cycles: 18,
        max_jobs_per_cycle: 1,
        max_ms: 70000,
        pump_depth: 0,
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: true,
      browser_auto_pump: false,
      backend_hot_continuation_launched: true,
      note: "Queued Base Starter History with executable mode delta_update for Morning Incremental readiness. Scoped repair/no-op behavior is preserved as safety metadata only; worker should not enter scoped-repair-only mode unless a dedicated repair route asks for it. No full sweep, no scoring, no board mutation."
    });
  }
  ...
  if (job === "orchestrator_enqueue_delta_team_game_logs") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Base/Delta Team Game Logs queue row found. Wait for completion before queueing another team-log job."
      });
    }
    ...
    const input = {
      ...
      worker_scope: "v0.3.1_delta_update_dynamic_latest_final_date_retained_stage",
      created_at: now,
      source_name: "MLB StatsAPI schedule plus game boxscore team totals delta_update",
      source_key: "mlb_statsapi_schedule_boxscore_team_totals_probe_v0_1_0",
      schedule_endpoint_pattern: "/api/v1/schedule?sportId=1&gameTypes=R&startDate={date}&endDate={date}",
      boxscore_endpoint_pattern: "/api/v1/game/{gamePk}/boxscore",
      delta_start_date: "2026-05-19",
      base_backfill_cutoff_date: "2026-05-18",
      delta_end_date_policy: "auto_latest_completed_final_mlb_game_date",
      retained_stage_required: true,
      retained_stage_restore_before_queue: true,
      scoped_delta_targets_only: true,
      schedule_noop_before_queue: true,
      no_full_sweep: true,
      allowed_writes: ["TEAM_DB.team_game_log_batches", "TEAM_DB.team_game_log_stage", "TEAM_DB.team_game_log_outcomes", "TEAM_DB.team_game_log_cursor", "TEAM_DB.team_game_log_certifications", "TEAM_DB.team_game_logs"],
      no_hitter_game_log_mutation: true,
      no_pitcher_game_log_mutation: true,
      no_splits_mutation: true,
      no_starter_history: true,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-team-game-logs', 'alphadog-v2-base-team-game-logs', 'Base', 'delta', 'Delta Team Game Logs v0.3.1', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_delta_team_game_logs', 'INFO', 'queued_delta_team_game_logs_update', 'Queued exact Delta Team Game Logs v0.3.1 dynamic delta_update from retained max date to latest final MLB date', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "DELTA > Team Game Logs", mode: "delta_update", requested_preflight_behavior: "delta_retained_stage_restore_before_queue", retained_stage_restore_before_queue: true, scoped_delta_targets_only: true, schedule_noop_before_queue: true, delta_start_date: "2026-05-19", delta_end_date_policy: "auto_latest_completed_final_mlb_game_date", retained_stage_required: true })).run();
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued Delta Team Game Logs with executable mode delta_update. Retained-stage restore/no-op behavior is preserved as metadata only; worker should not enter probe-only mode."
    });
  }
  ...
  if (job === "orchestrator_enqueue_delta_hitter_splits") {
    ...
    if (existing) {
      const hotStart = await callOrchestrator(env, "/pump", {
        source: "control_room_delta_hitter_splits_existing_direct_proxy_hot_continue",
        max_cycles: 4,
        max_jobs_per_cycle: 1,
        max_ms: 30000,
        pump_depth: 0,
      ...
      return jsonResponse({ ok: true, data_ok: true, version, job, status: "already_queued", request_id: existing.request_id, existing, visible_button: "DELTA > Hitter Splits", backend_scheduled_continuation: true, backend_direct_proxy_hot_start: orchestratorCallOk(hotStart), hot_start_route: hotStart.route || null, hot_start_http_status: hotStart.http_status, hot_start_body: hotStart.body, auto_pump_triggered: orchestratorCallOk(hotStart), browser_auto_pump: false, note: "Existing Base/Delta Hitter Splits queue row found. Control Room directly proxied one bounded backend pump so continuation starts immediately; browser still does not run the job." });
    }
    ...
    const input = {
      ...
      requested_preflight_behavior: "delta_retained_stage_restore_before_queue",
      created_at: now,
      worker_scope: "delta_noop_restore_scoped_repair_daily_affected_refresh_gate_v0_4_3",
      ...
      daily_affected_player_refresh_when_new_final_date: true,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-hitter-splits', 'alphadog-v2-base-hitter-splits', 'Delta', 'delta', 'Delta Hitter Splits No-op / Restore / Scoped Repair / Daily Affected Refresh v0.4.6', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_delta_hitter_splits', 'INFO', 'queued_delta_hitter_splits_scoped_repair_gate', 'Queued exact Delta Hitter Splits v0.4.3 scoped repair + daily affected-player refresh gate job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"DELTA > Hitter Splits", delta_update:true, noop_before_queue_if_current:true, restore_before_queue_if_retained_stage_available:true, no_new_mlb_calls_when_current:true, daily_affected_player_refresh_when_new_final_date:true})).run();

    const hotStart = await callOrchestrator(env, "/pump", {
      source: "control_room_delta_hitter_splits_direct_proxy_auto_start",
      max_cycles: 4,
      max_jobs_per_cycle: 1,
      max_ms: 30000,
      pump_depth: 0,
    ...
    return jsonResponse({ ok: true, data_ok: true, version, job, status: "queued", request_id: requestId, chain_id: chainId, visible_button: "DELTA > Hitter Splits", queued_job_key: "base-hitter-splits", queued_worker_name: "alphadog-v2-base-hitter-splits", delta_update: true, noop_before_queue_if_current: true, restore_before_queue_if_retained_stage_available: true, no_new_mlb_calls_when_current: true, daily_affected_player_refresh_when_new_final_date: true, no_full_base_split_mining: true, no_full_hitter_universe_refresh: true, backend_scheduled_continuation: true, backend_direct_proxy_hot_start: orchestratorCallOk(hotStart), hot_start_route: hotStart.route || null, hot_start_http_status: hotStart.http_status, hot_start_body: hotStart.body, auto_pump_triggered: orchestratorCallOk(hotStart), browser_auto_pump: false, backend_hot_continuation_launched: orchestratorCallOk(hotStart), note: "Queued v0.4.6 delta no-op / retained-registry restore / scoped-repair / daily affected-hitter refresh gate and directly proxied one bounded backend pump to start it immediately. Browser does not run the job. No full hitter-universe refresh, no scoring, no board mutation." });
  }
  ...
  if (job === "orchestrator_enqueue_delta_pitcher_splits") {
    ...
    if (existing) {
      const hotStart = await callOrchestrator(env, "/pump", {
        source: "control_room_delta_pitcher_splits_existing_direct_proxy_hot_continue",
        max_cycles: 4,
        max_jobs_per_cycle: 1,
        max_ms: 30000,
        pump_depth: 0,
      ...
      return jsonResponse({ ok: true, data_ok: true, version, job, status: "already_queued", request_id: existing.request_id, existing, visible_button: "DELTA > Pitcher Splits", backend_scheduled_continuation: true, backend_direct_proxy_hot_start: orchestratorCallOk(hotStart), hot_start_route: hotStart.route || null, hot_start_http_status: hotStart.http_status, hot_start_body: hotStart.body, auto_pump_triggered: orchestratorCallOk(hotStart), browser_auto_pump: false, note: "Existing Base Pitcher Splits queue row found. Control Room directly proxied one bounded backend pump so continuation starts immediately; browser still does not run the job." });
    }
    ...
    const input = {
      ...
      created_at: now,
      worker_scope: "delta_noop_restore_scoped_repair_daily_affected_refresh_gate_v0_5_10",
      ...
      delta_noop_restore_scoped_repair_gate: true,
      full_740_player_delta_disabled: true,
      retained_registry_restore_before_queue: true,
      scoped_repair_if_live_and_retained_missing: true,
      daily_affected_pitcher_refresh_when_new_final_date: true,
      no_full_pitcher_universe_refresh: true,
      no_balanced_split_assumption: true,
      outcome_rows_staged_non_authoritative: true,
      physical_truth_sources: ["pitcher_splits live rows", "pitcher_split_batches.rows_promoted"],
      no_broad_live_replacement: true,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-pitcher-splits', 'alphadog-v2-base-pitcher-splits', 'Delta', 'delta', 'Delta Pitcher Splits No-op / Restore / Scoped Repair / Daily Affected Refresh v0.5.10', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_delta_pitcher_splits', 'INFO', 'queued_delta_pitcher_splits_scoped_repair_gate', 'Queued Delta Pitcher Splits v0.5.10 no-op / retained-restore / scoped-repair / daily affected refresh gate', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "DELTA > Pitcher Splits", delta_noop_restore_scoped_repair_gate: true, retained_stage_restore_before_queue: true, daily_affected_pitcher_refresh_when_new_final_date: true, no_balanced_split_assumption: true })).run();
    const hotStart = await callOrchestrator(env, "/pump", {
      source: "control_room_delta_pitcher_splits_direct_proxy_auto_start",
      max_cycles: 4,
      max_jobs_per_cycle: 1,
      max_ms: 30000,
      pump_depth: 0,
    ...
    return jsonResponse({ ok: true, data_ok: true, version, job, status: "queued", request_id: requestId, chain_id: chainId, visible_button: "DELTA > Pitcher Splits", queued_job_key: "base-pitcher-splits", queued_worker_name: "alphadog-v2-base-pitcher-splits", delta_noop_restore_scoped_repair_gate: true, retained_stage_restore_before_queue: true, daily_affected_pitcher_refresh_when_new_final_date: true, no_balanced_split_assumption: true, outcome_rows_staged_non_authoritative: true, no_full_pitcher_universe_refresh: true, backend_scheduled_continuation: true, backend_direct_proxy_hot_start: orchestratorCallOk(hotStart), hot_start_route: hotStart.route || null, hot_start_http_status: hotStart.http_status, hot_start_body: hotStart.body, auto_pump_triggered: orchestratorCallOk(hotStart), browser_auto_pump: false, backend_hot_continuation_launched: orchestratorCallOk(hotStart), note: "Queued v0.5.10 delta no-op / retained-restore / scoped-repair / daily affected-pitcher refresh gate and directly proxied one bounded backend pump to start it immediately. Browser does not run the job. No full pitcher-universe refresh, no scoring, no board mutation." });
  }
