const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.47-base-starter-history-delta-noop";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_base_starter_history") {
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-starter-history', 'alphadog-v2-base-starter-history', 'Base', 'base', 'Base Starter History Delta Noop v0.4.1', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_starter_history', 'INFO', 'queued_base_starter_history_delta_noop_current_state', 'Queued Base Starter History v0.4.1 delta no-op current-state check', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "BASE > Starter History", mode: "delta_noop_current_state", delta_start_date: "2026-05-19", delta_end_date: new Date().toISOString().slice(0,10), base_backfill_cutoff_date: "2026-05-18", no_live_promotion: true, backend_hot_continuation: true, stage_retained: true })).run();

    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(callOrchestrator(env, "/pump", {
        source: "control_room_base_starter_history_delta_noop_auto_start",
        max_cycles: 18,
        max_jobs_per_cycle: 1,
        max_ms: 70000,
        pump_depth: 0,
    ...
    return jsonResponse({
      ...
      note: "Queued Base Starter History v0.4.1 delta no-op current-state check. Control Room launches one backend orchestrator pump via ctx.waitUntil. Browser may close now. ORCHESTRATOR > Wake is rescue only. No mining, no live mutation, no stage writes, no scoring, no board mutation."
    });
  }
  ...
  if (job === "orchestrator_enqueue_delta_team_game_logs") {
    ...
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "DELTA > Team Game Logs", mode: "delta_noop_current_state", delta_start_date: "2026-05-19", delta_end_date_policy: "auto_latest_completed_final_mlb_game_date", retained_stage_required: true })).run();
