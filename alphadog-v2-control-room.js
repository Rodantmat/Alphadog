const SYSTEM_VERSION = "alphadog-v2-control-room-v1.5.15-1101-recovery-sync-pump";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_static_players") {
    ...
    if (existing) {
      const pumpPayload = {source:"control_room_static_players_existing_sync_pump", request_id:existing.request_id, max_cycles:20, max_jobs_per_cycle:5, max_ms:27000, max_pump_chains:20};
      const pumpResult = await callOrchestrator(env, "/pump", pumpPayload);
      return jsonResponse({
        ok: true,
        data_ok: true,
        version,
        ...
        auto_pump_mode: "sync_awaited_recovery",
        auto_pump: pumpResult,
        note: "Existing Static Players queue row found. Control Room called orchestrator /pump synchronously and returned the pump result. Use ORCHESTRATOR > Logs and V2 SAFE ACTIONS > Queue."
      });
    }
    ...
    const pumpPayload = {source:"control_room_static_players_enqueue_sync_pump", request_id:requestId, max_cycles:20, max_jobs_per_cycle:5, max_ms:27000, max_pump_chains:20};
    const pumpResult = await callOrchestrator(env, "/pump", pumpPayload);
    ...
    return jsonResponse({
      ...
      auto_pump_mode: "sync_awaited_recovery",
      auto_pump: pumpResult,
      note: "Queued in CONTROL_DB and called orchestrator /pump synchronously. Use ORCHESTRATOR > Logs and V2 SAFE ACTIONS > Queue. Cron remains the fallback/weekly scheduler."
    });
  }
