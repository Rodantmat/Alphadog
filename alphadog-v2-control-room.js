const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.80-base-pitcher-metrics-sample-stage";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_base_hitter_metrics") {
    ...
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"BASE > Hitter Metrics", no_promotion:true, no_external_mlb_calls:true, no_scoring:true})).run();
  ...
  if (job === "orchestrator_enqueue_delta_hitter_metrics") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Hitter Metrics queue row found. Tap ORCHESTRATOR > Wake once only if it has not processed. Browser does not run the job."
      });
    }
  ...
  if (job === "orchestrator_enqueue_base_pitcher_metrics") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Base Pitcher Metrics sample-stage queue row found. Tap ORCHESTRATOR > Wake once only if it has not processed. Browser does not run the job."
      });
    }

    const requestId = "base_pitcher_metrics_sample_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_base_pitcher_metrics_sample_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Pitcher Metrics",
      mode: "sample_stage_calibration",
      created_at: now,
      worker_scope: "v0_2_0_sample_stage_calibration_only",
      source_name: "D1 audit from certified STATS_PITCHER_DB.pitcher_game_logs and STATS_PITCHER_DB.pitcher_splits",
      source_key: "d1_pitcher_game_logs_and_splits_sample_stage",
      source_season: 2026,
      config_profile_id: "pitcher_metrics_neutral_v0_2_0_sample_stage",
      formula_version: "pitcher_metrics_formula_v0_2_0_sample_stage",
      allowed_writes: [
        "STATS_PITCHER_DB.pitcher_metric_schema_migrations",
        "STATS_PITCHER_DB.pitcher_metric_batches",
        "STATS_PITCHER_DB.pitcher_metric_stage",
        "STATS_PITCHER_DB.pitcher_metric_outcomes",
        ...
        "STATS_PITCHER_DB.pitcher_metric_snapshot_stage",
      ...
      no_browser_auto_pump: true,
      wake_once_for_test_only: true,
      sample_stage_calibration_only: true,
      promotion_locked: true
    };

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-pitcher-metrics', 'alphadog-v2-base-pitcher-metrics', 'Base', 'base', 'Base Pitcher Metrics v0.2.0 Sample Stage Calibration', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_pitcher_metrics', 'INFO', 'queued_base_pitcher_metrics_v0_2_0_sample_stage_only', 'Queued exact Base Pitcher Metrics v0.2.0 sample-stage calibration job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"BASE > Pitcher Metrics", no_promotion:true, no_external_mlb_calls:true, no_scoring:true})).run();
    ...
    return jsonResponse({
      ...
      v0_1_0_sample_stage_calibration_only: true,
      backend_scheduled_continuation: true,
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued v0.2.0 sample-stage calibration. Tap ORCHESTRATOR > Wake to dispatch. Stage rows only; no live metric promotion, no source table mutation, no MLB calls, no scoring."
    });
  }
