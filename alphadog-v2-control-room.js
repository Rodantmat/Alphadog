const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.44-base-starter-history-hot-continuation";
  ...
  if (job === "orchestrator_enqueue_base_starter_history") {
    ...
    if (existing) {
      ...
      return jsonResponse({
        ...
        stage_only_base_backfill: true,
        no_live_promotion: true,
        backend_scheduled_continuation: true,
        auto_pump_triggered: true,
        browser_auto_pump: false,
        note: "Existing Base Starter History stage-only queue row found. Control Room launched the backend orchestrator pump as a hot-continuation nudge. Browser does not run the job; no repeated manual Wake required."
      });
    }

    const requestId = "starter_history_base_stage_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-starter-history', 'alphadog-v2-base-starter-history', 'Base', 'base', 'Base Starter History Stage-Only Base v0.2.0', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_starter_history', 'INFO', 'queued_base_starter_history_stage_only_base', 'Queued Base Starter History v0.2.1 stage-only base backfill with backend hot continuation', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({ request_id: requestId, chain_id: chainId, visible_button: "BASE > Starter History", mode: "base_backfill_stage_only", base_start_date: "2026-03-01", base_backfill_cutoff_date: "2026-05-18", max_games_per_tick: 50, no_live_promotion: true, backend_hot_continuation: true })).run();

    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(callOrchestrator(env, "/pump", {
        source: "control_room_base_starter_history_auto_start",
        max_cycles: 18,
        max_jobs_per_cycle: 1,
        max_ms: 70000,
        pump_depth: 0,
    ...
    return jsonResponse({
      ...
      note: "Queued Base Starter History v0.2.1 stage-only base backfill. Control Room launches one backend orchestrator pump via ctx.waitUntil; then orchestrator self-continues bounded chunks. Browser may close now. ORCHESTRATOR > Wake is rescue only. No live starter_history promotion, no delta execution."
    });
  }
