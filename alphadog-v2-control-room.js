const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.155-scoring-simulation-allowlist-fix";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_scoring_simulation") {
    ...
    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-score-audit','scoring-engine-simulation','11 Scoring','scoring','Scoring Engine Simulation Shadow',1,'https://alphadog-v2-score-audit.rodolfoaamattos.workers.dev','SCORE_AUDIT_WORKER',1,'Existing score-audit slot runs strict simulation shadow only. Writes SCORE_DB.scoring_engine_simulation_shadow and simulation audit tables only; compares Strict-B against Hybrid-Control; no scoring_engine_current mutation, no archive, no ranking, no final board.',CURRENT_TIMESTAMP)"
    ).run();
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'scoring-engine-simulation', 'alphadog-v2-score-audit', '11 Scoring', 'scoring', 'Scoring Engine Simulation Shadow', 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_scoring_engine_simulation', 'Queued exact Scoring Engine Simulation Shadow job', ?, CURRENT_TIMESTAMP)"
    ...
    return jsonResponse({
      ...
      thresholds_locked: false,
      scoring_enabled: false,
      note: "Queued Scoring Engine simulation. Writes only SCORE_DB scoring_engine_simulation_shadow/batches/issues, compares Strict-B to Hybrid-Control, and does not mutate scoring_engine_current, archive, ranking, or final board. Tap ORCHESTRATOR > Wake."
    });
  }
