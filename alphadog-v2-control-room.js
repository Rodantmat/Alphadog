const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.215-baseline-v5-classification-rescue-target-batch";
...
async function runJob(request, env, ctx) {
  ...
  try {
    ...
    if ([
      ...
      "orchestrator_enqueue_expansion_line_inventory",
      "orchestrator_enqueue_expansion_baseline_sanity",
      "orchestrator_enqueue_expansion_baseline_hp",
      "orchestrator_enqueue_expansion_baseline_v2_heb",
      "orchestrator_enqueue_expansion_baseline_v2_delta",
      "orchestrator_enqueue_baseline_v5_classification_base",
      "orchestrator_enqueue_baseline_v5_classification_rescue",
      "orchestrator_enqueue_baseline_v5_classification_delta",
      "orchestrator_enqueue_baseline_v5_base",
      "orchestrator_enqueue_baseline_v5_base_rescue",
      "orchestrator_enqueue_baseline_v5_delta",
      "orchestrator_enqueue_baseline_v5_state_hydrate",
      ...
      "orchestrator_enqueue_player_baseline_sanity",
      "orchestrator_enqueue_player_baseline_hp",
      ...
      "orchestrator_enqueue_prop_matrix_builder",
      "orchestrator_enqueue_scoring_engine",
      "orchestrator_enqueue_score_enrichment_v1",
      "orchestrator_enqueue_score_enrichment_v2_shadow",
      "orchestrator_enqueue_hit_probability_v3_shadow",
      "orchestrator_enqueue_final_score_v2_shadow",
      "orchestrator_enqueue_final_board_v3_shadow",
      "orchestrator_enqueue_hit_probability",
      "orchestrator_enqueue_hit_probability_v2",
      "orchestrator_enqueue_final_score_v1",
    ...
    if (job === "v2_hard_cleanup") {
      const activeQueueBefore = await queryAll(env.CONTROL_DB, "SELECT request_id, chain_id, job_key, worker_name, status, started_at, updated_at FROM control_job_queue WHERE status IN ('pending','running','requested','partial_continue') AND finished_at IS NULL ORDER BY datetime(created_at) ASC LIMIT 100");
      const activeLocksBefore = await queryAll(env.CONTROL_DB, "SELECT lock_key, lock_flag, owner_request_id, owner_worker_name, acquired_at, expires_at, updated_at FROM control_locks WHERE lock_flag = 1 ORDER BY updated_at DESC LIMIT 100");
      const unfinishedRunsBefore = await queryAll(env.CONTROL_DB, "SELECT run_id, request_id, chain_id, job_key, worker_name, status, certification_status, started_at, finished_at FROM control_job_runs WHERE finished_at IS NULL OR status IN ('running','queued') ORDER BY datetime(started_at) ASC LIMIT 100");

      const queueResult = await env.CONTROL_DB.prepare(
        "UPDATE control_job_queue SET status='cancelled', finished_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP, error_code=COALESCE(error_code,'hard_cleanup_cancelled_active_queue'), error_message=COALESCE(error_message,'Hard Cleanup cancelled active queue row; logs/history preserved, no deletion performed.') WHERE status IN ('pending','running','requested','partial_continue') AND finished_at IS NULL"
      ).run();

      const runsResult = await env.CONTROL_DB.prepare(
        "UPDATE control_job_runs SET status='failed', data_ok=0, certification_status='HARD_CLEANUP_RETIRED_ACTIVE_RUN_LEDGER', finished_at=CURRENT_TIMESTAMP, error_code=COALESCE(error_code,'hard_cleanup_retired_active_run_ledger'), error_message=COALESCE(error_message,'Hard Cleanup retired active/unfinished run ledger row; logs/history preserved, no deletion performed.') WHERE finished_at IS NULL OR status IN ('running','queued') OR certification_status='ORCHESTRATOR_DISPATCH_STARTED'"
      ).run();

      const locksResult = await env.CONTROL_DB.prepare(
        "UPDATE control_locks SET lock_flag=0, owner_request_id=NULL, owner_worker_name=NULL, expires_at=NULL, lock_json=NULL, updated_at=CURRENT_TIMESTAMP WHERE lock_flag=1 OR owner_request_id IS NOT NULL OR expires_at IS NOT NULL"
      ).run();

      const stateResult = await env.CONTROL_DB.prepare(
        "UPDATE control_system_state SET lock_flag=0, running_job_key=NULL, running_request_id=NULL, running_chain_id=NULL, status='IDLE', updated_at=CURRENT_TIMESTAMP WHERE lock_flag=1 OR running_job_key IS NOT NULL OR running_request_id IS NOT NULL OR status <> 'IDLE'"
      ).run();

      const activeQueueAfter = await queryAll(env.CONTROL_DB, "SELECT request_id, chain_id, job_key, worker_name, status, started_at, updated_at FROM control_job_queue WHERE status IN ('pending','running','requested','partial_continue') AND finished_at IS NULL ORDER BY datetime(created_at) ASC LIMIT 100");
      const activeLocksAfter = await queryAll(env.CONTROL_DB, "SELECT lock_key, lock_flag, owner_request_id, owner_worker_name, acquired_at, expires_at, updated_at FROM control_locks WHERE lock_flag = 1 ORDER BY updated_at DESC LIMIT 100");
      const unfinishedRunsAfter = await queryAll(env.CONTROL_DB, "SELECT run_id, request_id, chain_id, job_key, worker_name, status, certification_status, started_at, finished_at FROM control_job_runs WHERE finished_at IS NULL ORDER BY datetime(started_at) ASC LIMIT 100");

      await env.CONTROL_DB.prepare(
        "INSERT INTO control_worker_run_log (worker_name, job_key, level, event_key, message, data_json, created_at) VALUES ('alphadog-v2-control-room', 'v2_hard_cleanup', 'WARN', 'hard_cleanup_executed', 'Hard Cleanup cancelled active queue rows, retired unfinished run ledgers, and released locks without deleting audit history.', ?, CURRENT_TIMESTAMP)"
      ).bind(JSON.stringify({
        queue_rows_seen_before: activeQueueBefore.length,
        lock_rows_seen_before: activeLocksBefore.length,
        unfinished_run_rows_seen_before: unfinishedRunsBefore.length,
        queue_changes: queueResult.meta && queueResult.meta.changes,
        run_changes: runsResult.meta && runsResult.meta.changes,
        lock_changes: locksResult.meta && locksResult.meta.changes,
        state_changes: stateResult.meta && stateResult.meta.changes,
        active_queue_after: activeQueueAfter.length,
        active_locks_after: activeLocksAfter.length,
        unfinished_runs_after: unfinishedRunsAfter.length,
        no_deletes: true,
        logs_preserved: true,
        version: SYSTEM_VERSION
      })).run();
      ...
      return jsonResponse({
        ok: true,
        data_ok: activeQueueAfter.length === 0 && activeLocksAfter.length === 0 && unfinishedRunsAfter.length === 0,
        version: SYSTEM_VERSION,
        job,
        status: "hard_cleanup_completed_no_deletes",
        no_deletes: true,
        logs_preserved: true,
        queue_rows_seen_before: activeQueueBefore.length,
        lock_rows_seen_before: activeLocksBefore.length,
        ...
        lock_changes: locksResult.meta && locksResult.meta.changes,
        state_changes: stateResult.meta && stateResult.meta.changes,
        remaining_active_queue_rows: activeQueueAfter,
        remaining_active_locks: activeLocksAfter,
        remaining_unfinished_runs: unfinishedRunsAfter
      });
    }
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  const shadowScoringJobs = {
    orchestrator_enqueue_score_enrichment_v2_shadow: {
      visible_button: "SCORING > Enrich V2",
      job_key: "score-enrichment-v2-shadow",
      display_name: "Score Enrichment V2 Shadow",
      mode: "score_enrichment_v2_shadow",
      logical_worker_name: "alphadog-v2-score-enrichment-v2-shadow",
      priority: 7
    },
    orchestrator_enqueue_hit_probability_v3_shadow: {
      visible_button: "SCORING > HP V3",
      job_key: "hit-probability-v3-shadow",
      display_name: "Hit Probability V3 Shadow",
      mode: "hit_probability_v3_shadow",
      logical_worker_name: "alphadog-v2-hit-probability-v3-shadow",
      priority: 8
    },
    orchestrator_enqueue_final_score_v2_shadow: {
      visible_button: "SCORING > Final Score V2",
      job_key: "final-score-v2-shadow",
      display_name: "Final Score V2 Shadow",
      mode: "final_score_v2_shadow",
      logical_worker_name: "alphadog-v2-final-score-v2-shadow",
      priority: 9
    },
    orchestrator_enqueue_final_board_v3_shadow: {
      visible_button: "SCORING > Final Board V3",
      job_key: "final-board-v3-shadow",
      display_name: "Final Board V3 Shadow",
      mode: "score_final_board_v3_shadow",
      logical_worker_name: "alphadog-v2-final-board-v3-shadow",
      priority: 9
    }
  };
  if (shadowScoringJobs[job]) {
    const meta = shadowScoringJobs[job];
    return jsonResponse({ ok: false, data_ok: false, version, job, status: "retired_2026_07_14", certification: "LEGACY_SHADOW_SCORING_RETIRED", visible_button: meta.visible_button, retired_job_key: meta.job_key, note: `This legacy score-audit.js-based ${meta.display_name} was retired 2026-07-14 after being confirmed inactive since 2026-06-30 (superseded by the new dedicated Scoring Full Run chain, SCORING > Full Run / SCORING > Daily Full).` }, 410);
  }
  if (false) {
    const meta = shadowScoringJobs[job];
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = ? AND status IN ('pending','running') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).bind(meta.job_key).first();
    if (existing) {
      return jsonResponse({ ok:true, data_ok:true, version, job, status:"already_queued", request_id:existing.request_id, existing, visible_button:meta.visible_button, queued_job_key:meta.job_key });
    }
    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-score-audit', ?, '11 Scoring', 'scoring', ?, 1, 'https://alphadog-v2-score-audit.rodolfoaamattos.workers.dev', 'SCORE_AUDIT_WORKER', 1, 'V2/V3 shadow scoring chain. Review-only, worker-owned v2_* shadow tables, no production final board/live activation.', CURRENT_TIMESTAMP)"
    ).bind(meta.job_key, meta.display_name).run();
    const requestId = meta.job_key.replace(/-/g, "_") + "_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_" + meta.job_key.replace(/-/g, "_") + "_" + Date.now().toString(36);
    let latestFinalScoreV2BatchId = null;
    let latestFinalScoreOfficialDateFloor = null;
    if (meta.job_key === "final-board-v3-shadow" && env.SCORE_DB) {
      try {
        const latestFinalScoreRow = await env.SCORE_DB.prepare("SELECT batch_id FROM v2_final_score_batches WHERE status='completed' ORDER BY datetime(updated_at) DESC LIMIT 1").first();
        latestFinalScoreV2BatchId = latestFinalScoreRow && latestFinalScoreRow.batch_id ? latestFinalScoreRow.batch_id : null;
        if (latestFinalScoreV2BatchId) {
          const dateRow = await env.SCORE_DB.prepare("SELECT MIN(official_date) AS min_official_date FROM v2_final_score_current WHERE batch_id=? AND official_date IS NOT NULL").bind(latestFinalScoreV2BatchId).first();
          latestFinalScoreOfficialDateFloor = dateRow && dateRow.min_official_date ? String(dateRow.min_official_date).slice(0, 10) : null;
        }
      } catch (e) {
        latestFinalScoreV2BatchId = null;
        latestFinalScoreOfficialDateFloor = null;
      }
    }
    const input = {
      source: "control_room",
      ...
      mode: meta.mode,
      created_at: now,
      logical_worker_name: meta.logical_worker_name,
      deployed_worker_slot: "alphadog-v2-score-audit",
      exact_worker_only: true,
      v2_v3_shadow_chain: true,
      worker_owned_schema_creation: true,
      no_old_production_touch: true,
      no_live_activation: true,
      review_only: true,
      live_playable_forced_zero: true,
      goblin_demon_score_only_not_hp: true,
      source_line_variant_detection_required: true,
      queued_job_key: meta.job_key,
      queued_worker_name: "alphadog-v2-score-audit"
    };
    if (latestFinalScoreV2BatchId) {
      input.source_final_score_v2_batch_id = latestFinalScoreV2BatchId;
      input.final_score_v2_batch_id = latestFinalScoreV2BatchId;
      input.source_batch_forwarded_by_control_room = true;
    }
    if (latestFinalScoreOfficialDateFloor) {
      input.board_official_date_floor = latestFinalScoreOfficialDateFloor;
      input.board_official_date_floor_source = "control_room_source_final_score_min_official_date";
    }
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, ?, 'alphadog-v2-score-audit', '11 Scoring', 'scoring', ?, 'pending', ?, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, meta.job_key, meta.display_name, meta.priority, JSON.stringify(input)).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_v2_v3_shadow_scoring_job', 'Queued V2/V3 shadow scoring job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, job, JSON.stringify({ request_id:requestId, chain_id:chainId, queued_job_key:meta.job_key, visible_button:meta.visible_button })).run();
    return jsonResponse({ ok:true, data_ok:true, version, job, status:"queued", request_id:requestId, chain_id:chainId, visible_button:meta.visible_button, queued_job_key:meta.job_key, queued_worker_name:"alphadog-v2-score-audit", logical_worker_name:meta.logical_worker_name, mode:meta.mode, source_final_score_v2_batch_id:latestFinalScoreV2BatchId, board_official_date_floor:latestFinalScoreOfficialDateFloor, source_batch_forwarded_by_control_room:!!latestFinalScoreV2BatchId, review_only:true, live_playable_forced_zero:true, note:"Queued V2/V3 shadow scoring job. Use ORCHESTRATOR > Wake to process." });
  }
  ...
  if (job === "orchestrator_enqueue_expansion_baseline_full_run" || job === "orchestrator_enqueue_expansion_line_inventory" || job === "orchestrator_enqueue_expansion_baseline_sanity" || job === "orchestrator_enqueue_expansion_baseline_hp") {
    const map = {
      orchestrator_enqueue_expansion_baseline_full_run: { job_key:"expansion-baseline-full-run", mode:"expansion_baseline_full_run", label:"EXPANSION > Full Baseline", display:"Expansion Baseline Full Run" },
      orchestrator_enqueue_expansion_line_inventory: { job_key:"expansion-baseline-line-inventory", mode:"expansion_line_inventory", label:"EXPANSION > Line Inventory", display:"Expansion Baseline Line Inventory" },
      orchestrator_enqueue_expansion_baseline_sanity: { job_key:"expansion-baseline-sanity", mode:"expansion_baseline_sanity", label:"EXPANSION > Sanity", display:"Expansion Baseline Sanity" },
      orchestrator_enqueue_expansion_baseline_hp: { job_key:"expansion-baseline-hp", mode:"expansion_baseline_hp", label:"EXPANSION > HP", display:"Expansion Baseline HP" }
    };
    const cfg = map[job];
    const existing = await env.CONTROL_DB.prepare("SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = ? AND status IN ('pending','running','partial_continue') ORDER BY datetime(created_at) DESC LIMIT 1").bind(cfg.job_key).first();
    if (existing) return jsonResponse({ok:true,data_ok:true,version,job,status:"already_queued",request_id:existing.request_id,existing,visible_button:cfg.label,note:"Existing expansion job found. Use ORCHESTRATOR > Wake."});
    const requestId = cfg.job_key.replace(/-/g,"_") + "_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2,8);
    const chainId = "chain_" + cfg.job_key.replace(/-/g,"_") + "_" + Date.now().toString(36);
    const input = {source:"control_room",visible_button:cfg.label,mode:cfg.mode,expansion_mode:cfg.mode,created_at:now,expansion_only:true,baseline_only:true,writes_expansion_tables_only:true,write_table_whitelist_regex:"^expansion_",no_current_baseline_mutation:true,no_factor_mutation:true,no_matrix_mutation:true,no_scoring_mutation:true,no_final_board_mutation:true,no_scheduler_mutation:true,no_full_run_integration:true,no_market_context:true,no_daily_context:true,no_scoring_context:true,full_depth_base:true,mining_game_chunk_size:60,target_established_sample:25};
    await env.CONTROL_DB.prepare("INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-phase3a-first-inning-pitcher-context',?, 'Expansion','expansion_baseline',?,1,'https://alphadog-v2-phase3a-first-inning-pitcher-context.rodolfoaamattos.workers.dev','PHASE3A_FIRST_INNING_PITCHER_CONTEXT_WORKER',1,'Baseline-only expansion routed through first-inning pitcher context slot. Writes expansion_ tables only; no production baseline/scoring/final-board mutation.',CURRENT_TIMESTAMP)").bind(cfg.job_key,cfg.display).run();
    await env.CONFIG_DB.prepare("INSERT OR REPLACE INTO config_worker_definitions (worker_name, job_key, worker_group, phase_key, display_name, enabled, schedule_profile_key, notes) VALUES ('alphadog-v2-phase3a-first-inning-pitcher-context',?, 'Expansion','expansion_baseline',?,1,'manual_or_orchestrated','Baseline-only expansion job. Writes expansion_ tables only; no production mutation.')").bind(cfg.job_key,cfg.display).run();
    await env.CONTROL_DB.prepare("INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, ?, 'alphadog-v2-phase3a-first-inning-pitcher-context', 'Expansion', 'expansion_baseline', ?, 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)").bind(requestId,chainId,cfg.job_key,cfg.display,JSON.stringify(input)).run();
    await env.CONTROL_DB.prepare("INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_expansion_baseline_job', 'Queued expansion baseline job', ?, CURRENT_TIMESTAMP)").bind(requestId,job,JSON.stringify({request_id:requestId,chain_id:chainId,visible_button:cfg.label,queued_job_key:cfg.job_key,mode:cfg.mode})).run();
    return jsonResponse({ok:true,data_ok:true,version,job,status:"queued",request_id:requestId,chain_id:chainId,visible_button:cfg.label,queued_job_key:cfg.job_key,queued_worker_name:"alphadog-v2-phase3a-first-inning-pitcher-context",mode:cfg.mode,note:"Queued expansion baseline job. Use ORCHESTRATOR > Wake."});
  }



  if (["orchestrator_enqueue_baseline_v5_classification_base","orchestrator_enqueue_baseline_v5_classification_rescue","orchestrator_enqueue_baseline_v5_classification_delta","orchestrator_enqueue_baseline_v5_base","orchestrator_enqueue_baseline_v5_base_rescue","orchestrator_enqueue_baseline_v5_delta","orchestrator_enqueue_baseline_v5_state_hydrate","orchestrator_enqueue_baseline_v5_stateful_delta"].includes(job)) {
    const modeMap = {
      orchestrator_enqueue_baseline_v5_classification_base: "baseline_v5_classification_base",
      orchestrator_enqueue_baseline_v5_classification_rescue: "baseline_v5_classification_rescue",
      orchestrator_enqueue_baseline_v5_classification_delta: "baseline_v5_classification_delta",
      orchestrator_enqueue_baseline_v5_base: "baseline_v5_base",
      orchestrator_enqueue_baseline_v5_base_rescue: "baseline_v5_base_rescue",
      orchestrator_enqueue_baseline_v5_delta: "baseline_v5_delta",
      orchestrator_enqueue_baseline_v5_state_hydrate: "baseline_v5_state_hydrate",
      orchestrator_enqueue_baseline_v5_stateful_delta: "baseline_v5_stateful_delta"
    };
    const labelMap = {
      baseline_v5_classification_base: "FINAL SCORING SYSTEM > Classification Base",
      baseline_v5_classification_rescue: "FINAL SCORING SYSTEM > Classification Base Rescue",
      baseline_v5_classification_delta: "FINAL SCORING SYSTEM > Old Classification Delta Blocked",
      baseline_v5_base: "FINAL SCORING SYSTEM > Baseline Base",
      baseline_v5_base_rescue: "FINAL SCORING SYSTEM > Baseline Base Rescue",
      baseline_v5_delta: "FINAL SCORING SYSTEM > Old Baseline Delta Blocked",
      baseline_v5_state_hydrate: "FINAL SCORING SYSTEM > Baseline V5 State Hydrate",
      baseline_v5_stateful_delta: "FINAL SCORING SYSTEM > Baseline V5 Stateful Delta"
    };
    const mode = modeMap[job];
    const visibleButton = labelMap[mode];
    if (mode === "baseline_v5_classification_delta" || mode === "baseline_v5_delta") {
      return jsonResponse({ ok:false, data_ok:false, version, job, status:"BASELINE_V5_OLD_AFFECTED_PLAYER_CUMULATIVE_DELTA_BLOCKED", certification:"BASELINE_V5_OLD_AFFECTED_PLAYER_CUMULATIVE_DELTA_BLOCKED", certification_grade:"BLOCKED", visible_button:visibleButton, requested_mode:mode, queued:false, current_tables_mutated:false, history_tables_mutated:false, full_cumulative_history_recompute:true, blocked_reason:"Old Baseline/Classification V5 delta reloads cumulative player history and is banned. Use FINAL SCORING SYSTEM > Baseline V5 State Hydrate, then guarded Stateful Delta shadow/parity path." }, 200);
    }
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at, json_extract(input_json,'$.mode') AS mode FROM control_job_queue WHERE job_key = 'expansion-baseline-v2' AND status IN ('pending','running','partial_continue') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();
    if (existing) {
      if (ctx && typeof ctx.waitUntil === "function") {
        ctx.waitUntil(callOrchestrator(env, "/pump", { source:"control_room_baseline_v5_existing_hot_continue", max_cycles:18, max_jobs_per_cycle:1, max_ms:65000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, baseline_v5_watchdog:true }));
      }
      return jsonResponse({ ok:true, data_ok:true, version, job, status:"already_queued", request_id:existing.request_id, existing, visible_button:visibleButton, requested_mode:mode, backend_hot_continue_triggered:true, auto_pump_triggered:true, browser_auto_pump:false, note:"Existing Baseline V5 job found. Backend hot continuation was launched; no duplicate was queued." });
    }
    const requestId = mode + "_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_" + mode + "_" + Date.now().toString(36);
    const isBase = mode.endsWith("_base");
    const isStateHydrate = mode === "baseline_v5_state_hydrate";
    const isStatefulDelta = mode === "baseline_v5_stateful_delta";
    const isRescue = mode === "baseline_v5_classification_rescue" || mode === "baseline_v5_base_rescue";
    const isClassificationRescue = mode === "baseline_v5_classification_rescue";
    const isBaseRescue = mode === "baseline_v5_base_rescue";
    const isClassification = mode.indexOf("classification") >= 0;
    const input = {
      source:"control_room", visible_button:visibleButton, mode, expansion_mode:mode, created_at:now,
      logical_worker_name:"alphadog-v2-baseline-v5", deployed_worker_slot:"alphadog-v2-phase3a-first-inning-pitcher-context",
      baseline_v5:true, baseline_v5_function:isStateHydrate?"state_hydrate":(isStatefulDelta?"stateful_delta":(isClassification?"classification":"baseline")), baseline_v5_run_type:isStateHydrate?"state_hydrate":(isStatefulDelta?"stateful_delta_shadow_full_run":(isBaseRescue?"base_rescue":(isRescue?"rescue":(isBase?"base":"delta")))),
      v2_chunk_size:isRescue?40:(isBase?160:80), v2_all_current_chunk_size:isRescue?40:(isBase?160:80), v2_soft_yield_ms:isRescue?30000:(isBase?60000:45000),
      read_all_hp_v2_current:isBase, source_hp_v2_batch_id:"__BASELINE_V5_HISTORY_ONLY__", hp_v2_batch_id:"__BASELINE_V5_HISTORY_ONLY__",
      baseline_source_policy:"baseline_v5_history_only_static_base_delta_expansion_no_board_no_market_no_daily_no_app",
      history_only:true, no_board_context:true, no_market_context:true, no_daily_context:true, no_odds_context:true, no_app_context:true,
      no_prizepicks:true, no_sleeper:true, no_scoring:true, no_final_board:true, no_scheduler_mutation:true, no_current_baseline_mutation:(!isBaseRescue || isStateHydrate || isStatefulDelta), targeted_current_baseline_mutation:isBaseRescue,
      current_table_mutation_enabled:false, shadow_only:isStatefulDelta, hydrate_state_only:isStateHydrate, full_cumulative_history_recompute:false,
      writes_score_db_v5_tables_only:true,
      batch_id:isClassificationRescue?"player_baseline_v2_batch_mr1t5div_07sd4p":null,
      baseline_v5_classification_rescue_target_batch_id:isClassificationRescue?"player_baseline_v2_batch_mr1t5div_07sd4p":null,
      baseline_v5_base_rescue_target_batch_id:isBaseRescue?"player_baseline_v2_batch_mr1t5div_07sd4p":null,
      expected_baseline_base_rescue_rows:isBaseRescue?18620:null,
      expected_baseline_base_rescue_pairs:isBaseRescue?9310:null,
      expected_baseline_base_rescue_extreme_tail_conf_le_5:isBaseRescue?266:null,
      baseline_base_rescue_confidence_version:isBaseRescue?"baseline_v5_confidence_v0.5.3_history_sample_only_no_context_RESCUE_PITCHER_TIER_v1":null,
      allowed_reads:["STATS_HITTER_DB.hitter_game_logs","STATS_HITTER_DB.hitter_splits","STATS_PITCHER_DB.pitcher_game_logs","STATS_PITCHER_DB.pitcher_splits","TEAM_DB.starter_history","SCORE_DB.hit_probability_profile_configs","SCORE_DB.player_baseline_hp_v2_current","SCORE_DB.player_baseline_classification_v5_current","SCORE_DB.player_baseline_hp_v2_source_queue"],
      allowed_writes:isStateHydrate||isStatefulDelta?["SCORE_DB.player_baseline_v5_state_batches","SCORE_DB.player_baseline_v5_hp_state_current","SCORE_DB.player_baseline_v5_classification_state_current","SCORE_DB.player_baseline_v5_delta_events","SCORE_DB.player_baseline_v5_state_audit"]:["SCORE_DB.player_baseline_classification_v5_batches","SCORE_DB.player_baseline_classification_v5_stage","SCORE_DB.player_baseline_classification_v5_current","SCORE_DB.player_baseline_classification_v5_history","SCORE_DB.player_baseline_sanity_v2_batches","SCORE_DB.player_baseline_sanity_v2_stage","SCORE_DB.player_baseline_sanity_v2_current","SCORE_DB.player_baseline_sanity_v2_history","SCORE_DB.player_baseline_sanity_v2_issues","SCORE_DB.player_baseline_hp_v2_batches","SCORE_DB.player_baseline_hp_v2_stage","SCORE_DB.player_baseline_hp_v2_current","SCORE_DB.player_baseline_hp_v2_history","SCORE_DB.player_baseline_hp_v2_issues"]
    };
    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-phase3a-first-inning-pitcher-context','expansion-baseline-v2','Final Scoring System','final_scoring_system','Baseline V5 Classification/Baseline/State',1,'https://alphadog-v2-phase3a-first-inning-pitcher-context.rodolfoaamattos.workers.dev','PHASE3A_FIRST_INNING_PITCHER_CONTEXT_WORKER',1,'Baseline V5 history-only Classification/Base/Delta entrypoints. No board, market, PP/Sleeper, daily context, scoring, or final board mutation.',CURRENT_TIMESTAMP)"
    ).run();
    await env.CONFIG_DB.prepare(
      "INSERT OR REPLACE INTO config_worker_definitions (worker_name, job_key, worker_group, phase_key, display_name, enabled, schedule_profile_key, notes) VALUES ('alphadog-v2-phase3a-first-inning-pitcher-context','expansion-baseline-v2','Final Scoring System','final_scoring_system','Baseline V5 Classification/Baseline/State',1,'manual_or_orchestrated','Baseline V5 buttons: old cumulative deltas blocked; State Hydrate anchors HP and Classification state under Final Scoring System; Stateful Delta runs state shadow updates in the incremental full run after final Calendar/Tally proves complete source days; current/history mutation remains blocked until parity is approved.')"
    ).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'expansion-baseline-v2', 'alphadog-v2-phase3a-first-inning-pitcher-context', 'Final Scoring System', 'final_scoring_system', ?, 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, visibleButton, JSON.stringify(input)).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_baseline_v5_final_scoring_system', 'Queued Baseline V5 final scoring system job with backend hot continuation', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, job, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:visibleButton, queued_job_key:"expansion-baseline-v2", queued_worker_name:"alphadog-v2-phase3a-first-inning-pitcher-context", mode, baseline_v5_function:input.baseline_v5_function, baseline_v5_run_type:input.baseline_v5_run_type, backend_hot_continue_triggered:true})).run();
    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(callOrchestrator(env, "/pump", { source:"control_room_baseline_v5_enqueue", max_cycles:18, max_jobs_per_cycle:1, max_ms:65000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, baseline_v5_watchdog:true }));
      ctx.waitUntil((async()=>{ await sleepMs(25000); await callOrchestrator(env, "/pump", { source:"control_room_baseline_v5_watchdog_25s", max_cycles:18, max_jobs_per_cycle:1, max_ms:65000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, baseline_v5_watchdog:true }); })());
    }
    return jsonResponse({ ok:true, data_ok:true, version, job, status:"queued", request_id:requestId, chain_id:chainId, visible_button:visibleButton, queued_job_key:"expansion-baseline-v2", queued_worker_name:"alphadog-v2-phase3a-first-inning-pitcher-context", mode, baseline_v5_function:input.baseline_v5_function, baseline_v5_run_type:input.baseline_v5_run_type, auto_pump_triggered:true, browser_auto_pump:false, note:"Queued Baseline V5 Final Scoring System job. Old cumulative deltas are blocked; state hydrate/delta use guarded no-current-mutation path unless explicitly enabled." });
  }

  if (job === "orchestrator_enqueue_expansion_baseline_v2_delta") {
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = 'expansion-baseline-v2' AND status IN ('pending','running','partial_continue') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();
    if (existing) {
      if (ctx && typeof ctx.waitUntil === "function") {
        ctx.waitUntil(callOrchestrator(env, "/pump", { source:"control_room_expansion_baseline_v2_delta_existing_hot_continue", max_cycles:18, max_jobs_per_cycle:1, max_ms:65000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, expansion_baseline_v2_watchdog:true }));
      }
      return jsonResponse({ ok:true, data_ok:true, version, job, status:"already_queued", request_id:existing.request_id, existing, visible_button:"FINAL SCORING SYSTEM > Baseline Delta", backend_hot_continue_triggered:true, auto_pump_triggered:true, browser_auto_pump:false, note:"Existing Expansion V2 baseline job found. Backend hot continuation was launched; no duplicate delta was queued." });
    }
    const latest = await env.SCORE_DB.prepare("SELECT batch_id, status, certification, updated_at FROM hit_probability_v2_batches ORDER BY datetime(updated_at) DESC LIMIT 1").first();
    if (!latest || !latest.batch_id) {
      return jsonResponse({ ok:false, data_ok:false, version, job, status:"blocked_no_latest_hp_v2_batch", visible_button:"FINAL SCORING SYSTEM > Baseline Delta", note:"Baseline Delta requires latest SCORE_DB.hit_probability_v2_batches.batch_id. Run Baseline Base or upstream HP V2 first." }, 200);
    }
    const requestId = "expansion_baseline_v2_delta_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_expansion_baseline_v2_delta_" + Date.now().toString(36);
    const input = {
      source:"control_room", visible_button:"FINAL SCORING SYSTEM > Baseline Delta", mode:"baseline_v2_heb", expansion_mode:"baseline_v2_heb", v2_chunk_size:80, v2_all_current_chunk_size:80, v2_soft_yield_ms:45000, created_at:now,
      baseline_run_type:"BASELINE_DELTA_LATEST_HP_V2_BATCH", final_scoring_system:true, final_scoring_ui_route:"baseline_delta", latest_hp_v2_batch:latest,
      logical_worker_name:"alphadog-v2-expansion-baseline-v2-heb", deployed_worker_slot:"alphadog-v2-phase3a-first-inning-pitcher-context",
      source_hp_v2_batch_id:String(latest.batch_id), hp_v2_batch_id:String(latest.batch_id), read_all_hp_v2_current:false,
      v2_source_mode:"latest_hit_probability_v2_batch_delta_only", fast_safe_chunk_policy:"delta_latest_hp_batch_default_80_batched_stage_writes_soft_yield_45s", baseline_source_policy:"source_agnostic_unless_formula_profile_is_source_specific",
      parallel_v2_only:true, no_current_baseline_mutation:true, no_current_sanity_mutation:true, no_scoring:true, no_final_board:true, no_scheduler_mutation:true,
      writes_score_db_v2_tables_only:true,
      allowed_writes:["SCORE_DB.player_baseline_sanity_v2_batches","SCORE_DB.player_baseline_sanity_v2_stage","SCORE_DB.player_baseline_sanity_v2_current","SCORE_DB.player_baseline_sanity_v2_history","SCORE_DB.player_baseline_sanity_v2_issues","SCORE_DB.player_baseline_hp_v2_batches","SCORE_DB.player_baseline_hp_v2_stage","SCORE_DB.player_baseline_hp_v2_current","SCORE_DB.player_baseline_hp_v2_history","SCORE_DB.player_baseline_hp_v2_issues"]
    };
    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-phase3a-first-inning-pitcher-context','expansion-baseline-v2','Expansion','expansion','Expansion Baseline V2 HEB Parallel',1,'https://alphadog-v2-phase3a-first-inning-pitcher-context.rodolfoaamattos.workers.dev','PHASE3A_FIRST_INNING_PITCHER_CONTEXT_WORKER',1,'Parallel v2 HEB sanity/baseline HP generator. Baseline Base reads all-current; Baseline Delta reads latest HP V2 batch only. Writes only player_baseline_*_v2 tables.',CURRENT_TIMESTAMP)"
    ).run();
    await env.CONFIG_DB.prepare(
      "INSERT OR REPLACE INTO config_worker_definitions (worker_name, job_key, worker_group, phase_key, display_name, enabled, schedule_profile_key, notes) VALUES ('alphadog-v2-phase3a-first-inning-pitcher-context','expansion-baseline-v2','Expansion','expansion','Expansion Baseline V2 HEB Parallel',1,'manual_or_orchestrated','Parallel v2 HEB sanity/baseline HP generator. Base all-current; Delta latest HP V2 batch only. No current production mutation.')"
    ).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'expansion-baseline-v2', 'alphadog-v2-phase3a-first-inning-pitcher-context', 'Expansion', 'expansion', 'Expansion Baseline V2 Delta', 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_expansion_baseline_v2_delta', 'INFO', 'queued_expansion_baseline_v2_delta', 'Queued Expansion V2 HEB latest-batch delta baseline with backend hot continuation', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"FINAL SCORING SYSTEM > Baseline Delta", queued_job_key:"expansion-baseline-v2", queued_worker_name:"alphadog-v2-phase3a-first-inning-pitcher-context", mode:"baseline_v2_heb", source_hp_v2_batch_id:String(latest.batch_id), read_all_hp_v2_current:false, backend_hot_continue_triggered:true})).run();
    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(callOrchestrator(env, "/pump", { source:"control_room_expansion_baseline_v2_delta_enqueue", max_cycles:18, max_jobs_per_cycle:1, max_ms:65000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, expansion_baseline_v2_watchdog:true }));
      ctx.waitUntil((async()=>{ await sleepMs(25000); await callOrchestrator(env, "/pump", { source:"control_room_expansion_baseline_v2_delta_watchdog_25s", max_cycles:18, max_jobs_per_cycle:1, max_ms:65000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, expansion_baseline_v2_watchdog:true }); })());
    }
    return jsonResponse({ ok:true, data_ok:true, version, job, status:"queued", request_id:requestId, chain_id:chainId, visible_button:"FINAL SCORING SYSTEM > Baseline Delta", queued_job_key:"expansion-baseline-v2", queued_worker_name:"alphadog-v2-phase3a-first-inning-pitcher-context", mode:"baseline_v2_heb", source_hp_v2_batch_id:String(latest.batch_id), read_all_hp_v2_current:false, auto_pump_triggered:true, browser_auto_pump:false, note:"Queued latest HP V2 batch delta baseline. Existing orchestrator expansion-baseline-v2 dispatch handles this job_key." });
  }

  if (job === "orchestrator_enqueue_expansion_baseline_v2_heb") {
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = 'expansion-baseline-v2' AND status IN ('pending','running','partial_continue') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();
    if (existing) {
      if (ctx && typeof ctx.waitUntil === "function") {
        ctx.waitUntil(callOrchestrator(env, "/pump", { source:"control_room_expansion_baseline_v2_existing_hot_continue", max_cycles:18, max_jobs_per_cycle:1, max_ms:65000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, expansion_baseline_v2_watchdog:true }));
      }
      return jsonResponse({ ok:true, data_ok:true, version, job, status:"already_queued", request_id:existing.request_id, existing, visible_button:"EXPANSION > V2 HEB Baseline", backend_hot_continue_triggered:true, auto_pump_triggered:true, browser_auto_pump:false, note:"Existing Expansion V2 HEB all-current baseline job found. Backend hot continuation was launched; manual Wake is rescue only." });
    }
    const requestId = "expansion_baseline_v2_heb_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_expansion_baseline_v2_heb_" + Date.now().toString(36);
    const input = {
      source:"control_room", visible_button:"EXPANSION > V2 HEB Baseline", mode:"baseline_v2_heb", expansion_mode:"baseline_v2_heb", v2_chunk_size:160, v2_all_current_chunk_size:160, v2_soft_yield_ms:60000, created_at:now,
      logical_worker_name:"alphadog-v2-expansion-baseline-v2-heb", deployed_worker_slot:"alphadog-v2-phase3a-first-inning-pitcher-context",
      source_hp_v2_batch_id:"__ALL_HIT_PROBABILITY_V2_CURRENT__", hp_v2_batch_id:"__ALL_HIT_PROBABILITY_V2_CURRENT__", read_all_hp_v2_current:true,
      v2_source_mode:"all_hit_probability_v2_current", fast_safe_chunk_policy:"all_current_default_160_cap_220_batched_stage_writes_soft_yield_60s", baseline_source_policy:"source_agnostic_unless_formula_profile_is_source_specific",
      parallel_v2_only:true, no_current_baseline_mutation:true, no_current_sanity_mutation:true, no_scoring:true, no_final_board:true, no_scheduler_mutation:true,
      writes_score_db_v2_tables_only:true,
      allowed_writes:["SCORE_DB.player_baseline_sanity_v2_batches","SCORE_DB.player_baseline_sanity_v2_stage","SCORE_DB.player_baseline_sanity_v2_current","SCORE_DB.player_baseline_sanity_v2_history","SCORE_DB.player_baseline_sanity_v2_issues","SCORE_DB.player_baseline_hp_v2_batches","SCORE_DB.player_baseline_hp_v2_stage","SCORE_DB.player_baseline_hp_v2_current","SCORE_DB.player_baseline_hp_v2_history","SCORE_DB.player_baseline_hp_v2_issues"]
    };
    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-phase3a-first-inning-pitcher-context','expansion-baseline-v2','Expansion','expansion','Expansion Baseline V2 HEB Parallel',1,'https://alphadog-v2-phase3a-first-inning-pitcher-context.rodolfoaamattos.workers.dev','PHASE3A_FIRST_INNING_PITCHER_CONTEXT_WORKER',1,'Parallel v2 HEB sanity/baseline HP generator. Writes only player_baseline_*_v2 tables. Does not mutate current production sanity/baseline/scoring/final board.',CURRENT_TIMESTAMP)"
    ).run();
    await env.CONFIG_DB.prepare(
      "INSERT OR REPLACE INTO config_worker_definitions (worker_name, job_key, worker_group, phase_key, display_name, enabled, schedule_profile_key, notes) VALUES ('alphadog-v2-phase3a-first-inning-pitcher-context','expansion-baseline-v2','Expansion','expansion','Expansion Baseline V2 HEB Parallel',1,'manual_or_orchestrated','Parallel v2 HEB sanity/baseline HP generator. Writes only player_baseline_*_v2 tables. No current production mutation.')"
    ).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'expansion-baseline-v2', 'alphadog-v2-phase3a-first-inning-pitcher-context', 'Expansion', 'expansion', 'Expansion Baseline V2 HEB Parallel', 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_expansion_baseline_v2_heb', 'INFO', 'queued_expansion_baseline_v2_heb', 'Queued Expansion V2 HEB all-current baseline parallel job with backend hot continuation', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"EXPANSION > V2 HEB Baseline", queued_job_key:"expansion-baseline-v2", queued_worker_name:"alphadog-v2-phase3a-first-inning-pitcher-context", mode:"baseline_v2_heb", source_hp_v2_batch_id:"__ALL_HIT_PROBABILITY_V2_CURRENT__", read_all_hp_v2_current:true, v2_chunk_size:160, v2_all_current_chunk_size:160, v2_soft_yield_ms:60000, fast_safe_chunk_policy:"all_current_default_160_cap_220_batched_stage_writes_soft_yield_60s", parallel_v2_only:true, no_current_baseline_mutation:true, backend_hot_continue_triggered:true})).run();
    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(callOrchestrator(env, "/pump", { source:"control_room_expansion_baseline_v2_enqueue", max_cycles:18, max_jobs_per_cycle:1, max_ms:65000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, expansion_baseline_v2_watchdog:true }));
      ctx.waitUntil((async()=>{ await sleepMs(25000); await callOrchestrator(env, "/pump", { source:"control_room_expansion_baseline_v2_watchdog_25s", max_cycles:18, max_jobs_per_cycle:1, max_ms:65000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, expansion_baseline_v2_watchdog:true }); })());
    }
    return jsonResponse({ ok:true, data_ok:true, version, job, status:"queued", request_id:requestId, chain_id:chainId, visible_button:"EXPANSION > V2 HEB Baseline", queued_job_key:"expansion-baseline-v2", queued_worker_name:"alphadog-v2-phase3a-first-inning-pitcher-context", mode:"baseline_v2_heb", v2_chunk_size:160, v2_all_current_chunk_size:160, v2_soft_yield_ms:60000, parallel_v2_only:true, no_current_baseline_mutation:true, auto_pump_triggered:true, browser_auto_pump:false, source_hp_v2_batch_id:"__ALL_HIT_PROBABILITY_V2_CURRENT__", read_all_hp_v2_current:true, fast_safe_chunk_policy:"all_current_default_160_cap_220_batched_stage_writes_soft_yield_60s", note:"Queued all-current parallel V2 HEB baseline. Backend hot pump/watchdog was launched; manual Wake is rescue only." });
  }
  ...
  if (job === "orchestrator_enqueue_market_hitter_prop_context") {
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = 'market-line-shape-classifier' AND status IN ('pending','running') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "MARKET > Hitters",
        note: "Existing Market Hitter Prop Context queue row found. Use ORCHESTRATOR > Wake and ORCHESTRATOR > Logs."
      });
    }

    const requestId = "market_hitters_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_market_hitters_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "MARKET > Hitters",
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'market-line-shape-classifier', 'alphadog-v2-market-line-shape-classifier', '05 Market', 'market_context', 'External Hitter Prop Lines', 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_market_hitter_prop_context', 'INFO', 'queued_market_hitter_prop_context', 'Queued exact Market Hitter Prop Context worker job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"MARKET > Hitters", queued_job_key:"market-line-shape-classifier", queued_worker_name:"alphadog-v2-market-line-shape-classifier", no_teams_game_odds:true, no_market_current_lines_writes:true, no_score_db_mutation:true})).run();
    ...
    return jsonResponse({
      ...
      chain_id: chainId,
      visible_button: "MARKET > Hitters",
      queued_job_key: "market-line-shape-classifier",
      queued_worker_name: "alphadog-v2-market-line-shape-classifier",
      selected_worker_slot: "alphadog-v2-market-line-shape-classifier",
      today_tomorrow_retention_only: true,
      note: "Queued External Hitter Prop Lines only. It uses the verified unused Market Line Shape Classifier dummy slot, writes MARKET_DB evidence rows only, and does not write market_current_lines, mutate prepared board, score, rank, build matrix packets, or write final board. Tap ORCHESTRATOR > Wake."
    });
  }
  ...
  if (job === "orchestrator_enqueue_market_pitcher_prop_context") {
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = 'market-line-shape-classifier' AND status IN ('pending','running') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "MARKET > Pitchers",
        note: "Existing Market Line Shape Classifier queue row found. Use ORCHESTRATOR > Wake and ORCHESTRATOR > Logs."
      });
    }

    const requestId = "market_pitchers_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_market_pitchers_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "MARKET > Pitchers",
      mode: "market_pitcher_prop_line_context",
      created_at: now,
      exact_worker_only: true,
      selected_worker_slot: "alphadog-v2-market-line-shape-classifier",
      parlay_api_first_test: true,
      pitcher_player_props_only: true,
      same_worker_as_hitters_separate_mode: true,
      today_tomorrow_retention_only: true,
      evidence_tables_only: true,
      no_teams_game_odds: true,
      no_hitter_props_in_this_run: true,
      no_internal_pitcher_factors: true,
      no_market_current_lines_writes: true,
      no_prepared_board_mutation: true,
      no_score_db_mutation: true,
      no_scoring: true,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'market-line-shape-classifier', 'alphadog-v2-market-line-shape-classifier', '05 Market', 'market_context', 'External Pitcher Prop Lines', 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_market_pitcher_prop_context', 'INFO', 'queued_market_pitcher_prop_context', 'Queued exact Market Pitcher Prop Context worker job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"MARKET > Pitchers", queued_job_key:"market-line-shape-classifier", queued_worker_name:"alphadog-v2-market-line-shape-classifier", no_teams_game_odds:true, no_market_current_lines_writes:true, no_score_db_mutation:true, no_scoring:true})).run();
    ...
    return jsonResponse({
      ...
      selected_worker_slot: "alphadog-v2-market-line-shape-classifier",
      today_tomorrow_retention_only: true,
      note: "Queued External Pitcher Prop Lines only. Same worker as Hitters, separate pitcher mode. Writes MARKET_DB evidence rows only; no market_current_lines, no prepared-board mutation, no scoring, no ranking, no matrix, no final board. Tap ORCHESTRATOR > Wake."
    });
  }

  if (job === "orchestrator_enqueue_oddsapi_hitter_prop_context") {
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = 'oddsapi-reference' AND worker_name='alphadog-v2-oddsapi-reference' AND status IN ('pending','running') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "MARKET > Odds Hitters",
        note: "Existing OddsAPI Hitter Prop Context queue row found. Use ORCHESTRATOR > Wake and ORCHESTRATOR > Logs."
      });
    }

    const requestId = "market_odds_hitters_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_market_odds_hitters_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "MARKET > Odds Hitters",
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'oddsapi-reference', 'alphadog-v2-oddsapi-reference', '05 Market', 'market_context', 'OddsAPI Hitter Prop Lines', 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_oddsapi_hitter_prop_context', 'INFO', 'queued_oddsapi_hitter_prop_context', 'Queued exact OddsAPI Hitter Prop Context worker job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"MARKET > Odds Hitters", queued_job_key:"oddsapi-reference", queued_worker_name:"alphadog-v2-oddsapi-reference", no_parlay_api_calls:true, no_teams_game_odds:true, no_market_current_lines_writes:true, no_score_db_mutation:true})).run();
    ...
    return jsonResponse({
      ...
      chain_id: chainId,
      visible_button: "MARKET > Odds Hitters",
      queued_job_key: "oddsapi-reference",
      queued_worker_name: "alphadog-v2-oddsapi-reference",
      selected_worker_slot: "alphadog-v2-oddsapi-reference",
      today_tomorrow_retention_only: true,
      note: "Queued OddsAPI event-level Hitter Prop Lines only. It uses the unused OddsAPI Reference dummy slot, writes MARKET_DB evidence rows only, and does not write market_current_lines, mutate prepared board, score, rank, build matrix packets, or write final board. Tap ORCHESTRATOR > Wake."
    });
  }


  if (job === "orchestrator_enqueue_daily_full_run") {
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, chain_id, status, created_at, started_at, finished_at, updated_at FROM control_job_queue WHERE job_key = 'daily-full-run' AND status IN ('pending','running','partial_continue') AND finished_at IS NULL ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();

    if (existing) {
      return jsonResponse({ ok: true, data_ok: true, version, job, status: "already_queued", request_id: existing.request_id, existing, visible_button: "SCORING V3 > Daily Full", backend_chain_only: true, auto_pump_triggered: false, browser_auto_pump: false, note: "Existing Daily Full Run parent queue row found. It owns Daily Context Full Run, Board Full Run, then Market/Scoring Full Run. Backend auto-pump/watchdog/cron owns continuation; ORCHESTRATOR > Wake is manual testing/rescue only." });
    }

    const activeComponent = await env.CONTROL_DB.prepare(
      "SELECT request_id, chain_id, job_key, worker_name, status, created_at, started_at, updated_at FROM control_job_queue WHERE job_key IN ('daily-context-full-run','board-full-run','market-scoring-full-run') AND worker_name='alphadog-v2-orchestrator' AND status IN ('pending','running','partial_continue') AND finished_at IS NULL ORDER BY datetime(created_at) ASC LIMIT 1"
    ).first();

    if (activeComponent) {
      return jsonResponse({ ok: false, data_ok: false, version, job, status: "component_full_run_active", visible_button: "SCORING V3 > Daily Full", active_component: activeComponent, note: "A component full-run is already active. Do not create a Daily Full parent on top of an independent active full-run. Let the active run finish or use ORCHESTRATOR > Logs/Queue first." }, 409);
    }

    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-orchestrator','daily-full-run','12 Full Runs','daily_master','Expansion V3 Daily Full Run: Board + Daily Context + Market/Scoring',1,'https://alphadog-v2-orchestrator.rodolfoaamattos.workers.dev','ORCHESTRATOR_WORKER',1,'Top-level backend parent chain: Daily Context Full Run, Board Full Run including PrizePicks/Sleeper/Score Prep, then Market/Scoring Full Run including hit probability. Prepared for split slate, half slate, and single-game slate. No static/base/delta jobs.',CURRENT_TIMESTAMP)"
    ).run();

    await env.CONFIG_DB.prepare(
      "INSERT OR REPLACE INTO config_worker_definitions (worker_name, job_key, worker_group, phase_key, display_name, enabled, schedule_profile_key, notes) VALUES ('alphadog-v2-orchestrator','daily-full-run','12 Full Runs','daily_master','Expansion V3 Daily Full Run: Board + Daily Context + Market/Scoring',1,'manual_or_orchestrated','Top-level twice-daily parent chain for Daily Context Full Run, Board Full Run, and Market/Scoring Full Run. Uses existing component full runs and backend hot continuation; no manual SQL schema requirement.')"
    ).run();

    const requestId = "daily_full_run_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_daily_full_run_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "SCORING V3 > Daily Full",
      mode: "daily_full_run_backend_chain",
      created_at: now,
      exact_worker_only: true,
      ...
      daily_full_run_watchdog_requested: true,
      approved_chain_order: ["board-full-run", "daily-context-full-run", "market-scoring-full-run"],
      approved_stage_order: ["board_full_run", "daily_context_full_run", "market_scoring_full_run"],
      ...
      scoring_v3_sequence: true,
      includes_score_enrichment_v1: true,
      includes_score_enrichment_v2_shadow: true,
      includes_hit_probability_v3_shadow: true,
      includes_final_score_v2_shadow: true,
      includes_final_board_v3_shadow: true,
      ...
      no_old_production_touch: true,
      note: "Expansion V3 Daily Full parent chain: Board Full Run -> Daily Context Full Run -> Market/Scoring Full Run, with Market/Scoring wired to Enrich V1 -> Enrich V2 -> HP V3 -> Final Score V2 -> Final Board V3. Backend auto-pump/watchdog owns continuation; manual Wake is testing/rescue only."
    };

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'daily-full-run', 'alphadog-v2-orchestrator', '12 Full Runs', 'daily_master', 'Expansion V3 Daily Full Run: Board + Daily Context + Market/Scoring', 'pending', 3, 1, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT OR IGNORE INTO control_locks (lock_key, lock_flag, updated_at) VALUES ('DAILY_FULL_RUN', 0, CURRENT_TIMESTAMP)"
    ).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_daily_full_run', 'INFO', 'queued_daily_full_run', 'Queued Daily Full Run parent backend chain job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"SCORING V3 > Daily Full", approved_chain_order:input.approved_chain_order, backend_chain_only:true, auto_pump_requested:true, daily_full_run_watchdog_requested:true})).run();

    if (ctx && ctx.waitUntil) {
      ctx.waitUntil((async () => {
        await callOrchestrator(env, "/pump", { source:"control_room_daily_full_run_enqueue", max_cycles:18, max_jobs_per_cycle:1, max_ms:115000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, daily_full_run_watchdog:true, market_scoring_full_run_watchdog:true });
        await sleepMs(45000);
        await callOrchestrator(env, "/pump", { source:"control_room_daily_full_run_watchdog_45s", max_cycles:18, max_jobs_per_cycle:1, max_ms:115000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, daily_full_run_watchdog:true, market_scoring_full_run_watchdog:true });
        await sleepMs(90000);
        await callOrchestrator(env, "/pump", { source:"control_room_daily_full_run_watchdog_135s", max_cycles:18, max_jobs_per_cycle:1, max_ms:115000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, daily_full_run_watchdog:true, market_scoring_full_run_watchdog:true });
      })());
    }

    return jsonResponse({ ok: true, data_ok: true, version, job, status: "queued", request_id: requestId, chain_id: chainId, visible_button: "SCORING V3 > Daily Full", queued_job_key: "daily-full-run", queued_worker_name: "alphadog-v2-orchestrator", approved_chain_order: input.approved_chain_order, backend_chain_only: true, auto_pump_triggered: true, backend_watchdog_triggered: true, prepared_for_split_slate: true, prepared_for_half_slate: true, prepared_for_single_game_slate: true, includes_daily_context_full_run: true, includes_board_full_run: true, includes_market_scoring_full_run: true, includes_hit_probability: true, note: "Queued Expansion V3 Daily Full: Board Full Run, Daily Context Full Run, then Market/Scoring Full Run through Enrich V1 -> Enrich V2 -> HP V3 -> Final Score V2 -> Final Board V3. Backend auto-pump/watchdog was launched; browser may close now. ORCHESTRATOR > Wake is manual testing/rescue only." });
  }
  ...
  if (job === "orchestrator_enqueue_board_full_run") {
    ...
    const input = {
      ...
      child_modes: {
        ...
        "score-prep": "board_full_run_score_prep_enrichment",
        "parlay-underdog-board": "board_full_run_underdog_refresh"
      },
      stop_on_first_failed_stage: true,
      backend_chain_only: true,
  ...
  if (false) {
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "SCORING V3 > Market Full",
        note: "Existing Market/Scoring Full Run parent row found. Backend auto-pump/watchdog/cron owns continuation; ORCHESTRATOR > Wake is manual testing/rescue only."
      });
    }

    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-orchestrator','market-scoring-full-run','12 Full Runs','market_scoring','Expansion V3 Market + Factors + Matrix + Scoring Full Run',1,'https://alphadog-v2-orchestrator.rodolfoaamattos.workers.dev','ORCHESTRATOR_WORKER',1,'Backend parent chain: Market Teams, Market Hitters, Market Pitchers, Factor Hitters, Factor Pitchers, Matrix Build, Scoring Engine, Final Board, Hit Probability + HP Board. No source board mutation and no manual SQL schema requirement.',CURRENT_TIMESTAMP)"
    ).run();

    await env.CONFIG_DB.prepare(
      "INSERT OR REPLACE INTO config_worker_definitions (worker_name, job_key, worker_group, phase_key, display_name, enabled, schedule_profile_key, notes) VALUES ('alphadog-v2-orchestrator','market-scoring-full-run','12 Full Runs','market_scoring','Expansion V3 Market + Factors + Matrix + Scoring Full Run',1,'manual_or_orchestrated','Parent backend chain for market context, prop factors, matrix, real scoring engine current, final board, and same score-audit hit-probability/HP-board display calibration. Created by Control Room; no schema SQL required.')"
    ).run();

    const requestId = "market_scoring_full_run_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_market_scoring_full_run_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "SCORING V3 > Market Full",
      mode: "market_scoring_full_run",
      created_at: now,
      exact_worker_only: true,
      backend_chain_only: true,
      no_browser_loop: true,
      ...
      no_old_production_touch: true,
      auto_pump_requested: true,
      market_scoring_watchdog_requested: true,
      approved_chain_order: [
        "market-normalizer",
        "market-line-shape-classifier:hitter",
        "market-line-shape-classifier:pitcher",
        "prop-factor-miner:hitter",
        "prop-factor-miner:pitcher",
        "prop-matrix-builder",
        "score-enrichment-v1",
        "score-enrichment-v2-shadow",
        "hit-probability-v3-shadow",
        "final-score-v2-shadow",
        "final-board-v3-shadow"
      ],
      scoring_v3_sequence: true,
      includes_score_enrichment_v1: true,
      includes_score_enrichment_v2_shadow: true,
      ...
      includes_final_score_v2_shadow: true,
      includes_final_board_v3_shadow: true,
      hit_probability_included: true,
      hp_v3_included: true,
      final_score_v2_included: true,
      final_board_v3_included: true,
      no_true_hit_probability_claims: true,
      d1_limit_safe_chunking_required: true,
      service_binding_timeout_reconcile_from_tables: true,
      note: "Expansion V3 Market/Scoring parent chain: Market Teams -> Market Hitters -> Market Pitchers -> Factor Hitters -> Factor Pitchers -> Matrix -> Enrich V1 -> Enrich V2 -> HP V3 -> Final Score V2 -> Final Board V3. Board refresh is intentionally not included. Backend auto-pump/watchdog owns continuation; manual Wake is test/rescue only."
    };

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'market-scoring-full-run', 'alphadog-v2-orchestrator', '12 Full Runs', 'market_scoring', 'Expansion V3 Market + Factors + Matrix + Scoring Full Run', 'pending', 4, 1, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_market_scoring_full_run', 'INFO', 'queued_market_scoring_full_run', 'Queued Expansion V3 Market + Factors + Matrix + Scoring Full Run parent backend chain job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, queued_job_key:"market-scoring-full-run", queued_worker_name:"alphadog-v2-orchestrator", approved_chain_order:input.approved_chain_order})).run();

    if (ctx && ctx.waitUntil) {
      ctx.waitUntil((async () => {
        await callOrchestrator(env, "/pump", { source:"control_room_market_scoring_full_run_enqueue", max_cycles:18, max_jobs_per_cycle:1, max_ms:115000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, market_scoring_full_run_watchdog:true });
        await sleepMs(45000);
        await callOrchestrator(env, "/pump", { source:"control_room_market_scoring_full_run_watchdog_45s", max_cycles:10, max_jobs_per_cycle:1, max_ms:115000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, market_scoring_full_run_watchdog:true });
        await sleepMs(90000);
        await callOrchestrator(env, "/pump", { source:"control_room_market_scoring_full_run_watchdog_135s", max_cycles:10, max_jobs_per_cycle:1, max_ms:115000, max_pump_chains:80, no_browser_loop:true, cron_rescue_only:true, backend_budget_loop_requested:true, market_scoring_full_run_watchdog:true });
      })());
    }
    ...
    return jsonResponse({
      ...
      chain_id: chainId,
      visible_button: "SCORING V3 > Market Full",
      queued_job_key: "market-scoring-full-run",
      queued_worker_name: "alphadog-v2-orchestrator",
      approved_chain_order: input.approved_chain_order,
      backend_chain_only: true,
      auto_pump_triggered: true,
      backend_watchdog_triggered: true,
      no_source_board_mutation: true,
      no_prepared_board_mutation: true,
      hit_probability_included: true,
      hp_board_included: true,
      hp_board_display_calibration_required: true,
      no_true_hit_probability_claims: true,
      note: "Queued Expansion V3 Market/Scoring full run: Market -> Factors -> Matrix -> Enrich V1 -> Enrich V2 -> HP V3 -> Final Score V2 -> Final Board V3. Backend auto-pump/watchdog was launched; ORCHESTRATOR > Wake is manual testing/rescue only."
    });
  }

  if (job === "orchestrator_enqueue_prop_factor_hitters" || job === "orchestrator_enqueue_prop_factor_pitchers") {
    const isPitcher = job === "orchestrator_enqueue_prop_factor_pitchers";
    const visibleButton = isPitcher ? "FACTORS > Pitchers" : "FACTORS > Hitters";
    const mode = isPitcher ? "pitcher_prop_factor_mining" : "hitter_prop_factor_mining";
    const family = isPitcher ? "pitcher" : "hitter";
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = 'prop-factor-miner' AND status IN ('pending','running') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();
    ...
    if (existing) {
      return jsonResponse({
        ...
        note: "Existing Prop Factor Miner queue row found. Use ORCHESTRATOR > Wake / Logs / Queue to continue or inspect."
      });
    }

    const requestId = "prop_factor_" + family + "_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_prop_factor_" + family + "_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: visibleButton,
      mode,
      ...
      reads_score_prepared_board: true,
      reads_internal_context_tables: true,
      writes_score_factor_packets_only: true,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'prop-factor-miner', 'alphadog-v2-phase2b-recent-form', '09 Factors', 'factors', ?, 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, isPitcher ? "Prop Factor Miner: Pitcher Packets" : "Prop Factor Miner: Hitter Packets", JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_prop_factor_miner', 'Queued exact Prop Factor Miner internal packet worker job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, job, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:visibleButton, queued_job_key:"prop-factor-miner", queued_worker_name:"alphadog-v2-phase2b-recent-form", logical_worker_name:"alphadog-v2-prop-factor-miner", mode, retention_policy:"today_tomorrow_only", no_external_api_calls:true, no_scoring:true, no_ranking:true, no_matrix_builder:true, no_final_board:true})).run();
    ...
    return jsonResponse({
      ...
      chain_id: chainId,
      visible_button: visibleButton,
      queued_job_key: "prop-factor-miner",
      queued_worker_name: "alphadog-v2-phase2b-recent-form",
      logical_worker_name: "alphadog-v2-prop-factor-miner",
      mode,
      factor_family: family,
      retention_policy: "today_tomorrow_only",
      note: "Queued internal-only Prop Factor Miner. Writes SCORE_DB prop_factor_* packets/current/issues for today's and tomorrow's games only; no external calls, no scoring, no ranking, no matrix, no final board. Tap ORCHESTRATOR > Wake."
    });
  }

  if (job === "orchestrator_enqueue_prop_matrix_builder") {
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = 'prop-matrix-builder' AND status IN ('pending','running') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "MATRIX > Build",
        note: "Existing Prop Matrix Builder queue row found. Use ORCHESTRATOR > Wake / Logs / Queue to continue or inspect."
      });
    }

    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-phase2b-certifier','prop-matrix-builder','10 Matrix','matrix','Prop Matrix Builder',1,'https://alphadog-v2-phase2b-certifier.rodolfoaamattos.workers.dev','PHASE2B_CERTIFIER_WORKER',1,'Repurposed existing Phase2B Certifier dummy slot as internal-only Prop Matrix Builder. Writes SCORE_DB prop_matrix_* today/tomorrow only; no external calls, no scoring, no ranking, no final board.',CURRENT_TIMESTAMP)"
    ).run();

    const requestId = "prop_matrix_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_prop_matrix_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "MATRIX > Build",
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'prop-matrix-builder', 'alphadog-v2-phase2b-certifier', '10 Matrix', 'matrix', 'Prop Matrix Builder', 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_prop_matrix_builder', 'Queued exact Prop Matrix Builder internal matrix-certifier job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, job, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"MATRIX > Build", queued_job_key:"prop-matrix-builder", queued_worker_name:"alphadog-v2-phase2b-certifier", logical_worker_name:"alphadog-v2-prop-matrix-builder", mode:"prop_matrix_build", retention_policy:"today_tomorrow_only", no_external_api_calls:true, no_scoring:true, no_ranking:true, no_final_board:true})).run();
    ...
    return jsonResponse({
      ...
      chain_id: chainId,
      visible_button: "MATRIX > Build",
      queued_job_key: "prop-matrix-builder",
      queued_worker_name: "alphadog-v2-phase2b-certifier",
      logical_worker_name: "alphadog-v2-prop-matrix-builder",
      mode: "prop_matrix_build",
      retention_policy: "today_tomorrow_only",
      one_row_per_safe_prepared_row: true,
      note: "Queued internal-only Prop Matrix Builder. Writes SCORE_DB prop_matrix_* current/issue/coverage/batch tables for today's and tomorrow's games only; no external calls, no scoring, no ranking, no final board. Tap ORCHESTRATOR > Wake."
    });
  }


  if (job === "orchestrator_enqueue_final_score_v1") {
    return jsonResponse({ ok: false, data_ok: false, version, job, status: "retired_2026_07_14", certification: "LEGACY_FINAL_SCORE_V1_RETIRED", note: "This legacy score-audit.js-based Final Score V1 was retired 2026-07-14 after being confirmed inactive since 2026-06-23 (superseded by the new dedicated Scoring Full Run chain, SCORING > Full Run / SCORING > Daily Full)." }, 410);
  }
  if (false) {
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = 'final-score-v1' AND status IN ('pending','running') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "SCORING > Final Score",
        note: "Existing Final Score V1 queue row found. Use ORCHESTRATOR > Wake / Logs to continue or inspect."
      });
    }

    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-score-audit','final-score-v1','11 Scoring','scoring','Final Score V1 HP-Dominant Overall',1,'https://alphadog-v2-score-audit.rodolfoaamattos.workers.dev','SCORE_AUDIT_WORKER',1,'Same queued score-audit slot is intercepted by orchestrator direct Final Score V1. Reads SCORE_DB.hit_probability_v2_current and score_enrichment_current and writes only final_score_v1_* tables. Does not mutate HP V2, scoring_engine_current, score_final_board_current, prepared board, source boards, ranks, or live/play gates.',CURRENT_TIMESTAMP)"
    ).run();

    const requestId = "final_score_v1_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_final_score_v1_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "SCORING > Final Score",
      mode: "final_score_v1_current",
      created_at: now,
      logical_worker_name: "alphadog-v2-final-score-v1",
      deployed_worker_slot: "alphadog-v2-score-audit",
      exact_worker_only: true,
      worker_owned_schema_creation: true,
      reads_hit_probability_v2_current: true,
      reads_score_enrichment_current: true,
      writes_final_score_v1_tables_only: true,
      side_expanded: true,
      final_score_hp_dominant_overall: true,
      hp_v2_remains_probability_source: true,
      no_hp_v2_mutation: true,
      ...
      no_live_playable_activation: true,
      live_playable_forced_zero: true,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'final-score-v1', 'alphadog-v2-score-audit', '11 Scoring', 'scoring', 'Final Score V1 HP-Dominant Overall', 'pending', 8, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_final_score_v1', 'Queued exact Final Score V1 HP-dominant overall job through score-audit slot', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, job, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"SCORING > Final Score", queued_job_key:"final-score-v1", queued_worker_name:"alphadog-v2-score-audit", logical_worker_name:"alphadog-v2-final-score-v1", no_hp_v2_mutation:true, no_final_board_mutation:true, live_playable_forced_zero:true})).run();
    ...
    return jsonResponse({
      ...
      chain_id: chainId,
      visible_button: "SCORING > Final Score",
      queued_job_key: "final-score-v1",
      queued_worker_name: "alphadog-v2-score-audit",
      logical_worker_name: "alphadog-v2-final-score-v1",
      mode: "final_score_v1_current",
      writes_score_db_final_score_v1_only: true,
      no_hp_v2_mutation: true,
      no_final_board_mutation: true,
      live_playable_forced_zero: true,
      note: "Queued Final Score V1 in the existing score-audit slot. Orchestrator will intercept it directly, read HP V2 + Score Enrichment V1, and write only final_score_v1_* tables. Tap ORCHESTRATOR > Wake."
    });
  }
  ...
  if (false) {
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "SCORING > Final Board V2",
        note: "Existing Final Board V2 queue row found. Use ORCHESTRATOR > Wake / Logs to continue or inspect."
      });
    }

    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-score-audit','final-board-v2','11 Scoring','scoring','Final Board V2 HP-First Review Board',1,'https://alphadog-v2-score-audit.rodolfoaamattos.workers.dev','SCORE_AUDIT_WORKER',1,'Queued in score-audit slot but intercepted by orchestrator direct Final Board V2. Reads latest final_score_v1_current HP-dominant overall scores and writes only score_final_board_v2_* tables. Does not mutate legacy score_final_board_current or live/play gates.',CURRENT_TIMESTAMP)"
    ).run();

    const requestId = "final_board_v2_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_final_board_v2_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "SCORING > Final Board V2",
      mode: "score_final_board_v2_current",
      created_at: now,
      logical_worker_name: "alphadog-v2-final-board-v2",
      deployed_worker_slot: "alphadog-v2-score-audit",
      exact_worker_only: true,
      worker_owned_schema_creation: true,
      reads_final_score_v1_current: true,
      requires_hp_dominant_final_score_v1: true,
      writes_score_final_board_v2_tables_only: true,
      hp_first_sort: true,
      review_board_only: true,
      no_legacy_score_final_board_mutation: true,
      no_score_final_board_current_mutation: true,
      no_hp_v2_mutation: true,
      no_final_score_v1_mutation: true,
      no_prepared_board_mutation: true,
      no_source_board_mutation: true,
      live_playable_forced_zero: true,
      no_live_playable_activation: true
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'final-board-v2', 'alphadog-v2-score-audit', '11 Scoring', 'scoring', 'Final Board V2 HP-First Review Board', 'pending', 8, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_final_board_v2', 'Queued exact Final Board V2 HP-first review board through score-audit slot', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, job, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"SCORING > Final Board V2", queued_job_key:"final-board-v2", queued_worker_name:"alphadog-v2-score-audit", logical_worker_name:"alphadog-v2-final-board-v2", no_legacy_final_board_mutation:true, live_playable_forced_zero:true})).run();
    ...
    return jsonResponse({
      ...
      chain_id: chainId,
      visible_button: "SCORING > Final Board V2",
      queued_job_key: "final-board-v2",
      queued_worker_name: "alphadog-v2-score-audit",
      logical_worker_name: "alphadog-v2-final-board-v2",
      mode: "score_final_board_v2_current",
      writes_score_db_final_board_v2_only: true,
      hp_first_sort: true,
      review_board_only: true,
      no_legacy_final_board_mutation: true,
      live_playable_forced_zero: true,
      note: "Queued Final Board V2 in the existing score-audit slot. Orchestrator will intercept it directly, read latest HP-dominant Final Score V1, and write only score_final_board_v2_* tables. Tap ORCHESTRATOR > Wake."
    });
  }


  if (job === "orchestrator_enqueue_scoring_engine") {
    return jsonResponse({ ok: false, data_ok: false, version, job, status: "retired_2026_07_14", certification: "LEGACY_SCORING_ENGINE_RETIRED", note: "This legacy score-audit.js-based Scoring Engine was retired 2026-07-14 after being confirmed inactive since 2026-06-14 (superseded by the new dedicated Scoring Full Run chain, SCORING > Full Run / SCORING > Daily Full)." }, 410);
  }
  if (false) {
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = 'scoring-engine' AND status IN ('pending','running') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "SCORING > Engine",
        note: "Existing Scoring Engine queue row found. Use ORCHESTRATOR > Wake / Logs / Queue to continue or inspect."
      });
    }

    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-score-audit','scoring-engine','11 Scoring','scoring','Scoring Engine Current Scoring Producer',1,'https://alphadog-v2-score-audit.rodolfoaamattos.workers.dev','SCORE_AUDIT_WORKER',1,'Existing score-audit slot repurposed as Scoring Engine current scoring producer. Worker-owned idempotent schema creation; one row per matrix variation; side-aware/source-line-type-aware/variation-aware; no final threshold lock, no ranking, no final board.',CURRENT_TIMESTAMP)"
    ).run();

    const requestId = "scoring_engine_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_scoring_engine_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "SCORING > Engine",
      ...
      worker_owned_schema_creation: true,
      no_manual_schema_required: true,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'scoring-engine', 'alphadog-v2-score-audit', '11 Scoring', 'scoring', 'Scoring Engine Current Scoring Producer', 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_scoring_engine_current', 'Queued exact Scoring Engine current scoring producer job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, job, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"SCORING > Engine", queued_job_key:"scoring-engine", queued_worker_name:"alphadog-v2-score-audit", logical_worker_name:"alphadog-v2-scoring-engine", production_scoring_current:true, no_ranking:true, no_final_board:true})).run();
    ...
    return jsonResponse({
      ...
      final_qualification_threshold_locked: false,
      one_score_row_per_matrix_row: true,
      note: "Queued Scoring Engine current scoring producer. Worker creates idempotent SCORE_DB/ARCHIVE_DB schema on first run, writes one scored row per matrix variation, preserves blocked/deferred rows, keeps Goblin/Demon More-only, does not rank or write final board. Tap ORCHESTRATOR > Wake."
    });
  }
  ...
  if (false) {
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "SCORING > Enrichment V1",
        note: "Existing Enrichment V1 queue row found. Use ORCHESTRATOR > Wake / Logs / Queue to continue or inspect."
      });
    }

    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-score-audit','score-enrichment-v1','11 Scoring','scoring','Score Enrichment V1 Side Expanded',1,'https://alphadog-v2-score-audit.rodolfoaamattos.workers.dev','SCORE_AUDIT_WORKER',1,'Same deployed score-audit slot runs isolated Enrichment V1 phase. Writes only SCORE_DB.score_enrichment_* tables. Side-expanded MORE/LESS rows from prop_matrix_current, joins player_baseline_hp_current and factor packets, no HP V2, no current scoring, no final score, no final board.',CURRENT_TIMESTAMP)"
    ).run();

    const requestId = "score_enrichment_v1_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_score_enrichment_v1_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "SCORING > Enrichment V1",
      ...
      reads_prop_matrix_current: true,
      reads_player_baseline_hp_current: true,
      reads_prop_factor_packets: true,
      writes_score_enrichment_tables_only: true,
      no_hp_v2: true,
      no_hit_probability_mutation: true,
      no_scoring_engine_current_mutation: true,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'score-enrichment-v1', 'alphadog-v2-score-audit', '11 Scoring', 'scoring', 'Score Enrichment V1 Side Expanded', 'pending', 7, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_score_enrichment_v1', 'Queued Score Enrichment V1 side-expanded job through score-audit slot', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, job, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"SCORING > Enrichment V1", queued_job_key:"score-enrichment-v1", queued_worker_name:"alphadog-v2-score-audit", logical_worker_name:"alphadog-v2-score-enrichment-v1", side_expanded:true, no_hp_v2:true, no_current_scoring_mutation:true, no_final_board:true})).run();
    ...
    return jsonResponse({
      ...
      chain_id: chainId,
      visible_button: "SCORING > Enrichment V1",
      queued_job_key: "score-enrichment-v1",
      queued_worker_name: "alphadog-v2-score-audit",
      logical_worker_name: "alphadog-v2-score-enrichment-v1",
      mode: "score_enrichment_v1_side_expanded",
      side_expanded: true,
      expected_output: "SCORE_DB.score_enrichment_*",
      no_hp_v2: true,
      no_current_scoring_mutation: true,
      no_final_board: true,
      note: "Queued Enrichment V1 in the existing score-audit worker slot. It writes only score_enrichment_* tables and does not alter current scoring, Hit Prob, Final Score, or Final Board. Tap ORCHESTRATOR > Wake."
    });
  }


  if (job === "orchestrator_enqueue_hit_probability") {
    return jsonResponse({ ok: false, data_ok: false, version, job, status: "retired_2026_07_14", certification: "LEGACY_HIT_PROBABILITY_RETIRED", note: "This legacy score-audit.js-based Hit Probability was retired 2026-07-14 after being confirmed inactive since 2026-06-14 (superseded by the new dedicated Scoring Full Run chain, SCORING > Full Run / SCORING > Daily Full)." }, 410);
  }
  if (false) {
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = 'hit-probability' AND status IN ('pending','running') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "SCORING > Hit Prob",
        note: "Existing Hit Probability queue row found. Use ORCHESTRATOR > Wake / Logs / Queue to continue or inspect."
      });
    }

    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-score-audit','hit-probability','11 Scoring','scoring','Hit Probability Current Estimator',1,'https://alphadog-v2-score-audit.rodolfoaamattos.workers.dev','SCORE_AUDIT_WORKER',1,'Same deployed score-audit slot runs isolated Hit Probability phase. Writes only SCORE_DB.hit_probability_batches/current/issues. Does not mutate scoring_engine_current, score_final_board_current, prepared board, source boards, rank, score, live/review gates, or old production tables.',CURRENT_TIMESTAMP)"
    ).run();

    const requestId = "hit_probability_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_hit_probability_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "SCORING > Hit Prob",
      ...
      exact_worker_only: true,
      worker_owned_schema_creation: true,
      estimated_hit_probability_phase: true,
      estimated_not_true_probability: true,
      reads_score_final_board_current_first: true,
      fallback_reads_scoring_engine_current: true,
      writes_score_db_hit_probability_only: true,
      no_scoring_engine_current_mutation: true,
      no_score_final_board_mutation: true,
      no_prepared_board_mutation: true,
      no_source_board_mutation: true,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_hit_probability', 'Queued exact Hit Probability job through score-audit slot', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, job, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"SCORING > Hit Prob", queued_job_key:"hit-probability", queued_worker_name:"alphadog-v2-score-audit", logical_worker_name:"alphadog-v2-hit-probability", no_score_mutation:true, no_final_board_mutation:true})).run();
    ...
    return jsonResponse({
      ...
      chain_id: chainId,
      visible_button: "SCORING > Hit Prob",
      queued_job_key: "hit-probability",
      queued_worker_name: "alphadog-v2-score-audit",
      logical_worker_name: "alphadog-v2-hit-probability",
      mode: "hit_probability_current_estimate",
      estimated_hit_probability_phase: true,
      ...
      no_score_mutation: true,
      no_final_board_mutation: true,
      note: "Queued Hit Probability in the existing score-audit worker slot. It writes only hit_probability_* tables and does not alter scoring, final board, prepared board, or source boards. Tap ORCHESTRATOR > Wake."
    });
  }
  ...
  if (false) {
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "SCORING > Simulation",
        note: "Existing Scoring Simulation queue row found. Use ORCHESTRATOR > Wake / Logs / Queue to continue or inspect."
      });
    }

    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-score-audit','scoring-engine-simulation','11 Scoring','scoring','Scoring Engine Simulation Shadow v0.2.4 DB Config',1,'https://alphadog-v2-score-audit.rodolfoaamattos.workers.dev','SCORE_AUDIT_WORKER',1,'Existing score-audit slot runs v0.2.4 simulation shadow only. All tunable thresholds/penalties/caps/formula metadata live in SCORE_DB.scoring_engine_simulation_profile_configs; writes simulation audit tables only; no scoring_engine_current mutation, no archive, no ranking, no final board.',CURRENT_TIMESTAMP)"
    ).run();
    ...
    const input = {
      source: "control_room",
      visible_button: "SCORING > Simulation",
      mode: "scoring_engine_simulation_shadow_strict_b",
      created_at: now,
      logical_worker_name: "alphadog-v2-scoring-engine-simulation",
      deployed_worker_slot: "alphadog-v2-score-audit",
      exact_worker_only: true,
      simulation_only: true,
      primary_profile: "STRICT_B",
      comparison_profile: "HYBRID_CONTROL",
      worker_owned_schema_creation: true,
      writes_score_db_simulation_shadow_only: true,
      no_scoring_engine_current_mutation: true,
      no_archive_mutation: true,
      thresholds_locked: false,
      ...
      no_true_hit_probability_claims: true,
      strict_b_76_plus_floor: 50,
      strict_b_76_plus_ceiling: 300,
      strict_b_82_plus_ceiling: 75,
      strict_b_88_plus_exact: 0,
      regular_lines_two_sided: true,
      goblin_demon_more_only: true,
      goblin_demon_under_blocker: "GOBLIN_DEMON_UNDER_NOT_SELECTABLE",
      no_candidate_board_write: true,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'scoring-engine-simulation', 'alphadog-v2-score-audit', '11 Scoring', 'scoring', 'Scoring Engine Simulation Shadow v0.2.4 DB Config', 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', ?, 'INFO', 'queued_scoring_engine_simulation', 'Queued exact Scoring Engine Simulation Shadow v0.2.4 DB Config job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, job, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"SCORING > Simulation", queued_job_key:"scoring-engine-simulation", queued_worker_name:"alphadog-v2-score-audit", logical_worker_name:"alphadog-v2-scoring-engine-simulation", primary_profile:"STRICT_B", comparison_profile:"HYBRID_CONTROL", shadow_only:true, no_production_mutation:true, no_ranking:true, no_final_board:true})).run();
    ...
    return jsonResponse({
      ...
      chain_id: chainId,
      visible_button: "SCORING > Simulation",
      queued_job_key: "scoring-engine-simulation",
      queued_worker_name: "alphadog-v2-score-audit",
      logical_worker_name: "alphadog-v2-scoring-engine-simulation",
      mode: "scoring_engine_simulation_shadow_strict_b",
      simulation_only: true,
      ...
      thresholds_locked: false,
      scoring_enabled: false,
      note: "Queued Scoring Engine v0.2.3 simulation. DB-configured thresholds/penalties/caps only; writes only SCORE_DB scoring_engine_simulation_shadow/batches/issues/profile_configs, compares Strict-B to Hybrid-Control, and does not mutate scoring_engine_current, archive, ranking, or final board. Tap ORCHESTRATOR > Wake."
    });
  }


  if (job === "orchestrator_enqueue_score_final_board") {
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = 'score-final-board' AND status IN ('pending','running') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "SCORING > Final Board",
        note: "Existing Score Final Board queue row found. Use ORCHESTRATOR > Wake / Logs / Queue to continue or inspect."
      });
    }

    await env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO control_worker_registry (worker_name, job_key, worker_group, phase_key, display_name, enabled, endpoint_url, service_binding_name, safe_mode, notes, updated_at) VALUES ('alphadog-v2-score-final-board','score-final-board','11 Scoring','scoring','Score Final Board Generator',1,'https://alphadog-v2-score-final-board.rodolfoaamattos.workers.dev','SCORE_FINAL_BOARD_WORKER',1,'Generates SCORE_DB.score_final_board_current from latest completed STRICT_B scoring simulation live rows. No external calls, no raw board mutation, no simulation mutation.',CURRENT_TIMESTAMP)"
    ).run();

    await env.CONFIG_DB.prepare(
      "INSERT OR REPLACE INTO config_worker_definitions (worker_name, job_key, worker_group, phase_key, display_name, enabled, schedule_profile_key, notes) VALUES ('alphadog-v2-score-final-board','score-final-board','11 Scoring','scoring','Score Final Board Generator',1,'manual_or_orchestrated','Final board current table generator for final UI; reads scoring_engine_simulation_shadow only; writes score_final_board_current/history/batches/issues')"
    ).run();

    const requestId = "score_final_board_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_score_final_board_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "SCORING > Final Board",
      ...
      service_binding_name: "SCORE_FINAL_BOARD_WORKER",
      profile_key: "STRICT_B",
      source_simulation_batch_policy: "latest_completed_pass_simulation_unless_explicit_batch_supplied",
      writes_score_final_board_current: true,
      writes_score_final_board_history: true,
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'score-final-board', 'alphadog-v2-score-final-board', '11 Scoring', 'scoring', 'Score Final Board Generator', 'pending', 7, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_score_final_board', 'INFO', 'queued_score_final_board', 'Queued Score Final Board generator', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"SCORING > Final Board", queued_job_key:"score-final-board", queued_worker_name:"alphadog-v2-score-final-board"})).run();
    ...
    return jsonResponse({
      ...
      chain_id: chainId,
      visible_button: "SCORING > Final Board",
      queued_job_key: "score-final-board",
      queued_worker_name: "alphadog-v2-score-final-board",
      final_ui_table: "SCORE_DB.score_final_board_current",
      note: "Queued Score Final Board generator. It reads latest completed STRICT_B simulation live rows and replaces SCORE_DB.score_final_board_current for the final UI. Tap ORCHESTRATOR > Wake."
    });
  }
  ...
  if (job === "orchestrator_enqueue_daily_context_full_run") {
    ...
    const input = {
      ...
      created_at: now,
      approved_chain_order: ["daily-certifier", "daily-probable-pitchers", "daily-lineups", "daily-player-availability", "daily-weather", "daily-bullpen-availability", "daily-team-schedule-spot", "daily-umpire-context", "daily-certifier"],
      approved_stage_order: ["daily_context_certifier_first_pass", "daily_starters", "daily_lineups", "daily_player_availability", "daily_weather_roof", "daily_bullpen_availability", "daily_team_schedule_spot", "daily_umpire_context", "daily_context_certifier"],
      stop_on_first_failed_stage: true,
      backend_chain_only: true,
      auto_pump_requested: true,
      no_browser_auto_pump: true,
    ...
    return jsonResponse({ ok: true, data_ok: true, version, job, status: "queued", request_id: requestId, chain_id: chainId, visible_button: "DAILY JOBS > Full Run", queued_job_key: "daily-context-full-run", queued_worker_name: "alphadog-v2-orchestrator", approved_chain_order: input.approved_chain_order, backend_chain_only: true, auto_pump_triggered: true, browser_auto_pump: false, note: "Queued Daily Context Full Run parent chain: Context Cert (First Pass - finds gaps), then Starters, Lineups, Availability, Weather/Roof, Bullpen, Team Spot, Umpire (each mines real data or a temporary derived fallback), then Context Cert (closes gaps). Catcher Context rides within the Lineups stage. Backend service-binding auto-pump was launched; browser may close now. ORCHESTRATOR > Wake is manual testing/rescue only." });
  }
  ...
  if (job === "orchestrator_enqueue_player_baseline_sanity") {
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = 'player-baseline-sanity' AND status IN ('pending','running','partial_continue') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();

    if (existing) {
      return jsonResponse({ ok: true, data_ok: true, version, job, status: "already_queued", request_id: existing.request_id, existing, visible_button: "DELTA > Baseline Sanity", auto_pump_triggered: false, browser_auto_pump: false, note: "Existing Player Baseline Sanity queue row found. Use ORCHESTRATOR > Wake and MANUAL SQL validation." });
    }

    const requestId = "player_baseline_sanity_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_player_baseline_sanity_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "DELTA > Baseline Sanity",
      mode: "history_only_layer_1_sanity_baseline",
      created_at: now,
      ...
      one_worker_two_modules: true,
      module_a: "historical_sanity_classification",
      module_b: "history_based_baseline_calculation",
      history_only: true,
      ...
      no_final_board: true,
      no_prepared_board_mutation: true,
      writes_score_db_baseline_tables_only: true,
      allowed_writes: ["SCORE_DB.player_baseline_sanity_batches", "SCORE_DB.player_baseline_sanity_stage", "SCORE_DB.player_baseline_sanity_current", "SCORE_DB.player_baseline_sanity_history", "SCORE_DB.player_baseline_sanity_issues"],
      reads_historical_logs: ["STATS_HITTER_DB.hitter_game_logs", "STATS_PITCHER_DB.pitcher_game_logs"],
      sanity_profiles_locked: ["ESTABLISHED_REGULAR", "ROOKIE_LIMITED_SAMPLE", "LOW_USAGE_HITTER", "PLATOON_SPLIT_SENSITIVE", "STABLE_STARTER", "LOW_SAMPLE_STARTER", "UNUSUAL_OR_SPOT_STARTER", "OPENER_PROFILE", "BULK_FOLLOWER_PROFILE", "ROLE_TRANSITION_PITCHER", "RAMP_UP_OR_RETURNING_PLAYER", "EXTREME_LINE_DIFFICULTY", "EXTREME_HEAVY_TAIL_VOLATILITY_PLAYER"]
    };

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'player-baseline-sanity', 'alphadog-v2-phase2b-pitcher-role', 'Delta', 'delta', 'Player Baseline Sanity', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_player_baseline_sanity', 'INFO', 'queued_player_baseline_sanity', 'Queued Layer 1 history-only Player Baseline Sanity worker', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"DELTA > Baseline Sanity", logical_worker_name:"alphadog-v2-player-baseline-sanity", deployed_worker_slot:"alphadog-v2-phase2b-pitcher-role", history_only:true, no_scoring:true})).run();

    return jsonResponse({ ok: true, data_ok: true, version, job, status: "queued", request_id: requestId, chain_id: chainId, visible_button: "DELTA > Baseline Sanity", queued_job_key: "player-baseline-sanity", queued_worker_name: "alphadog-v2-phase2b-pitcher-role", logical_worker_name: "alphadog-v2-player-baseline-sanity", mode: "history_only_layer_1_sanity_baseline", history_only: true, no_daily_live_context: true, no_market_context: true, no_external_api_calls: true, no_scoring: true, no_final_board: true, note: "Queued Layer 1 history-only sanity/baseline worker. Tap ORCHESTRATOR > Wake. It writes SCORE_DB baseline sanity tables only and does not touch prepared board, daily context, market, scoring, or final board." });
  }


  if (job === "orchestrator_enqueue_player_baseline_hp") {
    const existing = await env.CONTROL_DB.prepare(
      "SELECT request_id, status, created_at, updated_at FROM control_job_queue WHERE job_key = 'player-baseline-hp' AND status IN ('pending','running','partial_continue') ORDER BY datetime(created_at) DESC LIMIT 1"
    ).first();

    if (existing) {
      return jsonResponse({ ok: true, data_ok: true, version, job, status: "already_queued", request_id: existing.request_id, existing, visible_button: "DELTA > Baseline HP", auto_pump_triggered: false, browser_auto_pump: false, note: "Existing Player Baseline HP queue row found. Use ORCHESTRATOR > Wake and MANUAL SQL validation." });
    }

    const requestId = "player_baseline_hp_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_player_baseline_hp_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "DELTA > Baseline HP",
      mode: "history_only_baseline_hp_enrichment",
      source_chunk_rows: 80,
      history_chunk_size: 750,
      run_after_delay_seconds: 1,
      created_at: now,
      logical_worker_name: "alphadog-v2-player-baseline-sanity",
      deployed_worker_slot: "alphadog-v2-phase2b-pitcher-role",
      layer: "history_only_baseline_hp_enrichment",
      same_worker_as_sanity_baseline: true,
      reads_layer_1_sanity_current: true,
      history_only: true,
      no_daily_live_context: true,
      no_market_context: true,
      no_external_api_calls: true,
      ...
      no_final_board: true,
      no_prepared_board_mutation: true,
      baseline_hp_anchor: "SCORE_DB.player_baseline_sanity_current.line_baseline_json selected_side baseline_pre_context_0_100",
      hp_adjustment_v1: 0,
      no_old_hp_blend: true,
      confidence_only_profile_enrichment: true,
      no_sample_double_jeopardy: true,
      d1_microchunked: true,
      cooldown_between_chunks_seconds: 1,
      allowed_reads: ["SCORE_DB.player_baseline_sanity_current"],
      allowed_writes: ["SCORE_DB.player_baseline_hp_batches", "SCORE_DB.player_baseline_hp_stage", "SCORE_DB.player_baseline_hp_current", "SCORE_DB.player_baseline_hp_history", "SCORE_DB.player_baseline_hp_issues"],
      allowed_modes: ["baseline_hp_begin", "baseline_hp_stage_chunk", "baseline_hp_promote_current", "baseline_hp_write_history_chunk", "baseline_hp_finalize"],
      prop_families: ["CONTACT_STANDARD", "CONTACT_EXTRA_BASE", "RARE_POWER", "RARE_SPEED", "COMPOSITE_BATTING", "CONTEXT_COUNTING", "PLATE_DISCIPLINE", "PITCHER_STRIKEOUT", "PITCHER_VOLUME", "PITCHER_DAMAGE_ALLOWED", "PITCHER_COMMAND_ALLOWED"]
    };

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'player-baseline-hp', 'alphadog-v2-phase2b-pitcher-role', 'Delta', 'delta', 'Player Baseline HP', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_player_baseline_hp', 'INFO', 'queued_player_baseline_hp', 'Queued history-only Player Baseline HP enrichment in the Sanity/Baseline worker', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"DELTA > Baseline HP", logical_worker_name:"alphadog-v2-player-baseline-sanity", deployed_worker_slot:"alphadog-v2-phase2b-pitcher-role", history_only:true, no_daily_live_context:true, no_market_context:true, no_scoring:true, no_old_hp_blend:true})).run();

    return jsonResponse({ ok: true, data_ok: true, version, job, status: "queued", request_id: requestId, chain_id: chainId, visible_button: "DELTA > Baseline HP", queued_job_key: "player-baseline-hp", queued_worker_name: "alphadog-v2-phase2b-pitcher-role", logical_worker_name: "alphadog-v2-player-baseline-sanity", mode: "history_only_baseline_hp_enrichment", source_chunk_rows: 80, history_chunk_size: 750, run_after_delay_seconds: 1, history_only: true, no_daily_live_context: true, no_market_context: true, no_external_api_calls: true, no_scoring: true, no_final_board: true, no_prepared_board_mutation: true, allowed_writes: input.allowed_writes, note: "Queued history-only Baseline HP enrichment in the same Sanity/Baseline worker. Tap ORCHESTRATOR > Wake. It writes SCORE_DB player_baseline_hp_* tables only and does not touch daily context, market, scoring, prepared board, or final board." });
  }
