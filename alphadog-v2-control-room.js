const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.20-base-hitter-splits-stage-only";
  ...
  if (job === "orchestrator_enqueue_base_hitter_splits") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        auto_pump_triggered: false,
        browser_auto_pump: false,
        note: "Existing Base Hitter Splits stage-only queue row found. Use ORCHESTRATOR > Wake once as a test/rescue accelerator; browser does not run the job."
      });
    }

    const requestId = "base_hitter_splits_stage_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
    const chainId = "chain_base_hitter_splits_stage_" + Date.now().toString(36);
    const input = {
      source: "control_room",
      visible_button: "BASE > Hitter Splits",
      mode: "base_backfill_stage_only",
      created_at: now,
      worker_scope: "base_backfill_stage_only_v0_2_0",
      ...
      endpoint_pattern: "/people/{playerId}/stats?stats=statSplits&group=hitting&season={season}&sitCodes=vl%2Cvr",
      source_season: 2026,
      source_snapshot_required: true,
      source_snapshot_model: "season_to_date_aggregate_snapshot",
      no_2026_05_18_cutoff_claim: true,
      group_type: "hitting",
      ingestion_mode: "base_backfill",
      chunk_size: 8,
      hitter_universe_source_required: "STATS_HITTER_DB.hitter_game_log_player_outcomes locked base batch hitter_base_backfill_batch_mpelpq0t_akyyu3",
      allowed_writes: ["STATS_HITTER_DB.hitter_split_stage", "STATS_HITTER_DB.hitter_split_batches", "STATS_HITTER_DB.hitter_split_outcomes", "STATS_HITTER_DB.hitter_split_cursor", "STATS_HITTER_DB.hitter_split_certifications", "STATS_HITTER_DB.hitter_schema_migrations"],
      additive_hitter_splits_lineage_columns_allowed: true,
      ...
      backend_scheduled_continuation: true,
      no_browser_auto_pump: true,
      control_room_backend_launches_orchestrator_pump: false,
      wake_once_for_test_only: true
    };

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'base-hitter-splits', 'alphadog-v2-base-hitter-splits', 'Base', 'base', 'Base Hitter Splits Stage-Only Backfill v0.2.0', 'pending', 5, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();

    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_base_hitter_splits', 'INFO', 'queued_base_hitter_splits_stage_only', 'Queued exact Base Hitter Splits v0.2.0 base_backfill stage-only job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"BASE > Hitter Splits", base_backfill_stage_only:true, no_live_promotion:true, no_delta_update:true, chunk_size:8})).run();
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued v0.2.0 stage-only base backfill in CONTROL_DB. Tap ORCHESTRATOR > Wake once to dispatch. No live promotion, no delta."
    });
  }
