const SYSTEM_VERSION = "alphadog-v2-control-room-v1.5.21-parlay-sleeper-probe-button";
  ...
  if (job === "orchestrator_enqueue_parlay_sleeper_board") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        request_id: existing.request_id,
        existing,
        visible_button: "MARKET > Sleeper Board"
      });
    }
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_job_queue (request_id, chain_id, job_key, worker_name, worker_group, phase_key, display_name, status, priority, cascade, input_json, run_after, created_at, updated_at) VALUES (?, ?, 'parlay-sleeper-board', 'alphadog-v2-parlay-sleeper-board', '05 Market', 'market', 'Parlay Sleeper Board Source Probe', 'pending', 6, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    ).bind(requestId, chainId, JSON.stringify(input)).run();
    ...
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"MARKET > Sleeper Board"})).run();
