const SYSTEM_VERSION = "alphadog-v2-control-room-v1.5.16-backend-scheduled-continuation";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_static_players") {
    ...
    if (existing) {
      return jsonResponse({
        ...
        note: "Existing Static Players queue row found. Control Room queues only. Orchestrator scheduled backend/cron will continue it; ORCHESTRATOR > Wake is manual one-tick testing only."
      });
    }
    ...
    return jsonResponse({
      ...
      auto_pump_triggered: false,
      browser_auto_pump: false,
      note: "Queued in CONTROL_DB only. Browser may close now. Orchestrator scheduled backend/cron owns continuation; ORCHESTRATOR > Wake is manual one-tick testing only."
    });
  }
  ...
  if (job === "orchestrator_tick") {
    ...
    return jsonResponse({
      ...
      note: "Control Room queues only. Static Players continuation is owned by alphadog-v2-orchestrator scheduled backend/cron. ORCHESTRATOR > Wake is a manual one-tick backend test only.",
