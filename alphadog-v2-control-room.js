const SYSTEM_VERSION = "alphadog-v2-control-room-v1.6.191-expansion-v2-full-group";
...
async function v12OrchestratorLocalBridge(job, env, ctx = null) {
  ...
  if (job === "orchestrator_enqueue_expansion_baseline_v2_heb") {
    ...
    if (existing) {
      return jsonResponse({ ok:true, data_ok:true, version, job, status:"already_queued", request_id:existing.request_id, existing, visible_button:"EXPANSION > V2 HEB Baseline", note:"Existing Expansion V2 HEB baseline job found. Use ORCHESTRATOR > Wake." });
    }
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO control_worker_run_log (request_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, 'alphadog-v2-control-room', 'orchestrator_enqueue_expansion_baseline_v2_heb', 'INFO', 'queued_expansion_baseline_v2_heb', 'Queued Expansion V2 HEB baseline parallel job', ?, CURRENT_TIMESTAMP)"
    ).bind(requestId, JSON.stringify({request_id:requestId, chain_id:chainId, visible_button:"EXPANSION > V2 HEB Baseline", queued_job_key:"expansion-baseline-v2", queued_worker_name:"alphadog-v2-phase3a-first-inning-pitcher-context", mode:"baseline_v2_heb", parallel_v2_only:true, no_current_baseline_mutation:true})).run();
    return jsonResponse({ ok:true, data_ok:true, version, job, status:"queued", request_id:requestId, chain_id:chainId, visible_button:"EXPANSION > V2 HEB Baseline", queued_job_key:"expansion-baseline-v2", queued_worker_name:"alphadog-v2-phase3a-first-inning-pitcher-context", mode:"baseline_v2_heb", v2_chunk_size:80, parallel_v2_only:true, no_current_baseline_mutation:true, note:"Queued parallel V2 HEB baseline. Use ORCHESTRATOR > Wake until completed." });
  }
