...
async function ensureSchema(env) {
  await env.MARKET_DB.batch([
    ...
    env.MARKET_DB.prepare(`CREATE TABLE IF NOT EXISTS market_context_probe_game_odds (probe_row_id TEXT PRIMARY KEY, batch_id TEXT, slate_window_key TEXT, official_date TEXT, game_pk INTEGER, source_key TEXT, source_event_id TEXT, source_commence_time_utc TEXT, source_home_team TEXT, source_away_team TEXT, bookmaker_key TEXT, bookmaker_title TEXT, market_key TEXT, market_last_update TEXT, outcome_name TEXT, outcome_side TEXT, price_american REAL, point REAL, mapping_status TEXT, mapping_confidence TEXT, raw_json TEXT, created_at TEXT DEFAULT CURRENT_TIMESTAMP)`),
    ...
    env.MARKET_DB.prepare("CREATE INDEX IF NOT EXISTS idx_market_context_game_odds_oddsapi ON market_context_probe_game_odds(batch_id, official_date, game_pk, market_key)")
  ]);
}
...
async function runWorker(env, input) {
  ...
  if (gameOddsRows.length) {
    await batchRun(env.MARKET_DB, `INSERT INTO market_context_probe_game_odds (probe_row_id, batch_id, slate_window_key, official_date, game_pk, source_key, source_event_id, source_commence_time_utc, source_home_team, source_away_team, bookmaker_key, bookmaker_title, market_key, market_last_update, outcome_name, outcome_side, price_american, point, mapping_status, mapping_confidence, raw_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`, gameOddsRows, 50);
  }
  ...
  const output = { ok:blockerCount === 0, data_ok:blockerCount === 0, version:VERSION, worker_name:WORKER_NAME, job_key:JOB_KEY, mode:MODE, request_id:requestId, run_id:runId, batch_id:batchId, status, certification, certification_status:certification, certification_grade:grade, rows_read:preparedRows.length, rows_written:rowsWritten + coverageRows.length + gameOddsRows.length, prepared_rows_read:preparedRows.length, prepared_games_checked:indexes.byGame.size, prepared_players_checked:playerIds.length, prepared_prop_keys_checked:new Set(preparedRows.map(r => r.canonical_prop_key)).size, odds_api_config_present:sourceHas(env, "ODDS_API_KEY") ? 1 : 0, odds_api_events_seen:eventsSeen, odds_api_events_mapped:eventsMapped, odds_api_event_calls: eventCalls, oddsapi_prop_rows_written:rowsWritten, oddsapi_prop_rows_matched:matchedRows, oddsapi_prop_rows_unmatched:unmatchedRows, oddsapi_game_odds_rows_written:gameOddsRows.length, coverage_rows_written:coverageRows.length, strong_book_coverage_against_prepared_board:{ denominator_prepared_pickable_rows:boardBaseRows, strong_books_raw_rows:strongBooks.rows, strong_books_matched_rows:strongBooks.matched, strong_books_raw_percent:strongRawPct, strong_books_matched_percent:strongMatchedPct }, book_summary:bookSummary, warning_count:warningCount, blocker_count:blockerCount, external_calls_performed:eventCalls, comprehensive_capture_mode:!preparedRows.length, d1_request_reduction:"v0.2.0 batches prop inserts and builds coverage from in-memory matchedCoverageSet; no per-prepared-row COUNT probes", output_json:{ source_scope:"odds_api_event_level_mlb_all_props_plus_game_odds", official_docs_shape:"player props accessed one event at a time through /events/{eventId}/odds; game-level odds (h2h/spreads/totals) captured directly from the main events call", selected_dummy_slot:"alphadog-v2-oddsapi-reference", slate_window:{ today, tomorrow, slateWindowKey }, event_fetch:{ ok:eventsFetch.ok || false, http_status:eventsFetch.http_status || null }, event_summaries:eventSummaries.slice(0, 24), markets_requested:ODDS_MLB_MARKETS, game_level_markets_requested:GAME_LEVEL_MARKETS, source_boundaries:baseIdentity(env).boundaries }, timestamp_utc:nowUtc() };
...
const HISTORICAL_BACKFILL_2025_DATES = [
  "2025-03-18","2025-03-30","2025-04-04","2025-04-09","2025-04-14","2025-04-19","2025-04-24","2025-04-29",
  "2025-05-04","2025-05-09","2025-05-14","2025-05-19","2025-05-24","2025-05-29",
  "2025-06-03","2025-06-08","2025-06-13","2025-06-18","2025-06-23","2025-06-28",
  "2025-07-03","2025-07-08","2025-07-13","2025-07-22","2025-07-27",
  "2025-08-01","2025-08-06","2025-08-11","2025-08-16","2025-08-21","2025-08-26","2025-08-31",
...
async function runHistoricalBackfillPitchers2025(env, input) {
  ...
  return {
    ...
    batch_id: batchId,
    date_processed: date,
    events_call_http_status: eventsRes.http_status,
...
async function runHistoricalBackfill2025(env, input) {
  if (!env.ODDS_API_KEY) return { ok: false, data_ok: false, version: VERSION, worker_name: WORKER_NAME, job_key: JOB_KEY, status: "odds_api_key_missing", timestamp_utc: nowUtc() };
  const base = String(input.odds_api_base_url || env.ODDS_API_BASE_URL || DEFAULT_ODDS_API_BASE_URL).replace(/\/+$/, "");
  const budgetCap = Number(input.credits_budget_cap || 19500);
  const batchId = String(input.batch_id || `market_historical_2025_${Date.now()}`);

  let progress = await env.MARKET_DB.prepare("SELECT * FROM market_historical_backfill_progress WHERE progress_key = ?").bind("market_historical_2025").first();
  if (!progress) {
    await env.MARKET_DB.prepare(
      "INSERT INTO market_historical_backfill_progress (progress_key, batch_id, total_dates_planned, dates_processed, games_processed, games_no_data, games_error, credits_used_estimate, credits_budget_cap, status, remaining_dates_json, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)"
    ).bind("market_historical_2025", batchId, HISTORICAL_BACKFILL_2025_DATES.length, 0, 0, 0, 0, 0, budgetCap, "pending", JSON.stringify(HISTORICAL_BACKFILL_2025_DATES)).run();
    progress = await env.MARKET_DB.prepare("SELECT * FROM market_historical_backfill_progress WHERE progress_key = ?").bind("market_historical_2025").first();
  }

  if (progress.status === "completed" || progress.status === "budget_exhausted") {
    return { ok: true, data_ok: true, version: VERSION, worker_name: WORKER_NAME, job_key: JOB_KEY, status: progress.status, certification: "MARKET_HISTORICAL_BACKFILL_ALREADY_DONE", progress, continuation_required: false, timestamp_utc: nowUtc() };
  }

  const remainingDates = JSON.parse(progress.remaining_dates_json || "[]");
  if (remainingDates.length === 0) {
    await env.MARKET_DB.prepare("UPDATE market_historical_backfill_progress SET status='completed', updated_at=CURRENT_TIMESTAMP WHERE progress_key='market_historical_2025'").run();
    return { ok: true, data_ok: true, version: VERSION, worker_name: WORKER_NAME, job_key: JOB_KEY, status: "completed", certification: "MARKET_HISTORICAL_BACKFILL_COMPLETED", continuation_required: false, timestamp_utc: nowUtc() };
  }
  ...
  for (const ev of events) {
    if (Number(progress.credits_used_estimate) + creditsUsedThisTick + HISTORICAL_BACKFILL_CREDITS_PER_EVENT > budgetCap) break;
    const oddsUrl = new URL(`${base}/historical/sports/baseball_mlb/events/${encodeURIComponent(ev.id)}/odds`);
    oddsUrl.searchParams.set("apiKey", String(env.ODDS_API_KEY));
    oddsUrl.searchParams.set("regions", "us");
    oddsUrl.searchParams.set("markets", HISTORICAL_BACKFILL_CORE_MARKETS.join(","));
    oddsUrl.searchParams.set("date", `${date}T20:00:00Z`);
    const oddsRes = await fetchJson(oddsUrl.toString());
    creditsUsedThisTick += HISTORICAL_BACKFILL_CREDITS_PER_EVENT;
    ...
    for (const bk of bookmakers) {
      for (const mk of (bk.markets || [])) {
        for (const oc of (mk.outcomes || [])) {
          const rowId = `${ev.id}_${bk.key}_${mk.key}_${normalizeText(oc.name)}_${normalizeText(oc.description || "")}`.slice(0, 190);
          insertStmts.push(env.MARKET_DB.prepare(
            "INSERT OR REPLACE INTO market_historical_props_2025 (row_id, batch_id, official_date, odds_api_event_id, home_team, away_team, commence_time_utc, bookmaker_key, market_key, player_name, outcome_name, line_point, price_american, snapshot_timestamp, raw_json) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
          ).bind(rowId, batchId, date, ev.id, ev.home_team || null, ev.away_team || null, ev.commence_time || null, bk.key, mk.key, oc.description || null, oc.name || null, oc.point ?? null, oc.price ?? null, oddsRes.json.timestamp || null, JSON.stringify(oc)));
        }
  ...
  await env.MARKET_DB.prepare(
    "UPDATE market_historical_backfill_progress SET dates_processed = dates_processed + 1, games_processed = games_processed + ?, games_no_data = games_no_data + ?, games_error = games_error + ?, credits_used_estimate = ?, status = ?, last_date_processed = ?, remaining_dates_json = ?, updated_at = CURRENT_TIMESTAMP WHERE progress_key = 'market_historical_2025'"
  ).bind(gamesProcessed, gamesNoData, gamesError, newCreditsUsed, newStatus, date, JSON.stringify(budgetExhausted ? [] : nextRemaining)).run();

  return {
    ok: true, data_ok: true, version: VERSION, worker_name: WORKER_NAME, job_key: JOB_KEY,
    status: newStatus,
    certification: newStatus === "budget_exhausted" ? "MARKET_HISTORICAL_BACKFILL_BUDGET_EXHAUSTED" : (newStatus === "completed" ? "MARKET_HISTORICAL_BACKFILL_COMPLETED" : "MARKET_HISTORICAL_BACKFILL_PARTIAL_CONTINUE"),
    batch_id: batchId,
    date_processed: date,
    real_events_found: events.length,
    games_processed_this_date: gamesProcessed,
