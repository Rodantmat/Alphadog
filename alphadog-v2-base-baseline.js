    ...
    try {
      if (url.pathname === "/" || url.pathname === "/health") {
        return new Response(JSON.stringify({
          ...
          deprecation_warning: "DEAD/STALE as of 2026-08-14, NOT read by live scoring - phase3c-certifier.js (the real, live HP-board worker) reads classification.baseline_v6_current instead, never this worker's output table (classification.baseline_current). This worker's code still functions correctly if invoked - it simply is not being called anymore. See COMMENT ON TABLE on the affected table for full detail."
