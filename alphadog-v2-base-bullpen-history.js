...
async function acquireBatchLock(sql, batchId, owner, staleSeconds) {
  const rows = await sql`SELECT locked_by, lock_acquired_at, lock_expires_at FROM team.bullpen_history_batches WHERE batch_id=${batchId} LIMIT 1`;
  ...
  await sql`UPDATE team.bullpen_history_batches SET locked_by=${owner}, lock_acquired_at=now(), lock_expires_at=now() + make_interval(secs => ${staleSeconds}), updated_at=now() WHERE batch_id=${batchId}`;
  return { ok: true, owner };
}
async function releaseBatchLock(sql, batchId, owner) {
  await sql`UPDATE team.bullpen_history_batches SET locked_by=NULL, lock_acquired_at=NULL, lock_expires_at=NULL, updated_at=now() WHERE batch_id=${batchId} AND locked_by=${owner}`;
}
async function buildPrePromotionChecks(sql, batchId) {
  const dupRows = await sql`SELECT COUNT(*)::int AS c FROM (SELECT team_id, game_pk, player_id, COUNT(*) AS n FROM team.bullpen_history_stage WHERE batch_id=${batchId} GROUP BY team_id, game_pk, player_id HAVING COUNT(*) > 1) sub`;
  const missingRows = await sql`SELECT COUNT(*)::int AS c FROM team.bullpen_history_stage WHERE batch_id=${batchId} AND (team_id IS NULL OR game_pk IS NULL OR game_date IS NULL OR player_id IS NULL)`;
  const totalRows = await sql`SELECT COUNT(*)::int AS c FROM team.bullpen_history_stage WHERE batch_id=${batchId}`;
...
async function runBaseBackfillTick(env, sql, input) {
  ...
  try {
    ...
    if (status === "BASE_BACKFILL_MINING") {
      const ins = await deriveBullpenStageRows(sql, batchId, runId, "base_backfill", batch.source_season, "2026-03-25", batch.base_backfill_cutoff_date);
      await sql`UPDATE team.bullpen_history_batches SET status='BASE_BACKFILL_STAGED_READY_FOR_CERTIFICATION', rows_staged=${ins.inserted}, updated_at=now() WHERE batch_id=${batchId}`;
      return { ok: true, data_ok: true, mode: "base_backfill", batch_id: batchId, status: "BASE_BACKFILL_STAGED_READY_FOR_CERTIFICATION", rows_staged: ins.inserted, continuation_required: true };
    }
    if (status === "BASE_BACKFILL_STAGED_READY_FOR_CERTIFICATION") {
      const pre = await buildPrePromotionChecks(sql, batchId);
      if (!pre.pass) {
        await sql`UPDATE team.bullpen_history_batches SET status='CERTIFICATION_FAILED', certification_status='BASE_BULLPEN_HISTORY_CERTIFICATION_FAILED', certification_grade='BASE_FAIL', certification_json=${JSON.stringify(pre.checks)}, updated_at=now() WHERE batch_id=${batchId}`;
        return { ok: true, data_ok: false, mode: "base_backfill", batch_id: batchId, status: "CERTIFICATION_FAILED", checks: pre.checks };
      }
      await sql`UPDATE team.bullpen_history_batches SET status='BASE_BACKFILL_CERTIFIED_READY_TO_PROMOTE', certification_status='BASE_BULLPEN_HISTORY_CERTIFIED', certification_grade='BASE_PASS', certification_json=${JSON.stringify(pre.checks)}, certified_at=now(), updated_at=now() WHERE batch_id=${batchId}`;
