...
async function runPlayerPropContext(env, input = {}) {
  ...
  try {
    ...
    for (let i = 0; i < effectivePropRows.length; i += BATCH_CHUNK) await pgClient`INSERT INTO market.context_probe_player_props ${pgClient(effectivePropRows.slice(i, i + BATCH_CHUNK), ...propCols)}
      ON CONFLICT (probe_row_id) DO UPDATE SET batch_id=EXCLUDED.batch_id, slate_window_key=EXCLUDED.slate_window_key, official_date=EXCLUDED.official_date, prepared_row_id=EXCLUDED.prepared_row_id, source_key=EXCLUDED.source_key, source_event_id=EXCLUDED.source_event_id, source_line_id=EXCLUDED.source_line_id, game_pk=EXCLUDED.game_pk, resolved_mlb_player_id=EXCLUDED.resolved_mlb_player_id, source_player_name=EXCLUDED.source_player_name, canonical_prop_key=EXCLUDED.canonical_prop_key, source_market_key=EXCLUDED.source_market_key, line_value=EXCLUDED.line_value, price_american=EXCLUDED.price_american, price_decimal=EXCLUDED.price_decimal, outcome_side=EXCLUDED.outcome_side, mapping_status=EXCLUDED.mapping_status, coverage_status=EXCLUDED.coverage_status, raw_json=EXCLUDED.raw_json`;
...
async function runHistoricalBackfill(env, input = {}) {
  ...
  try {
    ...
    const teamAliases = await loadTeamAliasMap(pgClient);
    const playerAliases = await loadPlayerAliasMap(pgClient);
    const gameRows = await pgClient`SELECT official_date::text as d, game_pk, home_team_id, away_team_id FROM calendar.game_calendar WHERE official_date::text = ANY(${pgClient.array(dates)})`;
