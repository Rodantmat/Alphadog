...
const PARLAY_PITCHER_ODDS_MARKETS = ["player_strikeouts", "player_pitcher_strikeouts", "pitcher_strikeouts", "player_pitcher_outs", "pitcher_outs", "player_outs", "player_outs_recorded", "pitcher_outs_recorded", "player_hits_allowed", "pitcher_hits_allowed", "player_walks_allowed", "pitcher_walks_allowed", "player_earned_runs", "player_earned_runs_allowed", "pitcher_earned_runs", "pitcher_earned_runs_allowed", "player_runs_allowed", "pitcher_runs_allowed"];
...
const MARKET_FULL_WORKER_SOFT_DEADLINE_MS = 48000;
...
const MARKET_FULL_BACKEND_BATCH_CHUNK_SIZE = 60;
...
const PITCHER_MARKET_KEY_TO_PROP = {
  ...
  pitcher_earned_runs_allowed: "earned_runs",
  earned_runs: "earned_runs",
  earned_runs_allowed: "earned_runs",
  player_runs_allowed: "runs_allowed",
  pitcher_runs_allowed: "runs_allowed",
  runs_allowed: "runs_allowed",
  pitcher_strikeouts_combo: "pitcher_strikeouts_combo",
  player_pitcher_strikeouts_combo: "pitcher_strikeouts_combo"
};
...
function modeConfig(input = {}) {
  ...
  return isPitcher ? {
    mode,
    prop_family: "pitcher",
    prop_keys: PITCHER_PROP_KEYS,
    market_map: PITCHER_MARKET_KEY_TO_PROP,
    parlay_markets: PARLAY_PITCHER_MARKETS,
    parlay_odds_markets: PARLAY_PITCHER_ODDS_MARKETS,
    source_suffix: "pitcher_props",
    batch_prefix: "market_pitcher_props_batch",
    request_prefix: "market_pitchers",
    issue_prefix: "PITCHER_PROP",
    coverage_prefix: "PITCHER_PROP",
    player_props_label: "pitcher",
    no_opposite_boundary_key: "no_hitter_props"
  } : {
    mode,
    prop_family: "hitter",
    prop_keys: HITTER_PROP_KEYS,
    market_map: HITTER_MARKET_KEY_TO_PROP,
    parlay_markets: PARLAY_HITTER_MARKETS,
    parlay_odds_markets: PARLAY_HITTER_ODDS_MARKETS,
    source_suffix: "hitter_props",
    batch_prefix: "market_hitter_props_batch",
    request_prefix: "market_hitters",
    issue_prefix: "HITTER_PROP",
    coverage_prefix: "HITTER_PROP",
    player_props_label: "hitter",
    no_opposite_boundary_key: "no_pitcher_props"
  };
}
...
function authHeaders(env) {
  const headers = new Headers({ "accept": "application/json", "user-agent": "AlphaDog-v2-market-player-prop-context/0.2.8" });
...
function extractPlayerFromMarketLabel(value) {
  ...
  for (const re of patterns) {
    ...
    if (m) return cleanPlayerCandidate(m[1]);
  }
  return null;
}
...
function collectDeepPlayerCandidates(obj, out = [], path = "", depth = 0) {
  ...
  if (Array.isArray(obj)) {
    for (let i = 0; i < Math.min(obj.length, 20); i++) collectDeepPlayerCandidates(obj[i], out, `${path}[${i}]`, depth + 1);
    return out;
  }
...
function normalizeParlayRow(row, config = modeConfig()) {
  ...
  let canonicalInfo = plus && plus.canonical_prop_key
    ? { canonical: plus.canonical_prop_key, market_key: `player_${plus.canonical_prop_key}`, reason: "plus_threshold_market_parser" }
    : canonicalFromMarket(marketTitleRaw || marketKeyRaw, marketKey, config);
  ...
  if ((!playerNormNow || playerNormNow === "odd" || playerNormNow === "even" || String(playerName || "").includes(" @ ")) && extractPlayerFromMarketLabel(marketTitleRaw || marketKeyRaw)) {
    playerName = extractPlayerFromMarketLabel(marketTitleRaw || marketKeyRaw);
  }
  ...
  if (plus && plus.normalized_line_value !== null) {
    thresholdLineValue = plus.normalized_line_value;
    lineValue = thresholdLineValue;
    side = "over";
  }
  ...
  if (directHadTemplate && playerExtract.reason === "no_usable_player_candidate_found") {
    normalizationStatus = "quarantined_template_player_label";
    issues.push("template_player_label_unresolved");
  } else if (directHadTemplate) {
    issues.push("template_player_label_recovered");
  }
  ...
  return {
    raw: row,
    market_key: marketKey || String(marketKeyRaw || marketTitleRaw || ""),
    canonical_prop_key: canonical,
    player_name: playerName ? String(playerName) : null,
    player_name_normalized: normalizeText(playerName),
    line_value: lineValue,
    threshold_value: plus ? plus.threshold : null,
    normalized_threshold: plus ? plus.threshold : null,
    threshold_line_value: thresholdLineValue,
    source_event_id: eventId ? String(eventId) : null,
    source_line_id: lineId ? String(lineId) : null,
    commence_time: commenceTime ? String(commenceTime) : null,
    home_team: homeTeam ? String(homeTeam) : null,
    away_team: awayTeam ? String(awayTeam) : null,
    bookmaker: bookmaker ? String(bookmaker) : "sleeper",
    bookmaker_key: bookmakerKey,
    book_quality: bookmakerQuality(bookmakerKey),
    normalization_status: normalizationStatus,
    normalization_issues: issues,
    normalization_reason: canonicalInfo.reason,
    player_name_resolution_reason: playerExtract.reason,
    threshold_source_text: plus ? plus.source_text : null,
    over_price: overPrice,
    under_price: underPrice,
    side,
    price_american: directPrice
  };
}
...
function parlayBaseUrlFromEndpoint(endpoint) {
  if (endpoint && endpoint.url) return endpoint.url;
  return `${DEFAULT_PARLAY_API_BASE_URL}${DEFAULT_PARLAY_PROPS_ENDPOINT}`;
}
function parlayOddsUrlFromPropsUrl(propsUrl, input = {}) {
  ...
  if (explicit) {
    const raw = /^https?:\/\//i.test(String(explicit)) ? String(explicit) : `${u.origin}${String(explicit).startsWith("/") ? explicit : "/" + explicit}`;
    return raw;
  }
...
function flattenOddsEndpointRows(json, probe = {}) {
  ...
  if (json && typeof json === "object") {
    for (const key of ["data", "events", "results"]) {
      if (Array.isArray(json[key])) roots.push(...json[key]);
    }
  }
  const rows = [];
    ...
    for (const book of bookmakers) {
      ...
      for (const market of markets) {
        ...
        for (const outcome of outcomes) {
          ...
          rows.push({
            __probe_id: probe.probe_id || "odds_endpoint_probe",
            __endpoint_kind: "odds",
            bookmaker: book.key || book.bookmaker || book.title || book.name,
            bookmaker_title: book.title || book.name || book.key,
            player,
            market_key: marketKey,
            market: marketKey,
            line: outcome.point ?? outcome.line ?? outcome.line_value ?? market.point ?? null,
            side: directSide,
            price: outcome.price ?? outcome.american_price ?? outcome.odds ?? null,
            event_id: event.id || event.event_id || event.canonical_event_id,
            canonical_event_id: event.canonical_event_id || event.id || event.event_id,
            commence_time: event.commence_time || event.start_time || event.game_time,
            home_team: event.home_team || event.homeTeam,
            away_team: event.away_team || event.awayTeam,
            last_update: book.last_update || market.last_update || outcome.last_update,
            source_shape: "parlay_odds_endpoint_nested_event_market_outcome",
            raw_odds_event_id: event.id || null,
            raw_market_key: marketKey,
            raw_outcome: outcome
          });
        }
      }
...
async function fetchOneParlayProbe(env, probe, deadlineAt = Date.now() + PARLAY_TOTAL_FETCH_BUDGET_MS) {
  ...
  try {
    for (let page = 0; page < PARLAY_MAX_PAGES && nextUrl; page++) {
      ...
      if (probe.endpoint_kind === "odds") {
        extractedRows = flattenOddsEndpointRows(json, probe);
        detectedPath = "events[].bookmakers[].markets[].outcomes[]";
      } else {
        const extracted = extractRawRows(json);
        extractedRows = extracted.rows || [];
        detectedPath = extracted.path;
        candidates.push(...(extracted.candidates || []).map(c => ({ ...c, page, probe_id: probe.probe_id })));
      }
    ...
    return { ok: false, error: safeText(err && err.message ? err.message : err), rows, candidates, pages };
  }
}
async function fetchParlayProps(env, input = {}, config = modeConfig(input)) {
  ...
  for (const [book, count] of Object.entries(bookCounts)) {
    const q = bookmakerQuality(book);
    bookQualityCounts[q] = (bookQualityCounts[q] || 0) + count;
  }
...
async function runPlayerPropContext(env, input = {}) {
  ...
  const playerIds = [...new Set(preparedRows.map(r => Number(r.resolved_mlb_player_id)).filter(Number.isFinite))];
  const propKeys = [...new Set(preparedRows.map(r => String(r.canonical_prop_key || "")).filter(Boolean))];
  // v0.2.11: early heartbeat for Market Full Run child lifecycle proof.
  // The final batch row still replaces this row; while the worker is live,
  // SQL can now distinguish an active child from an unentered/stale dispatch.
  await run(env.MARKET_DB, `INSERT OR REPLACE INTO market_context_probe_batches (batch_id, request_id, run_id, worker_name, worker_version, mode, slate_window_key, window_start_date, window_end_date, status, prepared_rows_read, prepared_games_checked, prepared_players_checked, prepared_prop_keys_checked, odds_api_config_present, parlay_inventory_rows_seen, parlay_props_mapped_to_prepared, parlay_coverage_grade, warning_count, blocker_count, certification_status, certification_grade, output_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`, batchId, requestId, runId, WORKER_NAME, VERSION, config.mode, slateWindowKey, today, tomorrow, `running_${config.prop_family}_prop_context_started`, preparedRows.length, gamePks.length, playerIds.length, propKeys.length, sourceHas(env, "ODDS_API_KEY") ? 1 : 0, 0, 0, "RUNNING", 0, 0, `MARKET_${config.issue_prefix}_CONTEXT_RUNNING_STARTED`, "RUNNING", safeJson({ retention, started_heartbeat: true, request_id: requestId, run_id: runId, prop_family: config.prop_family }, 3000));
  const issues = [];
  let blockerCount = 0;
  let warningCount = 0;
  ...
  if (!preparedRows.length) {
    ...
    await run(env.MARKET_DB, `INSERT OR REPLACE INTO market_context_probe_batches (batch_id, request_id, run_id, worker_name, worker_version, mode, slate_window_key, window_start_date, window_end_date, status, prepared_rows_read, prepared_games_checked, prepared_players_checked, prepared_prop_keys_checked, odds_api_config_present, parlay_inventory_rows_seen, parlay_props_mapped_to_prepared, parlay_coverage_grade, warning_count, blocker_count, certification_status, certification_grade, output_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`, batchId, requestId, runId, WORKER_NAME, VERSION, config.mode, slateWindowKey, today, tomorrow, "blocked_no_prepared_safe_player_prop_rows", 0, 0, 0, 0, sourceHas(env, "ODDS_API_KEY") ? 1 : 0, 0, 0, "NO_PREPARED_ROWS", warningCount, blockerCount, "MARKET_PLAYER_PROP_CONTEXT_NO_PREPARED_SAFE_ROWS", "BLOCKED", safeJson(output, 9000));
  ...
  if (backendSequence && normalizedRowsForMapping.length > MARKET_FULL_BACKEND_MAX_NORMALIZED_PARLAY_ROWS && input.full_market_prop_evidence_scan !== true) {
    normalizedRowsForMapping = normalizedRowsForMapping.slice(0, MARKET_FULL_BACKEND_MAX_NORMALIZED_PARLAY_ROWS);
    normalizedRowsTruncatedForBackendTimebox = true;
  }
  await run(env.MARKET_DB, "UPDATE market_context_probe_batches SET status=?, parlay_inventory_rows_seen=?, updated_at=CURRENT_TIMESTAMP WHERE batch_id=?", `running_${config.prop_family}_mapping_player_props`, Number((parlay && parlay.rows_seen) || normalizedRowsSeenBeforeBackendCap || 0), batchId);
  ...
  const uniqueCoverageSummary = {
    prepared_unique_player_prop_units: preparedUniquePlayerPropUnits.size,
    ...
    primary_comparison_book_pct: pct(coveredUnits.primary_comparison_book.size, preparedUniquePlayerPropUnits.size),
    pickem_comparison_pct: pct(coveredUnits.pickem_comparison.size, preparedUniquePlayerPropUnits.size),
    exchange_or_sharp_reference_pct: pct(coveredUnits.exchange_or_sharp_reference.size, preparedUniquePlayerPropUnits.size),
    requires_shape_validation_pct: pct(coveredUnits.requires_shape_validation.size, preparedUniquePlayerPropUnits.size),
    total_non_owned_pct: pct(coveredUnits.total_non_owned.size, preparedUniquePlayerPropUnits.size)
  };
  ...
  const output = {
    ...
    selected_worker_slot: WORKER_NAME,
    mode: config.mode,
    read_scope: `SCORE_DB.score_board_prepared_current prepared-safe ${config.prop_family} rows + ParlayAPI split book ${config.prop_family} prop probes`,
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (method === "POST" && path === "/run") {
      ...
      await controlLifecycleHeartbeat(env, input, "running_player_prop_context_worker_started", { prop_family: modeConfig(input).prop_family });
      try {
