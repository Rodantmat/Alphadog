...
const DEFAULT_PROMOTE_ROWS_PER_TICK = 300;
...
async function getLockedBaseIntegrity(sql) {
  ...
  const pass = !!batch && String(batch.status) === "COMPLETED_PROMOTED_CLEANED" && asInt(dup.c, 0) === 0 && asInt(after.c, 0) === 0 && asInt(live.c, 0) > 0 && asInt(outcomes.c, 0) > 0;
...
async function getOrCreateDeltaState(env, sql, input, inputJson, windowInfo) {
  ...
  await sql`
    ...
    VALUES (${batchId}, ${runId}, ${WORKER_NAME}, ${VERSION}, 'delta_update', 'DELTA_RUNNING', ${DATA_FEED_KEY}, ${SOURCE_KEY}, ${LOCKED_SOURCE_ENDPOINT_PATTERN}, ${sourceSeason}, 'R', ${DEFAULT_BASE_BACKFILL_CUTOFF_DATE}, ${windowInfo.delta_start_date}, 0, ${JSON.stringify(cursorJson)}, ${tickConfigAtDeltaCreate.chunk_size_players}, ${tickConfigAtDeltaCreate.chunk_size_players}, ${DEFAULT_MAX_ROWS_PER_TICK}, 'not_certified', ${`delta_update window ${windowInfo.delta_start_date} through ${windowInfo.delta_end_date}; base batch ${LOCKED_BASE_BATCH_ID} gate required`}, now())
    ON CONFLICT (batch_id) DO UPDATE SET run_id=excluded.run_id, status=excluded.status, cursor_state_json=excluded.cursor_state_json, updated_at=now()
  `;
  await sql`
    INSERT INTO stats_pitcher.game_log_cursor (cursor_key,batch_id,run_id,mode,status,source_season,base_backfill_cutoff_date,delta_start_date,current_player_offset,players_total,players_processed,requests_done,next_run_after,cursor_json,updated_at)
...
async function runBaseBackfillTick(env, sql, input, startedAtMs) {
  ...
  try {
    ...
    return { ok: true, data_ok: true, mode: "base_backfill", phase: doneScanning ? "mining_complete_finalization_started" : "mining", batch_id: batchId, run_id: runId, status: finalization ? finalization.status : nextStatus, continuation_required: !doneScanning, orchestrator_should_self_continue: !doneScanning, players_total: players.length, players_processed: offset, processed_this_tick: processedThisTick, rows_staged_this_tick: rowsThisTick, per_player_results: perPlayerResults, finalization };
  } finally { await releaseBatchLock(sql, batchId, owner); }
}
