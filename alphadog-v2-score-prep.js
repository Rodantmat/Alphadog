...
const VERSION = "alphadog-v2-score-prep-v0.5.2-cpu-limit-fix";
...
function pgClient(env) {
  return postgres(env.HYPERDRIVE.connectionString, { max: 1, fetch_types: false, prepare: false, connect_timeout: 8, connection: { statement_timeout: 20000, idle_in_transaction_session_timeout: 20000 } });
}
...
// Simple, driver-agnostic bulk insert: build a real, parameterized multi-row VALUES clause.
...
async function permanentlyRecordBoardLegs(env) {
  const rows = await allRows(env.pg, "SELECT prepared_row_id, official_date, source_key, source_row_id, player_name, resolved_mlb_player_id, team, opponent, canonical_prop_key, source_prop_name, line_value, official_game_pk, official_game_time_utc, pickable_safe, prep_status, raw_source_json FROM score.board_prepared_current").catch(() => []);
  if (!rows.length) return { copied: 0, checked: 0 };
  const cols = ["prepared_row_id", "official_date", "source_key", "source_row_id", "player_name", "resolved_mlb_player_id", "team", "opponent", "canonical_prop_key", "source_prop_name", "line_value", "official_game_pk", "official_game_time_utc", "pickable_safe", "prep_status", "raw_source_json", "captured_at"];
  const CHUNK = 200;
  const chunks = [];
  for (let i = 0; i < rows.length; i += CHUNK) chunks.push(rows.slice(i, i + CHUNK));
  for (const chunk of chunks) {
    const values = chunk.map(r => ({ ...r, captured_at: new Date().toISOString() }));
    await pgBulkInsert(env.pg, "archive.board_leg_history", cols, values, "ON CONFLICT (prepared_row_id) DO NOTHING");
  }
...
function preparedRowBase({ batchId, sourceKey, sourceRowId, sourceEventId, projectionId, playerName, propKey, sourcePropName, lineValue, sourceStartTime, sourcePickable, isGoblin, isDemon, isUnderAllowed, rawJson, payloadJson, calendarResolution, playerResolution, side, now }) {
  ...
  return {
    ...
    is_goblin: isGoblin == null ? 0 : Number(isGoblin),
    is_demon: isDemon == null ? 0 : Number(isDemon),
    is_under_allowed: isUnderAllowed == null ? 1 : Number(isUnderAllowed),
...
async function loadMarketRows(env) {
  ...
  let underdogRows = [];
  try {
    underdogRows = await allRows(env.pg, "SELECT * FROM market.underdog_board_current");
  } catch (err) {
...
function preparePrizePicksRows(rows, ref, calendar, batchId, now) {
  ...
  for (const r of rows) {
    ...
    out.push(preparedRowBase({
      ...
      isGoblin: Number(r.is_goblin || 0),
      isDemon: Number(r.is_demon || 0),
      isUnderAllowed: r.is_under_allowed == null ? 1 : Number(r.is_under_allowed),
