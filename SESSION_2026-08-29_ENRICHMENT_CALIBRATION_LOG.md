...
Two separate findings that initially looked like large, active problems (`batter_quality_of_contact`, `lineup_slot`) both turned out, on closer inspection, to be non-issues — one a real historical bug already fixed, the other a stale-backtest-coefficient artifact. This is not a failure of the session; it's the methodology working as intended (day-level tests, lookahead checks, and now live-coefficient checks catching false positives before they get acted on). What remains: confirmed real cell-coverage gaps (`defensive_quality_oaa`/`hits_allowed`, `lineup_surrounding_quality`'s untested props), one thin-but-suggestive lead (`lineup_surrounding_quality` on `rbis`, 7 days), and a large bucket of fully-wired factors with real variance but no day-level-detectable value. No single dominant villain has been found. The clearest remaining path forward, per the dossier's updated next-steps, may be testing a combined "dampen everything without proven value" variant rather than continuing to chase individual factors one at a time.
