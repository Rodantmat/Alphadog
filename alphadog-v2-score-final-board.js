  ...
  while (true) {
    const page = await pgClient`
      SELECT
        ...
        bp.is_under_allowed AS prep_is_under_allowed,
...
function addQuotaReserveRows(mappedRows, baseRows) {
  ...
  for (const side of sides) {
    ...
    if (!floor) continue;
    const predicate = r => String(r.selected_side || '').toLowerCase() === side;
    const available = countAvailable(predicate);
    const target = Math.min(floor, available);
    const before = countSelected(predicate);
    const added = addBest(predicate, Math.max(0, target - before), `side_floor:${side}`);
    diagnostics.push({ floor_type: 'side', key: side, target, available, before, added, after: countSelected(predicate) });
  }
  ...
  for (let i = 0; i < rows.length; i += chunkSize) {
    ...
    await pgClient`INSERT INTO score.final_board_history ${pgClient(values, ...historyCols)}
      ON CONFLICT (prepared_row_id) DO UPDATE SET
        ...
        goblin_demon_tier=EXCLUDED.goblin_demon_tier, goblin_demon_anchor_line=EXCLUDED.goblin_demon_anchor_line, goblin_demon_tier_direction=EXCLUDED.goblin_demon_tier_direction`;
...
async function generateFinalBoard(pgClient, input) {
  ...
  const hpRead = await fetchHpFinalBoardCandidateRows(pgClient, simBatchId, 20000);
