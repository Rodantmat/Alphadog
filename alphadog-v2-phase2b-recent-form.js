...
const SYSTEM_VERSION = "alphadog-v2-prop-factor-miner-v0.2.0-classification-v6-realigned";
const DEPLOYED_SLOT_VERSION = "alphadog-v2-phase2b-recent-form-v0.3.0-classification-v6-realigned";

const REQUIRED_DB_BINDINGS = ["CONTROL_DB", "CONFIG_DB", "REF_DB", "STATS_HITTER_DB", "STATS_PITCHER_DB", "TEAM_DB", "DAILY_DB", "MARKET_DB", "SCORE_DB", "SCORING_DB", "ARCHIVE_DB"];

const HITTER_PACKET_FLUSH_SIZE = 100;
const PITCHER_PACKET_FLUSH_SIZE = 250;
const HITTER_MAX_FACTOR_ROWS_PER_INVOCATION = 900;
...
async function retentionCleanup(env, dates, family) {
  ...
  await run(env.SCORING_DB, `DELETE FROM prop_factor_hitter_packets WHERE official_date NOT IN (${inClause})`, ...dates);
  await run(env.SCORING_DB, `DELETE FROM prop_factor_pitcher_packets WHERE official_date NOT IN (${inClause})`, ...dates);
  await run(env.SCORING_DB, `DELETE FROM prop_factor_issues WHERE official_date IS NOT NULL AND official_date NOT IN (${inClause})`, ...dates);
  await run(env.SCORING_DB, `DELETE FROM prop_factor_coverage_current WHERE official_date NOT IN (${inClause})`, ...dates);
  await run(env.SCORING_DB, `DELETE FROM prop_factor_batches WHERE window_start NOT IN (${inClause}) AND window_end NOT IN (${inClause})`, ...dates, ...dates);
  const packetTable = family === "pitcher" ? "prop_factor_pitcher_packets" : "prop_factor_hitter_packets";
  await run(env.SCORING_DB, `DELETE FROM ${packetTable} WHERE official_date IN (${inClause})`, ...dates);
  await run(env.SCORING_DB, `DELETE FROM prop_factor_issues WHERE factor_family=? AND official_date IN (${inClause})`, family, ...dates);
  await run(env.SCORING_DB, `DELETE FROM prop_factor_coverage_current WHERE factor_family=? AND official_date IN (${inClause})`, family, ...dates);
}
...
async function loadContext(env, dates) {
  ...
  ctx.lineups = latestRowsBy(await all(env.DAILY_DB, `SELECT lineup_row_id, batch_id, game_pk, official_date, team_side, team_id, player_id, lineup_slot, batting_order_code, bat_side, active_position, lineup_status, confidence_label, updated_at FROM daily_lineups_current WHERE official_date IN (${inClause})`, ...dates), r => key(r.game_pk, r.player_id));
  ctx.availability = latestRowsBy(await all(env.DAILY_DB, `SELECT availability_key, batch_id, official_date, game_pk, player_id, mlb_player_id, team_mlb_id, opponent_mlb_id, availability_status, roster_status, availability_confidence, active_roster_flag, injured_list_flag, transaction_warning_flag, transaction_block_flag, team_mismatch_flag, source_missing_flag, reason, evaluation_json, updated_at FROM daily_player_availability_current_v1 WHERE official_date IN (${inClause})`, ...dates), r => key(r.game_pk, r.mlb_player_id || r.player_id));
  ctx.weather = latestRowsBy(await all(env.DAILY_DB, `SELECT weather_key, batch_id, game_pk, official_date, weather_status, weather_confidence, temperature_f, humidity_pct, wind_speed_mph, wind_gust_mph, wind_direction_cardinal, wind_context, rain_risk_flag, delay_risk_flag, roof_type, roof_status, indoor_flag, weather_applicable_flag, park_weather_notes, updated_at FROM daily_game_weather_current WHERE official_date IN (${inClause})`, ...dates), r => key(r.game_pk));
  ctx.starters = latestRowsBy(await all(env.DAILY_DB, `SELECT current_key, batch_id, game_pk, official_date, team_id, opponent_team_id, is_home, starter_player_id, starter_name, starter_hand, starter_status, starter_confidence, source_status, game_status, scratch_flag, opener_flag, bulk_pitcher_flag, tbd_flag, unavailable_flag, updated_at FROM daily_starters_current WHERE official_date IN (${inClause})`, ...dates), r => key(r.game_pk, r.team_id));
  ctx.bullpen = latestRowsBy(await all(env.DAILY_DB, `SELECT bullpen_availability_key, batch_id, official_date, game_pk, team_id, opponent_team_id, is_home, bullpen_status, bullpen_confidence, availability_grade, games_played_last_1_day, games_played_last_2_days, games_played_last_3_days, bullpen_pitches_last_1_day, bullpen_pitches_last_2_days, bullpen_pitches_last_3_days, high_usage_reliever_count, back_to_back_reliever_count, likely_unavailable_reliever_count, rested_reliever_count, bullpen_fatigue_score, bullpen_risk_level, details_json, updated_at FROM daily_bullpen_availability_current WHERE official_date IN (${inClause})`, ...dates), r => key(r.game_pk, r.team_id));
  ctx.pitcherBullpen = latestRowsBy(await all(env.DAILY_DB, `SELECT pitcher_availability_key, batch_id, official_date, team_id, pitcher_id, pitcher_hand, role_hint, active_roster_flag, availability_status, availability_confidence, pitches_last_1_day, pitches_last_2_days, pitches_last_3_days, outs_last_1_day, outs_last_2_days, outs_last_3_days, back_to_back_flag, high_pitch_recent_flag, likely_unavailable_flag, notes, details_json, updated_at FROM daily_bullpen_pitcher_availability_current WHERE official_date IN (${inClause})`, ...dates), r => key(r.official_date, r.team_id, r.pitcher_id));
  ctx.schedule = latestRowsBy(await all(env.DAILY_DB, `SELECT schedule_spot_key, batch_id, official_date, game_pk, team_id, opponent_team_id, is_home, schedule_spot_status, schedule_spot_confidence, days_rest, games_last_1_day, games_last_2_days, games_last_3_days, games_last_5_days, played_yesterday_flag, back_to_back_flag, three_in_four_flag, four_in_six_flag, doubleheader_today_flag, travel_required_flag, travel_distance_bucket, timezone_transition_flag, schedule_fatigue_score, schedule_risk_level, details_json, updated_at FROM daily_team_schedule_spot_current WHERE official_date IN (${inClause})`, ...dates), r => key(r.game_pk, r.team_id));
  ctx.umpire = latestRowsBy(await all(env.DAILY_DB, `SELECT umpire_context_key, batch_id, official_date, game_pk, umpire_context_status, umpire_context_confidence, source_status, home_plate_umpire_id, home_plate_umpire_name, umpire_assignment_status, assignment_confirmed_flag, assignment_pending_flag, assignment_missing_flag, umpire_tendency_status, strike_zone_context_status, run_environment_context_status, walk_context_status, strikeout_context_status, details_json, updated_at FROM daily_umpire_context_current WHERE official_date IN (${inClause})`, ...dates), r => key(r.game_pk));
  ...
  for (const r of await all(env.DAILY_DB, `SELECT catcher_context_key, game_pk, official_date, team_id, player_id AS catcher_player_id, player_name AS catcher_name, framing_runs_total AS framing_runs, framing_pct_total AS strike_rate_shadow_zone, pop_time_2b_sba AS pop_time_seconds, NULL AS arm_strength, metrics_available_flag, updated_at FROM daily_catcher_context_current WHERE official_date IN (${inClause})`, ...dates)) {
