...
async function getPreparedRows(pgClient, dates) {
  ...
  return pgClient`SELECT prepared_row_id, prep_batch_id, source_key, source_row_id, source_event_id, projection_id,
      ...
      official_game_time_utc, official_date::text AS official_date, matchup_status, pickable_safe, prep_status, block_reason
    FROM score.board_prepared_current
...
async function runFactorMining(request, env, pgClient) {
  ...
  try {
    ...
    const relevantPlayerIds = [...new Set(prepared.map(r => r.resolved_mlb_player_id || r.resolved_player_id).filter(Boolean).map(String))];
    const ctx = await loadContext(pgClient, dates, family, relevantPlayerIds);
