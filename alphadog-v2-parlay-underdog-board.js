...
const VERSION = "alphadog-v2-parlay-underdog-board-v0.2.2-control-db-only-binding";
...
const DEFAULT_PARLAY_UNDERDOG_PROBE_ENDPOINT = "/sports/baseball_mlb/props?bookmakers=underdog&limit=10000&dfsOdds=effective";

// Confirmed live via ParlayAPI's real bookmaker registry (https://parlay-api.com/v1/bookmakers):
// {"key":"underdog","title":"Underdog","region":"us","status":"active","endpoints":["props"]}
// Same API, same auth, same response shape as the Sleeper worker - only the bookmakers= filter differs.

// Market key mapping carried over from the Sleeper worker as a starting point, since ParlayAPI
// normalizes market_key naming across bookmakers on the same endpoint family. NOT yet verified
// against real live Underdog data - the first real /run probe should be checked for any
...
const CONFIG_KEYS = [
  "PARLAY_API_BASE_URL",
  "PARLAY_UNDERDOG_PROBE_ENDPOINT",
  "PARLAY_API_UNDERDOG_ENDPOINT",
  "PARLAY_API_AUTH_HEADER_NAME",
  "PARLAY_API_AUTH_HEADER_PREFIX"
];

const EXPECTED_UNDERDOG_STAGE_COLUMNS = ["stage_id", "batch_id", "source_key", "slate_date", "fetched_at", "staged_at", "source_event_id", "source_line_id", "source_player_id", "player_name", "team", "opponent", "league", "sport", "source_stat_name", "canonical_prop_key", "line_value", "side", "price", "decimal_price", "is_pickable", "start_time", "raw_line_json", "parse_status", "parse_error", "certification_status", "created_at"];
const EXPECTED_UNDERDOG_BATCH_COLUMNS = ["batch_id", "source_key", "slate_date", "fetched_at", "staged_at", "certified_at", "source_base_url", "source_endpoint", "source_http_status", "source_size_bytes", "top_level_shape", "total_rows", "staged_rows", "valid_rows", "invalid_rows", "unmapped_stat_types", "certification_status", "certification_reason", "certification_json", "promoted_at", "cleaned_at", "created_at", "updated_at"];
const EXPECTED_UNDERDOG_CURRENT_COLUMNS = ["current_row_id", "batch_id", "source_key", "slate_date", "source_event_id", "source_line_id", "source_player_id", "player_name", "team", "opponent", "league", "sport", "source_stat_name", "canonical_prop_key", "line_value", "side", "price", "decimal_price", "is_pickable", "start_time", "raw_line_json", "row_payload_json", "promoted_at", "updated_at"];
const EXPECTED_UNDERDOG_ACTIVE_COLUMNS = ["source_key", "slate_date", "active_batch_id", "certification_status", "row_count", "valid_rows", "activated_at", "updated_at"];
...
CREATE INDEX IF NOT EXISTS idx_underdog_board_stage_batch ON market.underdog_board_stage(batch_id);
CREATE INDEX IF NOT EXISTS idx_underdog_board_stage_source_slate ON market.underdog_board_stage(source_key, slate_date);
CREATE INDEX IF NOT EXISTS idx_underdog_board_stage_stat ON market.underdog_board_stage(source_stat_name, canonical_prop_key);
CREATE INDEX IF NOT EXISTS idx_underdog_board_batches_source_slate ON market.underdog_board_batches(source_key, slate_date);
CREATE INDEX IF NOT EXISTS idx_underdog_board_batches_cert ON market.underdog_board_batches(certification_status);
CREATE INDEX IF NOT EXISTS idx_underdog_board_current_source_slate_batch ON market.underdog_board_current(source_key, slate_date, batch_id);
CREATE INDEX IF NOT EXISTS idx_underdog_board_current_player_prop ON market.underdog_board_current(player_name, canonical_prop_key);`);
    await client.unsafe(
      "INSERT INTO market.schema_migrations (migration_key, package_version, applied_at, notes) VALUES ('schema_market_db_underdog_board_v0_1_0', $1, now(), 'Additive Underdog board lifecycle tables only. No Sleeper/PrizePicks table changes.') ON CONFLICT (migration_key) DO UPDATE SET package_version=excluded.package_version, applied_at=now()",
      [VERSION]
    );
  } finally {
  ...
  const validation = await validateUnderdogSchema(env);
...
async function validateUnderdogSchema(env) {
  const stage = await validateTable(env, "underdog_board_stage", EXPECTED_UNDERDOG_STAGE_COLUMNS);
  const batches = await validateTable(env, "underdog_board_batches", EXPECTED_UNDERDOG_BATCH_COLUMNS);
  const current = await validateTable(env, "underdog_board_current", EXPECTED_UNDERDOG_CURRENT_COLUMNS);
  const active = await validateTable(env, "underdog_board_active_batches", EXPECTED_UNDERDOG_ACTIVE_COLUMNS);
  return { ok: stage.ok && batches.ok && current.ok && active.ok, underdog_board_stage: stage, underdog_board_batches: batches, underdog_board_current: current, underdog_board_active_batches: active };
}
...
function configuredEndpoint(env, input = {}) {
  const rawEndpoint = String(
    ...
    env.PARLAY_UNDERDOG_PROBE_ENDPOINT ||
    env.PARLAY_API_UNDERDOG_ENDPOINT ||
    DEFAULT_PARLAY_UNDERDOG_PROBE_ENDPOINT ||
    ""
  ).trim();
  ...
  if (!rawEndpoint) return { ok: false, reason: "PARLAY_UNDERDOG_PROBE_ENDPOINT_missing", base_url_present: true, endpoint_present: false, base_url_host: safeHost(baseUrl) };
...
async function ensureRfiNrfiUnderdogBoardInventorySupport(env) {
  ...
  try {
    ...
    const hasUnderdog = parts.some(s => s.toLowerCase() === "underdog");
    const nextSources = hasUnderdog ? current : Array.from(new Set([...parts, "Underdog"])).join(",");
    if (!hasUnderdog) {
...
function taxonomySupportsUnderdog(taxonomyRow) {
...
function auditCanonicalMapping(sourceStatName, taxonomy, isPitcher) {
  ...
  if (!taxonomySupportsUnderdog(taxonomyRow)) {
    return { ok: false, canonical_prop_key: canonical, status: "canonical_not_underdog_supported", reason: "CONFIG_DB_taxonomy_does_not_support_Underdog_for_this_prop_key" };
  }
...
function deriveSourceLineId(row) {
  ...
  return safeString(parts.join("|"), 500) || rid("underdog_source_line");
}
...
function toStageRow(row, batchId, fetchedAt, taxonomy, positionByPlayerTeam) {
  const required = rowRequiredAudit(row);
  const sourceStatName = normalizeText(row && row.market_key) || normalizeText(row && row.market);
  const posKey = `${normalizeAliasName(row && row.player)}|${normalizeAliasName(row && row.team)}`;
  const isPitcher = positionByPlayerTeam ? !!positionByPlayerTeam.get(posKey) : false;
...
function rowPayloadForCurrent(row) {
  ...
  return JSON.stringify({
    ...
    rfi_nrfi_note: isRfiNrfi ? "Real Underdog board inventory promoted only; RFI/NRFI logic/mining/scoring remains blocked pending future design." : null,
...
async function promoteBoardInventory(env, batchId, stageRows, fetchedAt) {
  ...
  const currentRows = promotableRows.map(row => ({
    current_row_id: rid("underdog_current"),
    batch_id: batchId,
  ...
  try {
    await client.begin(async (tx) => {
      await tx.unsafe("DELETE FROM market.underdog_board_current WHERE source_key=$1", [SOURCE_KEY]);
      await tx.unsafe("DELETE FROM market.underdog_board_active_batches WHERE source_key=$1", [SOURCE_KEY]);

      const chunkSize = 200;
      for (let i = 0; i < currentRows.length; i += chunkSize) {
        const chunk = currentRows.slice(i, i + chunkSize);
        await tx`INSERT INTO market.underdog_board_current ${tx(chunk, ...currentCols)}`;
      }

      if (expectedCurrentRows > 0) {
        await tx.unsafe(
          "INSERT INTO market.underdog_board_active_batches (source_key, slate_date, active_batch_id, certification_status, row_count, valid_rows, activated_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, now(), now()) ON CONFLICT (source_key, slate_date) DO UPDATE SET active_batch_id=excluded.active_batch_id, certification_status=excluded.certification_status, row_count=excluded.row_count, valid_rows=excluded.valid_rows, activated_at=now(), updated_at=now()",
          [SOURCE_KEY, activeSlateDate, batchId, "PROMOTED_BOARD_INVENTORY_ONLY_NO_SCORING", expectedCurrentRows, expectedCurrentRows]
        );
      }

      await tx.unsafe(
        "UPDATE market.underdog_board_batches SET promoted_at=now(), cleaned_at=NULL, certification_status='PROMOTED_BOARD_INVENTORY_ONLY_NO_SCORING', certification_reason='Promoted valid source-proven Underdog rows as board inventory only. Scoring/ranking/final-board logic remains disabled; rfi_nrfi rows are inventory-only and logic-blocked pending future design.', updated_at=now() WHERE batch_id=$1 AND source_key=$2",
        [batchId, SOURCE_KEY]
      );

      await tx.unsafe("DELETE FROM market.underdog_board_stage WHERE batch_id=$1 AND source_key=$2", [batchId, SOURCE_KEY]);
      await tx.unsafe("DELETE FROM market.underdog_board_batches WHERE source_key=$1 AND batch_id<>$2", [SOURCE_KEY, batchId]);
      await tx.unsafe("UPDATE market.underdog_board_batches SET cleaned_at=now(), updated_at=now() WHERE batch_id=$1 AND source_key=$2", [batchId, SOURCE_KEY]);
    });

    const finalRows = await client.unsafe(
      "SELECT (SELECT COUNT(*) FROM market.underdog_board_stage WHERE source_key=$1) AS stage_rows_for_source, (SELECT COUNT(*) FROM market.underdog_board_stage WHERE batch_id=$2 AND source_key=$1) AS stage_rows_for_batch, (SELECT COUNT(*) FROM market.underdog_board_batches WHERE source_key=$1) AS batch_rows_for_source, (SELECT COUNT(*) FROM market.underdog_board_current WHERE batch_id=$2 AND source_key=$1) AS current_rows_for_batch, (SELECT COUNT(*) FROM market.underdog_board_active_batches WHERE source_key=$1 AND active_batch_id=$2) AS active_batch_rows, (SELECT COUNT(*) FROM market.underdog_board_current WHERE source_key=$1 AND batch_id<>$2) AS other_current_rows",
      [SOURCE_KEY, batchId]
    );
    ...
    if (!parityOk) {
      ...
      return {
        ...
        certification: "PARLAY_UNDERDOG_PROMOTION_PARITY_FAILED",
        reason: "Promotion parity failed after single-board replacement cleanup; refusing to certify an empty, duplicate-active, or contaminated Underdog board.",
        final_parity: parity,
        no_scoring: true,
        no_ranking: true,
        no_final_board: true,
...
async function stageOnlyRows(env, rows, sourceMeta, shape) {
  ...
  const taxonomyPatch = await ensureRfiNrfiUnderdogBoardInventorySupport(env);
  ...
  const unsupportedRows = stageRows.filter(row => String(row.parse_status || "").includes("canonical_not_underdog_supported")).length;
  ...
  const aliasWrite = await replaceParlayUnderdogPropAliases(env, distinctSourceToCanonical);
  ...
  return {
    ...
    underdog_baseball_rows: filtered.length,
...
async function safeProbe(env, input = {}) {
  ...
  const effective_config_defaults_used = {
    ...
    PARLAY_UNDERDOG_PROBE_ENDPOINT: !present(env, "PARLAY_UNDERDOG_PROBE_ENDPOINT") && !present(env, "PARLAY_API_UNDERDOG_ENDPOINT"),
  ...
  if (!schema.ok) {
    return blockedProbe("SCHEMA_NOT_READY", "underdog_schema_missing_or_invalid", readiness, endpoint, auth);
  }
  if (!endpoint.ok) {
  ...
  const headers = new Headers({ "accept": "application/json", "user-agent": "AlphaDog-v2-Parlay-Underdog-BoardInventory/0.1.0" });
  ...
  try {
    ...
    return {
      ...
      certification: "PARLAY_UNDERDOG_SOURCE_PROBE_FETCH_EXCEPTION",
  ...
  return {
    ...
    job_key: JOB_KEY,
    source_key: SOURCE_KEY,
    status: promotionFailed ? "promotion_parity_failed_no_certification" : (stagedOk ? (stageResult.promotion && stageResult.promotion.ok ? "promoted_underdog_board_inventory_only_no_scoring" : "stage_certify_alias_audit_completed_no_promotion") : (response.ok && parsed ? "source_probe_completed_shape_captured_stage_blocked" : "source_probe_completed_shape_unverified")),
    certification: promotionFailed ? "PARLAY_UNDERDOG_PROMOTION_PARITY_FAILED" : (stagedOk ? (stageResult.promotion && stageResult.promotion.ok ? "PARLAY_UNDERDOG_BOARD_INVENTORY_PROMOTED_NO_SCORING" : (stageResult.mapping_blocked_rows === 0 && stageResult.invalid_rows === 0 ? "PARLAY_UNDERDOG_STAGE_CERTIFIED_READY_NO_PROMOTION" : "PARLAY_UNDERDOG_STAGE_ALIASES_WRITTEN_MAPPING_BLOCKED_NO_PROMOTION")) : (response.ok && parsed ? "PARLAY_UNDERDOG_SOURCE_PROBE_SHAPE_CAPTURED_STAGE_BLOCKED" : "PARLAY_UNDERDOG_SOURCE_PROBE_SHAPE_UNVERIFIED_NO_PROMOTION")),
...
function blockedProbe(certSuffix, reason, readiness, endpoint, auth) {
  return {
    ...
    certification: `PARLAY_UNDERDOG_${certSuffix}`,
    ...
    no_prizepicks_mutation: true,
    no_promotion: true,
    next_required_proof: "Configure an explicit Underdog/Parlay probe endpoint and explicit auth header config, then rerun source probe."
  };
}
...
function baseIdentity(env, extra = {}) {
  ...
  return {
    ...
    phase: "parlay_underdog_board_inventory_promotion_v0_1_0",
    notes: [
      "Underdog board inventory promotion worker with safe public endpoint/header fallbacks and full-body JSON parsing before bounded preview slicing.",
      "Creates/validates additive Underdog lifecycle schema when /run executes.",
      ...
      "Applies the smallest rfi_nrfi taxonomy adjustment for Underdog board inventory support only while forcing scoring_enabled=0.",
      "Promotes valid source-proven Underdog rows to underdog_board_current as board inventory only. No scoring, ranking, final board, recommendations, RFI/NRFI logic, PrizePicks/Sleeper mutation, or old production touch.",
      "Direct adaptation of the Sleeper worker (alphadog-v2-parlay-sleeper-board.js) - same ParlayAPI, same auth, same response shape, only the bookmakers= filter and dedicated tables differ. Confirmed live via ParlayAPI's real bookmaker registry that Underdog is a supported operator on this API."
    ],
    binding_summary: {
      required_db_bindings_present: allTrue(db),
      ...
      effective_default_config_available: {
        ...
        PARLAY_UNDERDOG_PROBE_ENDPOINT: true,
