...
const VERSION = "alphadog-v2-parlay-underdog-board-v0.2.1-postgres-rewire-redeploy";
...
async function ensureUnderdogSchema(env) {
  ...
  try {
    ...
    await client.unsafe("CREATE INDEX IF NOT EXISTS idx_underdog_board_stage_batch ON market.underdog_board_stage(batch_id)");
    await client.unsafe("CREATE INDEX IF NOT EXISTS idx_underdog_board_stage_source_slate ON market.underdog_board_stage(source_key, slate_date)");
    await client.unsafe("CREATE INDEX IF NOT EXISTS idx_underdog_board_stage_stat ON market.underdog_board_stage(source_stat_name, canonical_prop_key)");
    await client.unsafe("CREATE INDEX IF NOT EXISTS idx_underdog_board_batches_source_slate ON market.underdog_board_batches(source_key, slate_date)");
    await client.unsafe("CREATE INDEX IF NOT EXISTS idx_underdog_board_batches_cert ON market.underdog_board_batches(certification_status)");
    await client.unsafe("CREATE INDEX IF NOT EXISTS idx_underdog_board_current_source_slate_batch ON market.underdog_board_current(source_key, slate_date, batch_id)");
    await client.unsafe("CREATE INDEX IF NOT EXISTS idx_underdog_board_current_player_prop ON market.underdog_board_current(player_name, canonical_prop_key)");
    await client.unsafe(
