...
async function ensureUnderdogSchema(env) {
  ...
  await env.MARKET_DB.batch([
    env.MARKET_DB.prepare(`CREATE TABLE IF NOT EXISTS underdog_board_stage (
    ...
    env.MARKET_DB.prepare(`CREATE TABLE IF NOT EXISTS underdog_board_batches (
    ...
    env.MARKET_DB.prepare(`CREATE TABLE IF NOT EXISTS underdog_board_current (
    ...
    env.MARKET_DB.prepare(`CREATE TABLE IF NOT EXISTS underdog_board_active_batches (
      source_key TEXT,
      slate_date TEXT,
    ...
    env.MARKET_DB.prepare("CREATE INDEX IF NOT EXISTS idx_underdog_board_stage_batch ON underdog_board_stage(batch_id)"),
    env.MARKET_DB.prepare("CREATE INDEX IF NOT EXISTS idx_underdog_board_stage_source_slate ON underdog_board_stage(source_key, slate_date)"),
    env.MARKET_DB.prepare("CREATE INDEX IF NOT EXISTS idx_underdog_board_stage_stat ON underdog_board_stage(source_stat_name, canonical_prop_key)"),
    env.MARKET_DB.prepare("CREATE INDEX IF NOT EXISTS idx_underdog_board_batches_source_slate ON underdog_board_batches(source_key, slate_date)"),
    env.MARKET_DB.prepare("CREATE INDEX IF NOT EXISTS idx_underdog_board_batches_cert ON underdog_board_batches(certification_status)"),
    env.MARKET_DB.prepare("CREATE INDEX IF NOT EXISTS idx_underdog_board_current_source_slate_batch ON underdog_board_current(source_key, slate_date, batch_id)"),
    env.MARKET_DB.prepare("CREATE INDEX IF NOT EXISTS idx_underdog_board_current_player_prop ON underdog_board_current(player_name, canonical_prop_key)"),
    env.MARKET_DB.prepare("INSERT OR REPLACE INTO market_schema_migrations (migration_key, package_version, applied_at, notes) VALUES ('schema_market_db_underdog_board_v0_1_0', ?, CURRENT_TIMESTAMP, 'Additive Underdog board lifecycle tables only. No Sleeper/PrizePicks table changes.')").bind(VERSION)
  ]);
