...
async function promoteBoardInventory(env, batchId, stageRows, fetchedAt) {
  ...
  // Single-board replacement rule (same as Sleeper worker):
  // A certified Underdog refresh replaces the entire parlay_underdog board state.
  ...
  await run(env.MARKET_DB, "DELETE FROM underdog_board_current WHERE source_key=?", SOURCE_KEY);
  ...
  await runWithBoundedConcurrency(currentChunks, chunk => env.MARKET_DB.batch(chunk.map(row => env.MARKET_DB.prepare(`INSERT INTO underdog_board_current (
  ...
  await run(env.MARKET_DB, `UPDATE underdog_board_batches
    SET promoted_at=CURRENT_TIMESTAMP,
        cleaned_at=NULL,
        certification_status='PROMOTED_BOARD_INVENTORY_ONLY_NO_SCORING',
        certification_reason='Promoted valid source-proven Underdog rows as board inventory only. Scoring/ranking/final-board logic remains disabled; rfi_nrfi rows are inventory-only and logic-blocked pending future design.',
        updated_at=CURRENT_TIMESTAMP
    WHERE batch_id=? AND source_key=?`, batchId, SOURCE_KEY);

  await run(env.MARKET_DB, "DELETE FROM underdog_board_stage WHERE batch_id=? AND source_key=?", batchId, SOURCE_KEY);
  await run(env.MARKET_DB, "DELETE FROM underdog_board_batches WHERE source_key=? AND batch_id<>?", SOURCE_KEY, batchId);
  await run(env.MARKET_DB, "UPDATE underdog_board_batches SET cleaned_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP WHERE batch_id=? AND source_key=?", batchId, SOURCE_KEY);

  const finalRows = await all(env.MARKET_DB, `
    SELECT
      (SELECT COUNT(*) FROM underdog_board_stage WHERE source_key=?) AS stage_rows_for_source,
      (SELECT COUNT(*) FROM underdog_board_stage WHERE batch_id=? AND source_key=?) AS stage_rows_for_batch,
      (SELECT COUNT(*) FROM underdog_board_batches WHERE source_key=?) AS batch_rows_for_source,
      (SELECT COUNT(*) FROM underdog_board_current WHERE batch_id=? AND source_key=?) AS current_rows_for_batch,
      (SELECT COUNT(*) FROM underdog_board_active_batches WHERE source_key=? AND active_batch_id=?) AS active_batch_rows,
      (SELECT COUNT(*) FROM underdog_board_current WHERE source_key=? AND batch_id<>?) AS other_current_rows
  ...
  if (!parityOk) {
    await run(env.MARKET_DB, `UPDATE underdog_board_batches
      SET certification_status='PARLAY_UNDERDOG_PROMOTION_PARITY_FAILED',
