...
### Remaining score-audit.js legacy handlers not yet retired (next candidates, per Section 7.9's table)
Still live and dormant, not yet touched: `hit-probability` (just confirmed still-functional and now has a stray-but-cancelled test row — safe to retire same as above), `final-score-v1`, `final-board-v2`, `score-enrichment-v1`, `score-enrichment-v2-shadow`, `hit-probability-v3-shadow`, `final-score-v2-shadow`, `final-board-v3-shadow`, and the legacy `score-final-board` ("Legacy Final" button, distinct from the new dedicated Scoring Full Run's final board stage).
