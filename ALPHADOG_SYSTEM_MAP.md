...
### Net result: the legacy Market+Scoring Full Run chain is now completely removed from the orchestrator, not just unreachable
Combined with Change #9, this removes roughly **1,400 lines total** (Change #9: ~230, Change #10: ~1,170) from `alphadog-v2-orchestrator.js` — the file dropped from 20,027 lines / 1,409,408 bytes to approximately 18,650 lines / ~1,386,846 bytes after Change #9, and further after Change #10. This is the first and only part of the orchestrator's dead code physically removed so far. The `isScoringEngineJob`/`isExpansionBaselineJob`-style guard functions and other dead surface area identified earlier in the map (Sections 7.9, 11) have not yet been touched — same method applies to those next.
