...
### Updated status: score-audit.js legacy generation cleanup
**6 of 7 planned legacy handlers now retired** (scoring-engine, scoring-engine-simulation, final-score-v1, final-board-v2, score-enrichment-v1, hit-probability) — all confirmed dormant since 2026-06-06 through 2026-06-30, all verified via `run_job` returning the clean retirement response post-deploy. **Only `score_final_board`'s "Legacy Final" button remains, intentionally left alone pending Rodolfo's decision** (see above — it's a different category of thing, not simply dormant). The 3 table-driven "-shadow" handlers (`score_enrichment_v2_shadow`, `hit_probability_v3_shadow`, `final_score_v2_shadow`, `final_board_v3_shadow` — via the `shadowScoringJobs` object structure, a different code pattern than the individual `if` blocks used above) have not yet been located/retired — next candidate for a future pass, using a different technique since they're table-driven rather than individual if-blocks.
