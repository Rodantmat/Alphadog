...
### What this proves for the remaining ~1,038 lines still in the dead block (1284–2320, everything before the function just removed)
The method works on this file: capture exact text via grep with generous context windows, use it as a precise `old_str`/`new_str` pair, verify via real deploy + a direct orchestrator tick. The remaining constants, guard functions, `MARKET_SCORING_FULL_RUN_STAGES` array, `marketScoringFullRunChildInput`, lock helpers, and the proof/reconciliation helper functions (`synthesizeMarketPlayerPropTerminalProofFromEvidence`, `synthesizeScoringEngineCurrentTerminalProofFromEvidence`, `synthesizeHitProbabilityTerminalProofFromEvidence`, `rescueHitProbabilityCurrentReadyForHpBoardChild`, `rescueMarketScoringFullRunTerminalEvidenceChild`, `reconcileMarketScoringFullRunChildFromProof`, `isMarketScoringFullRunPartialChildOutput`, `childPassedMarketScoringFullRun`) are the next candidates for the same treatment — all confirmed part of the same dead chain, not yet removed.
