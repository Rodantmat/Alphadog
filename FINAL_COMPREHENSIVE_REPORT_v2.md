...
- The corrected baseline logic was deployed to production on 2026-09-02, then rolled back at explicit request. **The complete, validated fix is preserved and ready for future redeployment — see the version ledger in Section 7.5 for the exact commit SHA and one required addition before that redeployment.** The live file currently runs the original, pre-fix logic.
