...
| **State** | **CLEAN 0/3 · 9 passes · ALL FOUR STRATA READ.** 1,081 segments (largest yet), 944 uncovered vs twelve (87.3%). Tail at `scratchpad/t7/t7_tail.json`. **Pass 9 (cross-document consistency) found a 🔴 defect and reset the count from 2/3** — §T7.30a claimed a "single global 25% cap" from a quote that is **MLB's rule**, in a document whose next line says **NBA does not have it**. Corrected; live caps are **35 cells, 15 factors, 22 tier-keyed, 13 not** (§T7.38). |
| **Stratum** | owner ✅ (7) · reasoning ✅ (478) · commands ✅ (233) · results ✅ (233). |
| **Exact next step** | **T7 needs 3 consecutive clean passes from here.** Angles used: judgment (6, defects), mid-band seam (7, clean), wiring (8, clean), cross-document consistency (9, **defect**). **Unused: structural value sanity, live numeric re-verification, referential integrity.** Run **structural value sanity** as pass 10, then **live numeric re-verification** as 11, then a **second two-direction judgment** as 12 (the judgment pass must be among the three). Then close T7 → **T8** (`2026-09-09-20-48-33-nba-classification-baseline-backtest-calibration.txt`). ⚠ T7 built few tables, so referential integrity yields less here than on T4–T6. |
| **⚠ Lesson from pass 9** | The cheapest defects left in this sweep are **in the sweep's own citations**, not in the transcripts. Pass 9's angle — *re-read every document this sweep quoted, at the quote* — found one in its first hour after two clean passes had suggested T7 was done. **Run it on every transcript before closing.** |
