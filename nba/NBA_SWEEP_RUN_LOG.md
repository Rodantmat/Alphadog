...
| **State** | **CLEAN 0/3 · 16 passes · ALL FOUR STRATA READ.** Pass 16 (**composite-key sweep**, the new angle) found 🔴 **two broken joins pass 12 never ran** — `variation_band='continuous'` resolves to nothing, and `calibration_log` joins `factor_profile_cells` at **0%, 8 of 8**, with all values NULL under `status='applied'`. Coverage: uncovered vs twelve **944 → 932**. |
