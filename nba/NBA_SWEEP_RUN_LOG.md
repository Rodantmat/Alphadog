...
| **Exact next step for T8** | **Pass 12 = a fourth two-direction judgment** (`scratchpad/t8/judge8.py`) — **required among the closing three**; the band has been 61 for three runs, so judge the newest entries' claims under rules 12 and 13 rather than the extraction. Then **pass 13 = self-contradiction audit** over §T8.30–§T8.33, then **pass 14 = live numeric re-verification**. **3 consecutive clean at 3 different angles closes it.** ⚠ **Two habits from passes 9 and 11, apply them everywhere**: (a) when a figure comes from a `GROUP BY`, **write the prose from the same grouping** or say which grouping it uses — otherwise it mis-assigns members; (b) **before reusing a distinctive phrase, grep it** and see what it already denotes in these documents. |
