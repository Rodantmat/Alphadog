...
| **Exact next step** | **T7 needs 3 consecutive clean passes from here.** Angles used: judgment (6, defects), mid-band seam (7, clean), wiring (8, clean), cross-document consistency (9, **defect**), structural value sanity (10, **2 defects**), live numeric re-verification (11, **1 new live finding + 2 document defects**), referential integrity (12, **✅ clean — 1/3**). **Next: a second two-direction judgment as pass 13** (`sed` the transcript path in `scratchpad/t7/judge7.py`) — **the judgment pass must be among the closing three** — then **cross-document consistency again** as pass 14, re-run over everything §T7.38–§T7.41 added. If both are clean, T7 closes at 3/3. Then close T7 → **T8** (`2026-09-09-20-48-33-nba-classification-baseline-backtest-calibration.txt`). |
