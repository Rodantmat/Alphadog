...
| **Exact next step** | **T7 needs 3 consecutive clean passes from here.** Angles used: judgment (6, defects), mid-band seam (7, clean), wiring (8, clean), cross-document consistency (9, **defect**), structural value sanity (10, **2 defects**), live numeric re-verification (11, **1 new live finding + 2 document defects**), referential integrity (12, **✅ clean**), two-direction judgment 2nd run (13, **🔴 defect — reset to 0/3**), **novelty audit (14, ✅ clean — 1/3)**. **Next: pass 15 = cross-document consistency, run over everything §T7.38–§T7.43 added** (each new claim re-read at the document it cites, the §T7.38a failure mode). **Then pass 16 = a third two-direction judgment — required among the closing three.** If both are clean, T7 closes at 3/3 → **T8** (`2026-09-09-20-48-33-nba-classification-baseline-backtest-calibration.txt`). |
