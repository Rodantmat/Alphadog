...
| | |
|---|---|
| **Current transcript** | ▶ **T8** — `2026-09-09-20-48-33-nba-classification-baseline-backtest-calibration.txt` · **CLEAN 0/3 · 11 passes · all strata read.** |
...
| **Exact next step for T8** | **Pass 10 = live numeric re-verification** of every figure §T8.25–§T8.30 states — 28 props · A 13 / B 15 · 6 banded / 22 unbanded (7 A + 15 B) · the 5-vs-1 band-family split · 1,218 `canonical_prop_key` refs · 0 under `nba/` — **each re-derived from its authority, and every partition summed** (rule 11). Then **pass 11 = novelty audit vs `/tmp/t8base/nba/`** for §T8.27–§T8.31, and **pass 12 = a fourth two-direction judgment** (required among the closing three). **3 consecutive clean at 3 different angles closes it.** ⚠ **New habit from pass 9, apply it everywhere**: when a figure comes from a `GROUP BY`, write the prose **from the same grouping** — if the sentence groups by anything else (kind, family, "the obvious categories"), say so explicitly or it will mis-assign members. |
