...
| **Exact next step for T8** | **Pass 3 = the two-direction judgment** — `sed` the transcript path in `scratchpad/t7/judge7.py` into `scratchpad/t8/judge8.py` and run it. **Then**: live numeric re-verification · novelty audit vs **`/tmp/t8base/nba/`** · referential-integrity / composite-key · self-contradiction audit. **3 consecutive clean at 3 different angles closes it, judgment among them.** ⚠ **Apply the eleven rules before writing any severity marker** — T8's own pass 1 is the proof they work: all three owner directives turned out to be already recorded, which is exactly where T7 lost thirteen passes. |
