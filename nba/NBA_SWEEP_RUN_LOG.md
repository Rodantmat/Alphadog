...
| **Exact next step for T9** | **Pass 11 = a third two-direction judgment** (`scratchpad/t9/judge9.py`) — **required among the closing three**; judge §T9.16–§T9.25 under rules 12/13/14 rather than re-running the extraction. Then **pass 12 = a novelty audit vs `/tmp/t9base/nba/`** covering everything passes 7–11 added, then **pass 13 = referential/structural re-verification** at whatever angle passes 11–12 leave untested. **3 consecutive clean at 3 different angles closes T9** → then **T10** (next in `/home/claude/nbadoc/transcripts/`). ⚠ **Pass 10's lesson, apply on every correction from here**: after fixing a claim, `grep` the **ledger row**, the **closing summary** and the **run log** for the same figure — the correction stops there otherwise. |
