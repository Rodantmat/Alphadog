...
| **Exact next step for T8** | **Pass 7 = referential integrity / composite key** on the T8-seeded layer — `prop_taxonomy.canonical_prop_key` ↔ `factor_relevance` ↔ `variation_bands` ↔ `factor_profile_cells`, and the `apps` array against the three board scrapers' sports. **Enumerate each table's full column list and its design-stated key first** (rules 5 and 12). Then **pass 8 = self-contradiction audit** across §T8.22–§T8.28, then **pass 9 = a third two-direction judgment**. **3 consecutive clean at 3 different angles closes it, judgment among them.** |
