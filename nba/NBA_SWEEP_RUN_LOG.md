...
| **State** | **CLEAN 1/3 · 12 passes · ALL FOUR STRATA READ.** Pass 12 (referential integrity) **clean — 4 joins, 0 orphans**. Pass 11 re-verified **13 of 13** documented numbers exactly and found 🔴 **the 2026-27 schedule is uniformly 2 games short per team on all thirty** (1,200 vs 1,230). |
...
| **Exact next step** | **T7 needs 3 consecutive clean passes from here.** Angles used: judgment (6, defects), mid-band seam (7, clean), wiring (8, clean), cross-document consistency (9, **defect**), structural value sanity (10, **2 defects**), live numeric re-verification (11, **1 new live finding + 2 document defects**). **Unused: referential integrity.** Run **referential integrity** as pass 12 (do T7's config tables join what they claim — `factor_relevance` ↔ `factor_registry` ↔ `factor_profile_cells` on `factor_key`, and `role_tier_key` ↔ `role_tiers`), then a **second two-direction judgment** as 13, then **cross-document consistency again** as 14 — the judgment pass must be among the closing three. Then close T7 → **T8** (`2026-09-09-20-48-33-nba-classification-baseline-backtest-calibration.txt`). |
