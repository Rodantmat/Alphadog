...
| **Exact next step for T8** | **Pass 9 = a third two-direction judgment** (`scratchpad/t8/judge8.py`) — **required among the closing three**; the band has been 61 for two runs, so judge the entries' claims under rule 12 rather than the extraction. Then **pass 10 = live numeric re-verification** of the pass-7 figures (28 props · A 13 / B 15 · 6 banded · 22 unbanded · the 5-vs-1 band-family split), **each re-derived from `information_schema` and the tables themselves, not from the query that produced them** — and check the partition sums (rule 11: 6 + 22 = 28). Then **pass 11 = novelty audit vs `/tmp/t8base/nba/`** for §T8.27–§T8.30. **3 consecutive clean at 3 different angles closes it.** |
