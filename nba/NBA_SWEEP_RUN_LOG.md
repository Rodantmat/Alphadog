...
| **Exact next step for T8** | **Pass 5 = novelty audit vs `/tmp/t8base/nba/`** (snapshot `700a999b`) for everything §T8.22–§T8.25 added — **rule 8: grep the claim you are most confident about first.** Then **pass 6 = a second two-direction judgment** (`scratchpad/t8/judge8.py`, already pointed at T8), then **pass 7 = referential integrity / composite key** on the T8-seeded config layer. **3 consecutive clean at 3 different angles closes it, judgment among them.** |
