...
| **State** | **CLEAN 1/3 · 14 passes · ALL FOUR STRATA READ.** Pass 14 (**novelty audit**) ✅ clean — every claim passes 9–13 added was grepped against a `git archive` snapshot of the twelve from before pass 9; **no third duplicate**, and the two known ones were already downgraded. Coverage moved as the documents grew: uncovered vs twelve **944 → 932**. |
...
| **Exact next step** | **T7 needs 3 consecutive clean passes from here.** Angles used: judgment (6, defects), mid-band seam (7, clean), wiring (8, clean), cross-document consistency (9, **defect**), structural value sanity (10, **2 defects**), live numeric re-verification (11, **1 new live finding + 2 document defects**), referential integrity (12, **✅ clean**), two-direction judgment 2nd run (13, **🔴 defect — reset to 0/3**). **Next: pass 14 = a NOVELTY AUDIT of everything this sweep added to T7** — for each claim in §T7.38–§T7.42, grep the twelve for its key number or object name and confirm it was not already recorded elsewhere. **This is the check that has now failed twice in a row (§T7.39c, §T7.42a); run it deliberately as its own pass rather than trusting it inline.** Then pass 15 = structural value sanity or cross-document consistency, then pass 16 = a third two-direction judgment (**required among the closing three**). Then close T7 → **T8** (`2026-09-09-20-48-33-nba-classification-baseline-backtest-calibration.txt`). |
