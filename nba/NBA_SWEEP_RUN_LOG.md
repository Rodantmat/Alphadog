...
| **State** | **CLEAN 0/3 · 15 passes · ALL FOUR STRATA READ.** Pass 14 (novelty audit) was clean; **pass 15 (cross-document consistency) found the third instance of one habit** — the 13 cells I twice called unkeyed are keyed on `variation_band`, the fourth dimension of a **six-dimensional key the design document states in writing**. Reset to 0/3. Coverage: uncovered vs twelve **944 → 932**. |
...
| **Exact next step** | **T7 needs 3 consecutive clean passes from here.** Angles used: judgment (6, defects), mid-band seam (7, clean), wiring (8, clean), cross-document consistency (9, **defect**), structural value sanity (10, **2 defects**), live numeric re-verification (11, **1 new live finding + 2 document defects**), referential integrity (12, **✅ clean**), two-direction judgment 2nd run (13, **🔴 defect — reset to 0/3**), novelty audit (14, ✅ clean), cross-document consistency 2nd run (15, **🔴 defect — reset to 0/3**). **Next: pass 16 = a COMPOSITE-KEY / SCHEMA-COMPLETENESS sweep** — for every live object these T7 entries describe, enumerate **all** its columns and rows from `information_schema` and from the design document that specifies it, and re-check each claim against the full set. **This is the habit that has now failed three times (§T7.38a, §T7.39a, §T7.44a); run it as its own pass, like the novelty audit.** Then pass 17 = a third two-direction judgment (**required among the closing three**), then pass 18 = live numeric re-verification. Three consecutive clean ones close T7 → **T8** (`2026-09-09-20-48-33-nba-classification-baseline-backtest-calibration.txt`). |
