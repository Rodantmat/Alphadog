...
| **Exact next step for T8** | **⚠ FIRST, change the procedure, because the same failure has now happened in T7 and T8 for the same reason: run the novelty grep AT THE MOMENT OF WRITING, in the same tool call as drafting the claim — not in a later audit pass.** The audit pass stays as a backstop, but it is catching what should never have been written. **Then pass 6 = a second two-direction judgment** (`scratchpad/t8/judge8.py`, already pointed at T8), **pass 7 = referential integrity / composite key** on the T8-seeded config layer, **pass 8 = self-contradiction audit**. **3 consecutive clean at 3 different angles closes it, judgment among them.** |
