...
| **Exact next step for T9** | **Pass 6 = novelty audit vs `/tmp/t9base/nba/`** (snapshot `213800e7`) for everything §T9.16–§T9.20 added — **rule 8 (most confident first) and rule 14 (open every hit)**. Then **pass 7 = a second two-direction judgment** (required among the closing three), then **pass 8 = live numeric re-verification**. **3 consecutive clean at 3 different angles closes T9** → then **T10**. ⚠ **Rule 14 has now changed the output of three consecutive passes** (T9 passes 2, 4 and 5) — in each case a finding I was confident was new turned out to be on file, and opening the hit was what caught it. **It is the highest-yield check in the set; run it before writing, not after.** |
