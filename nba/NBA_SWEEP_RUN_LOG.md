...
| **Exact next step for T8** | **1.** `git fetch origin main && git reset --hard origin/main`. **2.** Point the tail script at T8: `sed` the transcript path in `scratchpad/t7/tail7.py`, run it, and read the four strata in order — **owner · reasoning · commands · results**. **3.** Then the angles, in this order, because T7 showed where the defects live: **two-direction judgment** (`judge7.py`, same `sed`) → **live numeric re-verification** → **novelty audit against a `git archive` snapshot taken BEFORE the first T8 write** (take it first, note the commit sha here) → **referential integrity / composite-key** → **self-contradiction audit**. **3 consecutive clean at 3 different angles closes it, judgment among them.** |
