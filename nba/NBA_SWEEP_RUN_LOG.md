...
| **Exact next step for T9** | **Pass 2 = the tail stratum** — read `scratchpad/t9/t9_tail.json` (703 segments) lowest-coverage first, **classifying by shape first** (T8 pass 2 found ~60% of its tail was mechanism (0): command output, patch bodies, tracebacks, web-page chrome). **Then the angles**: two-direction judgment (`t9/judge9.py`) · live numeric re-verification · novelty audit vs `/tmp/t9base/nba/` · referential integrity / composite key · self-contradiction audit · **dated-verdict audit**. **3 consecutive clean at 3 different angles closes it, judgment among them.** |
