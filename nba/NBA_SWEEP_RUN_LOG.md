...
| **Exact next step for T8** | **Pass 2 = the reasoning stratum.** `git fetch origin main && git reset --hard origin/main`, then read `scratchpad/t8/t8_tail.json` (542 segments, assistant-heavy) in order. **Then the angles**: two-direction judgment (`sed` the transcript path in `scratchpad/t7/judge7.py` → `t8/judge8.py`) · live numeric re-verification · novelty audit vs `/tmp/t8base/nba/` · referential-integrity / composite-key · self-contradiction audit. **3 consecutive clean at 3 different angles closes it, judgment among them.** |
