...
| | |
|---|---|
| **Current transcript** | ▶ **T8** — `2026-09-09-20-48-33-nba-classification-baseline-backtest-calibration.txt` · **CLEAN 1/3 · 13 passes · all strata read.** |
...
| **Exact next step for T8** | **Pass 13 = a DATED-VERDICT AUDIT — the angle passes 8 and 12 stumbled into twice, run deliberately.** Sweep the twelve for every *"correctly empty"*, *"correctly X"*, *"not yet"*, *"starts on the second run"* style verdict that was **true when written and carries a date or a condition**, and re-test each one live. **Two of these have now been retired in five passes, both from the same audit table, and both found by accident.** Then **pass 14 = a fifth two-direction judgment** (required among the closing three) and **pass 15 = live numeric re-verification**. **3 consecutive clean at 3 different angles closes it.** ⚠ **Three habits now, apply them everywhere**: (a) prose must use the **same grouping** as the `GROUP BY` behind its numbers; (b) **grep a distinctive phrase before reusing it**; (c) **when you supersede one row of a table, test every other row of that table before leaving it.** |
