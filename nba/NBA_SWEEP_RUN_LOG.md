...
| **Exact next step for T9** | **Pass 5 = referential integrity / composite key** across the production layer — `baseline_ladder` ↔ `baseline_ladder_runs` (do the 3 asof days, row counts and prop arrays agree?), `player_id` ↔ `nba_ref.players`, and the `prop` ↔ taxonomy join already known to fail on 4 values. **Enumerate each table's full column list first** (rule 5) and **sum every partition** (rule 11). Then **pass 6 = novelty audit vs `/tmp/t9base/nba/`** for §T9.16–§T9.20, then **pass 7 = a second two-direction judgment** (required among the closing three). **3 consecutive clean at 3 different angles closes it.** |
