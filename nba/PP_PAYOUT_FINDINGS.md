...
| **`T26-9` / `A2`** board liveness | Nothing could tell "the app posted nothing" from "our scraper broke": the certifier's only board gate was an aggregate count, and the per-app meta files were unread. | The P3 certifier reads each app's `boards/<app>_nba_current_meta.json`: a scraper that RAN and found nothing has a fresh stamp with `legs 0`; a BROKEN one has a stale stamp. PrizePicks stale/not-ok is RED; the others warn per app, so a known owner decision does not train anyone to ignore red. Tested on the real files: Underdog/Sleeper/Fliff alive with 0 NBA lines (the apps have not opened); **Betr 394 h stale, token expiring in 13 d — WARN every day until decided.** (PrizePicks' producer spells its stamp `finished_at`; handled.) |
| **`T26-10` / `A5`** factor-gate units | Six writers, three meanings per column; `ORDER BY gain_vs_anchor` crowned the N1 starter ablation with "gains" of 0.70–0.80 that are accuracies on n = 1,322. | `metric` column added and backfilled: 5 rows `accuracy (starter-prediction ablation, N1; NOT a log-loss gain)`, 104 rows `logloss_gain_vs_anchor`; column comments say units vary by row. |
| **`T26-11` / `A0`** period collision | The sweep feared `final_hp` period rungs colliding on the upsert key. | **False alarm, verified in the write path**: period rows carry `prop` = the LABEL (`points_q1`), base props are pinned to `period = 'FULL'`; `points_q1 5.5` and `points 5.5` are different keys. Made impossible to break later: an assertion on both invariants in `build_final_hp.py`. |
| **`T26-7`** baseball in the NBA archive | Underdog (2026-09-12) and Fliff (09-13) let MLB markets through their sport filter; the apps had NO NBA lines those days. | **Moved, not deleted**: 7,921 rows into `nba_market.board_snapshots_quarantine` (0 DFS rows left in September). The archiver now refuses rows in another sport's vocabulary, loudly, before the insert — tested: 6/6 MLB keys caught, 0 NBA keys (including `field_goals`) hit. |
| **`T26-6` / `A4`** period prune "inversion" | The day a period market appears on a board, that period's ladder becomes board-scoped. | **This is the owner's retention rule applied consistently** (never-derived props keep their raw material; once a board carries them they are board-scoped like every other prop). Left as is; owner to confirm if read differently. |
| §T23.5 residual | `build_defender_ratings.py` was the one unguarded index-creating script in any pipeline. | Guarded with `to_regclass` like the other seven. |
| **`F7-1`** | `sweep_coverage.py` writes credential-shaped text to `scored.json` in the working tree. | `.gitignore`: `scored.json`, `nba/tools/*.json`, `nba/transcripts/*.txt` (journal kept). |
