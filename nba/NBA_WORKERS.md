...
> 📏 **CENSUS STAMP — `2026-09-23`, `§F7.5`. THE SAME SWEEP RAN ON ALL TWELVE AND ONLY ONE OF THE THREE FIGURES SURVIVED IT.** ~~***`sections` is authoritative and was re-derived at this date.***~~ 🔴🔴 **RETRACTED `2026-09-23`, `§F7.14` — `sections` WENT STALE TOO: exact `4` of `11` across the twelve, NOT `12` of `12`.** *(This file's own count happens to still hold — **that is luck, not authority**.)* ***No figure below is authoritative.*** ⇒ 🔁 **`RULE 59` — RE-DERIVE, NEVER QUOTE:** `` grep -cE '^(> *)*#{1,6} ' nba/<FILE>.md `` *(the `` (> *)* `` prefix is mandatory — a `^#` detector is blind to blockquoted headings, `294` across the twelve, `6.0%`, `§F2.14`).* 🔑 **This file already carries the same principle for the workflow counts at `§0.0`/`§T22.29` — `RULE 59` is that rule generalised to the census.** *Measured against the live files the same day: **sections exact `12` of `12`** · **bytes exact `1` of `11`** · **lines exact `0` of `6`**. 🔑 **This file's byte figure is the honest one of the eleven: it was written as `~180,000`, an explicit approximation, and it lands `96` bytes high.** *A figure that admits its precision cannot mislead about it — **the tilde is the whole repair the other ten needed.*** ⚠ **The cause is structural: a census lives inside the index it measures, so the patch that publishes a figure invalidates it in the same commit** *(`§F6.19`)*. ⇒ **Re-derive sections with `^(?:>\s*)*#{1,6}\s` — never `^#`, which is blind to the `294` blockquoted headings across the twelve.**
