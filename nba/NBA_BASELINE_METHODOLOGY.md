...
1. **Base per-36-minute rate (EWMA)**: an Exponentially Weighted Moving Average of the player's per-36 rate for the target stat, computed from `nba_stats.player_game_log`. Recent games count more without a hard cutoff.
   - **Per-stat decay rates, not one alpha (revised 2026-09-08 after the baseline data-universe research pass)**: a single alpha wrongly assumes every skill is equally "sticky". They are not. DARKO — the most predictive public projection system — projects every box-score stat separately with its own decay rate, and the reasoning holds up: true 3PT% takes hundreds of attempts to stabilize (a 10-game streak is mostly noise → long memory, alpha ≈0.03), rebounding is a physical/role tendency that ~20 games indicate well (medium, ≈0.08), while usage/assist rate can change overnight with a trade or teammate injury (short, ≈0.15) and minutes — the single biggest error source in props, set by abrupt coaching decisions — shortest of all (≈0.20). **The values live in `nba_config.stat_decay_config`** (one row per stat: `ewma_alpha`, `min_lookback_games`, `shrinkage_stabilization_games`, `memory_class`, `rationale`), never hardcoded, and the pipeline must read them from there. The seeded values are informed starting points to be tuned against the walk-forward backtest, not final.
   - **Small-sample handling (Bayesian shrinkage)**: for players with few games this season (rookies, injury returners), blend the EWMA toward a prior from `nba_stats.player_career_season_totals` (age-adjusted) or a league/position average for true rookies. Blend weight scales with games played toward each stat's own `shrinkage_stabilization_games` from the config (3PT% needs far more games to trust than rebounding), capped at 1.0.
