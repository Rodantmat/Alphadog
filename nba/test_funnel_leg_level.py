...
def main():
    ...
    for name, lo, hi in (("B2", -0.05, 0.05), ("B3", -0.10, 0.10), ("BF", -0.10, 0.10)):
        ...
        globals()[name] = bv
        base_mae = best
    print(f"fitted efficiency betas: B2(def_fg on 2P%) {B2:+.4f}  B3(def_3p on 3P%) {B3:+.4f}  BF(def_foul on FTA) {BF:+.4f}", flush=True)
