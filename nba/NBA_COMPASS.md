...
48. BOARD SOURCES (evidence, config `board_sources_decision`): PrizePicks = our producer (ParlayAPI drops ~25% of rungs); Sleeper = our scraper (`scrape_sleeper_board.py`, parity + native multipliers); Underdog = our scraper v3 (`scrape_underdog_board.py`, 854 lines ≥ ParlayAPI 726 on every stat); Fliff = ParlayAPI. Outputs in repo `boards/<app>_<sport>_current.json`.
