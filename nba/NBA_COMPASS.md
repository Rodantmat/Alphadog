...
50. Fliff API (reverse-engineered from `sports.getfliff.com` bundles): POST `https://app.getfliff.com/api/v1/sports_book_public/` with `{header, message_name:"SPORTS_BOOK_REQUEST", operation:{code,config_version}, load_data_requests, load_data_requests_on_error}`; header needs app version `5.0.34` / cap `285`, device id, `usa_state_code`, `location_token`; no login needed (validation errors, not auth); feed sync = `FCM__Public_Feed_Sync__Request` codes 3055/3056/3062; data hosts `m-c1/m-c2/m-c701/702.app.getfliff.com`; live updates via PubNub. Not finished — ParlayAPI remains the Fliff source unless the owner captures one `sports_book_public` cURL.
