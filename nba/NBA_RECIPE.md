...
## STEPS 8+ — the journal's summary table *(superseded as a recipe by `STEP 8`–`STEP 13` below)*

> ### ✅ **WHAT THIS BLOCK IS NOW — updated `2026-09-23`**
>
> **The table below is a RECORD of what was done, not an instruction for doing it again.** *It is
> accurate; it is just a summary.*
>
> ✅ ***The gap it used to warn about is CLOSED.*** **`STEP 8`–`STEP 13` follow immediately and are
> real steps**, each written from the workflow file it describes: **`STEP 8` `P1` weekly-static** ·
> **`STEP 9` `P2` overnight-heavy** · **`STEP 10` `P3` afternoon-light** · **`STEP 11` the build order**
> · **`STEP 12` the game-day timeline and the four clocks** · **`STEP 13` the opening-night dry run.**
> ⇒ *The `1:15 PM PT` cutoff, the gating, the certifier and the build order are all in this document.*
> 📚 *What this block said before, and why: `NBA_SWEEP_RUN_LOG.md` → `§T20.86`, `§F7.20`.*
>
...
🔴 **WHAT BREAKS IT TODAY.** `defender_ratings` is stale since **`max(as_of_date) = 2026-04-09`** — **`169` days as of `2026-09-25`**, ⚠ **and this number GROWS BY ONE EVERY DAY, so re-derive it and never quote it** *(`RULE 59`; it was published as `166` and was wrong three days later — `§T26.25`)*: `` SELECT CURRENT_DATE - max(as_of_date) FROM nba_ref.defender_ratings; ``, so **the first P1 check is RED right now** — see the frozen-static-layer item and `§T20.51`. ⚠ **And `§T20.31`: `nba_control.job_runs` and `worker_run_log` are both EMPTY — the detector exists; nothing runs it.**
...
| **4** | 🔑🔑 **The baseline ladder** *(P2)* | stages 2–3 | `nba_score.baseline_history` — ▶ **2024-25 `162` dates / `4,233,434` rows · 2025-26 `163` / `4,474,899` · `29` props · `8,708,333` total** *(re-derived `2026-09-26`, `§T26.49`)*. ⚠⚠ ~~2024-25 **162** × **30** *(9,537,535)* · 2025-26 **163** × **30** *(9,805,813)*~~ — **the BOARD-SCOPED PRUNE roughly HALVED it** *(`§T26.7`, `prune_baseline_to_board.py`)*, **and the prop count fell `30` → `29`**. 🔑 **DATE COVERAGE IS UNCHANGED (`162`/`163`) — that is the integrity test; the row fall is the off-board ladder being removed on purpose.** ⚠ *Which prop left is **NOT RECORDED** (`RULE 6`). The `29` live are:* `assists` `assists_q1` `blocks` `dreb` `fantasy_score` `fg3a` `fga` `fgm` `fta` `ftm` `oreb` `personal_fouls` `points` `points_h1` `points_h2` `points_q1` `points_q4` `points_q4_otx` `pra` `pts_ast` `pts_reb` `reb_ast` `rebounds` `rebounds_q1` `steals` `stocks` `threes_made` `threes_made_q1` `turnovers`. 🔑 **`49.28%` of these rows are PERIOD rungs** *(`Q1`/`Q4`/`H1`/`H2`)*, *because the prune keeps a full ladder for any prop no board carried — which is the owner's **"ALL PROPS STAY, NO EXCEPTION"** (`§T26.27`)* | ✅✅ ~~🔴 **`T20-13`: October carries only `22`**~~ **CLOSED `2026-09-25`, `§T26.30`** — *the gate moved `>= 25` → **`>= 20`**, scoped to `period='FULL'` and the slate's own date. **Under the OLD gate `0` of `78` October+November dates passed, in EITHER season.** ⚠ **Margin is ONE prop: every opening-window date carries exactly `21`.*** |
| **5** | **As-of calibration** *(P2)* | stage 4 | `nba_score.ladder_calibration_asof` — ▶ **`25,962` rows · `47` as-of dates** *(re-derived `2026-09-26`, `§T26.49`)*. ⚠ ~~**9,904** rows · **24** as-of dates · `2024-10-29 → 2026-01-15`~~ — *that was `2026-09-23`; the refit ran against the restored `final_hp` (`§T26.15`), which is why it **`2.6×`'d***. 🔑 *`2025-26` now fits from its OWN evidence — `55.1%` own / `44.9%` prior-season — instead of inheriting `2024-25` wholesale* | P2: `as-of calibration available` · ⚠ **the parity rule: every value computed IN-RUN from history AS OF THE DAY — nothing pasted** *(COMPASS fact 6; `NBA_BASELINE_CALIBRATION.md` §T17.2 records the one violation of it, found and fixed)* |
| **6** | **Board archive** *(P3)* | *independent of 1–5* | `nba_market.board_snapshots` — ▶ **`14` sources · `27,068,327` rows**; newest NBA `game_date` **`2026-04-12`** *(correct for an off-season)*. ⚠ ~~**12** sources~~ *(re-derived `2026-09-26`, `§T26.49`)* 🔑 **AND `P3` NOW ALSO REFRESHES `nba_market.board_rung_keys` FROM THIS TABLE** *(`refresh_board_rung_keys()`, scoped to boards archived in the last `6` hours)* — ***three P2 scripts `SystemExit` without it, so this step is a P3→P2 dependency*** *(`§T26.45`, item `T26-3`)* | P3: `board archived today` · ⚠ **threshold is `> 0` against a real magnitude of **~71,000 legs per date** (`T20-6`)** |
| **7** | **The grader** *(P2, next morning)* | stages 2 + 6 | `nba_market.board_outcomes` — **6,905,452** ✅ *(re-derived `2026-09-26`: EXACT, unchanged)* | ✅✅✅ ~~🔴 **`T20-5`: `GRADE_END` defaults to `"2026-04-12"`…**~~ **CLOSED `2026-09-25`, `§T26.30`** — *the grader now defaults to a **rolling `7`-day window ending today**, and `build_rung_market` to **the current month**. Its own comment names what it removed: **"the old ceiling was dead from 2026-27 on — it would have graded ZERO dates and exited green, starving the calibration."*** ⚠ **The same edit also killed a critical-path defect the brief never knew about**: *the old default re-graded the WHOLE archive nightly — **`2.48M` legs / `132` dates in the first 13 minutes of a replay, to add ONE night*** |
| **8** | **The scoring engine** — 🔴 **NOT P3. `P2` OWNS `final_hp` SINCE `2026-09-23` (step `6b`)**, *which is what closed `NBA_SYSTEM_DESIGN.md` §4b's "rebuilt by NOTHING"* | stages 4 + 5 + 6 | `nba_score.final_hp` — ▶ **`7,215,296` rows · `21` props · 2024-25 `162` dates / 2025-26 `163` dates** *(re-derived `2026-09-26`, `§T26.49`)*. ⚠⚠ ~~**19,215,200** *(2024-25 `19,075,070` · 2025-26 `140,130`)*~~ — ***the `140,130`-on-one-date figure was `T16-7`'s DATA LOSS; it was RESTORED (`§T26.15`) and then the store was INVERTED to BOARD-SCOPED, which is the population the OWNER asked for.*** 🔑 **READ THE DATE COVERAGE, NOT THE ROW COUNT**: *`−62.5%` of rows is the full ladder the owner called "unneeded" being removed ON PURPOSE; `162`/`163` dates is the loss test, and it is whole.* ⚠ *The table is LIVE — it read `7,210,912` on `2026-09-25` and `7,215,296` a day later. **`RULE 59`: re-derive, never quote.*** · confidence · score ✅ **`0` of `7,215,296` rows negative** *(`T16-8` closed, `§T26.16`)* | P3: `final_hp has today` · `confidence populated` · `score in range 0-100` · `confidence model loaded` |
