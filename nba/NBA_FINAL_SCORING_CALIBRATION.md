...
> 📏 **CENSUS STAMP — `2026-09-23`, `§F7.5`. THE SAME SWEEP RAN ON ALL TWELVE AND ONLY ONE OF THE THREE FIGURES SURVIVED IT.** ~~***`sections` is authoritative and was re-derived at this date.***~~ 🔴🔴 **RETRACTED `2026-09-23`, `§F7.14` — `sections` WENT STALE TOO: exact `4` of `11` across the twelve, NOT `12` of `12`; this file drifted `+21`, the worst after the summary.** ***No figure below is authoritative.*** ⇒ 🔁 **`RULE 59` — RE-DERIVE, NEVER QUOTE:** `` grep -cE '^(> *)*#{1,6} ' nba/<FILE>.md `` *(the `` (> *)* `` prefix is mandatory — a `^#` detector is blind to blockquoted headings, `294` across the twelve, `6.0%`, `§F2.14`).* *Measured against the live files the same day: **sections exact `12` of `12`** · **bytes exact `1` of `11`** · **lines exact `0` of `6`**. 🔑 **This file holds that one exact byte figure — `347,306` — and it is exact for the least reassuring reason available: nothing has been patched into it since the figure was written.** *Not accuracy; stillness.* ⚠ **The cause is structural: a census lives inside the index it measures, so the patch that publishes a figure invalidates it in the same commit** *(`§F6.19`)*. ⇒ **Re-derive sections with `^(?:>\s*)*#{1,6}\s` — never `^#`, which is blind to the `294` blockquoted headings across the twelve.**
