...
> 📏 **`332` sections · re-derived `2026-09-26` (pass 29, RUN)** *(was `274` on `2026-09-23`, then `316` · `322` · `327` across `2026-09-25`–`26`; the last `+5` is §T26.74's; **`RULE 59` — re-derive AND RUN, never quote**):* `` grep -cE '^(> *)*#{1,6} ' nba/NBA_FINAL_SCORING_CALIBRATION.md ``
...
> ✅✅✅ **`§T26.78` ADDED `2026-09-26` — `§T26.74` IS RETRACTED IN FULL. THERE IS NO COLLISION AND THERE NEVER WAS.** 🔴 *`build_final_hp.py` writes period rungs under a **suffixed prop label** — `prop = 'points_q1'`, not `prop='points'` — so the existing conflict key already separates a `Q1 5.5` from a `FULL 5.5`. **`T26-11`/`A0` is closed.*** ⚠⚠ **HOW I GOT IT WRONG**: *two commits landed in the same minute (`09:12`) — I read `00c71c6b` ("board keys carry every period") and **not** `0afcb970` ("period rungs stored under the suffixed prop label … **no schema change**"), then built an alarm on the first.* 🔑 **The deeper fault is a `RULE 57` failure this sweep has catalogued all day: I CHECKED THE SCHEMA AND NOT THE VALUE** — *"`23` columns, no `period`" was true; "so what is `prop` set to?" was never run.* ✅ *The convention is the scorer's and predates all of it (`score_board_legs.MARKET_TO_PROP`: `"player_points_q1": "points_q1"`), and its `16` labels align exactly with `refresh_board_rung_keys`'s `16` period mappings.* ✅✅ **AND THE BUILD CHAT ANSWERED BY NAME IN TEN MINUTES** *(`09:27` my alarm → `09:37` `9d9c922c`, "answers T26-11/A0")* **with two live `assert`s that harden the invariant** — *the right response to a false alarm: not "you are wrong" but "you are wrong AND here is the assertion that keeps it wrong". The sweep's error produced a guard the system did not have.* ⚠ *SURVIVES: the `baseline_history` measurements (`7,796` groups / `31,332` rows / `3,801` with `FULL` on a board key) are correct — **the inference from the store's shape to `final_hp`'s shape was not**. `T26-6` is unaffected and still stands.*
>
> ~~🆕🔴🔴🔴 **`§T26.74` ADDED `2026-09-26`**~~ 🔴 **RETRACTED — see `§T26.78` above** ~~**THE MOST URGENT ENTRY IN THIS FILE AND THE NEWEST DEFECT IN THE SYSTEM, NINE HOURS OLD.**~~ **`build_final_hp.py` WAS CHANGED AT `09:12` TODAY (`00c71c6b`, *"board keys carry every period"*) TO DROP ITS `WHERE period = 'FULL'` GATE — AND `nba_score.final_hp` STILL HAS NO `period` COLUMN** *(`24` columns, checked; and the `INSERT` writes `23` with `ON CONFLICT (game_date, player_id, prop, line, side)` — **no `period` in either**)*. ⇒ 🔴🔴 ***THE READER NOW ADMITS FIVE PERIODS AND THE WRITER HAS ONE SLOT FOR THEM.*** **MEASURED, NOT FEARED**: *`baseline_history` already holds **`7,796` colliding key groups / `31,332` rows**; **`3,801` of them contain a `FULL` rung AND are on a real `board_rung_keys` key today — every single one — covering `17,049` rows**; one key carries all five periods.* ⚠⚠⚠ **THE ONLY GUARD LEFT IS THAT `board_rung_keys` IS `100%` `FULL` (`§T26.59`) — the reader's filter WAS the guard and it was removed.** *The refresh function carries `16` period mappings, so **the day one app posts a `player_points_q1` line**: ① `final_hp` receives period rungs it cannot label ⇒ `§T26.17`/`§T26.18`'s defect resurrected; ② the upsert overwrites on `3,801` live keys, silently, last-write-wins; ③ `_prune_scope` gains period triples ⇒ `T26-6`'s inversion on `4,296,237` rows.* ⇒ ***ONE TRIGGER, THREE SILENT FAILURES, routine in-season, `24` days out, in pipelines the owner has stated run unmonitored (`§T26.70`).*** ⇒ **`T26-11`.** ✅ *Remedy, unambiguous: **add `period` to `nba_score.final_hp` and to the conflict key BEFORE the key table can carry a period row** — the reader is already period-aware; only the destination is not.* 📜 **`RULE 61` INVERTED — first instance in this sweep: the CODE now assumes a schema the STORE does not have.** ⚠ *It also RETRACTS a claim `§T26.68` and `§T26.72` each published within the hour — that `final_hp` "asks for `FULL` only" — which was already false when cited. `RULE 63` caught its own author twice in one pass.*
>
> 🆕🆕🆕 **`§T26.66` ADDED `2026-09-26`** — ✅✅✅ **`T26-4` ANSWERED, AND THE ALARM INVERTS: THE BELOW-CHANCE ANCHOR IS `21` OF `109` ROWS IN ONE EXPERIMENT, WHILE THE PRODUCTION EVALUATION SCORES `0.56431` ON `1,248,826` LEGS.** ⚠⚠ ***READ IT BEFORE `§T26.55` BELOW — IT CORRECTS IT.*** *Over all `45` slices: `21` rows above `ln 2 = 0.69315` and **all `21` belong to the `A2`-novelty family** (`all`/`fires`/`low_novelty`/`high_novelty`, `n` `866`–`15,024`); **`40` of `45` slices contain not one above-chance row** — `remaining_factors` `0.56431` at `n = 1,248,826`, `allprop:*` as low as `0.0761`, `ladder_all` `0.6281`.* ⇒ ***the system is not below chance; one experiment's LOCAL anchor is, which makes that family's `gain_vs_anchor` figures UNINTERPRETABLE rather than wrong — fix the anchor, then re-run the gate.*** ⚠ **AND `§T26.55`'s "the slice is undefined" WAS MY OWN ERROR**: *`all` is defined by its siblings in the same table, one `GROUP BY` away — `RULE 58` turned on myself.* 🔴 *The `2026-09-25` rerun also regressed the harness: `n` `15,024`→`7,128` (`−52.6%`), `shrink_beta` `0.9285`→`NULL`, and **only `all` rewritten**, leaving its defining siblings `12` days staler than it.* 🔴🔴 **AND A LIVE TRAP — `T26-10`**: *six writers share this one schema with **at least three meanings per column**, and the key exists only in a `print()` — for `n1_ablation`, **`brier` is AUC** and **`gain_vs_anchor` is ACCURACY**, so `ORDER BY gain_vs_anchor DESC` crowns `n1_ablation` with apparent gains of `0.70`–`0.80` against a best real gain of `+0.0068`.*
>
> 🆕🆕 **`§T26.55` ADDED `2026-09-26`** — 🔴🔴🔴 **`F5-1`'s "MISSING" FACTOR-GATE RESULTS WERE NEVER MISSING — AND EVERY VARIANT FAILED, THE INTERACTIONS WORST** — *the verdicts are in `nba_score.factor_gate_results`, all written `2026-09-25 22:42:57Z`, `n = 7,128` legs: `anchor` log-loss `0.72604`; `anchor_x_defender` **identical to five decimals** (gain `0.00000`); `anchor_x_A2` and `anchor_x_A2_x_defender` both `0.97044` (gain **`−0.24441`**); `anchor_x_A5_pstart_minutes` `0.74408` (`−0.01804`). **Not one variant beat the anchor, and the two interactions that were supposed to be the answer are catastrophically worse.*** 🔑🔑 *AND THE ANCHOR ITSELF IS **BELOW CHANCE** on this slice — `0.72604` against `ln 2 = 0.69315`, Brier `0.26359` against `0.25` — so the comparison has no valid floor; `shrink_beta` is NULL on all five rows.* ⚠⚠ **CORRECTED `2026-09-26` BY `§T26.66` — READ IT FIRST**: ~~*and the slice is undefined*~~ — **the slice IS defined**, by its own siblings `fires` / `low_novelty` / `high_novelty` in the same table, and ✅ **the below-chance result is confined to that one `A2`-novelty experiment — `21` of the table's `109` rows, `n` between `866` and `15,024` — while the PRODUCTION evaluation scores `0.56431` on `1,248,826` legs.** ⇒ ***`T26-4` is ANSWERED and the headline INVERTS: the system is not below chance; the experiment's local anchor is, which makes that family's `gain_vs_anchor` figures — including the five above — UNINTERPRETABLE rather than wrong.*** ⇒ `T26-4`.
>
...
## ✅✅✅ **§T26.78 — `§T26.74` IS WRONG AND IS RETRACTED IN FULL. THE UPSERT CANNOT COLLIDE, BECAUSE THE WRITER RELABELS THE PROP — AND THE COMMIT THAT PROVES IT LANDED FIFTEEN MINUTES BEFORE I PUBLISHED THE ALARM** *(source, 2026-09-26; a self-correction, not a finding)*

> 🔴🔴 **THE ERROR, STATED PLAINLY BEFORE ANYTHING ELSE.** *`§T26.74` claimed that because `nba_score.final_hp` has no `period` column and its upsert conflicts on `(game_date, player_id, prop, line, side)`, a `Q1 5.5` and a `FULL 5.5` would collide and overwrite — `3,801` live keys, `17,049` rows. **That is false.*** ⚠ ***`build_final_hp.py` writes period rungs under a SUFFIXED PROP LABEL — `prop = 'points_q1'`, not `prop = 'points'` — so the existing key already separates them. There is no collision and there never was.***

### ⚠⚠ **HOW I GOT IT WRONG, EXACTLY — BECAUSE THE MECHANISM MATTERS MORE THAN THE APOLOGY**

> *Two commits landed **in the same minute**, `09:12` PT:*
> | commit | what it did | did I read it? |
> |---|---|---|
> | `00c71c6b` | *"board keys carry every period"* — dropped `WHERE period = 'FULL'` from the reader | ✅ **yes — I built `§T26.74` on it** |
> | `0afcb970` | *"period rungs stored under the **suffixed prop label** (`points_q1` …) — the same label the scorer and calibration use; **no schema change**, the unique index keeps `points_q1` apart from `points`"* | 🔴 **NO** |
>
> ⇒ ***I read one of two sibling commits and drew a conclusion the other refutes.*** 🔑 **AND THE DEEPER FAULT IS A `RULE 57` FAILURE OF THE EXACT KIND THIS SWEEP HAS CATALOGUED ALL DAY: I CHECKED THE SCHEMA AND NOT THE VALUE.** *The column list was real — `23` columns, no `period`, conflict key without it — and every word I wrote about it was true. **What I never ran was the question "so what is `prop` actually set to?"** The answer is four lines above the `INSERT`:*
> ```python
...
> h = pd.read_sql("""SELECT h.game_date, h.game_id, h.player_id, %s AS prop, …      -- the SUFFIXED label
>                    WHERE h.season=%s AND h.prop=%s AND coalesce(h.period,'FULL') = %s""", params=(prop, season, _base, _per, …))
> ```
> *The store keeps `prop='points', period='Q1'`; the read splits the label to query it and **relabels the output to `points_q1`**. `RULE 58` in its purest form: **my "no `period` column" was a true observation and a false claim about behaviour.***

### ✅ **AND THE CONVENTION IS NEITHER NEW NOR IMPROVISED — IT IS THE SCORER'S, AND IT PREDATES ALL OF THIS**

> *`score_board_legs.MARKET_TO_PROP` has carried it all along:* `"player_points_q1": "points_q1"` · `"player_rebounds_q1": "rebounds_q1"` · `"player_assists_q1": "assists_q1"` · `"player_threes_q1": "threes_made_q1"`. ⇒ **`final_hp` was brought INTO an existing system-wide convention, not given an ad-hoc one** — which is why the commit could say *"no schema change"* and be right. ✅ *The `16` labels (`4` bases × `q1`/`q4`/`h1`/`h2`) align exactly with the `16` period mappings `§T26.59` found in `refresh_board_rung_keys`.* ⚠ *`nba_score.board_scored` currently holds `12` distinct props and **`0`** suffixed — **the convention has never been exercised, because no board has ever posted a period market** (`§T26.59`, `§T26.63`). That is why it was invisible to me and why it will be exercised for the first time on the same day `T26-6` turns on.*

### 🔑🔑 **AND THE BUILD CHAT ANSWERED BY NAME IN TEN MINUTES — WITH A BETTER FIX THAN THE ONE I PROPOSED**

> | time (PT) | event |
> |---|---|
> | `09:12` | `0afcb970` *(the relabel)* and `00c71c6b` *(every period)* |
> | `09:13` | `976173a3` *(P2 rebuilds yesterday's `final_hp`)* |
> | 🔴 **`09:27`** | **`f7af4cd4` — my `§T26.74`, the false alarm** |
> | ✅ **`09:37`** | **`9d9c922c` — *"assert the period-label invariant (answers T26-11/A0 — the writer labels by period, so no key collides)"*** |
>
> ✅✅ *And it did not merely rebut — it **hardened the invariant so a future edit cannot quietly make my claim true**:*
> ```python
...
> ```
> *(both live at HEAD)* ⇒ 🔑 ***That is the correct response to a false alarm: not "you are wrong", but "you are wrong AND here is the assertion that keeps it wrong". The sweep's error produced a guard the system did not have.***

### ⚠ **WHAT SURVIVES FROM `§T26.74`, AND WHAT DOES NOT**

> ✅ **SURVIVES — the measurement, as a statement about `baseline_history` only**: *`7,796` key groups on one `(game_date, player_id, prop, line)` carry more than one period; `31,332` rows; `3,801` groups contain a `FULL` rung and sit on a real `board_rung_keys` key; one key carries all five periods.* **Those numbers are correct and re-derivable. What was wrong was the INFERENCE from the store's shape to `final_hp`'s shape.**
> 🔴 **RETRACTED**: *the collision, the `17,049` rows "at risk", the "reader takes five, the writer has one slot" framing, the "`RULE 61` inverted" reading (there is no schema mismatch — the design deliberately avoids one), and the proposed remedy (adding a `period` column, which would have duplicated a convention that already exists).*
> ✅ **UNAFFECTED — `T26-6` STILL STANDS**: *the prune's `_prune_scope` groups on `(game_date, prop, period)` using the **base** prop from `board_rung_keys`, which is untouched by how `final_hp` labels its output. The `4,296,237` period rows in `baseline_history` still survive only via the `kept as NEVER-DERIVED` branch, and that still inverts when a period market appears.*

> 🔁 **RE-DERIVE** *(`RULE 59`)*:
> ```bash
> git show 0afcb970 -- nba/build_final_hp.py     # the relabel I failed to read
> git log -1 --pretty='%ad %h %s' --date=format:'%H:%M' f7af4cd4 9d9c922c
> sed -n '344,346p' nba/build_final_hp.py        # _PERIOD_LABELS, 16
