...
2. **D1 referee assignments** — build the live daily capture so the archive starts now; keep historical
   officials as target-only.
3. **M1 integration** — fold measured defender quality into the harness as a factor layer.
4. **Day-by-day replay harness** — a runner that walks a date range and produces baseline + enrichment
   for each day exactly as production would, so the engine can be trained and evaluated on real
   sequences rather than a single slate.
