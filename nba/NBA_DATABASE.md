...
> 📏 **`199` sections · re-derived `2026-09-26` (pass 28, RUN)** *(was `148` on `2026-09-23`, then `173` · `177` · `182` · `191` · `195` across `2026-09-25`–`26`; the last `+4` is §T26.72's; **`RULE 59` — re-derive AND RUN, never quote**; the `+5` is §T26.52/§T26.57's own headings — **the stamp drifted a third time from my own additions, which is why it is re-RUN at the end of every pass and never carried forward**):* `` grep -cE '^(> *)*#{1,6} ' nba/NBA_DATABASE.md ``
>
> 🆕🆕🆕 **`§T26.72` ADDED `2026-09-26`** — ⚠⚠⚠ **`RULE 63` FIRES A SECOND TIME, NINE HOURS AFTER IT WAS WRITTEN — READ IT WITH `§T26.68`, WHICH IT HALF-SUPERSEDES.** *`P2` moved AGAIN (`976173a3`, `09:13` PT) — two changes in nine hours, both inside the mechanism `§T26.59`–`§T26.68` describe.* ✅ *The new step `3a` at line `224` rebuilds **yesterday's** `final_hp` right after the key refresh and the prune, because "the day-of board P3 captured at 13:15, late lines, period props — never entered it, and the calibration and confidence refits learn ONLY from `final_hp`".* ⇒ 🔑 ***`build_final_hp.py` now runs TWICE in `P2` — line `236` (yesterday, ✅ COVERED by the `213` refresh) and line `404` (the slate, 🔴 still NOT) — so `§T26.68`'s "not covered" row splits in two and is HALF-SUPERSEDED, not wrong.*** *`T26-3`'s residual is unchanged in substance, smaller in scope.* 🔑🔑 **AND THE MECHANISM IS NOW CORROBORATED FROM THREE DIRECTIONS IN ONE DAY** — *the sweep's PERIOD instance, the build chat's LATE-BOARD instance, and now its INCOMPLETE-`final_hp` instance: **"the key table's view of the board is a SNAPSHOT, and everything downstream treats it as the board."*** ✅ *It also bounds `T26-6`: the refits learn only from `final_hp`, which asks for `FULL` only, so the `4.3M` period rows reach the calibration path by **no** route today.*
>
> 🆕🆕🆕 **`§T26.68` ADDED `2026-09-26`** — ⚠⚠⚠ **`§T26.59` WAS ACTED ON TWENTY-FOUR MINUTES AFTER IT WAS WRITTEN, AND RE-DERIVING IT CORRECTS BOTH `§T26.59` AND `§T26.45`. READ IT WITH `§T26.59` BELOW.** *P2 now calls `refresh_board_rung_keys(yesterday, yesterday)` immediately before the prune (`3667076e`, `00:05` PT) — **the exact call `§T26.59` derived, now on the cron'd path** — for a reason the commit states itself: a board captured after P3's `13:15` refresh "**would be pruned away**".* 🔑🔑 ***That is an independent confirmation of `T26-6`'s mechanism from a different trigger: `§T26.59` found the PERIOD instance, the build chat found the LATE-BOARD instance, and both are "the prune deletes what the key table does not know about".*** ⚠⚠ **AND THE BLAST RADIUS WAS OVERSTATED — corrected here**: *`§T26.45` said "three consumers that `SystemExit`, two of them P2". In fact* ✅ *the **prune** is now self-healing;* 🔴 *`build_final_hp.py` is **NOT** covered — the new step refreshes YESTERDAY while the scorer asks for `period = 'FULL' AND game_date = FE_DATE`, **today's slate**;* 🔴🔴 *and `load_baseline_history.py` **is not in P2 at all** — it is dispatched by three workflows, **none of which has a `cron`**.* ⇒ ***a `P3` failure on day `N−1` now stops exactly ONE cron'd `P2` step, and `T26-3`'s residual is one line: extend the refresh to the slate `final_hp` scores.*** ✅ *Also a consumer-side confirmation of the `FULL`-only finding: the scorer ASKS for `FULL` only, so the `24` dead map rows cost nothing today — and will matter on the same date `T26-6` turns on.*
>
> 🆕🆕🆕 **`§T26.59` ADDED `2026-09-26`** — ✅✅✅ **`T26-3` CLOSED, BOTH HALVES, WITHOUT A WRITE — AND THE PERIOD HALF IS LOAD-BEARING** — ▶ *RECOVERY: the `6`-hour window is in the **P3 workflow step**, not in `nba_market.refresh_board_rung_keys`; the function body (`pg_get_functiondef`) is `DELETE … BETWEEN d1 AND d2` then an `INSERT … SELECT` from the archive with **no `fetched_at` predicate**, so `SELECT nba_market.refresh_board_rung_keys('<lo>','<hi>')` repairs any range at any later date — the call the two abort messages already print.* ▶ *PERIODS: the function carries `16` period mappings and does not drop them, but **`24` of its `38` map rows match nothing in `board_snapshots`** — all `16` period rows plus `8` `FULL` ones. `91` distinct market keys ever archived; **no NBA period market in any spelling** (`RULE 20`, three vocabularies). ⇒ `RULE 61`: **the input is `FULL`-only, not the store and not the code**.* 🔴🔴 ***THE CONSEQUENCE: `board_rung_keys` is `4,522,924` rows `100%` `FULL` while `baseline_history` is `49.3%` period (`4,296,237` rows) — and they survive `prune_baseline_to_board.py` only via its `kept as NEVER-DERIVED` branch, which inverts the day a board posts `player_points_q1`.*** ⇒ **`T26-6`.** 🔴🔴 *AND SEPARATELY: **`7,951` baseball rows sit in this NBA table across `68` non-NBA market keys, with NO sport or league column on it** — `189` hit an NBA map key (`player_fantasy_points`, `underdog`, `2026-09-12`); `0` reach `board_rung_keys`, but only because the `player_name_map` inner join is a de-facto sport filter built on roster-name disjointness.* ⇒ **`T26-7`.**
>
...
> | 🆕🔴🔴 **THE RETENTION RULE — why the big tables SHRANK, and what a rerun does** | **`§T26.7`, below.** ***One set of data per day; a rerun OVERWRITES, it never stacks.*** **`baseline_history` `13 GB→2.8 GB`, `final_hp` `9.4 GB→1.8 GB`** *(live `2026-09-25`)* — **that is the rule executing, not data loss.** ⚠ **`nba_score.baseline_prune_log` (`325` rows) records every prune, and the scorer REFUSES to re-score a pruned slate.** |
> | 🆕🔑 **"where is the INJURY REPORT?"** *(the binding availability input)* | **`nba_daily.injury_report_snapshots`** — **`1,338,020` rows · `330` game dates · `12,066` SNAPSHOTS · `2024-10-22→2026-04-14`** *(live `2026-09-25`)*. ⚠ ***It had NEVER been in Postgres before `2026-09-24`*** — the doc specified the loader and it was never built; the data existed only as repo files. 🔑 **Every snapshot is kept, not a daily summary**, so `as-known-at-12:30` stays separable from `as-known-at-19:45` — *which is what makes the availability fallback backtestable* **(`§T26.5`)**. |
> | 🆕 **the AVAILABILITY / STARTER fallback models** | **`nba_score.availability_training`** `27,265` *(leakage-free)* · **`availability_prior`** `699` *(`4` levels, shrinkage `15/10/5`)* · **`starter_training`** `78,556` · **`starter_prior_v2`** `125` · **`nba_ref.official_tendency`** `78`. *Fit and Brier scores: **`§T26.5`** in `NBA_FINAL_SCORING_CALIBRATION.md`.* |
> | ⚠⚠ **"do we have REFEREE data?"** — *two tables hold it and only one is named for it* | **HISTORY: `nba_stats.game_officials`** — *`3,681` crew rows, `1,227` games, `2025-10-21→2026-04-12`, `83` officials* · **IDENTITIES: `nba_ref.officials`** *(`80`)* · **UPCOMING: `nba_ref.referee_assignments`** — 🔴 ***`0` rows, and that is CORRECT before the season***: it is forward-looking and the next game is `2026-10-03`. **Grepping `referee` alone finds only the empty one** *(`§T26.2`)*. |
> | 🔑 **which table holds a graded leg OUTCOME** *(what actually happened)* | **`nba_score.board_scored`** *(plus the `nba_score.board_scored_snapshot_*` set)* — *written by `score_board_legs.py`; see* **`§T23.13`** *(`NBA_SYSTEM_DESIGN.md`) for what that scorer SKIPS* |
