#!/usr/bin/env python3
"""
Load mined PrizePicks per-leg prices into nba_market.pp_mined_leg (NBA, default) or nba_market.pp_mined_leg_wnba.

Scans <PP_OUT_DIR>/pp_payout_map_*.json and loads EVERY file whose run_file is not yet in the table, so:
...
LEAGUES (2026-09-21). WNBA runs write to their own directory and table, so NBA validation views never see a WNBA row;
the extraction below is the SAME function for both - a cross-league comparison is only meaningful with identical math.
Env: DATABASE_URL, PP_OUT_DIR (default nba/data/pp_payouts), PP_MINED_TABLE (default nba_market.pp_mined_leg).
"""
import glob
import json
import os
...
def main():
    import psycopg
    table = os.getenv("PP_MINED_TABLE") or TABLES[0]
    if table not in TABLES:
        raise ValueError(f"unexpected PP_MINED_TABLE {table!r}")
    out_dir = os.getenv("PP_OUT_DIR") or "nba/data/pp_payouts"
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(f"SELECT DISTINCT run_file FROM {table}")
        loaded = {r[0] for r in cur.fetchall()}
    files = sorted(glob.glob(os.path.join(out_dir, "pp_payout_map_*.json")))
    total = 0
    for path in files:
        ...
        conn.commit()
        print(f"LOADED|{table}|{name}|{len(rows)} per-leg prices", flush=True)
        total += len(rows)
    print(f"LOAD_DONE|{table}|files={len(files)}|previously_loaded={len(loaded)}|new_rows={total}", flush=True)
    # PRICE THE SEASON AS IT ARRIVES (NBA only): rescue centers for recent no-center legs, create missing Price IDs,
    # and price them under every registered model version. Idempotent - on an unchanged board it adds nothing.
    if table == TABLES[0]:
        with conn.cursor() as cur:
