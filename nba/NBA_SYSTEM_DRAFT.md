...
**One deliberate, narrow exception — the operational control-plane**: per the blueprint's own Section 7e (a legitimate, already-established exception even under MLB's own "zero new tables" discipline), NBA workers register into the *existing, shared* `config.worker_definitions` / `config.worker_schedules` and `control.job_queue` / `control.job_runs` tables — using `nba-` prefixed `job_key`/`worker_name` values, never new copies of these tables. This is pure bookkeeping plumbing, genuinely sport-agnostic, and the person's own instruction that "no runner/orchestrator" is needed doesn't preclude using the *registry* — it precludes building automated dispatch on top of it. NBA jobs are still triggered manually, worker by worker, via Coworker sessions; the registry is just where they're catalogued so they're visible and don't collide with MLB's own `job_key` space.
