...
// worker's entire invocation forever. This completes fixing all 6 master-run stage workers.
...
export default {
  async scheduled(event, env, ctx) {
    ...
    // directly instead - this handler is now a guaranteed no-op. Completes fixing all 6
    // master-run stage workers plus the 3 Delta workers fixed earlier today.
    ctx.waitUntil((async () => {
      try {
        await postgres(env.HYPERDRIVE.connectionString, { max: 1, fetch_types: false, prepare: false, connect_timeout: 8 })`
          ...
          VALUES ('AUTOMATED_scoring_runner_part2_run', 'Cron fired (cron: ' || ${event.cron || "unknown"} || ') but this handler is now a deliberate no-op - Cowork runs LAYER 1-4 directly instead.', 'CRON_NOOP_RETIRED', null)`;
      } catch (_) {}
    })());
  },
