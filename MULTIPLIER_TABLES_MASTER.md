...
### The one real exception found: `pitcher_strikeouts/less/Tier2`
- **Real, critical correction (found by a coworker research pass, 2026-08-22, and directly verified against the live deployed code)**: this section previously named the wrong prop — it said `hits_runs_rbis/less/Tier2`, but the actual deployed pool (`DEMON_HIGH_HIT_TIER_POOL` in `alphadog-v2-certification-center.js`) is, and has always been, `pitcher_strikeouts/less/Tier2`. The real evidence below (the 71.6% figure, the real 6-pick Sale/Misiorowski/Webb/Boyd placed-slip test) was always about `pitcher_strikeouts` — this was a pure documentation error, not a live deployment error. **The live strategy itself was correct the entire time.**
- **Real, critical table-selection warning**: `backtest.tiered_full_fixed` and `backtest.demon_full_history_dedup` are NOT interchangeable, despite both having a `tier` column. `tiered_full_fixed` is confirmed rotten for this purpose: 98 of its 105 `hits_runs_rbis` "tier 0" rows have `line_value = anchor_line` (the anchor was circularly derived from the leg's own line, fabricating that bucket), and it has a massive real NULL-tier gap. `demon_full_history_dedup` is clean — 3,155 rows, 26 days through 08-20, zero NULLs, correctly 1-indexed matching the live formula directly. **Always use `demon_full_history_dedup` for Demon tier analysis; `tiered_full_fixed` and its three sibling tables should be treated as stale and not read by any future session.**
- **Real, important leave-one-day-out finding (2026-08-22)**: on `demon_full_history_dedup`, exhaustive 3-combination enumeration (no arbitrary leg ranking) shows the real `pitcher_strikeouts/less/Tier2` pool has genuine support across 5 real days, and **remains positive on 4 independent days even with 08-11 fully excluded**: +311.9% Power / +378.5% Flex (56.5%→27.5% full-hit rate once 08-11 is removed). This is the more conservative, honest number to use going forward — the earlier +767.6%/+821.9% figures included 08-11, the same day already flagged elsewhere in this document as a real, recurring outlier (see the rejected `runs+singles<0.5` signal above, which also leaned on 08-11). **Caveat on the ex-08-11 number**: real full-hit rate (27.5%) runs meaningfully below what independence would predict from the real per-leg rate (71.1% → 35.9% implied) — real legs underperform an independence assumption, so treat +311.9%/+378.5% as a plausible but somewhat optimistic estimate, not a tight bound.
...
- Real finding: PrizePicks' own regular line for this specific prop runs genuinely too high — real hit rate positive on 11 of 12 real days
- Real backtest, 6-pick, 12-day window (2026-08-06 → 2026-08-18): Power **+1105.4%** (28 slips, 9 full hits), Flex **+779.3%** — the strongest number found anywhere in the entire session on this specific window
- **Real, important correction (found by a coworker research pass, 2026-08-22)**: this 12-day window is NOT the full available real record — real graded data actually begins 2026-07-24, not 07-06 as earlier stated here. On the FULL real record (18 days, including 10 real days this 12-day window omits — several materially worse, e.g. 08-05 at 58.8% and 07-28 at 62.5%), the real ROI is **+725%**, not +1105.4%. Neither number is "wrong" — they're genuinely different windows of the same real underlying data. **+725% is the correct figure to use for any full-record analysis; +1105.4% only describes the narrower 08-06→08-18 window specifically.**
