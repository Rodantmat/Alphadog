...
- **Real, critical table-selection warning (found by a coworker research pass, 2026-08-22)**: `backtest.tiered_full_fixed` and `backtest.demon_full_history_dedup` are NOT interchangeable, despite both having a `tier` column. `tiered_full_fixed` was built earlier in the session, before the switch-point anchor fix existed — it has a massive real gap (e.g., 752 NULL-tier rows for `hits_runs_rbis/less` alone) and its tier numbering does not match the current live formula (confirmed: it shows tier=0 on genuine demon legs, which is structurally impossible under the current `round(abs(line-anchor))` formula where tier=0 is reserved for non-goblin/non-demon regular legs). `demon_full_history_dedup` was built later, is switch-point-aware, and matches the current live tier definition exactly. **Always use `demon_full_history_dedup` for any Demon tier analysis; treat `tiered_full_fixed` as a known-stale table.**
