...
async function certifyAndPromoteIfClean(sql, batchId, runId, cutoffDate, options = {}) {
  ...
  const tickConfigForPromote = await getWorkerTickConfig(sql, WORKER_NAME, DEFAULT_CHUNK_SIZE_PLAYERS, DEFAULT_MAX_TICK_RUNTIME_MS, DEFAULT_PROMOTE_ROWS_PER_TICK);
  const promoteLimit = cap(options.promote_rows_per_tick || tickConfigForPromote.promote_rows_per_tick, 1, 2000);
...
async function runDeltaUpdateTick(env, sql, input, inputJson) {
  ...
  const maxRows = cap(inputJson.max_rows_per_tick || tickConfigDelta.promote_rows_per_tick || batch.max_rows_per_tick || env.MAX_ROWS_PER_TICK || DEFAULT_MAX_ROWS_PER_TICK, 100, 5000);
