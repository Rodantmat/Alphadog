...
const VERSION = "alphadog-v2-phase3a-first-inning-pitcher-context-v0.1.62-baseline-v5-base-efficiency-only-calibration-frozen";
...
async function runBaselineV2(env,input={}){
  ...
  // v0.1.62 efficiency-only: Baseline Base calibration is frozen at formula v0.5.3.
  // Do not let stale/low continuation payloads pin the run to 50/80/110-row chunks.
  // The source_queue is locked/read-only and rows are ordered by queue_row_id, so the safe
  // fast path is a bounded multi-player chunk with trailing partial-player removal below.
  ...
  const allCurrentFastDefault=forcedExactChunk ? (requestedFastChunk || baselineV5FastFloor) : Math.max(requestedFastChunk || 0, baselineV5FastFloor);
  ...
  const chunkSize=clamp(effectiveRequestedChunk || (readAllHpV2Current?baselineV5FastFloor:8),1,readAllHpV2Current?(forcedExactChunk?baselineV5FastCap:baselineV5FastCap):32);
  ...
  const output=baseOutput(input,{request_id:requestId,run_id:runId,batch_id:batchId,source_hp_v2_batch_id:sourceHpV2BatchId,mode:requestedV5Mode,status:cert,certification:cert,certification_grade:grade,current_system_mutated:changed.length>0,source_rows_read:total,raw_hp_v2_source_rows:rawHpV2SourceRows,null_only_source_rows:nullOnlySourceRows,source_hp_v2_batch_id:sourceHpV2BatchId,rows_staged:staged,rows_promoted:staged,history_rows:staged,issue_rows:actualIssueRows,mutation_guard:{changed_tables:changed},reporting_fix_version:"v0.1.62_efficiency_only_calibration_frozen",read_all_hp_v2_current:readAllHpV2Current,canonical_group_by:logicalGroupBy,baseline_source_policy:"baseline_v5_history_only_static_base_delta_expansion_no_board_no_market_no_daily_no_app",fast_safe_chunk_policy:"v0_1_62_efficiency_only_min700_compact_trace_calibration_frozen",formula_version:BASELINE_V2_FORMULA_VERSION,confidence_version:BASELINE_V2_CONFIDENCE_VERSION,stage_shape_repair:stageShapeRepair,stage_validation:baselineStageValidation,classification_readonly_guard:readOnlyClassificationBaseline?{ok:readonlyCompare.ok,before:lockedClassificationBefore,after:lockedClassificationAfter,changed:readonlyCompare.changed}:null,pitcher_fantasy_scope:"source_agnostic_component_baseline_no_win_no_qs_enforced_v0_1_59"});
