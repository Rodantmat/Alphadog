...
const VERSION = "alphadog-v2-phase3a-first-inning-pitcher-context-v0.1.25-baseline-v2-timeout-safe-reporting-trace";
...
async function runBaselineV2(env,input={}){
  ...
  const allCurrentFastDefault=Number(input.v2_all_current_chunk_size||80);
  // All-current V2 can hit uneven rows where direct history lookup is heavier than average.
  // Keep chunks bounded below the 75s service-binding ceiling; do not override smaller retry chunks upward.
  const effectiveRequestedChunk=requestedChunkSize || allCurrentFastDefault;
  const chunkSize=clamp(effectiveRequestedChunk || (readAllHpV2Current?80:8),1,readAllHpV2Current?96:16);
  const cursor=Number(input.v2_cursor_offset||0);
  const startedMs=Date.now();
  const softYieldMs=clamp(Number(input.v2_soft_yield_ms || (readAllHpV2Current?35000:18000)),12000,readAllHpV2Current?42000:24000);
  if(cursor===0){
  ...
  const stageBatchSize=readAllHpV2Current?30:24;
  ...
  const output=baseOutput(input,{request_id:requestId,run_id:runId,batch_id:batchId,source_hp_v2_batch_id:sourceHpV2BatchId,mode:"baseline_v2_heb",status:cert,certification:cert,certification_grade:grade,current_system_mutated:changed.length>0,source_rows_read:total,source_hp_v2_batch_id:sourceHpV2BatchId,rows_staged:staged,rows_promoted:staged,history_rows:staged,issue_rows:actualIssueRows,mutation_guard:{changed_tables:changed},reporting_fix_version:"v0.1.25_issue_counter_source_batch_trace_timeout_safe",read_all_hp_v2_current:readAllHpV2Current,canonical_group_by:logicalGroupBy,baseline_source_policy:"source_agnostic_unless_formula_profile_is_source_specific",fast_safe_chunk_policy:"all_current_default_80_cap_96_batched_stage_writes_soft_yield_35s_reporting_trace",formula_version:BASELINE_V2_FORMULA_VERSION});
