...
async function fetchScheduleRange(env, startDate, endDate, fetchTimeoutMs) {
  ...
  for (const d of dates) {
    ...
    for (const g of games) {
      ...
      const homeScore = asInt(home.score, null);
      const awayScore = asInt(away.score, null);
      if (!homeTeamId || !awayTeamId) continue;
      rows.push({ team_id: homeTeamId, opponent_team_id: awayTeamId, game_pk: gamePk, game_date: gameDate, is_home: 1, runs_scored: homeScore, runs_allowed: awayScore, raw_json: g });
      rows.push({ team_id: awayTeamId, opponent_team_id: homeTeamId, game_pk: gamePk, game_date: gameDate, is_home: 0, runs_scored: awayScore, runs_allowed: homeScore, raw_json: g });
    }
  }
...
async function insertStageRowsBulk(sql, batchId, runId, mode, sourceSeason, rows) {
  ...
  const values = dedupedRows.map(r => ({
    ...
    runs_scored: r.runs_scored, runs_allowed: r.runs_allowed,
    raw_json: JSON.stringify(r.raw_json),
    ingestion_mode: mode, source_key: SOURCE_KEY, source_confidence: "SOURCE_LOCKED_STATSAPI_SCHEDULE_TEAM_TOTALS",
  ...
  for (let i = 0; i < values.length; i += CHUNK) {
    ...
    await sql`
      INSERT INTO team.game_logs_stage ${sql(slice, "stage_id","batch_id","run_id","team_id","game_pk","game_date","opponent_team_id","is_home","runs_scored","runs_allowed","raw_json","ingestion_mode","source_key","source_confidence","source_endpoint","source_season","source_game_type","row_status")}
...
async function acquireBatchLock(sql, batchId, owner, staleSeconds) {
  const rows = await sql`SELECT locked_by, lock_acquired_at, lock_expires_at FROM team.game_log_batches WHERE batch_id=${batchId} LIMIT 1`;
  ...
  await sql`UPDATE team.game_log_batches SET locked_by=${owner}, lock_acquired_at=now(), lock_expires_at=now() + make_interval(secs => ${staleSeconds}), updated_at=now() WHERE batch_id=${batchId}`;
  return { ok: true, owner };
}
async function releaseBatchLock(sql, batchId, owner) {
  await sql`UPDATE team.game_log_batches SET locked_by=NULL, lock_acquired_at=NULL, lock_expires_at=NULL, updated_at=now() WHERE batch_id=${batchId} AND locked_by=${owner}`;
}
async function buildPrePromotionChecks(sql, batchId) {
  const dupRows = await sql`SELECT COUNT(*)::int AS c FROM (SELECT team_id, game_pk, COUNT(*) AS n FROM team.game_logs_stage WHERE batch_id=${batchId} GROUP BY team_id, game_pk HAVING COUNT(*) > 1) sub`;
  const missingRows = await sql`SELECT COUNT(*)::int AS c FROM team.game_logs_stage WHERE batch_id=${batchId} AND (team_id IS NULL OR game_pk IS NULL OR game_date IS NULL)`;
  const totalRows = await sql`SELECT COUNT(*)::int AS c FROM team.game_logs_stage WHERE batch_id=${batchId}`;
...
async function getOrCreateBaseBackfillState(env, sql, input) {
  ...
  const existingRows = await sql`SELECT * FROM team.game_log_batches WHERE mode='base_backfill' AND status != 'CERTIFICATION_FAILED' ORDER BY started_at DESC LIMIT 1`;
  if (existingRows[0]) return { is_new: false, batch: existingRows[0] };
  const runId = asText(input.run_id, rid("run_base_team_backfill"));
  const batchId = asText(inputJson.batch_id, rid("team_game_logs_base_backfill_batch"));
  ...
  await sql`
    INSERT INTO team.game_log_batches (batch_id, run_id, worker_name, worker_version, mode, status, data_feed_key, source_key, source_endpoint, source_season, source_game_type, base_backfill_cutoff_date, delta_start_date, promote_rows_per_tick, certification_status, source_confidence, notes, started_at, updated_at)
    VALUES (${batchId}, ${runId}, ${WORKER_NAME}, ${VERSION}, 'base_backfill', 'BASE_BACKFILL_MINING', ${DATA_FEED_KEY}, ${SOURCE_KEY}, ${LOCKED_SOURCE_ENDPOINT_PATTERN}, ${sourceSeason}, 'R', ${cutoffDate}, ${DEFAULT_DELTA_RESERVED_START_DATE}, ${tickConfig.promote_rows_per_tick}, 'not_certified', 'SOURCE_LOCKED_STATSAPI_SCHEDULE_TEAM_TOTALS', ${"Postgres-native build, base fills only through " + cutoffDate}, now(), now())
  `;
  const rows = await sql`SELECT * FROM team.game_log_batches WHERE batch_id=${batchId} LIMIT 1`;
...
async function runBaseBackfillTick(env, sql, input) {
  ...
  try {
    ...
    if (status === "BASE_BACKFILL_MINING") {
      ...
      if (!schedule.ok) {
        await sql`UPDATE team.game_log_batches SET status='SOURCE_ERROR', updated_at=now() WHERE batch_id=${batchId}`;
      ...
      await sql`UPDATE team.game_log_batches SET status='BASE_BACKFILL_STAGED_READY_FOR_CERTIFICATION', rows_staged=${ins.inserted}, updated_at=now() WHERE batch_id=${batchId}`;
    ...
    if (status === "BASE_BACKFILL_STAGED_READY_FOR_CERTIFICATION") {
      ...
      if (!pre.pass) {
        await sql`UPDATE team.game_log_batches SET status='CERTIFICATION_FAILED', certification_status='BASE_TEAM_GAME_LOGS_CERTIFICATION_FAILED', certification_grade='BASE_FAIL', certification_json=${JSON.stringify(pre.checks)}, updated_at=now() WHERE batch_id=${batchId}`;
        return { ok: true, data_ok: false, mode: "base_backfill", batch_id: batchId, status: "CERTIFICATION_FAILED", checks: pre.checks };
      }
      await sql`UPDATE team.game_log_batches SET status='BASE_BACKFILL_CERTIFIED_READY_TO_PROMOTE', certification_status='BASE_TEAM_GAME_LOGS_CERTIFIED', certification_grade='BASE_PASS', certification_json=${JSON.stringify(pre.checks)}, certified_at=now(), updated_at=now() WHERE batch_id=${batchId}`;
    ...
    if (status === "BASE_BACKFILL_CERTIFIED_READY_TO_PROMOTE" || status === "BASE_BACKFILL_PROMOTING") {
      ...
      await sql`UPDATE team.game_log_batches SET status=${nextStatus}, rows_promoted=rows_promoted + ${promoted.promoted_this_tick}, promoted_at=COALESCE(promoted_at, now()), updated_at=now() WHERE batch_id=${batchId}`;
      return { ok: true, data_ok: true, mode: "base_backfill", batch_id: batchId, status: nextStatus, promoted, continuation_required: nextStatus !== "BASE_BACKFILL_PROMOTED_READY_TO_CLEAN" || true };
    }
    if (status === "BASE_BACKFILL_PROMOTED_READY_TO_CLEAN" || status === "BASE_BACKFILL_CLEANING") {
      ...
      await sql`UPDATE team.game_log_batches SET status=${nextStatus}, cleaned_at=CASE WHEN ${cleaned.cleanup_done} THEN now() ELSE cleaned_at END, finished_at=CASE WHEN ${cleaned.cleanup_done} THEN now() ELSE finished_at END, updated_at=now() WHERE batch_id=${batchId}`;
...
async function getLockedBaseIntegrity(sql) {
  const batchRows = await sql`SELECT batch_id, status, rows_promoted, base_backfill_cutoff_date, cleaned_at FROM team.game_log_batches WHERE batch_id=${LOCKED_BASE_BATCH_ID} LIMIT 1`;
  const batch = batchRows[0] || null;
  const liveRows = await sql`SELECT COUNT(*)::int AS c FROM team.game_logs WHERE batch_id=${LOCKED_BASE_BATCH_ID}`;
  const dupRows = await sql`SELECT COUNT(*)::int AS c FROM (SELECT team_id, game_pk, COUNT(*) AS n FROM team.game_logs WHERE batch_id=${LOCKED_BASE_BATCH_ID} GROUP BY team_id, game_pk HAVING COUNT(*) > 1) sub`;
...
async function getDeltaWindow(env, sql, inputJson, fetchTimeoutMs) {
  ...
  const existingDeltaLiveRows = await sql`SELECT MAX(game_date) AS max_delta_game_date FROM team.game_logs WHERE ingestion_mode='delta_update' AND game_date::date >= ${deltaFloor}::date`;
...
async function getOrCreateDeltaState(env, sql, input, windowInfo) {
  const existingRows = await sql`SELECT * FROM team.game_log_batches WHERE mode='delta_update' AND status != 'CERTIFICATION_FAILED' AND delta_start_date=${windowInfo.delta_start_date} ORDER BY started_at DESC LIMIT 1`;
  if (existingRows[0]) return { is_new: false, batch: existingRows[0] };
  ...
  const tickConfig = await getWorkerTickConfig(sql, WORKER_NAME, DEFAULT_CHUNK_SIZE_GAMES, DEFAULT_MAX_TICK_RUNTIME_MS, DEFAULT_PROMOTE_ROWS_PER_TICK);
  await sql`
    INSERT INTO team.game_log_batches (batch_id, run_id, worker_name, worker_version, mode, status, data_feed_key, source_key, source_endpoint, source_season, source_game_type, base_backfill_cutoff_date, delta_start_date, promote_rows_per_tick, certification_status, notes, started_at, updated_at)
...
async function runDeltaUpdateTick(env, sql, input) {
  ...
  try {
    ...
    if (status === "DELTA_MINING") {
      ...
      await sql`UPDATE team.game_log_batches SET status='DELTA_STAGED_READY_FOR_CERTIFICATION', rows_staged=${ins.inserted}, updated_at=now() WHERE batch_id=${batchId}`;
    ...
    if (status === "DELTA_STAGED_READY_FOR_CERTIFICATION") {
      const pre = await buildPrePromotionChecks(sql, batchId);
      if (!pre.pass) {
        await sql`UPDATE team.game_log_batches SET status='CERTIFICATION_FAILED', certification_status='DELTA_TEAM_GAME_LOGS_CERTIFICATION_FAILED', certification_grade='DELTA_FAIL', certification_json=${JSON.stringify(pre.checks)}, updated_at=now() WHERE batch_id=${batchId}`;
        return { ok: true, data_ok: false, mode: "delta_update", batch_id: batchId, status: "CERTIFICATION_FAILED", checks: pre.checks };
      }
      await sql`UPDATE team.game_log_batches SET status='DELTA_CERTIFIED_READY_TO_PROMOTE', certification_status='DELTA_TEAM_GAME_LOGS_CERTIFIED', certification_grade='DELTA_PASS', certification_json=${JSON.stringify(pre.checks)}, certified_at=now(), updated_at=now() WHERE batch_id=${batchId}`;
