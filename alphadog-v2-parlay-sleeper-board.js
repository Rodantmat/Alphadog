...
const VERSION = "alphadog-v2-parlay-sleeper-board-v0.5.2-control-db-only-binding";
...
async function ensureSleeperSchema(env) {
  ...
  try {
    await client.unsafe(`
CREATE TABLE IF NOT EXISTS market.sleeper_board_stage (stage_id TEXT PRIMARY KEY, batch_id TEXT, source_key TEXT, slate_date TEXT, fetched_at TIMESTAMPTZ, staged_at TIMESTAMPTZ DEFAULT now(), source_event_id TEXT, source_line_id TEXT, source_player_id TEXT, player_name TEXT, team TEXT, opponent TEXT, league TEXT, sport TEXT, source_stat_name TEXT, canonical_prop_key TEXT, line_value DOUBLE PRECISION, side TEXT, price DOUBLE PRECISION, decimal_price DOUBLE PRECISION, is_pickable INTEGER DEFAULT 0, start_time TEXT, raw_line_json JSONB, parse_status TEXT, parse_error TEXT, certification_status TEXT, created_at TIMESTAMPTZ DEFAULT now());
CREATE TABLE IF NOT EXISTS market.sleeper_board_batches (batch_id TEXT PRIMARY KEY, source_key TEXT, slate_date TEXT, fetched_at TIMESTAMPTZ, staged_at TIMESTAMPTZ, certified_at TIMESTAMPTZ, source_base_url TEXT, source_endpoint TEXT, source_http_status INTEGER, source_size_bytes INTEGER, top_level_shape TEXT, total_rows INTEGER DEFAULT 0, staged_rows INTEGER DEFAULT 0, valid_rows INTEGER DEFAULT 0, invalid_rows INTEGER DEFAULT 0, unmapped_stat_types INTEGER DEFAULT 0, certification_status TEXT, certification_reason TEXT, certification_json JSONB, promoted_at TIMESTAMPTZ, cleaned_at TIMESTAMPTZ, created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now());
CREATE TABLE IF NOT EXISTS market.sleeper_board_current (current_row_id TEXT PRIMARY KEY, batch_id TEXT, source_key TEXT, slate_date TEXT, source_event_id TEXT, source_line_id TEXT, source_player_id TEXT, player_name TEXT, team TEXT, opponent TEXT, league TEXT, sport TEXT, source_stat_name TEXT, canonical_prop_key TEXT, line_value DOUBLE PRECISION, side TEXT, price DOUBLE PRECISION, decimal_price DOUBLE PRECISION, is_pickable INTEGER DEFAULT 0, start_time TEXT, raw_line_json JSONB, row_payload_json JSONB, promoted_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now());
CREATE TABLE IF NOT EXISTS market.sleeper_board_active_batches (source_key TEXT, slate_date TEXT, active_batch_id TEXT, certification_status TEXT, row_count INTEGER DEFAULT 0, valid_rows INTEGER DEFAULT 0, activated_at TIMESTAMPTZ, updated_at TIMESTAMPTZ DEFAULT now(), PRIMARY KEY (source_key, slate_date));
CREATE TABLE IF NOT EXISTS market.schema_migrations (migration_key TEXT PRIMARY KEY, package_version TEXT NOT NULL, applied_at TIMESTAMPTZ DEFAULT now(), notes TEXT);
CREATE INDEX IF NOT EXISTS idx_sleeper_board_stage_batch ON market.sleeper_board_stage(batch_id);
CREATE INDEX IF NOT EXISTS idx_sleeper_board_stage_source_slate ON market.sleeper_board_stage(source_key, slate_date);
CREATE INDEX IF NOT EXISTS idx_sleeper_board_stage_stat ON market.sleeper_board_stage(source_stat_name, canonical_prop_key);
CREATE INDEX IF NOT EXISTS idx_sleeper_board_batches_source_slate ON market.sleeper_board_batches(source_key, slate_date);
CREATE INDEX IF NOT EXISTS idx_sleeper_board_batches_cert ON market.sleeper_board_batches(certification_status);
CREATE INDEX IF NOT EXISTS idx_sleeper_board_current_source_slate_batch ON market.sleeper_board_current(source_key, slate_date, batch_id);
CREATE INDEX IF NOT EXISTS idx_sleeper_board_current_player_prop ON market.sleeper_board_current(player_name, canonical_prop_key);`);
    await client.unsafe(
...
function toStageRow(row, batchId, fetchedAt, taxonomy, positionByPlayer) {
  ...
  return {
    ...
    team: normalizeText(row && (row.team || row.home_team)),
    opponent: normalizeText(row && (row.opponent || row.away_team)),
    league: "MLB",
...
async function clearUnpromotedSleeperStage(env) {
  const client = pgClient(env);
  try {
    await client.unsafe("DELETE FROM market.sleeper_board_stage WHERE source_key = $1", [SOURCE_KEY]);
    await client.unsafe("DELETE FROM market.sleeper_board_batches WHERE source_key = $1 AND promoted_at IS NULL", [SOURCE_KEY]);
  } finally {
    await client.end({ timeout: 1 });
  }
}
...
async function insertStageRows(env, stageRows) {
  ...
  try {
    for (const chunk of chunks) {
      await client`INSERT INTO market.sleeper_board_stage ${client(chunk, ...cols)}`;
    }
  } finally {
    await client.end({ timeout: 1 });
  }
  ...
  try {
    ...
    if (missing.length) {
      ...
      try {
        ...
        // score-prep's prepareSleeperRows grounds the calendar on raw.home_team / raw.away_team,
        // which the Sleeper lines payload does not carry. Without them every row fails calendar
        // resolution and never reaches board_prepared_current.
        ...
        if (filled || teamed) console.log(`sleeper: start_time filled ${filled}, home/away injected ${teamed}`);
      } finally {
        try { await client.end({ timeout: 2 }); } catch (_) {}
      }
    }
...
async function lastFetchAge(env) {
  ...
  const client = pgClient(env);
  try {
    const rows = await client.unsafe("SELECT MAX(fetched_at) AS last_fetch FROM market.sleeper_board_batches WHERE source_key=$1 AND promoted_at IS NOT NULL", [SOURCE_KEY]);
...
async function safeProbe(env, input = {}) {
    ...
    if (lastFetch !== null) {
      ...
      if (ageMs < FRESHNESS_BUFFER_MS) {
        return {
          ...
          certification: "PARLAY_SLEEPER_SKIPPED_WITHIN_FRESHNESS_BUFFER",
          ...
          note: "Current Sleeper board data is under 3 hours old, so the external ParlayAPI call was skipped to avoid consuming credits unnecessarily. Pass {force_refresh: true} to bypass this buffer.",
          rows_read: 0,
          rows_written: 0,
  ...
  // SCRAPER FIRST (2026-09-10). See fetchScraperBoard above.
  ...
  try {
    ...
    if (sc.ok && sc.rows.length) {
      ...
      scraperInfo = { used: true, rows: sc.rows.length, raw_legs: sc.raw_legs, age_hours: Math.round(sc.age_hours * 100) / 100, source: SCRAPER_BOARD_URL };
    } else {
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (method === "POST" && path === "/diagnostic") {
      ...
      const schema = env.HYPERDRIVE ? await validateSleeperSchema(env) : { ok: false, reason: "missing_HYPERDRIVE_binding" };
      return jsonResponse(baseIdentity(env, {
        route: "/diagnostic",
        input_echo_safe: {
