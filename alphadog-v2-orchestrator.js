const SYSTEM_VERSION = "alphadog-v2-orchestrator-v0.2.30-base-hitter-splits-source-probe";
...
function base(env, extra = {}) {
  return {
    ...
    notes: [
      ...
      "v0.2.30 processes safe system-health, exact market-source-health, exact prizepicks-github-board, exact parlay-sleeper-board source-probe, exact base-hitter-game-logs self-continuing base_backfill with stale running recovery, exact base-hitter-splits schema/source-lock probe, exact base-pitcher-game-logs base/delta continuation, exact active static workers, exact static-certifier read-only validation, and exact static-full-run backend chain only.",
...
async function processBaseHitterSplitsJob(env, row, runId, trigger) {
  ...
  const input = {
    ...
    worker_name: row.worker_name,
    trigger,
    mode: "orchestrator_exact_base_hitter_splits_source_lock_probe_dispatch",
    input_json: rowInput
  };
  ...
  const rowsWritten = Number(output && output.rows_written ? output.rows_written : 0);
  const externalCalls = Number(output && output.external_calls_performed ? output.external_calls_performed : 0);
  const certification = String((output && output.certification) || (ok ? "base_hitter_splits_source_probe_completed" : "base_hitter_splits_source_probe_failed")).slice(0, 120);
  const queueStatus = ok ? "completed" : "failed";
  const runStatus = ok ? "completed" : "failed";
  const errorCode = ok ? null : "base_hitter_splits_worker_failed";
  const errorMessage = ok ? null : String((output && (output.error || output.status)) || "Base Hitter Splits worker failed").slice(0, 900);
  ...
  await run(env.CONTROL_DB,
    "INSERT INTO control_worker_run_log (request_id, run_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, ?, ?, ?, ?, 'base_hitter_splits_source_probe_dispatch_completed', 'Orchestrator completed exact base-hitter-splits v0.1.0 source-lock probe dispatch', ?, CURRENT_TIMESTAMP)",
    row.request_id, runId, WORKER_NAME, row.job_key, ok ? "INFO" : "ERROR", JSON.stringify({ request_id: row.request_id, status: queueStatus, run_status: runStatus, certification, rows_read: rowsRead, rows_written: rowsWritten, external_calls: externalCalls, no_live_promotion: true })
  );
...
async function processOneUnlocked(env, trigger) {
  ...
  if (isBaseHitterSplitsJob(row)) {
    ...
    return {
      status: output && output.ok ? "completed_one_base_hitter_splits_job" : "failed_one_base_hitter_splits_job",
      request_id: row.request_id,
      run_id: runId,
      output
    };
  ...
  if (!isSafeTestJob(row)) {
    const output = {
      ...
      job_key: row.job_key,
      worker_name: row.worker_name,
      note: "v0.2.22 only processes safe system-health, exact market-source-health, exact prizepicks-github-board, exact parlay-sleeper-board source-probe, exact base-hitter-game-logs self-continuing base_backfill, exact base-hitter-splits source-lock probe, exact base-pitcher-game-logs base/delta continuation with bounded tick recovery, exact active static workers, exact static-certifier, and exact static-full-run jobs. Generic dispatch remains blocked. Base Hitter hot continuation uses backend waitUntil, not browser pump; cron is rescue only. Base Pitcher supports locked base promotion and delta_update retained-stage continuation; base promotion makes no MLB calls, delta uses MLB StatsAPI only after base integrity gate."
    };

    await run(env.CONTROL_DB,
      "INSERT INTO control_job_runs (run_id, request_id, chain_id, job_key, worker_name, status, data_ok, certification_status, rows_read, rows_written, external_calls, started_at, finished_at, elapsed_ms, input_json, output_json, error_code, error_message) VALUES (?, ?, ?, ?, ?, 'blocked', 0, 'blocked_safe_shell', 1, 0, 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, ?, ?, 'unsupported_job_in_v0_2_26', 'Only safe system-health, exact market-source-health, exact prizepicks-github-board, exact parlay-sleeper-board source-probe, exact base-hitter-game-logs self-continuing base_backfill, exact base-hitter-splits source-lock probe, exact base-pitcher-game-logs base/delta continuation with bounded tick recovery, exact active static workers, exact static-certifier, and exact static-full-run jobs are enabled in orchestrator v0.2.18')",
      runId, row.request_id, row.chain_id, row.job_key, row.worker_name, JSON.stringify(row), JSON.stringify(output)
    );
...
async function tick(env, trigger = "manual", maxJobs = 3) {
  ...
  try {
    ...
    const partialContinue = processed.filter(x => x.status === "partial_continue_static_full_run_job" || x.status === "partial_continue_base_hitter_game_logs_job" || x.status === "partial_continue_base_pitcher_game_logs_job").length;
