const SYSTEM_VERSION = "alphadog-v2-orchestrator-v0.2.74-base-pitcher-metrics-snapshot-prep-dispatch";
...
async function processBasePitcherMetricsJob(env, row, runId, trigger) {
  ...
  const queueStatus = partialContinue ? "pending" : (ok ? "completed" : "failed");
  const runStatus = partialContinue ? "partial_continue" : (ok ? "completed" : "failed");
  const certification = String((output && output.certification) || (ok ? "BASE_PITCHER_METRICS_V0_3_3_DISPATCH_COMPLETED" : "BASE_PITCHER_METRICS_V0_3_3_DISPATCH_FAILED"));
  const rowsRead = Number((output && output.rows_read) || 0);
  const rowsWritten = Number((output && output.rows_written) || 0);
  const externalCalls = Number((output && output.external_calls_performed) || 0);
  const cappedOutput = { ...output, processed_by_orchestrator: SYSTEM_VERSION, trigger };
  const errorCode = ok ? null : "base_pitcher_metrics_dispatch_failed";
  const errorMessage = ok ? null : String((output && (output.error || output.status)) || "Base Pitcher Metrics v0.3.3 snapshot-prep/full-stage dispatch failed").slice(0, 500);
  ...
  await run(env.CONTROL_DB,
    "INSERT INTO control_worker_run_log (request_id, run_id, worker_name, job_key, level, event_key, message, data_json, created_at) VALUES (?, ?, ?, ?, ?, 'base_pitcher_metrics_dispatch_completed', 'Orchestrator completed exact base-pitcher-metrics v0.3.3 snapshot-prep/full-stage dispatch', ?, CURRENT_TIMESTAMP)",
