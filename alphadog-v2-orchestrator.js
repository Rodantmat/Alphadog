const SYSTEM_VERSION = "alphadog-v2-orchestrator-v0.2.1-market-source-health-dispatch";
...
function base(env, extra = {}) {
  return {
    ...
    notes: [
      ...
      "v0.2.1 processes safe system-health test jobs plus exact market-source-health v0.1 dispatch only.",
      "No generic worker dispatch, no mining beyond source health, no scoring, no old production writes."
    ],
    bindings: {
...
async function processOneUnlocked(env, trigger) {
  ...
  if (!isSafeTestJob(row)) {
    const output = {
      ...
      data_ok: false,
      version: SYSTEM_VERSION,
      status: "unsupported_in_v0_2_1_safe_shell",
      job_key: row.job_key,
      worker_name: row.worker_name,
      note: "v0.2.1 only processes safe system-health test jobs and exact alphadog-v2-market-source-health / market-source-health jobs. Generic dispatch remains blocked."
    };

    await run(env.CONTROL_DB,
      "INSERT INTO control_job_runs (run_id, request_id, chain_id, job_key, worker_name, status, data_ok, certification_status, rows_read, rows_written, external_calls, started_at, finished_at, elapsed_ms, input_json, output_json, error_code, error_message) VALUES (?, ?, ?, ?, ?, 'blocked', 0, 'blocked_safe_shell', 1, 0, 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, ?, ?, 'unsupported_job_in_v0_2_1', 'Only safe system-health and exact market-source-health jobs are enabled in orchestrator v0.2.1')",
      runId, row.request_id, row.chain_id, row.job_key, row.worker_name, JSON.stringify(row), JSON.stringify(output)
    );

    await run(env.CONTROL_DB,
      "UPDATE control_job_queue SET status='blocked', finished_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP, output_json=?, error_code='unsupported_job_in_v0_2_1', error_message='Only safe system-health and exact market-source-health jobs are enabled in orchestrator v0.2.1' WHERE request_id=?",
      JSON.stringify(output), row.request_id
    );
...
async function tick(env, trigger = "manual", maxJobs = 3) {
  ...
  try {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      if (result.status === "blocked_unsupported_job" || result.status === "failed_one_market_source_health_job") break;
    }

    await releaseLock(env, owner, "IDLE");

    const completed = processed.filter(x => x.status === "completed_one_safe_test_job" || x.status === "completed_one_market_source_health_job").length;
    const blocked = processed.filter(x => x.status === "blocked_unsupported_job" || x.status === "failed_one_market_source_health_job").length;
