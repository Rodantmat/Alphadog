...
async function runMarketFullRun(env, input) {
  ...
  try {
    const result = await runMarketFullRunLocked(env, input, runId, startedAt);
    ...
    ctx.waitUntil((async () => {
      try {
        await postgres(env.HYPERDRIVE.connectionString, { max: 1, fetch_types: false, prepare: false, connect_timeout: 8 })`
          ...
          VALUES ('AUTOMATED_market_runner_run', 'Cron fired (cron: ' || ${event.cron || "unknown"} || ') but this handler is now a deliberate no-op - Cowork runs LAYER 1-4 directly instead.', 'CRON_NOOP_RETIRED', null)`;
      } catch (_) {}
    })());
  },
