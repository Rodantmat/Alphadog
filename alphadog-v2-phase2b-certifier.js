...
async function loadChunkPrerequisites(pgClient, dates, chunkRows, globalCtx) {
  ...
  // PERFORMANCE FIX (2026-07-31): these 5 queries are fully independent of each other's
  // results - confirmed by reading the code, none reference another's output. Running them
  // sequentially with individual awaits was pure wasted wall-clock time, since each round-trip
  // to Postgres carries real network latency regardless of query complexity. Parallelized via
  // Promise.all, cutting the per-chunk latency overhead roughly in proportion to how many of
  // these ran serially before - a real, measurable win on boards needing many chunks.
  const [factorCoverageRows, factorIssuesRows, readinessRows, marketCoverageRows, playerPropSummaries] = await Promise.all([
    preparedIds.length ? pgClient`SELECT coverage_key,factor_family,prepared_row_id,game_pk,mlb_player_id,canonical_prop_key,normalized_factor_lane,factor_status,factor_grade,packet_id,latest_batch_id,latest_checked_at,blocking_for_matrix,missing_reason,details_json,official_date,updated_at FROM scoring.prop_factor_coverage_current WHERE prepared_row_id = ANY(${preparedIdsLit}::text[]) AND official_date::text = ANY(${datesLit}::text[])`.catch(() => []) : Promise.resolve([]),
    preparedIds.length ? pgClient`SELECT * FROM scoring.prop_factor_issues WHERE prepared_row_id = ANY(${preparedIdsLit}::text[]) AND official_date::text = ANY(${datesLit}::text[])`.catch(() => []) : Promise.resolve([]),
    preparedIds.length ? pgClient`SELECT readiness_key,batch_id,official_date,game_pk,game_time_utc,prepared_row_id,source_key,source_row_id,projection_id,player_id,player_name,team_id,opponent_team_id,canonical_prop_key,prepared_board_relevant,pickable_safe,context_status,context_grade,hard_blocker_count,warning_count,enrichment_gap_count,available_context_count,expected_context_count,starter_context_status,lineup_context_status,player_availability_status,weather_context_status,bullpen_context_status,schedule_spot_context_status,umpire_context_status,hard_block_reasons_json,warning_reasons_json,enrichment_gaps_json,details_json,updated_at FROM context_cert.readiness_current WHERE prepared_row_id = ANY(${preparedIdsLit}::text[]) AND official_date::text = ANY(${datesLit}::text[])`.catch(() => []) : Promise.resolve([]),
    (marketBatchIds.length && preparedIds.length) ? pgClient`SELECT coverage_row_id,batch_id,slate_window_key,official_date,prepared_row_id,source_key,game_pk,resolved_mlb_player_id,canonical_prop_key,board_line_value,game_market_status,player_prop_market_status,market_context_status,coverage_grade,details_json,created_at FROM market.context_probe_coverage WHERE prepared_row_id = ANY(${preparedIdsLit}::text[]) AND batch_id = ANY(${marketBatchIdsLit}::text[])`.catch(() => []) : Promise.resolve([]),
    (marketBatchIds.length && preparedIds.length) ? pgClient`SELECT prepared_row_id, COUNT(*) AS row_count, STRING_AGG(DISTINCT source_key, ',') AS source_keys, STRING_AGG(DISTINCT source_market_key, ',') AS source_markets, STRING_AGG(DISTINCT outcome_side, ',') AS outcome_sides, MIN(line_value) AS min_line_value, MAX(line_value) AS max_line_value, MIN(price_american) AS min_price_american, MAX(price_american) AS max_price_american FROM market.context_probe_player_props WHERE prepared_row_id = ANY(${preparedIdsLit}::text[]) AND batch_id = ANY(${marketBatchIdsLit}::text[]) GROUP BY prepared_row_id`.catch(() => []) : Promise.resolve([])
  ]);
...
async function insertMatrixRows(pgClient, matrixRows, issueRows, coverageRows) {
  const matrixCols = ["matrix_id", "batch_id", "prepared_row_id", "source_line_id", "source_key", "game_pk", "official_date", "official_game_time_utc", "mlb_player_id", "player_name", "team_id", "opponent_team_id", "is_home", "canonical_prop_key", "board_line_value", "line_value", "side", "prop_side", "factor_family", "factor_packet_id", "factor_status", "market_game_context_status", "market_prop_context_status", "daily_readiness_status", "matrix_status", "matrix_grade", "blocking_for_scoring", "warning_count", "blocker_count", "missing_component_count", "matrix_payload_json", "details_json", "is_goblin", "is_demon", "more_only", "source_variant_label"];
  ...
  for (let i = 0; i < issueRows.length; i += CHUNK) await pgClient`INSERT INTO scoring.prop_matrix_issues ${pgClient(issueRows.slice(i, i + CHUNK), ...issueCols)} ON CONFLICT (issue_id) DO NOTHING`;
...
async function runMatrixBuilder(request, env, pgClient) {
  ...
  if (!complete) {
    const remainingRows = finalRemainingPrepared.length;
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "POST" && (path === "/run" || path === "/build" || path === "/matrix")) {
      ...
      try {
        const workPromise = runMatrixBuilder(request, env, pgClient);
