...
const VERSION = "alphadog-v2-daily-games-status-v0.2.1-postgres-rewire-redeploy";
...
function firstJsonValue(objects, keys) {
  for (const obj of objects || []) {
    ...
    for (const k of keys) { const v = obj[k]; if (v !== undefined && v !== null && String(v).length) return v; }
  }
  return null;
}
function canonicalSleeperEventShell(rawLineJson, rowPayloadJson) {
  ...
  return { event_id: event_id || null, canonical_event_id: canonical_event_id || null, game_date: game_date || dateOnly(commence_time) || null, commence_time: commence_time || null, home_team: home_team || null, away_team: away_team || null };
}

function base(extra = {}) {
  return {
    ok: true, data_ok: true, version: VERSION, worker_name: WORKER_NAME, job_key: JOB_KEY, scope: "Daily Schedule + Game Status only",
    source_strategy: {
      mlb_endpoint: MLB_SCHEDULE_ENDPOINT_PATH, board_tables_read: ["market.prizepicks_board_current", "market.sleeper_board_current"],
      write_tables: ["daily.game_status_batches", "daily.game_status_stage", "daily.game_status_current", "daily.game_status_outcomes", "daily.game_status_certifications"],
      ...
      allowed_enrichment: ["read-only ref.team_aliases", "read-only ref.players/ref.teams active roster context", "Sleeper raw event shell home/away/game_date enrichment", "cross-board PrizePicks player/team/game enrichment", "official MLB schedule opponent/home-away fill"]
    },
...
async function ensureSchema(pg) {
  await pg.unsafe(`
CREATE TABLE IF NOT EXISTS daily.game_status_batches (
  batch_id TEXT PRIMARY KEY, job_key TEXT, source_key TEXT, mode TEXT,
  board_rows_read INTEGER DEFAULT 0, board_relevant_dates INTEGER DEFAULT 0, board_relevant_games INTEGER DEFAULT 0,
  mlb_schedule_dates_fetched INTEGER DEFAULT 0, mlb_schedule_games_seen INTEGER DEFAULT 0,
  staged_rows INTEGER DEFAULT 0, promoted_rows INTEGER DEFAULT 0, unsafe_rows INTEGER DEFAULT 0, warning_rows INTEGER DEFAULT 0,
  certification_status TEXT, certification_grade TEXT, certification_reason TEXT, certification_json JSONB,
  started_at TIMESTAMPTZ, certified_at TIMESTAMPTZ, promoted_at TIMESTAMPTZ, cleaned_at TIMESTAMPTZ,
  created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now()
);
CREATE TABLE IF NOT EXISTS daily.game_status_stage (
  stage_id TEXT PRIMARY KEY, batch_id TEXT, source_key TEXT, board_source_key TEXT, board_batch_id TEXT, board_row_id TEXT,
  source_line_id TEXT, source_event_id TEXT, source_player_id TEXT, player_name TEXT, canonical_prop_key TEXT,
  board_team TEXT, board_opponent TEXT, board_start_time TEXT, board_slate_date TEXT, board_pickable_flag INTEGER,
  resolved_game_key TEXT, game_pk BIGINT, official_date TEXT, official_start_time_utc TEXT,
  away_mlb_team_id BIGINT, away_team_name TEXT, home_mlb_team_id BIGINT, home_team_name TEXT,
  venue_id BIGINT, venue_name TEXT, double_header TEXT, game_number INTEGER,
  abstract_game_state TEXT, coded_game_state TEXT, detailed_state TEXT, status_code TEXT, game_status_class TEXT,
  safety_status TEXT, pickable_safe INTEGER DEFAULT 0, has_started INTEGER DEFAULT 0, is_final INTEGER DEFAULT 0,
  is_postponed INTEGER DEFAULT 0, is_suspended INTEGER DEFAULT 0, is_delayed INTEGER DEFAULT 0, is_in_progress INTEGER DEFAULT 0,
  warning_flags TEXT, block_reason TEXT, match_method TEXT, source_confidence TEXT, board_mlb_start_time_delta_minutes DOUBLE PRECISION,
  source_endpoint TEXT, source_fetched_at TEXT, raw_board_json TEXT, raw_mlb_game_json TEXT,
  staged_at TIMESTAMPTZ DEFAULT now()
);
CREATE TABLE IF NOT EXISTS daily.game_status_current (
  current_key TEXT PRIMARY KEY, batch_id TEXT, source_key TEXT, board_source_key TEXT, board_batch_id TEXT, board_row_id TEXT,
  source_line_id TEXT, source_event_id TEXT, source_player_id TEXT, player_name TEXT, canonical_prop_key TEXT,
  board_team TEXT, board_opponent TEXT, board_start_time TEXT, board_slate_date TEXT, board_pickable_flag INTEGER,
  resolved_game_key TEXT, game_pk BIGINT, official_date TEXT, official_start_time_utc TEXT,
  away_mlb_team_id BIGINT, away_team_name TEXT, home_mlb_team_id BIGINT, home_team_name TEXT,
...
CREATE TABLE IF NOT EXISTS daily.game_status_outcomes (
  outcome_id TEXT PRIMARY KEY, batch_id TEXT, outcome_key TEXT, outcome_status TEXT, outcome_json JSONB, created_at TIMESTAMPTZ DEFAULT now()
);
CREATE TABLE IF NOT EXISTS daily.game_status_certifications (
  certification_id TEXT PRIMARY KEY, batch_id TEXT, certification_status TEXT, certification_grade TEXT,
  certification_reason TEXT, certification_json JSONB, certified_at TIMESTAMPTZ DEFAULT now()
);
CREATE INDEX IF NOT EXISTS idx_game_status_current_game ON daily.game_status_current(game_pk, official_date);
CREATE INDEX IF NOT EXISTS idx_game_status_current_safety ON daily.game_status_current(safety_status, pickable_safe);
CREATE INDEX IF NOT EXISTS idx_game_status_current_board ON daily.game_status_current(board_source_key, board_slate_date, board_batch_id);
CREATE INDEX IF NOT EXISTS idx_game_status_stage_batch ON daily.game_status_stage(batch_id);`);
}

async function readTeamAliases(pg) {
  const aliases = new Map();
  const rows = await pg`SELECT alias_normalized, mlb_team_id FROM ref.team_aliases WHERE active=1 AND alias_normalized IS NOT NULL AND mlb_team_id IS NOT NULL LIMIT 1000`;
...
function aliasTeamId(aliases, value) {
  ...
  if (!n) return null;
  return aliases.has(n) ? aliases.get(n) : null;
}
async function readTeamDirectory(pg) {
  const byMlbId = new Map();
  const byTeamId = new Map();
  const rows = await pg`SELECT team_id, mlb_team_id, abbreviation, full_name, short_name FROM ref.teams WHERE COALESCE(active,1)=1 AND mlb_team_id IS NOT NULL LIMIT 1000`;
  for (const r of rows) {
    const obj = { team_id: r.team_id || null, mlb_team_id: Number(r.mlb_team_id || 0) || null, abbreviation: r.abbreviation || null, full_name: r.full_name || null, short_name: r.short_name || null };
...
function mergePlayerTeam(map, key, value) {
  ...
  if (Number(existing.mlb_team_id) !== Number(value.mlb_team_id)) map.set(n, { ambiguous: true, reason: "player_name_maps_to_multiple_active_teams" });
}
async function readPlayerTeamMap(pg, teamDir) {
  const out = new Map();
  const rows = await pg`
    SELECT pa.alias_name AS alias_name, pa.alias_normalized AS alias_normalized, pa.player_id AS player_id,
      p.player_name AS player_name, p.full_name AS full_name,
      COALESCE(pa.team_id, p.current_team_id, p.primary_team_id) AS team_id,
  ...
  for (const r of rows) {
    ...
    const value = { player_id: r.player_id || null, team_id: r.team_id || teamObj.team_id || null, mlb_team_id: mlbTeamId, abbreviation: r.abbreviation || teamObj.abbreviation || null, method: "ref_player_alias_active_team" };
...
function sleeperShellTeamIds(shell, aliases) {
  ...
  return { homeId: aliasTeamId(aliases, shell.home_team), awayId: aliasTeamId(aliases, shell.away_team) };
}
function buildPrizePicksPlayerTeamMap(prizePicksRows, aliases) {
  ...
  for (const r of prizePicksRows || []) {
    ...
    map.get(name).push({ player_name: r.player_name, team: r.team, team_id: teamId, opponent: r.opponent || null, start_time: r.start_time || null, slate_date: r.slate_date || dateOnly(r.start_time) || null, source_event_id: r.source_event_id || null, board_row_id: r.board_row_id || null });
  }
  return map;
}
function applySleeperRawEventShell(row, aliases, teamDir) {
  ...
  if (currentTeamId) {
    ...
    if (oppId && !row.opponent) { row.opponent = teamAbbrFromId(teamDir, oppId); row.enriched_opponent_mlb_team_id = Number(oppId); row.enriched_opponent_method = "sleeper_raw_event_shell_opponent"; row.enrichment_flags.push("opponent_filled_from_sleeper_raw_event_shell"); }
  }
...
function applyCrossBoardPrizePicksTeam(row, ppPlayerTeamMap, aliases, teamDir) {
  if (!row.sleeper_event_shell || !row.player_name) return false;
  ...
  if (currentTeamId && Number(currentTeamId) !== Number(teamId)) { row.enrichment_flags.push("cross_board_prizepicks_player_team_conflicts_existing_team"); return false; }
  const sample = candidates[0];
  if (!row.team) row.team = teamAbbrFromId(teamDir, teamId);
  row.enriched_mlb_team_id = row.enriched_mlb_team_id || teamId;
  row.enriched_team_method = row.enriched_team_method || "cross_board_prizepicks_player_team_with_sleeper_event_shell";
  const oppId = Number(teamId) === Number(ids.homeId) ? ids.awayId : ids.homeId;
  if (!row.opponent) row.opponent = teamAbbrFromId(teamDir, oppId);
  row.enriched_opponent_mlb_team_id = row.enriched_opponent_mlb_team_id || Number(oppId);
  row.enriched_opponent_method = row.enriched_opponent_method || "cross_board_prizepicks_player_team_with_sleeper_event_shell";
  row.cross_board_prizepicks_start_time = sample.start_time || row.cross_board_prizepicks_start_time || null;
  row.cross_board_prizepicks_slate_date = sample.slate_date || dateOnly(sample.start_time) || row.cross_board_prizepicks_slate_date || null;
  row.cross_board_prizepicks_source_event_id = sample.source_event_id || row.cross_board_prizepicks_source_event_id || null;
  row.enrichment_flags.push("cross_board_prizepicks_player_game_anchor_available");
  return true;
}
...
function enrichBoardRows(rows, playerTeamMap, aliases = new Map(), teamDir = null, prizePicksRows = []) {
  ...
  for (const r of rows) {
    ...
    if (!r.team && r.player_name) {
      ...
      if (hit && hit.ambiguous) { r.enrichment_flags.push("player_team_mapping_ambiguous"); ambiguous++; }
      else if (hit && hit.mlb_team_id && hit.abbreviation) { r.team = hit.abbreviation; r.enriched_mlb_team_id = Number(hit.mlb_team_id); r.enriched_team_method = hit.method || "ref_player_active_team"; r.enrichment_flags.push("team_filled_from_ref_player_team_context"); teamFilled++; }
    }
    if (String(r.board_source_key || r.source_key || "") === "parlay_sleeper") {
      if (applyCrossBoardPrizePicksTeam(r, ppPlayerTeamMap, aliases, teamDir)) crossBoardTeamFilled++;
  ...
  for (const [eventId, ev] of events.entries()) {
    ...
    if (teamIds.length === 2) {
      ...
      for (const r of ev.rows) {
        ...
        if (!r.opponent && otherId) { r.opponent = other?.abbreviation || other?.full_name || String(otherId); r.enriched_opponent_mlb_team_id = Number(otherId); r.enriched_opponent_method = "sleeper_source_event_two_team_cluster"; r.enrichment_flags.push("opponent_filled_from_sleeper_event_team_cluster"); opponentFilled++; }
      }
    } else if (teamIds.length > 2) { eventPairAmbiguous++; for (const r of ev.rows) r.enrichment_flags.push("sleeper_event_team_cluster_ambiguous"); }
  }
...
async function readPrizePicksRows(pg) {
  ...
  return { columns: ["current_row_id", "batch_id", "source_key", "slate_date", "projection_id", "player_id", "player_name", "team", "opponent", "stat_type", "line_score", "start_time", "board_time", "end_time", "game_id", "pickable_flag", "raw_projection_json", "row_payload_json"], rows: rows.map(r => ({
    ...
    team: r.team, opponent: r.opponent, start_time: r.start_time || r.board_time || r.end_time, slate_date: r.slate_date, pickable_flag: Number(r.pickable_flag ?? 1),
...
async function readSleeperRows(pg) {
  ...
  return { columns: ["current_row_id", "batch_id", "source_key", "slate_date", "source_event_id", "source_line_id", "source_player_id", "player_name", "team", "opponent", "canonical_prop_key", "source_stat_name", "line_value", "side", "is_pickable", "start_time", "raw_line_json", "row_payload_json"], rows: rows.map(r => {
    const rawLine = r.raw_line_json;
    const payload = r.row_payload_json;
    const shell = canonicalSleeperEventShell(rawLine, payload);
    return {
      source_key: "sleeper", board_source_key: r.source_key || "sleeper", board_batch_id: r.batch_id, board_row_id: r.current_row_id || r.source_line_id,
      source_line_id: r.source_line_id || r.current_row_id, source_event_id: r.source_event_id || shell?.event_id || null, source_player_id: r.source_player_id, player_name: r.player_name,
      canonical_prop_key: r.canonical_prop_key || r.source_stat_name, team: r.team, opponent: r.opponent, start_time: r.start_time || shell?.commence_time || null,
      slate_date: r.slate_date || shell?.game_date || dateOnly(shell?.commence_time), pickable_flag: Number(r.is_pickable ?? 1),
      raw_board_json: payload || rawLine, raw_line_json: rawLine, row_payload_json: payload, sleeper_event_shell: shell, raw_row: r
    };
  }) };
}
...
async function fetchMlbSchedule(env, dates) {
  ...
  for (const d of dates) {
    ...
    try {
      const resp = await fetch(url, { headers: { "accept": "application/json", "user-agent": String(env.MLB_API_USER_AGENT || "AlphaDogV2DailyGameStatus/0.2") } });
      ...
      for (const dateRow of dateRows) {
        ...
        for (const g of games) { out.games_seen += 1; out.games.push({ ...g, __source_endpoint: url, __source_fetched_at: fetchedAt, __schedule_date: d }); }
      }
    } catch (err) { out.errors.push({ date: d, endpoint: url, error: safeString(err && err.message ? err.message : err, 600), fetched_at: fetchedAt }); }
  }
  return out;
}
...
function scheduleTeamIds(game, teamDir = null) {
  ...
  return { away, home, awayName: game?.teams?.away?.team?.name || awayRef?.full_name || null, homeName: game?.teams?.home?.team?.name || homeRef?.full_name || null, awayAbbr: awayRef?.abbreviation || null, homeAbbr: homeRef?.abbreviation || null };
}
...
function gamesForUnorderedTeamPair(games, teamId, oppId, teamDir = null) {
  ...
  return games.filter(g => {
    ...
    return (Number(t.away) === Number(teamId) && Number(t.home) === Number(oppId)) || (Number(t.home) === Number(teamId) && Number(t.away) === Number(oppId)) || (Number(t.away) === Number(oppId) && Number(t.home) === Number(teamId)) || (Number(t.home) === Number(oppId) && Number(t.away) === Number(teamId));
  });
}
...
function syntheticSleeperShellGame(row, ids, teamDir, reason = "no_official_schedule_match") {
  ...
  return {
    gamePk: null, gameDate: start, officialDate: shell.game_date || dateOnly(start) || row.slate_date || null,
    teams: { away: { team: { id: Number(ids.awayId) || null, name: awayRef?.full_name || shell.away_team || teamAbbrFromId(teamDir, ids.awayId) || null } }, home: { team: { id: Number(ids.homeId) || null, name: homeRef?.full_name || shell.home_team || teamAbbrFromId(teamDir, ids.homeId) || null } } },
    status: { abstractGameState: "Preview", codedGameState: "S", detailedState: "Scheduled", statusCode: "S" },
    __source_endpoint: "sleeper_raw_event_shell_board_context", __source_fetched_at: nowUtc(), __synthetic_board_context: true, __synthetic_reason: reason,
...
function matchGame(row, games, aliases, teamDir = null) {
  ...
  if (shell) {
    ...
    if (ids.homeId && ids.awayId) {
      ...
      if (shellPair.length === 1) return { game: shellPair[0], method: "sleeper_raw_event_shell_unordered_team_pair_official_schedule", confidence: "HIGH" };
      ...
      if (row.cross_board_prizepicks_start_time) {
        ...
        if (crossCompatible) {
          ...
          if (crossTimeHit) return { game: crossTimeHit.game, method: "sleeper_raw_event_shell_cross_board_prizepicks_official_schedule", confidence: "HIGH" };
          if (crossPair.length === 1) return { game: crossPair[0], method: "sleeper_raw_event_shell_cross_board_prizepicks_team_pair", confidence: "HIGH" };
        } else { row.enrichment_flags.push("cross_board_prizepicks_anchor_date_mismatch_ignored"); }
      }
      if (shellPair.length > 1) return { game: syntheticSleeperShellGame(row, ids, teamDir, "ambiguous_official_schedule_but_source_event_shell_complete"), method: "sleeper_raw_event_shell_context_unambiguous_board_event", confidence: "HIGH" };
  ...
  if (teamId) {
    const teamMatches = possible.filter(g => { const t = scheduleTeamIds(g, teamDir); return Number(t.away) === Number(teamId) || Number(t.home) === Number(teamId); });
    ...
    if (teamMatches.length > 1 && row.start_time) {
      ...
      if (sorted.length && sorted[0].delta <= 15 && (sorted.length === 1 || sorted[1].delta > 15)) return { game: sorted[0].g, method: "single_team_doubleheader_exact_start_time", confidence: "HIGH" };
...
function stageRow(batchId, row, match, nowMs, aliases, teamDir) {
  ...
  if (safety === "safe_pregame" && confidence !== "HIGH") { safety = "review_required"; block = confidence === "MEDIUM" ? "medium_confidence_game_mapping_review_required" : "low_confidence_game_mapping_blocked"; }
  ...
  return {
    stage_id: rid("dgs_stage"), current_key: `${row.board_source_key || row.source_key}:${row.board_row_id || row.source_line_id || rid("row")}`, batch_id: batchId, source_key: MLB_SCHEDULE_SOURCE,
    board_source_key: row.board_source_key, board_batch_id: row.board_batch_id, board_row_id: row.board_row_id, source_line_id: row.source_line_id, source_event_id: row.source_event_id,
    source_player_id: row.source_player_id, player_name: row.player_name, canonical_prop_key: row.canonical_prop_key, board_team: row.team, board_opponent: derivedOpponent,
    board_start_time: row.start_time, board_slate_date: row.slate_date || dateOnly(row.start_time), board_pickable_flag: row.pickable_flag, resolved_game_key: gameKey,
    game_pk: g && g.gamePk ? Number(g.gamePk) : null, official_date: g ? (g.officialDate || dateOnly(g.gameDate) || g.__schedule_date) : null, official_start_time_utc: g ? g.gameDate : null,
    away_mlb_team_id: t.away || null, away_team_name: t.awayName || null, home_mlb_team_id: t.home || null, home_team_name: t.homeName || null,
    venue_id: g?.venue?.id || null, venue_name: g?.venue?.name || null, double_header: g?.doubleHeader || null, game_number: g?.gameNumber || null,
    abstract_game_state: c.abstractState || null, coded_game_state: c.coded || null, detailed_state: c.detailed || null, status_code: c.statusCode || null,
    ...
    safety_status: safety, pickable_safe: safety === "safe_pregame" && confidence !== "LOW" ? 1 : 0, has_started: c.hasStarted ? 1 : 0, is_final: c.isFinal ? 1 : 0,
    is_postponed: c.isPostponed ? 1 : 0, is_suspended: c.isSuspended ? 1 : 0, is_delayed: c.isDelayed ? 1 : 0, is_in_progress: c.isInProgress ? 1 : 0,
    warning_flags: JSON.stringify([...(c.warnings || []), ...((row.enrichment_flags || []).filter(f => /ambiguous|mismatch|missing/.test(String(f))))]),
    block_reason: block, match_method: match.method, source_confidence: confidence, board_mlb_start_time_delta_minutes: c.delta, source_endpoint: g?.__source_endpoint || null,
    source_fetched_at: g?.__source_fetched_at || null, raw_board_json: safeString(row.raw_board_json || row.raw_row, 7000), raw_mlb_game_json: safeString(g || {}, 7000)
  };
}

async function insertStageRows(pg, rows) {
  ...
  for (let i = 0; i < rows.length; i += CHUNK) {
    ...
    await pg`INSERT INTO daily.game_status_stage ${pg(chunk, ...cols)}`;
  }
}
async function promoteCurrent(pg, batchId) {
  await pg`DELETE FROM daily.game_status_current`;
  await pg.unsafe(`INSERT INTO daily.game_status_current (
    ...
    SELECT board_source_key || ':' || COALESCE(board_row_id, source_line_id, stage_id),batch_id,source_key,board_source_key,board_batch_id,board_row_id,source_line_id,source_event_id,source_player_id,player_name,canonical_prop_key,board_team,board_opponent,board_start_time,board_slate_date,board_pickable_flag,resolved_game_key,game_pk,official_date,official_start_time_utc,away_mlb_team_id,away_team_name,home_mlb_team_id,home_team_name,venue_id,venue_name,double_header,game_number,abstract_game_state,coded_game_state,detailed_state,status_code,game_status_class,safety_status,pickable_safe,has_started,is_final,is_postponed,is_suspended,is_delayed,is_in_progress,warning_flags,block_reason,match_method,source_confidence,board_mlb_start_time_delta_minutes,source_endpoint,source_fetched_at,raw_board_json,raw_mlb_game_json,now(),now()
    FROM daily.game_status_stage WHERE batch_id=$1`, [batchId]);
}
...
async function runDailyGameStatus(env, input = {}) {
  ...
  try {
    ...
    await insertStageRows(pg, stagedRows);
    await promoteCurrent(pg, batchId);
    const currentCountRows = await pg`SELECT COUNT(*) AS c FROM daily.game_status_current WHERE batch_id=${batchId}`;
    const duplicateKeysRows = await pg`SELECT COUNT(*) AS c FROM (SELECT current_key FROM daily.game_status_current GROUP BY current_key HAVING COUNT(*) > 1) x`;
    const stageRowsRows = await pg`SELECT COUNT(*) AS c FROM daily.game_status_stage WHERE batch_id=${batchId}`;
    await pg`DELETE FROM daily.game_status_stage WHERE batch_id=${batchId}`;
    const stageAfterCleanRows = await pg`SELECT COUNT(*) AS c FROM daily.game_status_stage WHERE batch_id=${batchId}`;
    const currentCount = currentCountRows[0], duplicateKeys = duplicateKeysRows[0], stageRowsCount = stageRowsRows[0], stageAfterClean = stageAfterCleanRows[0];
    ...
    const certJson = { board_rows_read: boardRows.length, prizepicks_rows_read: pp.rows.length, sleeper_rows_read: sl.rows.length, board_relevant_dates: dates, mlb_schedule_fetches: schedule.fetches, mlb_schedule_errors: schedule.errors, duplicate_current_keys: Number(duplicateKeys?.c || 0), source_endpoint_template: MLB_SCHEDULE_ENDPOINT_PATH, exact_board_columns_used: { prizepicks: pp.columns, sleeper: sl.columns }, enrichment, unsafe_rules: ["blocked_started", "blocked_final", "blocked_postponed", "blocked_suspended", "blocked_missing_game_time", "blocked_ambiguous_game_mapping", "blocked_board_time_past", "review_required"] };
    await pg`INSERT INTO daily.game_status_certifications ${pg([{ certification_id: rid("dgs_cert"), batch_id: batchId, certification_status: certification, certification_grade: grade, certification_reason: "Board-focused game status refresh completed without board/scoring mutation", certification_json: JSON.stringify(certJson) }], "certification_id", "batch_id", "certification_status", "certification_grade", "certification_reason", "certification_json")}`;
    await pg`INSERT INTO daily.game_status_outcomes ${pg([{ outcome_id: rid("dgs_outcome"), batch_id: batchId, outcome_key: "daily_game_status_summary", outcome_status: certification, outcome_json: JSON.stringify(certJson) }], "outcome_id", "batch_id", "outcome_key", "outcome_status", "outcome_json")}`;
    await pg.unsafe(`UPDATE daily.game_status_batches SET board_rows_read=$1, board_relevant_dates=$2, board_relevant_games=$3, mlb_schedule_dates_fetched=$4, mlb_schedule_games_seen=$5, staged_rows=$6, promoted_rows=$7, unsafe_rows=$8, warning_rows=$9, certification_status=$10, certification_grade=$11, certification_reason=$12, certification_json=$13, certified_at=now(), promoted_at=now(), cleaned_at=now(), updated_at=now() WHERE batch_id=$14`,
      [boardRows.length, dates.length, new Set(schedule.games.map(g => g.gamePk)).size, schedule.dates_fetched, schedule.games_seen, staged, Number(currentCount?.c || 0), unsafe, warnings, certification, grade, "Certified current replacement lifecycle; no board/scoring/final writes", JSON.stringify(certJson), batchId]);
    return base({
      status: certification, certification, certification_grade: grade, batch_id: batchId, rows_read: boardRows.length, rows_written: Number(currentCount?.c || 0),
      board_rows_read: boardRows.length, prizepicks_rows_read: pp.rows.length, sleeper_rows_read: sl.rows.length, board_relevant_dates: dates.length, board_relevant_dates_list: dates,
      mlb_schedule_dates_fetched: schedule.dates_fetched, mlb_schedule_games_seen: schedule.games_seen, current_rows_promoted: Number(currentCount?.c || 0),
      stage_rows_before_clean: Number(stageRowsCount?.c || 0), stage_rows_after_clean: Number(stageAfterClean?.c || 0), unsafe_rows: unsafe, warning_rows: warnings,
      duplicate_current_keys: Number(duplicateKeys?.c || 0), external_calls_performed: schedule.fetches.length, writes_performed: staged + Number(currentCount?.c || 0),
      board_mutation_performed: false, scoring_mutation_performed: false, final_board_mutation_performed: false, enrichment_summary: enrichment,
      source_details: { endpoint_template: MLB_SCHEDULE_ENDPOINT_PATH, fetches: schedule.fetches, errors: schedule.errors },
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "GET" && (path === "/" || path === "/health")) {
      return jsonResponse(base({ status: "READY", route: path, bindings: { HYPERDRIVE: !!env.HYPERDRIVE }, exact_worker_only: true }));
    }
