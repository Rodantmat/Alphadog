...
| `lineup_surrounding_quality` | 3 declared / **1 cell** (rbis) | Code is real (2026-07-31 fix wired OBP computation), not a stub. **Backfilled 2026-08-29** using `context.history_game_lineup` (40-day coverage) + real historical OBP computed from `stats_hitter.game_logs`: pooled corr -0.0317, day-level t=-1.650, but only 7 qualifying days (≥30 legs/day) — below the 15-day sample bar. Directionally suggestive of a possible wrong sign, not yet provable. | 🟡 Real 2-prop gap; thin-but-suggestive evidence on the 1 wired prop, needs more days before any action |
