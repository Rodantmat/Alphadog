...
1. **The real open question, per the principal's direct challenge (2026-08-29)**: does any factor or combination demonstrate genuine enhancement over baseline — `corr(variant, outcome) > corr(baseline, outcome)`, day-level, real outcomes — not just "avoids destroying" baseline? Not yet answered. `opposing_pitcher_quality` and `schedule_travel_fatigue` individually do not clear this bar; `catcher_framing` is untested at this rigor due to thin per-cell samples.
2. Test `catcher_framing` alone against baseline with a lower per-cell sample threshold or over a longer window, since it couldn't be tested at the ≥200-per-cell bar used for the other two.
3. Consider whether a different combination (not just these 3) might show real enhancement — this session's "residual correlation" test (§2.2) identifies factors with information baseline lacks, but per the finding above, passing that test does not guarantee the stricter "improves baseline's own correlation with outcome" bar. Both tests may be needed together to certify a factor as a genuine enhancer.
4. What remains solid and actionable regardless: the current full ~19-factor enrichment is confirmed, repeatedly, severely destructive relative to baseline. Even without a proven enhancer yet, removing/dampening the confirmed-destructive combination is itself a defensible backtest-validated improvement over the status quo — but should be labeled honestly as "stops the bleeding," not "achieves the original enhancement mandate."
