...
|---|---|---|---|
| **`batter_quality_of_contact`** | 4/4 declared, 4/4 cells | **t = -4.43 (23 days, backward-corrected)** | 🔴 **Largest, most robust finding this session.** Strongly, consistently wrong-signed or a genuine mean-reversion trap (baseline already reflects season aggregate; xwoba/QoC deviation may mostly capture streak/luck that reverts). No backtest data existed for this factor — self-backfilled from `ref.batter_quality_of_contact_history` (Aug 5-23 snapshots) + `stats_hitter.game_logs` for shrinkage weight. |
...
| `defensive_quality_oaa` | 4/4 declared (confirmed wired in code + `relevant_prop_keys_json`), but **only 1/4 has a config cell** (hits) | Hits: sign fixed 2026-08-13, weak proxy-based magnitude (-0.006, n=36,790). Singles/doubles/hits_allowed: coded and declared relevant, but **zero cells exist → always null/missing live**, never independently validated | 🟡 Real, confirmed 3-prop gap |
