...
## NEW SESSION — 2026-07-23: Board Full Run layer (PrizePicks, Sleeper, Underdog, Score Prep) fully migrated, individually certified, chain certified end-to-end

**Read HANDOFF.md + DOS_AND_DONTS.md in full at session start, per the standing requirement.** Confirmed Board Full Run was the real, correct next priority per the roadmap.

**Real stage sequence found in the orchestrator (not guessed)**: `BOARD_FULL_RUN_STAGES` = prizepicks-github-board → parlay-sleeper-board → parlay-underdog-board → score-prep. No separate certifier stage exists in this chain — the final `validateBoardFullRunFinalGuard` function is the gate.

**All 4 workers rewired D1→Postgres, each individually certified 3/3 clean passes, per the testing protocol:**
- `alphadog-v2-prizepicks-github-board.js` — schema already existed on Postgres matching D1 exactly, straightforward rewire.
- `alphadog-v2-parlay-sleeper-board.js` — real schema mismatch found on `market.sleeper_board_current` (pre-existing Postgres table only had 11 of 24 needed columns, likely from an older dev iteration) — reconciled via `ALTER TABLE ADD COLUMN IF NOT EXISTS`. Created 4 missing lifecycle tables mirroring D1 exactly.
- `alphadog-v2-parlay-underdog-board.js` — same exact schema mismatch pattern as Sleeper, same fix.
- `alphadog-v2-score-prep.js` — biggest, most complex rewire (touches REF_DB/TEAM_DB/MARKET_DB/SCORE_DB/ARCHIVE_DB in the D1 original). `score.board_prepared_current` was a stale shadow table with only 11 of 35 needed columns — reconciled. `ref.*`/`calendar.game_calendar`/`archive.board_leg_history` were already correctly migrated by an earlier session — confirmed via direct check before assuming backfill was needed, per Rule Three.

**Real, root-caused bugs found and fixed this session (not guessed, each confirmed via live failure before/after):**
1. **PrizePicks**: a template-literal `${i+1}` placeholder-builder got silently mangled in transmission to a bare `${i+1}` (missing dollar sign) in two separate spots, producing unparameterized integer literals instead of real `$1,$2` placeholders — caused a `text = integer` Postgres error. Fixed with plain string concatenation (`"$" + (i+1)`) instead of template-literal interpolation for building placeholder strings, to avoid the same transmission risk recurring.
2. **Score Prep — real, deep debugging saga on a `CONNECTION_CLOSED` failure.** Tried three different driver/config combinations (postgres.js `prepare:false`, node-postgres, postgres.js `prepare:true`) — all failed identically, which is itself real evidence the failure wasn't about driver choice (this finding is now in DOS_AND_DONTS PART 4, written by the immediately-preceding session's own real-time diagnosis). **The actual, precise root cause, found by the NEXT session (this one) via harder evidence**: `WRITE_ROWS_PER_INVOCATION` was hardcoded to 350 — a value tuned years earlier for D1's slower per-row `batch()` semantics. Real tick evidence this session showed Postgres bulk multi-row inserts already handling 1,400 rows/tick cleanly; raising the constant to 3,000 cut the total connection-establishments per full run from ~25 down to ~3, which resolved the failure (each fresh Hyperdrive connection was a real point of intermittent failure risk — fewer of them meant less exposure, not a different fix in kind, but a much more effective dose of the same idea DOS_AND_DONTS PART 4 already pointed at). **This refines, not contradicts, PART 4's existing entry** — "workload weight" was correct in direction; "stale hardcoded chunk constant from the D1 era" is the precise, actionable form of it. Also fixed one real, separate bug found once this got far enough to reach it: a `TEXT`→`TIMESTAMPTZ`/`DATE` cast was needed explicitly in the promote step's `INSERT...SELECT` (`NULLIF(col,'')::timestamptz`), since Postgres doesn't auto-cast in that context the way SQLite implicitly did.
3. **Sleeper + Underdog — the REAL final root cause of their own, separate 20-second timeout failures (distinct from Score Prep's issue above, confirmed by testing after Score Prep's fix didn't help these two at all)**: an **N+1 query pattern** — `replaceParlaySleeperPropAliases`/`replaceParlayUnderdogPropAliases` looped one `INSERT` per alias (13-15 real aliases each run) instead of a single bulk insert. Combined with the schema-setup DDL also being issued as 12-14 separate sequential statements (same class of bug, also fixed — consolidated into one multi-statement call each, mirroring the fix already proven for Score Prep's `ensureScoreTables`), the cumulative round-trip count pushed every real run right up against the orchestrator's exact 20000ms external dispatch ceiling. **Ruled out ParlayAPI itself as the cause first, with real evidence** (their public `/v1/status` endpoint showed `sla:"ok"`, healthy, fresh data for both sources at the time of the failures) before continuing to look at our own code — this is the right order of operations: rule out the external dependency with real evidence, don't blame it by default. Fixed by switching both alias-insert loops to `postgres.js`'s bulk `sql(array, ...cols)` tagged-template helper. Both workers dropped from a full 20000ms timeout to a clean ~15.8-15.9s real completion immediately after.

**Orchestrator's own bug found and fixed (not one of the 4 workers — the shared control-plane file)**: `validateBoardFullRunFinalGuard`'s parity check was reading `env.MARKET_DB`/`env.SCORE_DB` (D1) directly — frozen, orphaned tables that hadn't been written to since before this session's migration, causing every board-full-run chain to fail its final guard even when all 4 real Postgres-backed stages passed individually. Fixed by rewiring those 7 queries to the orchestrator's own pre-existing `pgSchedule()` Hyperdrive helper (already used elsewhere in the file for scheduled-jobs Postgres reads) pointed at the real `market.*`/`score.*` tables.

**Deploy pipeline reliability bug found and fixed (real, systemic, not specific to any one worker)**: `github_mobile_deploy_workers.py`'s changed-file detection only ever diffed each commit against its own immediate parent (`git diff HEAD~1 HEAD`). Under GitHub Actions' `cancel-in-progress:false` concurrency queueing, intermediate queued runs between rapid commits get silently superseded/cancelled — and since the diff was never cumulative, whatever a cancelled run was supposed to deploy was lost forever, with no self-healing. **Real fix**: track the last successfully-deployed commit SHA in a committed marker file (`deployed_sha.txt`), diff cumulatively from that marker instead of `HEAD~1`, and update the marker (with a `[skip ci]` commit message, to avoid a redeploy loop) only after a deploy actually succeeds. Verified end-to-end: the marker file exists with the correct SHA, and the marker-write commit itself correctly did NOT trigger another "AlphaDog v2 Mobile Auto Deploy" run.

**D1 binding audit (real, exhaustive, not assumed)**: confirmed via direct grep that all 4 workers have zero functional D1 read/write remaining (only cosmetic text in comments/notes referencing old D1 table names). Found and fixed a separate, real gap: `generate_wrangler_configs.py` was still attaching the FULL 11-database D1 binding set to all 4 fully-migrated workers (an unused-but-present binding, not a functional violation, but still real "wiring" per Rule Zero's letter) — restricted their `d1_databases` config to `CONTROL_DB` only (the one allowed operational-dispatch exception), for these 4 workers specifically. Did NOT touch the same gap for other already-Hyperdrive-whitelisted workers from earlier sessions (`static-*`, `base-*`) since their own code hasn't been personally re-audited this session — flagged, not silently expanded in scope.

**Final result, verified with real data, not assumed**: Board Full Run chain passed clean end-to-end — `BOARD_FULL_RUN_CERTIFIED_PRIZEPICKS_SLEEPER_AND_SCORE_PREP_PASS`, final guard parity matching exactly (market/score/score-prep counts identical across PrizePicks, Sleeper, and total rows), 1 distinct score-prep batch (no contamination), real window dates populated. Per HANDOFF's 2-successful-full-run gate: one fully clean pass is confirmed; a second clean pass should be run before this is considered fully done, per protocol (this got interrupted mid-session by several genuine-but-messy intermediate failures from overlapping manual test triggers, which were real self-inflicted contamination from testing too many things concurrently — not a code defect — and were cleaned up directly in Postgres each time before retrying isolated).

**Next up per the roadmap**: Daily Context layer. Per HANDOFF Section 3: `daily.team_schedule_spot_current`, `daily.bullpen_availability_current`, `daily.player_availability_current`, `daily.game_weather_current` are confirmed EMPTY (real work needed, not just rewiring). `daily.lineups_current` (303 rows), `daily.probable_pitchers` (46 rows), `daily.umpire_context_current` (15 rows) already have real data — investigate what's populating those before assuming the whole `daily` schema needs backfill from zero, per Rule Three's explicit instruction to check before assuming.