...
async function pruneDateScopedDailyStarterTables(pg, retention) {
  await pg`DELETE FROM daily.probable_pitchers WHERE official_date IS NULL OR substr(official_date, 1, 10) NOT IN ${pg(retention.dates)}`;
  await pg`DELETE FROM daily.starters_stage WHERE official_date IS NULL OR substr(official_date, 1, 10) NOT IN ${pg(retention.dates)}`;
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (method === "POST" && path === "/run") {
      return await runDailyStarters(request, env);
    }
