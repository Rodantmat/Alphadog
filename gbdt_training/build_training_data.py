...
def pull_weather_umpire(d1):
    weather = pd.DataFrame(d1.query_paginated(
        "CONTEXT_DB",
        "SELECT game_pk, temperature_f, wind_speed_mph, wind_direction_cardinal, humidity_pct, "
        "roof_status, indoor_flag FROM context_history_game_weather",
        order_by="game_pk",
    ))
...
def main():
    ...
    d1 = D1Client()
    print(f"Pulling real hitter training data for season {args.season}...")
    hitter_df = build_hitter_training_table(d1, args.season)
    print(f"Real hitter training rows: {len(hitter_df)}")

    print(f"Pulling real pitcher training data for season {args.season}...")
    pitcher_df = build_pitcher_training_table(d1, args.season)
    print(f"Real pitcher training rows: {len(pitcher_df)}")
    ...
    print(f"Wrote {hitter_path} and {pitcher_path}")
