...
**Tested on `total_bases` — does not converge, reported honestly.** Using the player's own season rate as the shrinkage prior (same approach that worked for `walks`) with reasoned M estimates (300, then 700, approaching the hits value): predictions barely move (90.3 → 89.4 → 89.2) against actual 83.5 — nowhere near the improvement seen for `hits`/`singles`/`doubles` or `walks`. The likely reason: `hits`/`singles`/`doubles` used a proper granular tier-matched prior (built from a broader reference population via quantile matching, §4), while `total_bases` and `walks` substituted the player's own season average as a shortcut. That shortcut worked well enough for `walks` but not for `total_bases` — meaning `total_bases` likely needs the same granular tier-prior construction as the hits-family fix, not yet built for it, rather than a larger M. Recorded as a real negative result, not forced into a second success to pad the count.

## 23. WHAT THIS DOES NOT YET ANSWER
