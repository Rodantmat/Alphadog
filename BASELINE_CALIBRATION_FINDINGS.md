...
**Tested generalization to `hits_allowed` — does not replicate cleanly, reported honestly rather than forced.** Confirmed the same severe overconfidence (n=71, predicted 88.4, actual 66.2, -22.2pp — even worse than `pitcher_strikeouts`). But testing the analogous opponent-context mechanism (opposing lineup's real hitting rate) gave a **backwards** result: low-hit (weak) opposing lineups showed the *worst* calibration gap (-30.0), high-hit (strong) lineups showed a smaller gap (-16.3) — the opposite of the walks/pitcher_strikeouts pattern. With only 12-15 legs per extreme bucket this may be pure noise, or `hits_allowed` may have a genuinely different driver not yet identified. Recorded as a real null/contradictory result rather than reinterpreted to fit the pattern — not every prop's mechanism generalizes from the ones already confirmed.
