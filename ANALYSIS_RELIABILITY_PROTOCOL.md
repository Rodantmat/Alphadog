...
- **BOTH sides of demon/goblin-tagged rows ARE bettable.** A retracted claim on this date said demons were MORE-only, citing PrizePicks help-center text. The system's own placed-slip record refutes it: 188 real LESS legs on demon-tagged rows across 91 slips with real multipliers (e.g. `total_bases 3.5 less` x28, `home_runs 0.5 less` x36, `singles 1.5 less` x39). The API `allowed_wager_types = under_or_over` flag is CORRECT. The rule stands: `odds_type` describes the MORE side; LESS gets the complement tag. Do not re-litigate this from web sources - the placed slips are the evidence.
