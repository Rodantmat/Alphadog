...
**Standing, top-priority blocker**: ~~`context.history_game_lineup` joins to the graded board on only ~2-5% of legs~~ **RESOLVED, Session 4 (2026-08-22)** — the join was filtering on `batting_order_code`, which stopped populating on 08-05; use `lineup_slot` instead. Coverage restored.
...
**Rule B0a**: every proposed pool must state, for each prop in it, both its real **class** (fixed-threshold or tiered — verified empirically, not assumed) and its real **lane** (Standard, Goblin, or Demon). Lane is the dominant driver of real EV — see `HIGH_HIT_RATE_METHODOLOGY.md` §3 for the mechanism (the `doubles/less/0.5` "+1298.7% standard-lane" example that used to illustrate this is itself RETRACTED — it was a data-defect artifact, not a real standard-lane leg; see the retraction notice at the top of §3). A report that gives a pool's ROI without stating its lane is incomplete.
