...
**`total_bases`'s remaining +3.08pp residual, further narrowed**: pulled the current config for the other 4 factors applied to this prop. `opposing_pitcher_quality` (coefficient -0.075) carries an explicit prior validation note — *"Residual-validated -0.088 (n=67,001)"* — meaning this is a confirmed-real, previously-validated signal, not a suspect value; it should not be zeroed out or treated as a bug candidate. The remaining three (`weather_temp_altitude_pressure`, `weather_wind`, `schedule_travel_fatigue`) each have genuinely different formula structures (a 3-coefficient continuous formula, a discrete wind-direction-tiered lookup, and a travel-transition-detection formula respectively) that haven't yet been individually re-verified against current config the same way `lineup_slot` and `bullpen_fatigue` were. This is real, concrete, scoped remaining work — not an open-ended mystery — but rebuilding each one correctly (finding the exact formula, verifying it against a past validated result the way `lineup_slot`'s SD=0.0664 vs. 0.067 match confirmed correctness) is substantial enough that it's the right next step to take deliberately, one factor at a time, rather than guess at all three in one pass.
