...
|---|---|---|---|---|
| `pitches_thrown` (>87, first-ever assessment of this prop) | M=20 vs. pure season | +11.02 / -8.89 / -27.22 / -40.55 (severe) | **-1.16 / +3.82 / -6.11 / -5.62** | **Pure season rate — LOCKED. Population-prior M was catastrophically wrong here (up to -40pp); pure season rate is a dramatic, clear improvement even though tier 3/4 residuals remain real.** |
| `hits_runs_rbis` (clears 0) | M=100 vs. pure season | +12.95 / +3.82 / -1.38 / -6.81 | **+5.75 / +1.89 / -1.11 / -1.69** | **Pure season rate — LOCKED for the baseline layer. The known, separate enrichment-layer bug (Section 5 of the original master report, +0.554 average log-adjustment) is untouched by this and still needs its own dedicated fix before this prop's full pipeline is trustworthy.** |
| `hits_allowed` (>4) | M=60 vs. pure season | +15.44 / +2.31 / -6.82 / -17.36 | **+8.30 / -0.13 / -7.66 / -10.79** | **Pure season rate — LOCKED, but with a genuine, disclosed limitation: I tested the original report's blowout-stratification hypothesis directly (short starts as an early-pull proxy) and it did NOT confirm — the residual was actually *larger* for normal/deep starts than short starts in 3 of 4 tiers, the opposite of what the blowout mechanism predicts. This residual (8-11pp at the tails) looks like the same general extreme-tier imperfection seen elsewhere, not a distinct, separately-fixable blowout effect. A more thorough test using real run-support data (not just outs_recorded as a rough proxy) might tell a different story — this is a disclosed gap, not a closed question.** |
| `rfi_nrfi` | Not built | — | — | **Flagged for a decision, not locked either way.** Real, testable data exists (497 legs, 33 days, confirmed directly) — the original report's join-methodology bug is real and the data is there. But this prop was explicitly set aside per prior instruction as a deliberate scope decision, and its raw data lives in a completely different source (`context.first_inning_pitcher`, joined to game logs only in-memory during mining, never persisted) — a structurally different pipeline than all 22 other props here, which all use standard `stats_hitter`/`stats_pitcher.game_logs`. Needs an explicit decision on whether to build this out or leave it set aside, rather than assuming either way. |

## Summary: 21 of 23 props locked, one flagged for a scope decision, one (`hits_allowed`) locked with an honestly disclosed residual limitation

**The single dominant finding across this entire investigation, worth stating plainly**: population-prior-based shrinkage (the method used for the first 13 props, before the tier-cancellation risk was discovered) is not wrong for every prop, but it is a real, recurring risk that must be checked for every time. Once checked, **pure season-to-date rate — with no recency blending at all — turned out to be the better, simpler, more robust choice for 10 of the last 10 props tested** (everything from `hitter_strikeouts` onward). This wasn't the original report's hypothesis, and it wasn't my initial hypothesis either — it emerged from directly testing the skill-tier-quartile check that Gemini's adversarial review first prompted, and then confirming it generalized far beyond the one prop where it was first found.

**A pattern worth carrying into the next phase**: extreme-tier residuals (tier 1 and tier 4, i.e., the most extreme true-rate populations) persist to some degree even under pure season rate for several props (`pitcher_fantasy_score` family, `pitches_thrown`, `hits_allowed`) — roughly 5-11pp, smaller than the original problem but not zero. Multiple targeted fixes were tried for `pitcher_strikeouts` specifically (compression toward 0.5, toward the player's own prior, logit-space versions of both, dynamic variance scaling, Platt recalibration) and none fully eliminated this without breaking something else. This looks like a genuine, not-yet-solved property of extreme-tier prediction under this general approach, not a prop-specific quirk — worth real attention in a dedicated follow-up rather than being treated as fully resolved.
