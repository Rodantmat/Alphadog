...
## Independent verification confirms the deployment is correct — and one important side-finding worth recording

A separate, independent chat re-checked the deployed code directly against the report and confirmed line-for-line correctness (`nEffWindowCols`, the Kish's-formula loop, `VALIDATED_M_BY_PROP`, `PURE_SEASON_RATE_PROPS` all present exactly as described) — a genuine, valuable second check beyond this document's own post-deploy re-read.

**A real question was raised and resolved directly**: that same review initially searched a different, unrelated function (`expansionPosteriorHp`) before finding the real fix location. Checked this immediately given the real stakes — `expansionPosteriorHp` belongs to a completely separate "expansion baseline" scoring subsystem (`EXPANSION_RUNS_ALLOWED`, `EXPANSION_PITCHER_FANTASY_SCORE`, `EXPANSION_FIRST_INNING_RUNS`) whose namespace prefixes coincidentally overlap with real canonical prop names. **Confirmed via an explicit in-code comment (line 12729) that this entire HP/scoring computation stage is already documented as dead/superseded** — `rfi_nrfi`'s real live data comes from `context.first_inning_pitcher`, fed by the still-active *mining* stage of this same subsystem, not the dead scoring stage containing `expansionPosteriorHp`. This is consistent with, not in conflict with, this investigation's own fix — `rfi_nrfi` is already correctly branched to read `context.first_inning_pitcher` inside the function that was actually edited. No live conflict between two competing scoring paths exists. Noted honestly: this is confirmed by a prior session's own in-code comment, not freshly re-derived from scratch this session — reasonable to trust, not yet independently re-proven from zero.

## Continuing to fix what can still be fixed — investigating `rbis`/`hits_runs_rbis`'s enrichment bug directly

Per instruction to complete anything still fixable. Checked current config for both props: `hits_runs_rbis` has only `batter_quality_of_contact` wired (coefficient 0.5, cap 0.2) — the exact factor already confirmed to have a real bug fixed on 2026-08-19. `rbis` has three factors wired: `market_implied_total` (0.55, already confirmed negative/exclude), `schedule_travel_fatigue` (0.02/0.01, already confirmed no effect), and `lineup_surrounding_quality` (1.2, cap 0.2) — genuinely never tested by anyone, with the largest coefficient of the three.

**`hits_runs_rbis`/`batter_quality_of_contact`, tested split at the 08-19 fix date using real snapshot data (`ref.batter_quality_of_contact_history`, 17 real dates, July 5-August 30) with a leakage-free, point-in-time xwoba-deviation computation**: pre-fix (125 days) showed the factor genuinely correlating with outcomes, t=3.840 — post-fix (13 days, the current live state) showed no significant effect, t=-0.693. **This is a real, important, positive-leaning finding**: the currently-live version of this factor shows no significant effect, positive or negative, on `hits_runs_rbis` — a marked change from the severe harm originally reported. Honestly caveated: only 13 real post-fix days exist, thinner than the 130+-day standard held elsewhere in this investigation, and a magnitude-based (not just correlation-based) breakdown of those 13 days still shows the combined version's calibration gap slightly larger than baseline's in 3 of 4 predicted-probability buckets — a small, real, still-slightly-negative pattern, not a clean resolution. **Genuine improvement from "severe active harm" to "small, inconclusive, needs more days" — not yet fully closed.**

**`lineup_surrounding_quality`, built and tested for the first time ever**: using real lineup-slot data (`context.history_game_lineup`) to find each target hitter's immediately preceding lineup-slot hitter, and that hitter's real, point-in-time season on-base rate as the "surrounding quality" signal (players who need to reach base for RBI opportunities to exist) — 6,335 real legs, 31 real days (constrained by the same lineup-data window used for `lineup_slot` earlier). Applying the real config coefficient (1.2, cap 0.2): **a positive-leaning trend, t=1.387, 19/31 days (61%) — directionally promising but not reaching conventional significance on this limited sample.** Reported honestly as directional, not proven, given this is a brand-new factor with no prior validation history to compare against.

**Net finding on `rbis`/`hits_runs_rbis`**: none of the four factors actually wired to these two props (across both) currently show the severe, confirmed active harm originally reported — two are already excluded on separate confirmed grounds (`market_implied_total`, `schedule_travel_fatigue`), `batter_quality_of_contact` shows a real improvement to near-neutral post-fix (though thinly tested), and `lineup_surrounding_quality` shows a new, unproven, directionally positive signal. **This meaningfully revises, though does not fully close, the "active enrichment harm" finding** — the severe bug as originally described does not appear to be reproducible using current config and real, current data. Recommended next step: monitor `hits_runs_rbis`'s real post-deployment outcomes over the same 10-14 day forward window already planned for the baseline fix, specifically watching whether `batter_quality_of_contact`'s small residual pattern persists or resolves with more real post-fix days.

## Attempting the three genuinely never-tested factors — two real, confirmed dead ends, and why

Per instruction to continue and fix anything still fixable. Attempted `defensive_quality_oaa` for its 3 untested props (`doubles`, `singles`, `hits_allowed`) and `player_availability`.

**`defensive_quality_oaa` (doubles/singles/hits_allowed) — confirmed a genuine, hard data dead end, not a gap left for later.** No dedicated OAA/outs-above-average table exists anywhere in this database, and the existing `hits` cell's own validation note explicitly states it was built from "a simplified team-average proxy, not the actual OF/IF-weighted, batted-ball-profile-specific signal the live code computes." Checked `team.game_logs` for any real fielding/error/defensive metric to build an equivalent proxy for the other three props — none exists. **This cannot be responsibly built with data currently in this database** — not a "never got to it" gap like the others in this document, a genuine absence of any underlying signal to compute from, real or proxy.

**`player_availability` — a real, dedicated history table exists (`context.history_player_availability`, real IL/roster-status flags), but the real underlying events are too sparse to test.** The factor is categorical, not continuous: two real, defined tiers (`recent_il_return`: -0.08 penalty, `platoon_only_role`: -0.05 penalty), both showing zero real validation ever (`real_sample_size_observed=0` for both, confirmed directly in config). Built a real test for the IL-return tier: identified players flagged `injured_list_flag=1` on some date, then found their real games in the following 10 days. **Only 5 real post-flag games exist across 5 distinct players, over the table's entire 27-day real window (July 24-August 19).** This is not a computation failure — it is a genuine, confirmed data sparsity too thin for any meaningful statistical statement in either direction. `platoon_only_role` has no clear data source to identify the category at all, a further, compounding gap.

**Both are reported as real, confirmed dead ends for now — not further "remaining work" to track the same way as `lineup_surrounding_quality` was before it got built.** Closing this gap for `defensive_quality_oaa` would require new, dedicated defensive-metrics data collection (real OAA/outs-above-average tracking) that doesn't currently exist in this system. Closing it for `player_availability` would require either much more real IL-transition history to accumulate over time, or a different, more frequent event to test (the underlying real event — a player returning from the IL — is simply rare in any 27-day window, and no amount of clever computation increases how many real IL-return events occurred).

## Updated final status after this pass

**Everything genuinely actionable with current, real data has now been attempted.** What remains open, in order of how likely more work could still move it:
1. **The 10-14 day forward validation window** — for both the baseline fix and the `hits_runs_rbis`/`batter_quality_of_contact` finding — needs real days to pass, not more analysis.
2. **`lineup_surrounding_quality`** — directionally positive (t=1.387) but unproven; would benefit from re-testing once more real lineup-history data accumulates past the current 31-day window.
3. **`defensive_quality_oaa`'s 3 untested props and `player_availability`** — genuine, confirmed dead ends requiring new data collection infrastructure, not further backtesting.
4. **The final production-code diff**, immediately before any further live change beyond what's already deployed.

**No further backtest-only work remains that could meaningfully move any of these forward right now.** This investigation has reached the point where real, elapsed time — either for new data to accumulate or for the deployed fix's own real outcomes to materialize — is the genuine, remaining constraint, not further analysis effort.
