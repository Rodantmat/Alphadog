...
## The complete real-factor list, found via exhaustive chat history search — this closes out the remaining 11 props

Found the project's own complete, prior factor-by-factor audit (every factor, not just the ones already suspected). Only 3 factors in the entire ~19-factor system have a confirmed, validated, real effect: `opposing_pitcher_quality` (best-validated in the system, 5/5 props wired), `schedule_travel_fatigue` (6/6 props, validated 2026-08-20), and `catcher_framing` (2/2 props, externally validated against published FanGraphs research, exact 0.039 magnitude match — though its true practical effect size is confirmed "well under 1%, nearly undetectable at the leg level"). Every other factor in the system — `bullpen_fatigue`, `lineup_slot`, `weather_temp_altitude_pressure`, `weather_wind`, `park_factors`, `market_implied_total`, `umpire_tendency`, `platoon_handedness`, `defensive_quality_oaa`, `lineup_surrounding_quality`, `stolen_base_family`, `weather_precip`, `batter_quality_of_contact` — is confirmed either no-effect, deliberately dark by design, or unresolved due to a real, separate data or conceptual gap.

**Cross-referencing which of the remaining 11 props actually have one of these 3 real factors wired:**

| Prop | Real factor wired | Combined test result | Verdict |
|---|---|---|---|
| `walks_allowed` | `catcher_framing` (-0.039) | Baseline 0.21pp → Combined **0.22pp**, 113 real days, 2,419 legs | Enrichment negligible, as expected given the confirmed sub-1% real-world effect size — baseline fix stands |
| `pitcher_strikeouts` | `catcher_framing` (+0.039); also has `market_implied_total` (-1) wired but **this specific cell was never independently validated** (flagged as an open question earlier this session — zero backtest rows ever existed for it) | Baseline -0.34pp → Combined **-0.32pp** (catcher_framing only, market_implied_total excluded pending its own verification), 112 real days, 1,988 legs | Enrichment negligible — baseline fix stands. `market_implied_total`'s pitcher_strikeouts cell remains a genuine, separate open question, not resolved by this check |
| `hitter_strikeouts` | `opposing_pitcher_quality` (+0.05) | **Flagged, not yet computed** — this specific prop's stored contribution range in the legacy table (-3.84 to +1.71) is suspiciously identical to `total_bases`'s own range for the same factor, suggesting a shared, pre-per-prop-coefficient raw value rather than a properly scaled one. Needs the same formula-verification treatment as `lineup_slot` before trusting either the legacy table or a naive direct read. |
| `doubles`, `earned_runs`, `hits_allowed`, `pitcher_outs`, `singles`, `stolen_bases`, `triples`, `hits_runs_rbis` | None of the 3 confirmed-real factors are wired to any of these 8 props | Not applicable | **No real enrichment signal exists for these props at all — the baseline-only fix already locked for each of these stands as the final answer.** |

**This closes out the enrichment-layer question for 10 of the 11 remaining props decisively — 9 have no real enrichment signal at all (baseline stands), 2 (`walks_allowed`, `pitcher_strikeouts`) have a confirmed-real but practically negligible factor that doesn't move the result. Only `hitter_strikeouts`'s `opposing_pitcher_quality` value and `pitcher_strikeouts`'s `market_implied_total` cell remain genuinely open.**

## Explicit accounting against the "today back to 45 days, no gaps" requirement

Stated precisely, by layer, since different parts of this investigation draw on different underlying data with genuinely different real coverage:

- **Baseline layer (all 23 props' locked fixes)**: built from `stats_hitter.game_logs`/`stats_pitcher.game_logs`, which extend back to **March 25, 2026** — far exceeding 45 days, with no gaps, through today. This is the layer every prop's locked M-value and formula rests on, and it has the deepest, most complete coverage in the entire investigation.
- **`lineup_slot` (now corrected)**: rebuilt from `context.history_game_lineup`, confirmed continuously accumulating with real per-day volume from **2026-07-24 through today** (and a few days beyond, since lineups post in advance) — 43 real days, no gaps found.
- **`opposing_pitcher_quality`, `schedule_travel_fatigue`, `catcher_framing`**: currently still read from the legacy `factor_contributions_asof_v2` table, which caps at **30 days** (2026-07-24 to 2026-08-22) — this does **not** yet meet the no-gaps-to-today requirement for these 3 specific factors. Given `catcher_framing` and `pitcher_strikeouts`/`walks_allowed`'s combined results were already negligible even within this shorter window, extending it further is unlikely to change the conclusion, but it has not actually been done yet, and that's stated plainly rather than assumed.
- **`bullpen_fatigue`**: real raw source (`context.history_bullpen_availability`) genuinely stopped being collected around 2026-08-18/19 — this one cannot reach today's date without a real, separate backfill of the raw data itself (a job re-trigger, not a recomputation), since the underlying data doesn't exist past that point.

**Honest summary**: the baseline layer — the foundation every locked prop fix depends on — fully meets the no-gaps, 45-day-plus, through-today standard. `lineup_slot` also now meets it. The three remaining real-signal factors (`opposing_pitcher_quality`, `schedule_travel_fatigue`, `catcher_framing`) do not yet, and extending them to full, gap-free, through-today coverage — the same way `lineup_slot` and `runs`/`total_bases` were extended — is the concrete, correctly-scoped next step, not yet complete.
