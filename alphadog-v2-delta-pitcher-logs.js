...
async function runProbe(env, input) {
  ...
  if (!fixturesRes.ok || !Array.isArray(fixturesRes.json)) {
    return { ok: true, data_ok: false, version: VERSION, worker_name: LOGICAL_WORKER_NAME, job_key: JOB_KEY, status: "fixtures_call_failed", certification: "ODDSPAPI_PROBE_FIXTURES_FAILED", http_status: fixturesRes.http_status, error: fixturesRes.error || "non_array_response", raw_len: fixturesRes.raw_len || 0, raw_text_safe: fixturesRes.raw_text_safe || null, account_check: { http_status: accountRes.http_status, ok: accountRes.ok, raw_preview: accountRes.raw_text_safe }, sports_check: { http_status: sportsRes.http_status, ok: sportsRes.ok, raw_preview: sportsRes.raw_text_safe, is_array: Array.isArray(sportsRes.json) }, external_calls_performed: 3, no_scoring: true, no_ranking: true, no_final_board: true, timestamp_utc: nowUtc() };
  }
