...
const VERSION = "alphadog-v2-base-pitcher-splits-postgres-v1.0.0-per-player-fetch";
const JOB_KEY = "base-pitcher-splits";
const DATA_FEED_KEY = "pitcher_splits";
const SOURCE_KEY = "mlb_statsapi_people_statsplits_pitching_v0_1_0";
const LOCKED_SOURCE_ENDPOINT_PATTERN = "/people/{playerId}/stats?stats=statSplits&group=pitching&season={season}&sitCodes=vl,vr";
...
function asFloat(v, fallback = null) { const n = Number(v); return Number.isFinite(n) ? n : fallback; }
...
async function ensureSchema(sql) {
  await sql`
    CREATE TABLE IF NOT EXISTS stats_pitcher.splits_stage (
      ...
      season INTEGER,
      split_key TEXT,
      innings_pitched DOUBLE PRECISION, batters_faced INTEGER, hits_allowed INTEGER, walks_allowed INTEGER, strikeouts INTEGER,
      avg_against TEXT, obp_against TEXT, slg_against TEXT, ops_against TEXT,
      raw_json JSONB,
      ingestion_mode TEXT,
      source_key TEXT,
  ...
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS stats_pitcher.splits_batches (
  ...
  `;
  await sql`CREATE UNIQUE INDEX IF NOT EXISTS idx_pitcher_splits_pk ON stats_pitcher.splits (split_id)`;
  await sql`CREATE INDEX IF NOT EXISTS idx_pitcher_splits_batch ON stats_pitcher.splits (batch_id)`;
  await sql`CREATE INDEX IF NOT EXISTS idx_pitcher_splits_stage_batch ON stats_pitcher.splits_stage (batch_id)`;
  return { ok: true };
}
...
async function fetchPlayerSplits(env, playerId, season, fetchTimeoutMs) {
  ...
  const endpoint = `${baseUrl}/people/${playerId}/stats?stats=statSplits&group=pitching&season=${season}&sitCodes=vl,vr`;
  ...
  for (const statBlock of statsArr) {
    ...
    for (const sp of splits) {
      ...
      rows.push({
        ...
        innings_pitched: asFloat(st.inningsPitched, null), batters_faced: asInt(st.battersFaced, null),
        hits_allowed: asInt(st.hits, null), walks_allowed: asInt(st.baseOnBalls, null), strikeouts: asInt(st.strikeOuts, null),
        avg_against: asText(st.avg, null), obp_against: asText(st.obp, null), slg_against: asText(st.slg, null), ops_against: asText(st.ops, null),
        raw_json: sp
      });
    }
  }
...
async function getPlayerUniverse(sql) {
  const rows = await sql`SELECT DISTINCT player_id FROM stats_pitcher.game_logs ORDER BY player_id`;
  return rows.map(r => Number(r.player_id));
}
async function getAffectedPlayerUniverse(sql) {
  const rows = await sql`SELECT DISTINCT player_id FROM stats_pitcher.game_logs WHERE game_date >= (CURRENT_DATE - ${DELTA_AFFECTED_LOOKBACK_DAYS}::int) ORDER BY player_id`;
  return rows.map(r => Number(r.player_id));
}
...
async function insertStageRowsBulk(sql, batchId, runId, mode, sourceSeason, rows) {
  ...
  const values = dedupedRows.map(r => ({
    ...
    innings_pitched: r.innings_pitched, batters_faced: r.batters_faced, hits_allowed: r.hits_allowed, walks_allowed: r.walks_allowed, strikeouts: r.strikeouts,
    avg_against: r.avg_against, obp_against: r.obp_against, slg_against: r.slg_against, ops_against: r.ops_against,
    raw_json: JSON.stringify(r.raw_json),
    ingestion_mode: mode, source_key: SOURCE_KEY, source_confidence: "SOURCE_LOCKED_STATSAPI_STATSPLITS_PITCHING",
  ...
  for (let i = 0; i < values.length; i += CHUNK) {
    ...
    await sql`
      INSERT INTO stats_pitcher.splits_stage ${sql(slice, "stage_id","batch_id","run_id","player_id","season","split_key","innings_pitched","batters_faced","hits_allowed","walks_allowed","strikeouts","avg_against","obp_against","slg_against","ops_against","raw_json","ingestion_mode","source_key","source_confidence","source_endpoint","source_season","row_status")}
      ON CONFLICT (stage_id) DO UPDATE SET
        innings_pitched=excluded.innings_pitched, batters_faced=excluded.batters_faced, hits_allowed=excluded.hits_allowed, walks_allowed=excluded.walks_allowed, strikeouts=excluded.strikeouts,
...
async function promoteStageRowsChunk(sql, batchId, grade, limit) {
  ...
  const result = await sql`
    WITH batch_rows AS (
      SELECT stage_id, player_id, season, split_key, innings_pitched, batters_faced, hits_allowed, walks_allowed, strikeouts,
             avg_against, obp_against, slg_against, ops_against, raw_json,
             batch_id, run_id, ingestion_mode, source_key, source_confidence, source_season
      FROM stats_pitcher.splits_stage
      WHERE batch_id=${batchId} AND row_status != 'promoted'
      LIMIT ${safeLimit}
    ),
    ins AS (
      INSERT INTO stats_pitcher.splits (
        split_id, player_id, season, split_key, innings_pitched, batters_faced, hits_allowed, walks_allowed, strikeouts,
        avg_against, obp_against, slg_against, ops_against, source_key, raw_json,
      ...
      SELECT
        ...
        b.player_id, b.season, b.split_key, b.innings_pitched, b.batters_faced, b.hits_allowed, b.walks_allowed, b.strikeouts,
        b.avg_against, b.obp_against, b.slg_against, b.ops_against, b.source_key, b.raw_json,
      ...
      ON CONFLICT (split_id) DO UPDATE SET
        ...
        batch_id=COALESCE(stats_pitcher.splits.batch_id, excluded.batch_id),
        run_id=COALESCE(stats_pitcher.splits.run_id, excluded.run_id),
        ingestion_mode=COALESCE(stats_pitcher.splits.ingestion_mode, excluded.ingestion_mode),
        certification_status=COALESCE(stats_pitcher.splits.certification_status, excluded.certification_status),
        certification_grade=COALESCE(stats_pitcher.splits.certification_grade, excluded.certification_grade),
        promoted_at=now(), updated_at=now()
      RETURNING 1
    ),
    upd AS (
      UPDATE stats_pitcher.splits_stage SET row_status='promoted', updated_at=now()
  ...
  const remainingRows = await sql`SELECT COUNT(*)::int AS c FROM stats_pitcher.splits_stage WHERE batch_id=${batchId} AND row_status != 'promoted'`;
...
async function cleanStageRowsChunk(sql, batchId, limit) {
  ...
  const res = await sql`DELETE FROM stats_pitcher.splits_stage WHERE ctid IN (SELECT ctid FROM stats_pitcher.splits_stage WHERE batch_id=${batchId} LIMIT ${safeLimit})`;
  const cleaned = Number(res.count || 0);
  const remainRows = await sql`SELECT COUNT(*)::int AS c FROM stats_pitcher.splits_stage WHERE batch_id=${batchId}`;
...
async function acquireBatchLock(sql, batchId, owner, staleSeconds) {
  const rows = await sql`SELECT locked_by, lock_acquired_at, lock_expires_at FROM stats_pitcher.splits_batches WHERE batch_id=${batchId} LIMIT 1`;
  ...
  await sql`UPDATE stats_pitcher.splits_batches SET locked_by=${owner}, lock_acquired_at=now(), lock_expires_at=now() + make_interval(secs => ${staleSeconds}), updated_at=now() WHERE batch_id=${batchId}`;
  return { ok: true, owner };
}
async function releaseBatchLock(sql, batchId, owner) {
  await sql`UPDATE stats_pitcher.splits_batches SET locked_by=NULL, lock_acquired_at=NULL, lock_expires_at=NULL, updated_at=now() WHERE batch_id=${batchId} AND locked_by=${owner}`;
}
async function buildPrePromotionChecks(sql, batchId) {
  const dupRows = await sql`SELECT COUNT(*)::int AS c FROM (SELECT player_id, season, split_key, COUNT(*) AS n FROM stats_pitcher.splits_stage WHERE batch_id=${batchId} GROUP BY player_id, season, split_key HAVING COUNT(*) > 1) sub`;
  const missingRows = await sql`SELECT COUNT(*)::int AS c FROM stats_pitcher.splits_stage WHERE batch_id=${batchId} AND (player_id IS NULL OR season IS NULL OR split_key IS NULL)`;
  const totalRows = await sql`SELECT COUNT(*)::int AS c FROM stats_pitcher.splits_stage WHERE batch_id=${batchId}`;
...
async function getOrCreateBaseBackfillState(env, sql, input) {
  const batchId = "pitcher_splits_base_backfill_singleton";
  const runId = asText(input.run_id, rid("run_base_pitcher_splits_backfill"));
  ...
  await sql`
    INSERT INTO stats_pitcher.splits_batches (batch_id, run_id, worker_name, worker_version, mode, status, data_feed_key, source_key, source_endpoint, source_season, promote_rows_per_tick, certification_status, source_confidence, notes, started_at, updated_at)
    VALUES (${batchId}, ${runId}, ${WORKER_NAME}, ${VERSION}, 'base_backfill', 'BASE_BACKFILL_MINING', ${DATA_FEED_KEY}, ${SOURCE_KEY}, ${LOCKED_SOURCE_ENDPOINT_PATTERN}, ${sourceSeason}, ${tickConfig.promote_rows_per_tick}, 'not_certified', 'SOURCE_LOCKED_STATSAPI_STATSPLITS_PITCHING', 'Postgres-native per-player pitching splits build', now(), now())
    ON CONFLICT (batch_id) DO NOTHING
  `;
  const rows = await sql`SELECT * FROM stats_pitcher.splits_batches WHERE batch_id=${batchId} LIMIT 1`;
...
async function runMiningTick(env, sql, batch, mode) {
  ...
  if (batch.players_total && Number(batch.players_total) > 0) {
    ...
    universe = await universeFn(sql);
    await sql`UPDATE stats_pitcher.splits_batches SET players_total=${universe.length}, updated_at=now() WHERE batch_id=${batchId}`;
  }
  ...
  await sql`UPDATE stats_pitcher.splits_batches SET cursor_player_id=${lastPid}, players_processed=players_processed + ${chunk.length}, rows_staged=rows_staged + ${ins.inserted}, updated_at=now() WHERE batch_id=${batchId}`;
...
async function runBaseBackfillTick(env, sql, input) {
  ...
  try {
    const freshRows = await sql`SELECT * FROM stats_pitcher.splits_batches WHERE batch_id=${batchId} LIMIT 1`;
    ...
    if (status === "BASE_BACKFILL_MINING") {
      ...
      await sql`UPDATE stats_pitcher.splits_batches SET status=${nextStatus}, updated_at=now() WHERE batch_id=${batchId}`;
    ...
    if (status === "BASE_BACKFILL_STAGED_READY_FOR_CERTIFICATION") {
      ...
      if (!pre.pass) {
        await sql`UPDATE stats_pitcher.splits_batches SET status='CERTIFICATION_FAILED', certification_status='BASE_PITCHER_SPLITS_CERTIFICATION_FAILED', certification_grade='BASE_FAIL', certification_json=${JSON.stringify(pre.checks)}, updated_at=now() WHERE batch_id=${batchId}`;
        return { ok: true, data_ok: false, mode: "base_backfill", batch_id: batchId, status: "CERTIFICATION_FAILED", checks: pre.checks };
      }
      await sql`UPDATE stats_pitcher.splits_batches SET status='BASE_BACKFILL_CERTIFIED_READY_TO_PROMOTE', certification_status='BASE_PITCHER_SPLITS_CERTIFIED', certification_grade='BASE_PASS', certification_json=${JSON.stringify(pre.checks)}, certified_at=now(), updated_at=now() WHERE batch_id=${batchId}`;
    ...
    if (status === "BASE_BACKFILL_CERTIFIED_READY_TO_PROMOTE" || status === "BASE_BACKFILL_PROMOTING") {
      ...
      await sql`UPDATE stats_pitcher.splits_batches SET status=${nextStatus}, rows_promoted=rows_promoted + ${promoted.promoted_this_tick}, promoted_at=COALESCE(promoted_at, now()), updated_at=now() WHERE batch_id=${batchId}`;
    ...
    if (status === "BASE_BACKFILL_PROMOTED_READY_TO_CLEAN" || status === "BASE_BACKFILL_CLEANING") {
      ...
      await sql`UPDATE stats_pitcher.splits_batches SET status=${nextStatus}, cleaned_at=CASE WHEN ${cleaned.cleanup_done} THEN now() ELSE cleaned_at END, finished_at=CASE WHEN ${cleaned.cleanup_done} THEN now() ELSE finished_at END, updated_at=now() WHERE batch_id=${batchId}`;
...
async function getLockedBaseIntegrity(sql) {
  const batchRows = await sql`SELECT batch_id, status, rows_promoted, cleaned_at FROM stats_pitcher.splits_batches WHERE batch_id=${LOCKED_BASE_BATCH_ID} LIMIT 1`;
  const batch = batchRows[0] || null;
  const liveRows = await sql`SELECT COUNT(*)::int AS c FROM stats_pitcher.splits WHERE batch_id=${LOCKED_BASE_BATCH_ID}`;
  const dupRows = await sql`SELECT COUNT(*)::int AS c FROM (SELECT player_id, season, split_key, COUNT(*) AS n FROM stats_pitcher.splits WHERE batch_id=${LOCKED_BASE_BATCH_ID} GROUP BY player_id, season, split_key HAVING COUNT(*) > 1) sub`;
...
async function getOrCreateDeltaState(env, sql, input) {
  ...
  const batchId = `pitcher_splits_delta_update_batch_${today}`;
  const existingRows = await sql`SELECT * FROM stats_pitcher.splits_batches WHERE batch_id=${batchId} LIMIT 1`;
  if (existingRows[0]) return { is_new: false, batch: existingRows[0] };
  const runId = asText(input.run_id, rid("run_delta_pitcher_splits"));
  const sourceSeason = DEFAULT_SOURCE_SEASON;
  const tickConfig = await getWorkerTickConfig(sql, WORKER_NAME, DEFAULT_CHUNK_SIZE_PLAYERS, DEFAULT_MAX_TICK_RUNTIME_MS, DEFAULT_PROMOTE_ROWS_PER_TICK);
  await sql`
    INSERT INTO stats_pitcher.splits_batches (batch_id, run_id, worker_name, worker_version, mode, status, data_feed_key, source_key, source_endpoint, source_season, promote_rows_per_tick, certification_status, notes, started_at, updated_at)
...
async function runDeltaUpdateTick(env, sql, input) {
  ...
  try {
    ...
    if (status === "DELTA_STAGED_READY_FOR_CERTIFICATION") {
      const pre = await buildPrePromotionChecks(sql, batchId);
      if (!pre.pass) {
        await sql`UPDATE stats_pitcher.splits_batches SET status='CERTIFICATION_FAILED', certification_status='DELTA_PITCHER_SPLITS_CERTIFICATION_FAILED', certification_grade='DELTA_FAIL', certification_json=${JSON.stringify(pre.checks)}, updated_at=now() WHERE batch_id=${batchId}`;
        return { ok: true, data_ok: false, mode: "delta_update", batch_id: batchId, status: "CERTIFICATION_FAILED", checks: pre.checks };
      }
      await sql`UPDATE stats_pitcher.splits_batches SET status='DELTA_CERTIFIED_READY_TO_PROMOTE', certification_status='DELTA_PITCHER_SPLITS_CERTIFIED', certification_grade='DELTA_PASS', certification_json=${JSON.stringify(pre.checks)}, certified_at=now(), updated_at=now() WHERE batch_id=${batchId}`;
