...
function baseIdentity(env) {
  ...
  return {
    ...
    source_lock: {
      ...
      daily_affected_pitcher_refresh_enabled: true
    },
    hard_blocks: {
...
function addDays(dateStr, days) {
  const d = new Date(String(dateStr || todayUtc()) + "T00:00:00Z");
...
async function refreshOneAffectedPitcherSplitPlayer(env, player, baseBatch, runId, sourceSnapshotDate) {
  ...
  for (const split of splits) {
    const row = makeDeltaStageRow(split, player, SOURCE_SEASON, baseBatch.batch_id, runId, endpoint, sourceSnapshotDate);
    if (!row.split_code || !["vs_left", "vs_right"].includes(row.split_code)) continue;
    row.ingestion_mode = "delta_daily_affected_pitcher_refresh_retained";
    row.certification_status = "DELTA_PITCHER_SPLITS_DAILY_AFFECTED_REFRESH_CERTIFIED_PROMOTED_RETAINED";
    row.certification_grade = "DELTA_REFRESH_PASS";
    row.source_confidence = "SOURCE_LOCKED_STATSAPI_STATSPLITS_PITCHING_SITCODES_VL_VR_DAILY_AFFECTED_REFRESH";
    row.row_status = "delta_daily_affected_pitcher_refresh_retained";
    await insertStageRow(env, row);
    rowsStaged++;
    row.ingestion_mode = "delta_daily_affected_pitcher_refresh_promoted_retained";
    await upsertPitcherSplitLiveRow(env, row, "SOURCE_LOCKED_STATSAPI_STATSPLITS_PITCHING_SITCODES_VL_VR_DAILY_AFFECTED_REFRESH");
    rowsPromoted++;
...
async function refreshDailyAffectedPitcherSplits(env, baseBatch, input, liveChecks, latestCompleteDate, coveredGameDate) {
  const runId = input.run_id || rid("run_delta_pitcher_splits_daily_affected_refresh");
  const cursor = await first(env.STATS_PITCHER_DB, "SELECT * FROM pitcher_split_cursor WHERE cursor_key='base_pitcher_splits_daily_affected_refresh_cursor' AND status='PARTIAL_CONTINUE_DELTA_PITCHER_SPLITS_DAILY_AFFECTED_REFRESH' ORDER BY datetime(updated_at) DESC LIMIT 1");
  let cursorJson = {};
  if (cursor && cursor.cursor_json) { try { cursorJson = JSON.parse(cursor.cursor_json || '{}'); } catch (_) { cursorJson = {}; } }
  const afterDate = dateOnlyUtc(cursorJson.covered_game_date_before) || dateOnlyUtc(coveredGameDate) || await inferPitcherSplitsCoveredGameDate(env, liveChecks, baseBatch);
  ...
  const affectedPlayers = await readAffectedPitcherSplitPlayers(env, afterDate, throughDate);
  ...
  for (const player of players) {
    const out = await refreshOneAffectedPitcherSplitPlayer(env, player, baseBatch, runId, throughDate);
  ...
  const liveAfter = await currentLiveIntegrity(env, baseBatch.batch_id);
  const checks = { locked_base_batch_id: baseBatch.batch_id, covered_game_date_before: afterDate, latest_complete_game_date: throughDate, affected_player_count: affectedPlayers.length, players_processed_this_tick: players.length, cursor_offset_before: startOffset, cursor_offset_after: nextOffset, rows_staged_this_tick: rowsStaged, rows_promoted_this_tick: rowsPromoted, external_calls_this_tick: externalCalls, no_full_sweep: true, no_full_pitcher_universe_refresh: true, source_errors_this_tick: sourceErrors, no_data_this_tick: noData, live_rows_after: liveAfter.live_rows, distinct_live_keys_after: liveAfter.distinct_live_keys, duplicate_live_keys: liveAfter.duplicate_live_keys, invalid_split_code: liveAfter.invalid_split_code, missing_raw_json: liveAfter.missing_raw_json, min_source_snapshot_date_after: liveAfter.min_source_snapshot_date, max_source_snapshot_date_after: liveAfter.max_source_snapshot_date, outcomes };
  await run(env.STATS_PITCHER_DB, `INSERT OR REPLACE INTO pitcher_split_cursor (cursor_key,batch_id,run_id,mode,status,source_season,source_snapshot_date,current_player_id,current_player_offset,players_total,players_processed,requests_done,next_run_after,last_error,cursor_json,updated_at)
...
async function runDeltaNoopRestoreScopedRepairGate(env, input) {
  ...
  if (exists) {
    ...
    const checks = { locked_base_batch_id: baseBatch.batch_id, expected_key: expectedKey, live_rows: liveChecks.live_rows, expected_rows: expectedRows, distinct_live_keys: liveChecks.distinct_live_keys, duplicate_live_keys: liveChecks.duplicate_live_keys, source_snapshot_date_today_utc: todayUtc(), covered_game_date: coveredGameDate, no_mlb_calls: true, no_full_sweep: true, no_live_mutation: true, retained_registry_available: Boolean(registry), does_not_require_balanced_splits: true, does_not_use_outcome_rows_staged_for_physical_truth: true, latest_complete_game_date_check: latestCompleteCheck };
    await run(env.STATS_PITCHER_DB, `INSERT OR REPLACE INTO pitcher_split_cursor (cursor_key,batch_id,run_id,mode,status,source_season,source_snapshot_date,current_player_id,current_player_offset,players_total,players_processed,requests_done,next_run_after,last_error,cursor_json,updated_at)
