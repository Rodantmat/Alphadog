...
## 3a. Rule B0b and B0c — added 2026-08-22, directly triggered by a real coworker over-claim

A 2026-08-22 coworker session reported a new Sleeper pool (`rbis`+`walks`+`home_runs`+`hits`+`hits_runs_rbis/more`, 6-pick Power) at **+216.5% ROI, 0 of 28 LODO folds negative**. Manually decomposed the same day using each prop's own real live per-leg price (pulled from `market.sleeper_board_current`, not assumed): real per-leg rates ranged from **1.152x (`home_runs`) to 2.094x (`hits_runs_rbis`)** — an 82% spread. Re-built the exact same pool pricing each leg by its own real rate instead of a blended assumption: **+32.9%**, with 15 of 28 real days negative (concentrated in an early cold-start window before enough props had data). The reported number was a real, ~6.5x overstatement — not fabricated, just priced wrong.

**Rule B0b — Combined pools must be priced per-prop, never blended.** Any pool spanning more than one (prop, side, tier) bucket must pull each bucket's real per-leg price independently (live moneyline for Sleeper via `market.sleeper_board_current.raw_line_json`, unwrapped with `(raw_line_json #>> '{}')::jsonb->>'field'`; the granular table for Goblin/Demon) and construct slips using each leg's own real rate — never a single averaged or assumed multiplier applied across the whole pool. Report the real spread between the cheapest and most expensive leg-type in the pool; if it exceeds ~20%, state explicitly that blended pricing would be materially wrong and must not be used.

**Rule B0c — Ranked-greedy results must report tie-break sensitivity.** This same 2026-08-22 session independently found `score_0_100` has no predictive power inside a homogeneous bucket (whole-board r=+0.414, but a single-bucket pool like `goblin/doubles/less/0.5` r=+0.001) and that the same config swung from +25.9% to +12.5% purely on tie-break order (37 distinct score values across 777 legs, 69.4% tied within-day). Any backtest built by ranking legs within a bucket must re-run with at least 2 alternate deterministic tie-break orders (e.g. player_id ascending vs. descending) and report the range, not a single run. A result that isn't stable across tie-break order is not a real finding yet — it is noise dressed as a result.

**Live pricing note for Goblin specifically**: as of 2026-08-22, real per-leg Goblin/Demon pricing is no longer only a static table — `score.pricing_layer1_prop_line`, `pricing_layer2_tier`, and `pricing_layer3_player` now accumulate real per-leg observations automatically from every saved real slip (see `alphadog-v2-certification-center.js`, `recordRealPricingObservation`). A coworker session proposing or re-validating a Goblin/Demon pool should read the current state of these three tables directly rather than assuming the static in-code table is the freshest available real data.

**Historical tier-reconstruction note, added after a 2026-08-22 session found `goblin_demon_tier` populated on only 25 of 264 `walks_allowed/more` legs (9.5%)**: that gap is real in the raw `score.final_board_history`/`prop_outcome_history` tables, but it does NOT affect the live deployed Goblin logic — the deployed query (`autoSelectHighHitSlipLegs` in `alphadog-v2-certification-center.js`) reconstructs the tier at query time via `COALESCE(goblin_demon_tier, ROUND(ABS(line_value - anchor_line)))`, the same fix documented in `MULTIPLIER_TABLES_MASTER.md` §2 that unlocked this pool from 42 to 707 real Tier1 legs. **Any historical backtest of a Goblin/Demon tier-based pool must apply this same reconstruction (or query the pre-built `backtest.unified_tiers_20260822` table, which already has it applied) — measuring against the raw, unreconstructed tier column will produce a much smaller, and possibly differently-biased, sample than what the live system actually uses.** If a finding about tier-based EV or tie-break sensitivity was built on the raw column, re-run it on the reconstructed data before treating the finding as applying to the live deployed strategy.
