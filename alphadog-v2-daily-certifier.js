...
const VERSION = "alphadog-v2-daily-certifier-v0.1.1-readiness-schema-on-run-complete";
...
function safeJson(value, max = 14000) { if (value === undefined || value === null) return null; let text; try { text = typeof value === "string" ? value : JSON.stringify(value); } catch (_) { text = String(value); } return text.length > max ? text.slice(0, max) + "...TRUNCATED" : text; }
...
function baseIdentity(env) {
  ...
  return { ok: true, data_ok: true, version: VERSION, worker_name: WORKER_NAME, job_key: JOB_KEY, status: "READY_DAILY_CONTEXT_READINESS_ENRICHMENT_CERTIFIER", timestamp_utc: nowUtc(), phase: "daily-context-readiness-enrichment-certifier", binding_summary: { required_db_bindings_present: allTrue(db), expected_vars_present: allTrue(vars) }, guardrails: { readiness_enrichment_only: true, not_strict_all_context_enforcement: true, no_external_calls: true, no_sidecar_repair: true, no_board_mutation: true, no_score_db_mutation: true, no_scoring: true, no_ranking: true, no_final_board: true, volatile_current_issue_retention_today_tomorrow_only: true, batches_retained_for_small_audit_metadata: true } };
}
...
function countIssues(list, severityNames) { return list.filter(x => severityNames.includes(String(x.severity || x.issue_severity || "").toLowerCase())).length; }
...
async function runCertifier(env, input) {
  ...
  const windowDates = [today, tomorrow];
  ...
  for (const p of prepared) {
    ...
    const starterStatus = stRows.length ? "available" : "missing"; if (stRows.length) availableContext++; else gaps.push({ layer: "starters", type: "missing_starter_context", reason: "No starter rows found for game in today/tomorrow current table" });
    for (const sr of stRows) { if (sr.tbd_flag === 1 || String(sr.starter_status || "").toLowerCase().includes("tbd")) warnings.push({ layer: "starters", type: "starter_tbd", reason: "Starter is TBD/probable context incomplete" }); if (sr.hand_missing_flag === 1) warnings.push({ layer: "starters", type: "starter_hand_missing", reason: "Starter hand is missing but starter identity exists" }); }

    let lineupStatus = "not_applicable";
    if (isHitterProp(p.canonical_prop_key)) { if (lineup) { lineupStatus = layerStatus(lineup.lineup_status, "posted_lineup"); availableContext++; } else { lineupStatus = "not_posted_or_player_not_in_lineup"; gaps.push({ layer: "lineups", type: "lineup_not_posted_or_player_not_found", reason: "Lineup context missing for hitter/player row" }); } }

    let availabilityStatus = "missing";
    if (av) { availabilityStatus = layerStatus(av.availability_status, "available"); availableContext++; if (isUnavailableAvailability(av)) hard.push({ layer: "player_availability", type: "player_unavailable", reason: av.reason || av.transaction_summary || "Player availability current marks player unavailable/blocked" }); else if (av.transaction_warning_flag === 1) warnings.push({ layer: "player_availability", type: "recent_transaction_warning", reason: av.transaction_summary || "Recent transaction warning but player not hard-blocked" }); }
    ...
    if (ss) { scheduleStatus = layerStatus(ss.schedule_spot_status, "available"); availableContext++; if (ss.played_yesterday_flag === 1) warnings.push({ layer: "schedule_spot", type: "played_yesterday", reason: "Team played yesterday" }); if (ss.three_in_four_flag === 1) warnings.push({ layer: "schedule_spot", type: "three_in_four", reason: "Team is in three-in-four schedule spot" }); if (ss.four_in_six_flag === 1) warnings.push({ layer: "schedule_spot", type: "four_in_six", reason: "Team is in four-in-six schedule spot" }); if (ss.travel_required_flag === 1) warnings.push({ layer: "schedule_spot", type: "travel_required", reason: "Team travel required" }); }
    ...
    if (hard.length) { contextStatus = isUnavailableAvailability(av) ? "blocked" : "blocked"; contextGrade = isUnavailableAvailability(av) ? "BLOCKED_PLAYER_UNAVAILABLE" : "BLOCKED_HARD_INTEGRITY"; counts.blocked++; }
    else if (gaps.length && warnings.length) { contextStatus = "partial_enrichment"; contextGrade = "READY_PARTIAL_ENRICHMENT"; counts.ready_partial++; }
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (method === "POST" && path === "/run") {
      ...
      catch (e) { return jsonResponse({ ok: false, data_ok: false, version: VERSION, worker_name: WORKER_NAME, job_key: JOB_KEY, status: "failed", certification: "DAILY_CONTEXT_READINESS_FAILED", error: String(e && e.message ? e.message : e), stack_preview: String(e && e.stack ? e.stack : "").slice(0, 900), external_calls: 0 }, 500); }
    }
