...
async function runCertifier(env, input) {
  ...
  try {
    ...
    const lineups = await pg.unsafe(`SELECT game_pk, player_id, official_date, batch_id, lineup_status, data_source_level, is_temporary_derived FROM daily.lineups_current WHERE official_date = ANY($1::text[])`, [boardWindowDatesLiteral]).catch(() => []);
    const availability = await pg.unsafe(`SELECT game_pk, player_id, mlb_player_id, official_date, batch_id, availability_status, roster_status, transaction_block_flag, injured_list_flag, transaction_warning_flag, team_mlb_id, team_id, opponent_mlb_id, reason, transaction_summary, data_source_level, is_temporary_derived FROM daily.player_availability_current WHERE official_date = ANY($1::text[])`, [boardWindowDatesLiteral]).catch(() => []);
    const weather = await pg.unsafe(`SELECT game_pk, official_date, batch_id, weather_status, rain_risk_flag, delay_risk_flag, retractable_roof_flag, roof_status, data_source_level, is_temporary_derived FROM daily.game_weather_current WHERE official_date = ANY($1::text[])`, [boardWindowDatesLiteral]).catch(() => []);
    const bullpen = await pg.unsafe(`SELECT game_pk, team_id, official_date, batch_id, bullpen_status, bullpen_risk_level, bullpen_fatigue_score, data_source_level, is_temporary_derived FROM daily.bullpen_availability_current WHERE official_date = ANY($1::text[])`, [boardWindowDatesLiteral]).catch(() => []);
    const schedule = await pg.unsafe(`SELECT game_pk, team_id, official_date, batch_id, schedule_spot_status, played_yesterday_flag, three_in_four_flag, four_in_six_flag, travel_required_flag, data_source_level, is_temporary_derived FROM daily.team_schedule_spot_current WHERE official_date = ANY($1::text[])`, [boardWindowDatesLiteral]).catch(() => []);
    const umpire = await pg.unsafe(`SELECT game_pk, official_date, batch_id, umpire_context_status, assignment_pending_flag, assignment_missing_flag, unknown_umpire_flag, source_failure_flag, data_source_level, is_temporary_derived FROM daily.umpire_context_current WHERE official_date = ANY($1::text[])`, [boardWindowDatesLiteral]).catch(() => []);
