...
function pgClient(env) {
  return postgres(env.HYPERDRIVE.connectionString, { max: 5, fetch_types: false, prepare: false, connect_timeout: 8, connection: { statement_timeout: 30000, idle_in_transaction_session_timeout: 30000 } });
}
...
async function runCertifier(env, input) {
  ...
  try {
    ...
    const gamesAllDates = gamePksAllDates.length ? await pg.unsafe(`SELECT game_pk, official_date::text AS official_date, game_time_utc, is_pregame, is_live, is_final, is_postponed, is_cancelled, home_team_id, away_team_id, home_team_name, away_team_name, venue_id, venue_name, detailed_state FROM calendar.game_calendar WHERE game_pk = ANY($1::bigint[])`, [pgArrayLiteral(gamePksAllDates, false)]) : [];
    ...
    const [starters, lineups, availability, weather, bullpen, schedule, umpire, batches] = await Promise.all([
      pg.unsafe(`SELECT starter_player_id AS player_id, starter_name AS player_name, game_pk, official_date, batch_id, tbd_flag, hand_missing_flag, starter_status, data_source_level, is_temporary_derived FROM daily.probable_pitchers WHERE official_date = ANY($1::text[])`, [boardWindowDatesLiteral]).catch(() => []),
      pg.unsafe(`SELECT game_pk, player_id, official_date, batch_id, lineup_status, data_source_level, is_temporary_derived FROM daily.lineups_current WHERE official_date = ANY($1::text[])`, [boardWindowDatesLiteral]).catch(() => []),
      pg.unsafe(`SELECT game_pk, player_id, mlb_player_id, official_date, batch_id, availability_status, roster_status, transaction_block_flag, injured_list_flag, transaction_warning_flag, team_mlb_id, team_id, opponent_mlb_id, reason, transaction_summary, data_source_level, is_temporary_derived FROM daily.player_availability_current WHERE official_date = ANY($1::text[])`, [boardWindowDatesLiteral]).catch(() => []),
      pg.unsafe(`SELECT game_pk, official_date, batch_id, weather_status, rain_risk_flag, delay_risk_flag, retractable_roof_flag, roof_status, data_source_level, is_temporary_derived FROM daily.game_weather_current WHERE official_date = ANY($1::text[])`, [boardWindowDatesLiteral]).catch(() => []),
      pg.unsafe(`SELECT game_pk, team_id, official_date, batch_id, bullpen_status, bullpen_risk_level, bullpen_fatigue_score, data_source_level, is_temporary_derived FROM daily.bullpen_availability_current WHERE official_date = ANY($1::text[])`, [boardWindowDatesLiteral]).catch(() => []),
      pg.unsafe(`SELECT game_pk, team_id, official_date, batch_id, schedule_spot_status, played_yesterday_flag, three_in_four_flag, four_in_six_flag, travel_required_flag, data_source_level, is_temporary_derived FROM daily.team_schedule_spot_current WHERE official_date = ANY($1::text[])`, [boardWindowDatesLiteral]).catch(() => []),
      pg.unsafe(`SELECT game_pk, official_date, batch_id, umpire_context_status, assignment_pending_flag, assignment_missing_flag, unknown_umpire_flag, source_failure_flag, data_source_level, is_temporary_derived FROM daily.umpire_context_current WHERE official_date = ANY($1::text[])`, [boardWindowDatesLiteral]).catch(() => []),
      readLatestBatchMap(pg)
    ]);
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (method === "POST" && path === "/run") {
      ...
      try {
        const out = await runCertifier(env, input);
